...
for i in range(0,$numgrp):
    ...
    def send_mcast_streams(self, vm_fixtures, traffic, int, **kwargs):
        '''
        ...
        for stream in traffic.values():
            src = stream['src'][0]
            self._generate_multicast_trafficv2(vm_fixtures[src], maddr=stream['maddr'], count=stream['count'], int=int)
        return True

    def _generate_multicast_trafficv2(self, bms_fixture,maddr,count,int):
        '''
            Generate IGMPv3 joins/leaves from multicast receiver VMs
        '''
...
for i in range(0,$numgrp):
    ...
    def send_verify_mcastv2(self, vm_fixtures, traffic, igmp, vxlan_id,assertFlag=True):
        ...
        result = self.send_mcast_streams(vm_fixtures, traffic,int)
    ...
    def setUp(self):
        for device_name, device_dict in self.inputs.physical_routers_data.items():
            ...
            if device_dict['role'] == 'leaf':
                ...
                #self.rb_roles[device_name] = ['CRB-Access']

        super(setupFabricDcGw, self).setUp()

class Evpnt6base(setupFabricDcGw):
    ...
    def configure_qfx(self,vxlan_id):
        ...
        cmd.append('set interfaces ' + port +' unit 0 family ethernet-switching vlan members bd-'+str(vxlan_id))
        cmd.append('set vlans bd-'+str(vxlan_id)+' vlan-id '+str(vxlan_id))
    ...
    def unconfigure_qfx(self,vxlan_id):
        ...
        vxlan = "vx" + str(vxlan_id)
        vn    = "vncom" + str(vxlan_id)
        cmd.append('delete interfaces '+ port)
        cmd.append('delete vlans')
        #cmd.append('delete vlans bd-'+str(vxlan_id))
        cmd.append('delete protocols igmp-snooping')
        mx_handle = NetconfConnection(host = mgmt_ip)
        mx_handle.connect()
        time.sleep(30)
        cli_output = mx_handle.config(stmts = cmd, timeout = 120)
...
class Evpnt6MultinodeBase(IGMPV2TestBase,Evpnt6base):
    ...
    def configure_evpn_mn_topology(self,mode='l2_l3'):
        ...
        #assert self.verify_eth1_ip_from_vm(vm1_fixture)
        #self.bringup_interface_forcefully(vm2_fixture)
        #cmd = 'dhclient eth1'
        #vm2_fixture.run_cmd_on_vm(cmds=[cmd], as_sudo=True)
        #assert self.verify_eth1_ip_from_vm(vm2_fixture)
        #self.bringup_interface_forcefully(vm3_fixture)
        #cmd = 'dhclient eth1'
        #vm3_fixture.run_cmd_on_vm(cmds=[cmd], as_sudo=True)
        #assert self.verify_eth1_ip_from_vm(vm3_fixture)
        ...
        time.sleep(10)
        time.sleep(40)
        self.configure_qfx(vxlan_id)
        self.addCleanup(self.unconfigure_qfx,vxlan_id)
