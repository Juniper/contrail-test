...
from evpn_multicast_base import *
...
class TestEvpnt6Mn(Evpnt6MultinodeBase):
    ...
    @preposttest_wrapper
    def verify_evpnt6_multinode_igmp_config(self):
        '''
        ...
        result = self.send_verify_mcast_traffic(vm_fixtures, traffic, igmp, vxlan,False)
    ...
    @preposttest_wrapper
    def verify_multicast_policy(self):
        ''' 
            VM1 ,VM2 and VM3 in different compute nodes.
        ...
        int = bms_fixtures.get_mvi_interface()
        bms_fixtures.config_mroute(int,'225.1.1.1','255.255.255.255')
        bms_fixtures.config_mroute(int,'225.1.1.2','255.255.255.255')
        self.allow_default_sg_to_allow_all_on_project(self.inputs.project_name)
        vxlan = 400
        ...
        name = 'policy1'
        policy_obj = self.useFixture(MulticastPolicyFixture(name,self.project.project_obj,self.connections,action='deny',source ='0.0.0.0',group ='225.1.1.1'))
        self.logger.info('!!!!!!!!!!!!!!!!!!!!! uuid is %s ' %(policy_obj.uuid))
        name2 = 'policy2'
        policy_obj2 = self.useFixture(MulticastPolicyFixture(name2,self.project.project_obj,self.connections,action='pass',source ='0.0.0.0',group ='225.1.1.2'))
    ...
    def verify_source_within_cluster(self):
        ''' 
            VM1 ,VM2 and VM3 in different compute nodes.
            Send IGMP join from all 3 . 
            Send IGMP leave from all 3 VMs. 
...
class TestEvpnt6Sn(Evpnt6SinglenodeBase):
    ...
    def verify_source_within_cluster(self):
        ...
        result = self.send_verify_mcast_traffic_within_cluster(vm_fixtures, traffic, igmp,vxlan)
