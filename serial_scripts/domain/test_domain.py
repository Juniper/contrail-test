...
class TestDomain(BaseDomainTest):
    ...
    def test_crud_domain(self):
        ...
        if not (domain_update and domain_found):
            return False
    ...
    def test_domain_sanity(self):
        ...
        self.admin_isolated_creds.create_and_attach_user_to_tenant(
            project_fix, username, password)
    ...
    def test_domain_user_group(self):
        ...
        self.admin_connections.auth.add_user_group_to_tenant(
            project=project_name, group=user_group,
    ...
    @preposttest_wrapper
    @skip_because(keystone_version = 'v3')
    def test_domain_with_diff_user_groups(self):
        ...
        domain_name = self.connections.domain_name
        project_fix1 = self.useFixture(ProjectFixture(
            domain_name=domain_name,
            ...
            password=password1,
            connections=self.admin_connections))
        project_fix2 = self.useFixture(ProjectFixture(
            domain_name=domain_name,
        ...
        self.admin_connections.auth.add_user_group_to_tenant(
            project=project_name1, group=user_group1,
            role='admin', domain=domain_name)
        self.admin_connections.auth.add_user_group_to_tenant(
            project=project_name2, group=user_group2,
            role='_member_', domain=domain_name)
        project_fix1.set_user_creds(username1, password1)
        proj_conn = project_fix1.get_project_connections(
            domain_obj=self.domain_obj)
        import pdb;pdb.set_trace()
        ret1 = self.setup_common_objects(
            connections=proj_conn, project_fix=project_fix1)
        project_fix2.set_user_creds(username2, password2)
        proj_conn = project_fix2.get_project_connections(
            domain_obj=self.domain_obj)
        ret2 = self.setup_common_objects(
            connections=proj_conn, project_fix=project_fix2)
