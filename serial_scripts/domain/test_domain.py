...
class TestDomain(BaseDomainTest):
    ...
    def test_crud_domain(self):
        ...
        if domain_update and domain_found:
            self.logger.info(
                'Domain Create, Read and Update for domain %s PASSED.' %
                (domain_name))
        else:
            self.logger.info(
                'Domain Create, Read and Update for domain %s FAILED.' %
                (domain_name))
        return True
    ...
    def test_domain_with_diff_user_groups(self):
        ...
        self.admin_connections.auth.add_user_group_to_tenant(
            project=project_name, group=user_group1,
            role='admin', domain=domain_name)
        self.admin_connections.auth.add_user_group_to_tenant(
            project=project_name, group=user_group2,
            role='_member_', domain=domain_name)
