...
class TestAsn4(ASN4Base, BaseBGPaaS, LocalASBase):
    ...
    def create_bgp_router(self, router_params, phy_device_fixture):
        ...
        if phy_router_fixture.bgp_router_already_present:
            ...
            self.addCleanup(self.set_bgp_router_asn,
                            phy_router_fixture.bgp_router.uuid, existing_asn, None)
    ...
    def configure_control_nodes(self, control_node_config, mx_config):
        ...
        for i, ctrl_node_name in enumerate(self.inputs.bgp_names):
            ...
            if ctrl_fixture.already_present:
                ...
                self.addCleanup(self.set_bgp_router_asn,
                                bgp_obj.uuid, existing_asn, prev_local_asn)
    ...
    def generate_expected_routes(self, control_node_config, vn_fixture, mx_config, mx1_config, mx2_config, vm_fixt_list, vm_config_list):
        ...
        for static_route in vm_config_list[2]['static_routes']:
            vsrx1_routes.append(r'%s\s+%s\s+%d %d' % (static_route,
                                                     vn_gw_ip, control_node_asn[0], peer_bgpaas_asn))
        for static_route in vm_config_list[2]['static_routes_aggr']:
            vsrx1_routes.append(r'%s\s+%s\s+%d %d' % (static_route,
                                                     vn_gw_ip, control_node_asn[0], peer_bgpaas_asn))
        vsrx1_routes.append(r'%s\s+%s\s+%d %d' % (
            vm_config_list[2]['aggr_route'], vn_gw_ip, control_node_asn[0], peer_bgpaas_asn))
        ...
        if control_mx_bgp_type == "internal":
            for static_route in mx_static_routes:
                vsrx1_routes.append(r'%s\s+%s\s+%d (I|\?)' %
        ...
        if control_node_config['cluster_4b_capability']:
           as_path = "%s %s"%(control_node_asn[0],peer_bgpaas_asn)
           as4_path = "-"
        else:
           if peer_bgpaas_asn > ASN4Base.AS_2BYTE_MAX:
              as_path = "%s %s"%(control_node_asn[0],ASN4Base.AS_TRANS)
              as4_path = "%s %s"%(control_node_asn[0],peer_bgpaas_asn)
           else:
              as_path = "%s %s"%(control_node_asn[0],peer_bgpaas_asn)
              as4_path = "-"
        vn_gw_ip = vn_fixture.get_subnets()[0]['gateway_ip']
        for static_route in vm_config_list[1]['static_routes']:
            vsrx2_routes.append(r'%s\s+%s\s+%d %d' % (static_route,
                                                     vn_gw_ip, control_node_asn[0], peer_bgpaas_asn))
        for static_route in vm_config_list[1]['static_routes_aggr']:
            vsrx2_routes.append(r'%s\s+%s\s+%d %d' % (static_route,
                                                     vn_gw_ip, control_node_asn[0], peer_bgpaas_asn))
        vsrx2_routes.append(r'%s\s+%s\s+%d %d' % (
            vm_config_list[1]['aggr_route'], vn_gw_ip, control_node_asn[0], peer_bgpaas_asn))
        # Routes from MX
        if control_mx_bgp_type == "internal":
            for static_route in mx_static_routes:
                vsrx2_routes.append(r'%s\s+%s\s+%d (I|\?)' %
                                    (static_route, vn_gw_ip, control_node_asn[0]))
            vsrx2_routes.append(r'%s\s+%s\s+%d (I|\?)' %
                                (mx_aggr_route, vn_gw_ip, control_node_asn[0]))
        else:
        ...
        if mx1_config: ## Routes from External MX:
        ...
        return introspect_prefix_info, mx_routes, vsrx1_routes, vsrx2_routes
