...
class TestvDNSRestart(BasevDNSTest):
    ...
    def test_agent_query_all_dns_servers_policy_fixed(self):
        ...
        for vm in vm_list:
            if 'agent1' in vm:
                vm_fixture[vm] = self.useFixture(VMFixture(project_name=self.inputs.project_name, 
                    connections=self.connections, vn_obj=vn_fixt.obj, vm_name=vm,
        ...
        for nodes in dns_list_all_compute_nodes[0]:
            ...
            self.addCleanup(self.inputs.start_service,'contrail-named', [self.inputs.bgp_ips[index]])
        verify = "once"
        ...
        for i in range(0,360):
            ...
            if i == 0 and new_dns_list[0] == new_dns_list[1] and new_dns_list[0]==dns_list_all_compute_nodes[0]:
                self.assertFalse(self.verify_ns_lookup_data(vm_fixture['vm1-agent1'], cmd_for_agent2, vm_fixture['vm2-agent2'].vm_ip))
                self.assertFalse(self.verify_ns_lookup_data(vm_fixture['vm1-agent1'], cmd_for_agent1, vm_fixture['vm2-agent1'].vm_ip))
                continue
            elif new_dns_list[0] != new_dns_list[1] and verify=="once" :
                if new_dns_list[0] == dns_list_all_compute_nodes[0] and new_dns_list[1] != dns_list_all_compute_nodes[1]:
                    self.assertFalse(self.verify_ns_lookup_data(vm_fixture['vm1-agent1'], cmd_for_agent1, vm_fixture['vm2-agent1'].vm_ip))
                    self.assertFalse(self.verify_ns_lookup_data(vm_fixture['vm1-agent2'], cmd_for_agent1, vm_fixture['vm2-agent1'].vm_ip))
                    self.assertTrue(self.verify_ns_lookup_data(vm_fixture['vm1-agent2'], cmd_for_agent2, vm_fixture['vm2-agent2'].vm_ip))
                    self.assertTrue(self.verify_ns_lookup_data(vm_fixture['vm1-agent1'], cmd_for_agent2, vm_fixture['vm2-agent2'].vm_ip))
                elif new_dns_list[0] != dns_list_all_compute_nodes[0] and new_dns_list[1] == dns_list_all_compute_nodes[1]:
                    self.assertFalse(self.verify_ns_lookup_data(vm_fixture['vm1-agent2'], cmd_for_agent2, vm_fixture['vm2-agent2'].vm_ip))
                    self.assertFalse(self.verify_ns_lookup_data(vm_fixture['vm1-agent1'], cmd_for_agent2, vm_fixture['vm2-agent2'].vm_ip))
                    self.assertTrue(self.verify_ns_lookup_data(vm_fixture['vm1-agent1'], cmd_for_agent1, vm_fixture['vm2-agent1'].vm_ip))
                    self.assertTrue(self.verify_ns_lookup_data(vm_fixture['vm1-agent2'], cmd_for_agent1, vm_fixture['vm2-agent1'].vm_ip))
                verify="done"
                continue
            elif new_dns_list[0] != dns_list_all_compute_nodes[0] and new_dns_list[1] != dns_list_all_compute_nodes[1]:
                self.assertTrue(self.verify_ns_lookup_data(vm_fixture['vm1-agent1'], cmd_for_agent2, vm_fixture['vm2-agent2'].vm_ip))
