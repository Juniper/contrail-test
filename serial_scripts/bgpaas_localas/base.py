...
class LocalASBase(test_v1.BaseTestCase_v1):
    ...
    def configure_bgpaas_obj_and_vsrx(self, bgpaas_fixture, bgpaas_vm1, vn_fixture, src_vm, dst_vm, bgp_ip, lo_ip, local_autonomous_system, peer_local = ''):
        ...
        self.config_bgp_on_vsrx(src_vm=src_vm, dst_vm=bgpaas_vm1, bgp_ip=bgp_ip, lo_ip=bgp_ip,address_families=address_families, autonomous_system=autonomous_system, neighbors=neighbors, bfd_enabled=False, local_autonomous_system=local_autonomous_system, peer_local = peer_local)
        bgpaas_vm1.wait_for_ssh_on_vm()
    ...
    def create_2_legs(self):
        ...
        bgpaas_vm1 = self.useFixture(VMFixture(
            project_name=self.inputs.project_name, connections=self.connections,
            vn_objs=[vn1_fixture.obj,
                     vn2_fixture.obj], vm_name='bgpaas_vm1', node_name=None,
            image_name='vsrx'))
    ...
    def config_2_ports(self, bgpaas_vm1, vn1_fixture, vn2_fixture, bgpaas1_fixture, bgpaas2_fixture, test_vm, left_local_autonomous_system, right_local_autonomous_system):
        ...
        self.addCleanup(self.detach_vmi_from_bgpaas,port1['id'], bgpaas1_fixture)
        self.addCleanup(self.detach_vmi_from_bgpaas,port2['id'], bgpaas2_fixture)
        bgp_router1_uuid = self.vnc_lib.bgp_as_a_service_read(id = bgpaas1_fixture.uuid).get_bgp_router_refs()[0]['uuid']
        ...
        bgp_router2_uuid = self.vnc_lib.bgp_as_a_service_read(id = bgpaas2_fixture.uuid).get_bgp_router_refs()[0]['uuid']
        ...
        self.config_2legs_on_vsrx(src_vm=test_vm, dst_vm=bgpaas_vm1, bgp_left_ip=bgpaas_vm1.vm_ips[0], bgp_right_ip=bgpaas_vm1.vm_ips[1], address_families=address_families, autonomous_system=autonomous_system, left_neighbors = left_neighbors, right_neighbors=right_neighbors, left_local_autonomous_system=left_local_autonomous_system, right_local_autonomous_system=right_local_autonomous_system, peer_local_left = peer_local1, peer_local_right = peer_local2)
    ...
    def verify_in_agent(self, vn2_fixture, bgpaas_vm1, test_vm):
        ...
        agent_vrf_obj = bgpaas_vm1.get_matching_vrf(agent_vrf_objs['vrf_list'],vn2_fixture.vrf_name)
        vn_vrf_id = agent_vrf_obj['ucindex']
        cidr_prefix = test_vm.vm_ip
        next_hops = inspect_h.get_vna_active_route( vrf_id=vn_vrf_id, ip=cidr_prefix, prefix='32')['path_list'][0]['nh']
        if not next_hops:
            result = False
