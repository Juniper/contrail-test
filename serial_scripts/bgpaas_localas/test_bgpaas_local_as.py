...
class TestBGPaaSlocalAS(LocalASBase, BaseBGPaaS):
    ...
    def test_basic_bgpaas_local_as(self):
        ...
        self.configure_bgpaas_obj_and_vsrx(bgpaas_fixture=bgpaas_fixture, bgpaas_vm1=bgpaas_vm1, vn_fixture=vn_fixture, src_vm=test_vm, dst_vm=bgpaas_vm1, bgp_ip=bgpaas_vm1.vm_ip, lo_ip=bgpaas_vm1.vm_ip,local_autonomous_system=local_autonomous_system)
    ...
    def test_basic_bgpaas_local_as_bgp_router(self):
        ...
        self.configure_bgpaas_obj_and_vsrx(bgpaas_fixture=bgpaas_fixture, bgpaas_vm1=bgpaas_vm1, vn_fixture=vn_fixture, src_vm=test_vm, dst_vm=bgpaas_vm1, bgp_ip=bgpaas_vm1.vm_ip, lo_ip=bgpaas_vm1.vm_ip,local_autonomous_system=local_autonomous_system, peer_local = peer_local)
        agent = bgpaas_vm1.vm_node_ip
        #ToDO skiranh: some setups take a lot of time for bgpaas connection to be up, hence the sleep. Need to figure out if this is expected.
        sleep(90)
        assert bgpaas_fixture.verify_in_control_node(bgpaas_vm1), 'BGPaaS Session not seen in the control-node'
    ...
    def test_basic_bgpaas_local_as_bgp_router_negative(self):
        ...
        peer_local = self.update_bgp_router(bgpaas_vm1, bgpaas_fixture)
        self.configure_bgpaas_obj_and_vsrx(bgpaas_fixture=bgpaas_fixture, bgpaas_vm1=bgpaas_vm1, vn_fixture=vn_fixture, src_vm=test_vm, dst_vm=bgpaas_vm1, bgp_ip=bgpaas_vm1.vm_ip, lo_ip=bgpaas_vm1.vm_ip,local_autonomous_system=local_autonomous_system, peer_local = peer_local+1)
        agent = bgpaas_vm1.vm_node_ip
        #ToDO skiranh: some setups take a lot of time for bgpaas connection to be up, hence the sleep. Need to figure out if this is expected.
        sleep(90)
        assert not bgpaas_fixture.verify_in_control_node(bgpaas_vm1), 'BGPaaS Session not seen in the control-node'
    ...
    def test_endtoend_bgpaas_local_as(self):
        ...
        bgpaas1_fixture = self.create_bgpaas(bgpaas_shared=True, autonomous_system=64500, bgpaas_ip_address=bgpaas_vm1.vm_ips[0], local_autonomous_system=left_local_autonomous_system)
        bgpaas2_fixture = self.create_bgpaas(bgpaas_shared=True, autonomous_system=64500, bgpaas_ip_address=bgpaas_vm1.vm_ips[1], local_autonomous_system=right_local_autonomous_system)
        bgpaas_vm1.wait_for_ssh_on_vm()
        self.config_2_ports(bgpaas_vm1, vn1_fixture, vn2_fixture, bgpaas1_fixture, bgpaas2_fixture, test_vm, left_local_autonomous_system, right_local_autonomous_system)

        agent = bgpaas_vm1.vm_node_ip
        #ToDO skiranh: some setups take a lot of time for bgpaas connection to be up, hence the sleep. Need to figure out if this is expected.
        sleep(90)
        assert bgpaas1_fixture.verify_in_control_node(bgpaas_vm1), 'BGPaaS Session not seen in the control-node'
        assert bgpaas2_fixture.verify_in_control_node(bgpaas_vm1), 'BGPaaS Session not seen in the control-node'
