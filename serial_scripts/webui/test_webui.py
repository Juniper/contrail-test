...
class WebuiTestSanity(base.WebuiBaseTest):
    ...
    def test3_2_edit_net_disp_name_change(self):
        ...
        if self.vn_disp_name:
            ...
            if not self.webui.verify_vn_after_edit_ui('Display Name', topo.vn_disp_name, opt_list):
                self.webui.logger.debug('Virtual networks config data verification in UI failed')
                result = result and False
            self.webui.logger.debug("Step 4 : Verify API server after editing")
            if not self.webui.verify_vn_after_edit_api('Display Name', topo.vn_disp_name, uuid, opt_list):
                self.webui.logger.debug('Virtual networks config data verification in API failed')
                result = result and False
            ...
            if not self.webui.verify_vn_after_edit_ui('Display Name', self.vn_disp_name, opt_list):
                self.webui.logger.debug('Virtual networks config data verification in UI failed')
                result = result and False
            self.webui.logger.debug("Step 8 : Verifying the VN after editing previous vn name in API")
            if not self.webui.verify_vn_after_edit_api('Display Name', self.vn_disp_name, uuid, opt_list):
                self.webui.logger.debug('Virtual networks config data verification in API failed')
                result = result and False
            ...
            self.webui.logger.error("Not able to get the display name. So Editing Vn is not possible")
            result = result and False
        return result
    #end test_edit_vn_witout_change
    ...
    def test3_3_edit_net_disp_name_change_with_spl_char(self):
        ...
        if self.vn_disp_name:
            ...
            if not self.webui_common.edit_vn_disp_name_change(topo.vn_disp_name_spl_char):
                ...
                result = result and False
            self.webui.logger.debug("Step 3 : Verify WebUI server after editing")
            if not self.webui.verify_vn_after_edit_ui('Display Name', topo.vn_disp_name_spl_char, opt_list):
                self.webui.logger.debug('Virtual networks config data verification in UI failed')
                result = result and False
            self.webui.logger.debug("Step 4 : Verify API server after editing")
    ...
    def test3_4_edit_net_by_add_policy(self):
        ...
        self.webui.logger.debug("Step 2 : Verify the VN for the attached policy through WebUI server")
        ...
        if not self.webui.verify_vn_after_edit_ops('Policy', self.vn_disp_name, self.vn_disp_name, opt_list):
            self.webui.logger.debug('Virtual networks config data verification in OPS failed')
            result = result and False
    ...
    def test3_5_edit_net_by_add_subnet(self):
        ...
        '''
        opt_list = [topo.subnet_edit, topo.mask, topo.subnet_sip, topo.subnet_eip, topo.subnet_dns_ip,
                    topo.subnet_gate_ip, topo.subnet_default_gate_ip]
        result = True
        if self.webui_common.click_configure_networks():
            ...
            for subnet_type in verify_list:
                ...
                self.webui.logger.debug("Step 1 - " + subnet_type + ": Add subnet with " + str1 + "options")
                if not self.webui_common.edit_vn_with_subnet(subnet_type, topo.subnet_edit + "/" + topo.mask, \
                                                             topo.subnet_sip + "-" + topo.subnet_eip, \
                                                             topo.subnet_gate_ip):
                ...
                self.webui.logger.debug("Step 2 - " + subnet_type + ": Verify the VN for subnet in WebUI")
                if not self.webui.verify_vn_after_edit_ui(subnet_type, subnet, opt_list):
                    self.webui.logger.debug('Virtual networks config data verification in UI failed')
                    result = result and False
                self.webui.logger.debug("Step 3 - " + subnet_type + ": Verify the VN for subnet in API server")
                if not self.webui.verify_vn_after_edit_api(subnet_type, subnet, uuid, opt_list):
                    self.webui.logger.debug('Virtual networks config data verification in API failed')
                    result = result and False
                self.webui.logger.debug("Step 4 - " + subnet_type + ": Verify the VN for subnet in OPS server")
                if not self.webui.verify_vn_after_edit_ops(subnet_type, self.vn_disp_name, uuid, opt_list):
                    self.webui.logger.debug('Virtual networks config data verification in OPS failed')
                    result = result and False
    ...
    def test3_6_edit_net_host_opt(self):
        ...
        if not self.webui_common.edit_vn_with_host_route('add', 'pos', topo.host_prefix, topo.host_nexthop):
        ...
        if not self.webui_common.edit_vn_with_host_route('remove', 'pos', topo.host_prefix, topo.host_nexthop):
            self.webui.logger.debug('Editing network with host routes failed')
            result = result and False
        return result
    ...
    def test3_7_edit_net_host_opt_neg(self):
        ...
        uuid = self.webui_common.get_vn_detail_ui('UUID')
        self.webui.logger.debug("Step 1 : Add Host Route under VN")
        assert self.webui_common.edit_vn_with_host_route('add', 'neg', topo.dns_ip, topo.host_nexthop), \
                                                         'Editing network with host routes failed as \
                                                         expected for negative scenario'
    ...
    @preposttest_wrapper
    def test3_13_edit_net_route_target_asn_num(self):
        ...
        if not self.webui_common.edit_vn_with_route_target('add', 'pos', 'RT',topo.asn_num, topo.target_num):
    ...
    # end test3_13_edit_net_route_target_asn_num

    @preposttest_wrapper
    def test3_14_edit_net_route_target_asn_ip(self):
        ''' Test to edit the existing network by Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
        ...
        opt_list = [topo.asn_num, topo.target_num, topo.asn_ip]
        self.webui.logger.debug("Step 1 : Add Route Target under VN")
        if not self.webui_common.edit_vn_with_route_target('add', 'pos', 'RT', topo.asn_ip, topo.target_num):
            self.webui.logger.debug('Editing network with Route target failed')
            result = result and False
        self.webui_common.wait_till_ajax_done(self.browser, wait=3)
        uuid = self.webui_common.get_vn_detail_ui('UUID')
        ...
        if not self.webui.verify_vn_after_edit_ops('RT', self.vn_disp_name, uuid, opt_list):
            ...
            result = result and False
        self.webui.logger.debug("Step 5 : Remove the Route Target which is added")
        if not self.webui_common.edit_vn_with_route_target('remove', 'pos', 'RT', topo.asn_ip, topo.target_num):
            self.webui.logger.debug('Editing network with Route Target is failed')
            result = result and False
        return result

    # end test3_14_edit_net_route_target_asn_ip

    @preposttest_wrapper
    def test3_15_edit_net_route_target_neg_asn_ip(self):
        ''' Test to edit the existing network by Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
        ...
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'RT', \
                                                           ...
                                                           'Editing network with Route target failed'
        self.webui_common.wait_till_ajax_done(self.browser, wait=3)

    # end test3_15_edit_net_route_target_neg_asn_ip

    @preposttest_wrapper
    def test3_16_edit_net_route_target_neg_asn_num(self):
        ''' Test to edit the existing network by Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
        ...
        result = True
        self.webui.logger.debug("Step 1 : Add Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'RT', topo.invalid_asn_num, \
                                                           topo.invalid_target_num), \
                                                           'Editing network with Route target failed'
        self.webui_common.wait_till_ajax_done(self.browser, wait=3)

    # end test3_16_edit_net_route_target_neg_asn_num

    @preposttest_wrapper
    def test3_17_edit_net_exp_route_target_asn_num(self):
        ...
        if not self.webui_common.edit_vn_with_route_target('add', 'pos', 'ERT', topo.asn_num, topo.target_num):
    ...
    # end test3_17_edit_net_exp_route_target_asn_num

    @preposttest_wrapper
    def test3_18_edit_net_exp_route_target_asn_ip(self):
        ''' Test to edit the existing network by Export Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
    ...
    # end test3_18_edit_net_exp_route_target_asn_ip

    @preposttest_wrapper
    def test3_19_edit_net_exp_route_target_neg_asn_ip(self):
        ''' Test to edit the existing network by Export Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
        ...
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'ERT', \
                                                           topo.invalid_asn_ip, \
                                                           topo.invalid_target_num), \
                                                           'Editing network with Export Route target failed'
        self.webui_common.wait_till_ajax_done(self.browser, wait=3)

    # end test3_19_edit_net_exp_route_target_neg_asn_ip

    @preposttest_wrapper
    def test3_20_edit_net_exp_route_target_neg_asn_num(self):
        ''' Test to edit the existing network by Export Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
    ...
    # end test3_20_edit_net_exp_route_target_neg_asn_num

    @preposttest_wrapper
    def test3_21_edit_net_imp_route_target_asn_num(self):
    ...
    # end test3_21_edit_net_imp_route_target_asn_num

    @preposttest_wrapper
    def test3_22_edit_net_imp_route_target_asn_ip(self):
        ''' Test to edit the existing network by Import Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
    ...
    # end test3_22_edit_net_route_target_asn_ip

    @preposttest_wrapper
    def test3_23_negative_case_edit_net_with_invalid_route_target_ip(self):
        ''' Test to edit the existing network by Import Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
        ...
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'IRT', \
                                                           topo.invalid_asn_ip, \
                                                           topo.invalid_target_num), \
                                                           'Editing network with Import Route target failed'
        self.webui_common.wait_till_ajax_done(self.browser, wait=3)

    # end test3_23_negative_case_edit_net_with_invalid_route_target_ip

    @preposttest_wrapper
    def test3_24_negative_case_edit_net_with_invalid_route_target_num(self):
        ''' Test to edit the existing network by Import Route Target
            1. Go to Configure->Networking->Networks. Then select any of the vn and click the edit button
    ...
    # end test3_24_negative_case_edit_net_with_invalid_route_target_num

    @preposttest_wrapper
    def test3_25_create_vn_with_spl_char(self):
        ''' Test to create vn with combination of spl char and verify in all API, OPS and WebUI
            1. Go to Configure->Networking->Networks. Then create VN with all combination of spl characters
        ...
        for vn in vn_list:
            ...
            if self.webui_common.click_configure_networks():
                ...
                add.click()
                self.webui_common.wait_till_ajax_done(self.browser, wait=3)
                self.webui_common.find_element("//input[contains(@name,'display_name')]", 'xpath').send_keys(vn)
                self.webui_common.wait_till_ajax_done(self.browser, wait=3)
                self.webui_common.click_element('configure-networkbtn1')
                self.webui_common.wait_till_ajax_done(self.browser)
                uuid = self.webui_common.get_vn_detail_ui('UUID')
                ...
                if not self.webui_common.edit_remove_option("Networks", 'remove'):
                    ...
                    result = result and False
        return result
    # test3_25_create_vn_with_spl_char
