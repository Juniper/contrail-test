...
class WebuiTestSanity(base.WebuiBaseTest):
    ...
    def test3_1_edit_net_without_change(self):
        ...
        assert self.webui_common.edit_vn_without_change(), 'Editing Network failed'
    ...
    def test3_2_edit_net_disp_name_change(self):
        ...
        if self.vn_disp_name:
            ...
            assert self.webui_common.edit_vn_disp_name_change(topo.vn_disp_name), 'Editing Network failed'
            ...
            assert self.webui_common.edit_vn_disp_name_change(self.vn_disp_name), 'Editing Network failed'
    ...
    def test3_3_edit_net_disp_name_change_with_spl_char(self):
        ...
        if self.vn_disp_name:
            ...
            assert self.webui_common.edit_vn_disp_name_change(topo.vn_disp_name_spl_char), 'Editing Network failed'
            logger.debug("Step 3 : Verify WebUI server after editing")
    ...
    def test3_4_edit_net_by_add_policy(self):
        ...
        pol_name = ""
        assert self.webui_common.add_vn_with_policy(pol_name), 'Editing network with policy failed' 
        opt_list = [pol_name]
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        ...
        assert self.webui_common.del_vn_with_policy(), 'Editing network with policy failed'
    ...
    def test3_5_edit_net_by_add_subnet(self):
        ...
        if self.webui_common.click_configure_networks():
            ...
            for i in verify_list:
                ...
                assert self.webui_common.edit_vn_with_subnet(i, topo.subnet_edit + "/" + topo.mask, topo.subnet_sip + "-" + topo.subnet_eip, topo.subnet_gate_ip), 'Editing network with subnet failed'
                uuid = self.webui_common.get_vn_display_name('UUID')
                self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
                ...
                assert self.webui_common.del_vn_with_subnet(), 'Editing network with subnet failed'
        logger.debug("Step 6 : Remove the VN which is added")
        assert self.webui_common.edit_option("Networks", 'remove'), 'Editing network with advanced options is failed'
    ...
    def test3_6_edit_net_host_opt(self):
        ...
        assert self.webui_common.edit_vn_with_host_route('add', 'pos', topo.host_prefix, topo.host_nexthop), 'Editing network with host routes failed'
        time.sleep(5)
        ...
        assert self.webui_common.edit_vn_with_host_route('remove', 'pos', topo.host_prefix, topo.host_nexthop), 'Editing network with host routes failed'
    ...
    def test3_7_edit_net_host_opt_neg(self):
        ...
        uuid = self.webui_common.get_vn_display_name('UUID')
        logger.debug("Step 1 : Add Host Route under VN")
        assert self.webui_common.edit_vn_with_host_route('add', 'neg', topo.dns_ip, topo.host_nexthop), 'Editing network with host routes failed as expected for negative scenario'
    ...
    def test3_8_edit_net_adv_opt(self):
        ...
        assert self.webui_common.edit_vn_with_adv_option(1, 'pos-phy', topo.phy_net, topo.vlan_id), 'Editing network with advanced options is failed'
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
    ...
    def test3_9_edit_net_adv_opt_neg(self):
        ...
        assert self.webui_common.edit_vn_with_adv_option(0, 'neg-phy', topo.phy_net, topo.invalid_vlan_id), 'Editing network with advanced option is failed'
    ...
    def test3_10_edit_net_dns(self):
        ...
        assert self.webui_common.edit_vn_with_dns('add', 'pos', topo.dns_ip), 'Editing network with dns is failed'
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        ...
        logger.debug("Step 5 : Remove the VN which is added")
        assert self.webui_common.edit_vn_with_dns('remove', 'pos', topo.dns_ip), 'Editing network with dns is failed'
    ...
    def test3_11_edit_net_dns_neg(self):
        ...
        '''
        logger.debug("Step 1 : Add dns server IP under VN")
        assert self.webui_common.edit_vn_with_dns('add', 'neg', topo.invalid_dns_ip), 'Editing network with dns is failed'
    ...
    def test3_12_edit_net_fip(self):
        ...
        assert self.webui_common.edit_vn_with_fpool('add', topo.fpool), 'Editing network with FIP is failed'
        time.sleep(3)
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        ...
        assert self.webui_common.edit_vn_with_fpool('remove', topo.fpool), 'Editing network with FIP is failed'
    ...
    def test3_13_edit_net_route_target_as_no(self):
        ...
        assert self.webui_common.edit_vn_with_route_target('add', 'pos', 'RT',topo.as_no, topo.target_no), 'Editing network with Route target failed'
        time.sleep(5)
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        ...
        assert self.webui_common.edit_vn_with_route_target('remove', 'pos', 'RT',topo.as_no, topo.target_no), 'Editing network with Route Target is failed'
    ...
    def test3_14_edit_net_route_target_as_ip(self):
        ...
        opt_list = [topo.as_no,topo.target_no,topo.as_ip]
        logger.debug("Step 1 : Add Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'pos', 'RT', topo.as_ip, topo.target_no), 'Editing network with Route target failed'
        time.sleep(3)
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        rt = self.webui_common.get_vn_display_name('RT')
        ...
        assert self.webui.verify_vn_after_edit_ops('RT', self.vn_disp_name, uuid, opt_list), 'Virtual networks config data verification in OPS failed'
        logger.debug("Step 5 : Remove the Route Target which is added")
        assert self.webui_common.edit_vn_with_route_target('remove', 'pos', 'RT', topo.as_ip, topo.target_no), 'Editing network with Route Target is failed'
    ...
    def test3_15_edit_net_route_target_neg_as_ip(self):
        ...
        '''
        logger.debug("Step 1 : Add Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'RT', topo.invalid_as_ip, topo.invalid_target_no), 'Editing network with Route target failed'
        time.sleep(3)
    ...
    def test3_16_edit_net_route_target_neg_as_no(self):
        ...
        '''
        logger.debug("Step 1 : Add Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'RT', topo.invalid_as_no, topo.invalid_target_no), 'Editing network with Route target failed'
        time.sleep(3)
    ...
    def test3_17_edit_net_exp_route_target_as_no(self):
        ...
        assert self.webui_common.edit_vn_with_route_target('add', 'pos', 'ERT', topo.as_no, topo.target_no), 'Editing network with Export Route target failed'
        time.sleep(3)
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        ...
        logger.debug("Step 5 : Remove the Route Target which is added")
        assert self.webui_common.edit_vn_with_route_target('remove', 'pos', 'ERT', topo.as_no, topo.target_no), 'Editing network with Export Route Target is failed'
    ...
    def test3_18_edit_net_exp_route_target_as_ip(self):
        ...
        opt_list = [topo.as_no,topo.target_no,topo.as_ip]
        logger.debug("Step 1 : Add Export Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'pos', 'ERT', topo.as_ip, topo.target_no), 'Editing network with Export Route target failed'
        time.sleep(3)
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        ert = self.webui_common.get_vn_display_name('ERT')
        ...
        assert self.webui_common.edit_vn_with_route_target('remove', 'pos', 'ERT', topo.as_ip, topo.target_no), 'Editing network with Export Route Target is failed'
    ...
    def test3_19_edit_net_exp_route_target_neg_as_ip(self):
        ...
        '''
        logger.debug("Step 1 : Add Export Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'ERT', topo.invalid_as_ip, topo.invalid_target_no), 'Editing network with Export Route target failed'
        time.sleep(3)
    ...
    def test3_20_edit_net_exp_route_target_neg_as_no(self):
        ...
        '''
        logger.debug("Step 1 : Add Export Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'ERT', topo.invalid_as_no, topo.invalid_target_no), 'Editing network with Export Route target failed'
        time.sleep(3)
    ...
    def test3_21_edit_net_imp_route_target_as_no(self):
        ...
        assert self.webui_common.edit_vn_with_route_target('add', 'pos', 'IRT', topo.as_no, topo.target_no), 'Editing network with import Route target failed'
        time.sleep(3)
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        ...
        assert self.webui_common.edit_vn_with_route_target('remove', 'pos', 'IRT', topo.as_ip, topo.target_no), 'Editing network with Import Route Target is failed'
    ...
    def test3_22_edit_net_imp_route_target_as_ip(self):
        ...
        logger.debug("Step 1 : Add Import Route Target under VN")
        time.sleep(5)
        assert self.webui_common.edit_vn_with_route_target('add', 'pos', 'IRT', topo.as_ip, topo.target_no), 'Editing network with Import Route target failed'
        time.sleep(3)
        uuid = self.webui_common.get_vn_display_name('UUID')
        self.vn_disp_name = self.webui_common.get_vn_display_name('Display Name')
        irt = self.webui_common.get_vn_display_name('IRT')
    ...
    def test3_23_edit_net_imp_route_target_neg_as_ip(self):
        ...
        '''
        logger.debug("Step 1 : Add Import Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'IRT', topo.invalid_as_ip, topo.invalid_target_no), 'Editing network with Import Route target failed'
        time.sleep(3)
    ...
    def test3_24_edit_net_imp_route_target_neg_as_no(self):
        ...
        '''
        logger.debug("Step 1 : Add Import Route Target under VN")
        assert self.webui_common.edit_vn_with_route_target('add', 'neg', 'IRT', topo.invalid_as_no, topo.invalid_target_no), 'Editing network with Import Route target failed'
        time.sleep(3)
