...
class AnalyticsTestSanityWithResource(base.AnalyticsBaseTest, ConfigSvcChain , VerifySvcChain):
    ...
    def test_object_log_verification_with_delete_add_in_network_mode(self):
        ...
        try:
            ...
            try:
                ...
                result = result and False
            try:
                assert self.analytics_obj.verify_connected_networks_in_vn_uve(self.vn1_fixture.\
            ...
            for si_fix in self.si_fixtures:
				...
				(vn_fq_name= self.vn1_fixture.vn_fq_name,vm_uuid_lst=[si_uuid])
                        ...
                        result = result and False
                    try:
                        assert self.analytics_obj.verify_vm_list_in_vn_uve(vn_fq_name =\
				 self.vn2_fixture.vn_fq_name,vm_uuid_lst=[si_uuid])
                    except Exception as e:
            ...
            for si_fix in self.si_fixtures:
                ...
                time.sleep(10)
                try:
                    self.analytics_obj.verify_si_uve_not_in_analytics(instance=si_name,
						st_name = self.st_name,left_vn=self.vn1_fq_name,
    ...
    def test_vrouter_uve_vm_on_vm_create(self):
        ...
        for vm_fixture in vm_fixture_list:
            ...
            assert self.analytics_obj.verify_vm_list_in_vrouter_uve(vm_uuid=vm_uuid,vn_fq_name=vn_of_vm,
					vrouter=vm_host,tap=interface_name)
    ...
    def test_verify_connected_networks_based_on_policy(self):
        ...
        policy1_fixture= self.useFixture( PolicyFixture( policy_name= policy1_name, 
					rules_list= rules, inputs= self.inputs,
                                    	connections= self.connections ))
        policy2_fixture= self.useFixture( PolicyFixture( policy_name= policy2_name, 
...
#        self.res.verify_common_objects()
    ...
    def test_verify_flow_series_table_query_range(self):
        ...
        vm_node_ip= self.res.vn1_vm1_fixture.inputs.host_data[self.res.\
			vn1_vm1_fixture.nova_fixture.get_nova_host_of_vm(\
...
#        self.res.verify_common_objects()
        ...
        pkts_before_traffic = self.analytics_obj.get_inter_vn_stats(self.inputs.collector_ips[0], 
						src_vn=vn1_fq_name, other_vn=vn2_fq_name, direction='in')
        ...
        vm_node_ip= self.res.vn1_vm1_fixture.inputs.host_data[self.res.vn1_vm1_fixture.\
			nova_fixture.get_nova_host_of_vm(self.res.vn1_vm1_fixture.vm_obj)]['host_ip']
        vm_host=self.res.vn1_vm1_fixture.inputs.host_data[vm_node_ip]['name']
...
#        time.sleep(60)
        pkts_after_traffic = self.analytics_obj.get_inter_vn_stats(self.inputs.collector_ips[0], 
					src_vn=vn1_fq_name, other_vn=vn2_fq_name, direction='in')
...
#        assert ( self.flow_record == 0)
        ...
        for ip in self.inputs.collector_ips: 
            ...
            self.res2=self.analytics_obj.ops_inspect[ip].post_query('FlowRecordTable',start_time=start_time,end_time='now'
                                                 		,select_fields=['sourcevn', 'sourceip', 'destvn', 
								'destip','setup_time','teardown_time','agg-packets'],
                                                    		where_clause=query)
            ...
            #Quering flow sreies table
            self.logger.info("Verifying flowSeriesTable through opserver %s"%(ip))    
            self.res1=self.analytics_obj.ops_inspect[ip].post_query('FlowSeriesTable',
