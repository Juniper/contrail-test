...
class Md5Base(test_v1.BaseTestCase_v1, VerifySecGroup, ConfigPolicy):
    ...
    def test_check_per_peer_md5(self):
        auth_data=None
        for host in self.list_uuid:
            self.config_per_peer(auth_data=auth_data)
            self.config_md5( host=host, auth_data=auth_data )
    ...
    def test_add_delete_per_peer_md5(self)
        auth_data=None
        for host in self.list_uuid:
            self.config_per_peer(auth_data=auth_data)
            self.config_md5( host=host, auth_data=auth_data )
    ...
    def test_diff_keys_per_peer_md5(self):
        auth_data=None
        for host in self.list_uuid:
            self.config_per_peer(auth_data=auth_data)
            self.config_md5( host=host, auth_data=auth_data )
    ...
    def test_precedence_per_peer_md5():
        auth_data=None
        host=self.list_uuid[1]
        self.config_per_peer(auth_data=auth_data)
        for host in self.list_uuid:
    ...
    def test_iter_keys_per_peer_md5(self):
        auth_data=None
        for host in self.list_uuid:
            self.config_per_peer(auth_data=auth_data)
            self.config_md5( host=host, auth_data=auth_data )
        ...
        for i in range(1, 11):
            for host in self.list_uuid:                key = i.__str__()
                auth_data={'key_items': [ { 'key':key,"key_id":0 } ], "key_type":"md5"}
                self.config_md5( host=host, auth_data=auth_data )
            sleep(95)
