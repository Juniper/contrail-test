...
class TestXmpptests(XmppBase, ConfigPolicy):
    ...
    def test_precedence_xmpp_auth(self):
        ...
        """
        for node in self.inputs.bgp_control_ips:
            self.update_contrail_conf(conf_file = '/etc/contrail/contrail-control.conf', operation = 'del', section = 'DEFAULT', knob = 'xmpp_auth_enable', node = node, service = 'supervisor-control')
        ...
        for node in self.inputs.bgp_control_ips:
            self.update_contrail_conf(conf_file = '/etc/contrail/contrail-control.conf', operation = 'set', section = 'DEFAULT', knob = 'xmpp_auth_enable', value = 'True', node = node, service = 'supervisor-control')
    ...
    def test_compute_negative_xmpp_auth(self):
        ...
        for node in self.inputs.compute_ips:
            self.update_contrail_conf(conf_file = '/etc/contrail/contrail-vrouter-agent.conf', operation = 'del', section = 'DEFAULT', knob = 'xmpp_auth_enable', node = node, service = 'supervisor-vrouter')
        assert (not (self.check_if_cluster_has_xmpp())), "XMPP connections should not be found"
        for node in self.inputs.bgp_control_ips:
        ...
        for node in self.inputs.compute_ips:
            self.update_contrail_conf(conf_file = '/etc/contrail/contrail-vrouter-agent.conf', operation = 'set', section = 'DEFAULT', knob = 'xmpp_auth_enable', value = 'True', node = node, service = 'supervisor-vrouter')
        assert (self.check_if_cluster_has_xmpp), "XMPP connections not found"
        for node in self.inputs.bgp_control_ips:
            assert (self.check_xmpp_status(node)), "XMPP between nodes not up after adding back xmpp auth"
