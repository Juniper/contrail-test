...
class Test1553316(BaseNeutronTest):
    ...
    def test_create_v6(self):
        ...
        mx_handle = self.phy_router_fixture.get_connection_obj('juniper', 
                    ...
                    logger=[self.logger])
        self.vn_index = self.vnc_lib.virtual_network_read(id = vn1_fixture.uuid).virtual_network_network_id
        cmd = 'show configuration groups __contrail__ routing-instances _contrail_%s-l3-%s' % (vn1_name, self.vn_index)
