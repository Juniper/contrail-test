    ...
    def tcpdump_start_on_all_compute(self):
        for compute_ip in self.inputs.compute_ips:
            ...
            if self.inputs.orchestrator == 'vcenter':
                ...
                sftp.get('/etc/contrail/contrail-vrouter-agent.conf', '/tmp/contrail-vrouter-agent.conf')
                self.config.read('/tmp/contrail-vrouter-agent.conf')
                comp_intf = self.config.get('VIRTUAL-HOST-INTERFACE', 'physical_interface', None)
            else:
