"""Test module for ip fabric forwarding for k8s
   this module contains the restart and reboot scenario based 
   testcases to verify the behavior of the ip fabric forwarding with k8s
"""
...
__version__ = "1.0.1"
__maintainer__ = "Venkatesh Velpula"
__email__ = "vvelpula@juniper.net"
...
class TestFabricFWDRestarts(BaseK8sTest):
    ...
    def verify_ping_between_pods_across_namespaces_and_public_network(self, client1, client2, client3):
        ...
        assert client1[1].ping_to_ip(google_dns_ip)
        assert client2[0].ping_to_ip(google_dns_ip)
        assert client1[0].ping_to_ip(client1[1].pod_ip)
        #verifying pods in isolated/default namespaces shoud not reach each other when fabric forwarding is enabled
