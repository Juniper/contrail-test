...
class TestNetworkAttachment(BaseK8sTest):
    ...
    def common_setup_for_multi_intf(self,
        ...
        pprint(metadata)
        left_pod = get_random_name('left')
        right_pod = get_random_name('right')
        ...
        #left_pod = self.setup_pod(name=left_pod, metadata=metadata, spec=spec)
        #create a cirros pod with network attachment definitions  
        #managent ,left and right 
        if nginx:
           left_pod = self.setup_nginx_pod(name=left_pod, metadata=metadata, namespace=namespace, labels=labels)
        else:
           left_pod = self.setup_cirros_pod(name=left_pod, metadata=metadata,namespace=namespace)
        assert left_pod.verify_on_setup()

        left_pod_mgmpt_ip = left_pod.pod_ip
        left_pod_ip1 = left_pod.get_pod_ip(left_nad.metadata["name"])
        left_pod_ip2 = left_pod.get_pod_ip(right_nad.metadata["name"])
        
        left_pod_details = [left_pod, left_pod_mgmpt_ip, left_pod_ip1, left_pod_ip2]

        metadata.update({"name" : right_pod})
        #checking if the ipod has created with rewreid interfaces
        if nginx:
           right_pod = self.setup_nginx_pod(name=right_pod, metadata=metadata, namespace=namespace, labels=labels)
        else:
           right_pod = self.setup_cirros_pod(name=right_pod, metadata=metadata, namespace = namespace)
        assert right_pod.verify_on_setup()
        #pick the ip address of the pods  from the corresponding network attachment
        right_pod_mgmt_ip = right_pod.pod_ip
        right_pod_ip1 = right_pod.get_pod_ip(left_nad.metadata["name"])
        right_pod_ip2 = right_pod.get_pod_ip(right_nad.metadata["name"])
       
        right_pod_details = [right_pod, right_pod_mgmt_ip, right_pod_ip1, right_pod_ip2]
        #self.verify_rechability_between_multi_intf_pods (left_pod_details, right_pod_details )
        return (left_pod_details, right_pod_details)

    def verify_rechability_between_multi_intf_pods(self, left_pod, right_pod, expectation=True):
        ...
        assert right_pod[0].ping_to_ip(left_pod[1], expectation=expectation)
        assert right_pod[0].ping_to_ip(left_pod[2], expectation=expectation)
        assert right_pod[0].ping_to_ip(left_pod[3], expectation=expectation)
        #verify the rechability across the left network attachment
        assert left_pod[0].ping_to_ip(right_pod[1], expectation=expectation)
        assert left_pod[0].ping_to_ip(right_pod[2], expectation=expectation)
        assert left_pod[0].ping_to_ip(right_pod[3], expectation=expectation)
    ...
    @preposttest_wrapper
    def test_pod_with_multi_intf_index_with_kube_manager_restart(self):
        """
           :return: None
           ...
           verify the index of the interfaces like eth0,eth1 and eth2
           restart the kube manager 
           Check the contrail cluster stability
           repeat the steps before the kube managre restart 
        """
        left_pod, right_pod = self.common_setup_for_multi_intf()
        self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
        cluster_status, error_nodes_before = ContrailStatusChecker(self.inputs).wait_till_contrail_cluster_stable()
        eth0_ip = self.get_intf_address('eth0',left_pod[0])
        eth1_ip = self.get_intf_address('eth1',left_pod[0])
        eth2_ip = self.get_intf_address('eth2',left_pod[0])
        #check the index of the interfaces 
        assert eth0_ip == left_pod[1]  
        assert eth1_ip == left_pod[2]  
        assert eth2_ip == left_pod[3]  
        self.restart_kube_manager()
        time.sleep(15) 
        cluster_status, error_nodes_after = ContrailStatusChecker(self.inputs).wait_till_contrail_cluster_stable() 
        #cluster should stable and status should same as before the kube manager restart
        assert cmp(error_nodes_before, error_nodes_after) == 0 , "cluster is not stable after kube manager restart"
        #network attachments should eb reachable even after the kubemanager restart
        self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
        #get the index of the all the interfaces
        eth0_ip = self.get_intf_address('eth0',right_pod[0])
        eth1_ip = self.get_intf_address('eth1',right_pod[0])
        eth2_ip = self.get_intf_address('eth2',right_pod[0])
        #check the index of the interfaces
        assert eth0_ip == right_pod[1]
        assert eth1_ip == right_pod[2]
        assert eth2_ip == right_pod[3]
    #end test_pod_with_multi_intf_index_with_kube_manager_restart
    ...
    def test_pod_with_multi_intf_defualt_route(self):
        ...
        self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
        self.verify_default_route(left_pod[0])
    ...
    def test_basic_pod_with_multi_intf(self):
        """
           ...
           Ping should pass from
               left_pod:eth0_ip <-> eth0_ip:right_pod
               left_pod:eth1_ip <-> eth1_ip:right_pod
               left_pod:eth2_ip <-> eth2_ip:right_pod
        """
        left_pod, right_pod = self.common_setup_for_multi_intf()
        eth0_ip = self.get_intf_address('eth0',left_pod[0])
        eth1_ip = self.get_intf_address('eth1',left_pod[0])
        ...
        #management to management i.e eth0 of left_pod to eth0 of right_pod
        #additional network attachment definitions i.e
        #eth1 of left_pod to eth1 of right_pod
        self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
        #default route should be installed via eth0
        #which is mapped to default pod network im this case
        self.verify_default_route(left_pod[0])
        self.verify_default_route(right_pod[0])
    ...
    def test_basic_pod_with_multi_intf_non_default_namespace(self):
        ...
        left_pod, right_pod = self.common_setup_for_multi_intf(namespace = namespace.name)
        self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
    ...
    def test_basic_pod_with_multi_intf_isolated_namespace(self):
        ...
        self.verify_rechability_between_multi_intf_pods(iso_pod1, right_pod ,expectation = False)
    ...
    def test_pod_with_multi_intf_kube_manager_restart(self):
        ...
        self.verify_pod_intf_index(left_pod)
        self.verify_pod_intf_index(right_pod)
    ...
    @preposttest_wrapper
    def test_pod_multi_intf_with_docker_restart_on_master(self):
        """
            1.verifies pods can reach to public network when snat is enabled
            2.restart the docker service
            3.re verify  pods can reach to public network when snat is enabled
        ...
        self.inputs.restart_service(service_name = "docker",
                                            host_ips = self.inputs.k8s_slave_ips)
        time.sleep(120) # Wait timer for all kubernetes pods to stablise.
        self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
        cluster_status, error_nodes = ContrailStatusChecker(self.inputs).wait_till_contrail_cluster_stable()
        assert cluster_status, 'All nodes and services not up. Failure nodes are: %s' % (
                    error_nodes)

        time.sleep(90) # Wait timer for all kubernetes pods to stablise.
        verify_rechability_between_multi_intf_pods (left_pod, right_pod)
    #end test_pod_multi_intf_with_docker_restart_on_master
    ...
    def test_multi_intf_pod_with_fabric_fwd_snat(self):
        """
            ...
            3.re verify  pods can reach to public network when fabric forwarding is enabled
        """
        left_pod, right_pod = self.common_setup_for_multi_intf(ip_fabric_forwarding=True, ip_fabric_snat=True)
        self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
    ...
    #@preposttest_wrapper
    #def test_multi_intf_pod_with_master_reboot(self):
    #    left_pod, right_pod = self.common_setup_for_multi_intf(ip_fabric_forwarding=True, ip_fabric_snat=True)
    #    self.verify_rechability_between_multi_intf_pods(left_pod, right_pod)
