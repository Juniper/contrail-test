...
class BaseDM(test_v1.BaseTestCase_v1, VerifySecGroup):
...
#            if self.inputs.use_devicemanager_for_md5:
    ...
    def check_mx_output(self, cmd):
        ...
        for i in range(len(self.output_from_mx)):
            if 'invalid command' in self.output_from_mx[i]:
                ...
                return True, str(self.phy_router_fixture[i].name)
    ...
    def is_dm_going_through(self):
        ...
        assert not (self.iteration == 9), "DM VN config not pushed even after 90 sec"
        assert self.op, "DM config on %s not going through. Check the MX" % self.bad_mx

    def is_dm_removed(self):

        assert (self.iteration == 9), "DM VN config not removed even after 90 sec"
        assert not self.op, "DM config on %s not removed. Check the MX" % self.bad_mx
