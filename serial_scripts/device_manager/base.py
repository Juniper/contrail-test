...
class BaseDM(VerifySecGroup):
    ...
    def check_policy_added_on_mx(self, cmd):

        for i in range(len(self.output_from_mx)):
            for j in range(1,13):
                self.does_mx_have_config(cmd)
                self.is_dm_going_through()
                if not '54321' in self.output_from_mx[i]:
                    sleep(10)
                else:
                    break
            if j == 12:
    ...
    def check_policy_removed_on_mx(self, cmd):

        for i in range(len(self.output_from_mx)):
            for j in range(1,13):
                self.does_mx_have_config(cmd)
                self.is_dm_removed()
                if '54321' in self.output_from_mx[i]:
                    sleep(10)
                else:
                    break
            if j == 12:
