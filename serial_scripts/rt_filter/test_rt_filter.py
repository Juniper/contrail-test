...
from base import BaseRtFilterRestartTest
from common import isolated_creds
import inspect

import test

class TestBasicRTFilterRestart(BaseRtFilterRestartTest):
    ...
    def test_vpnv4_route_entry_only_on_RT_interest_receipt(self):
        ...
        if (('MX_GW_TEST' in os.environ) and (os.environ.get('MX_GW_TEST') == '1')):
            ...
            ip= vm1_fixture.vm_ip + '/32'
            user_def_rt= "target:%s:%s" % (self.inputs.router_asn, self.inputs.mx_rt)
            routing_instance = vn1_fixture.ri_name
            self.logger.info('Will add RT %s to the RI %s'%(user_def_rt, routing_instance))
            vn1_fixture.add_route_target(routing_instance, self.inputs.router_asn, self.inputs.mx_rt)                        
            sleep(10)
            active_ctrl_node= self.get_active_control_node(vm1_fixture)
            peers1= self.cn_inspect[active_ctrl_node].get_cn_rtarget_group(user_def_rt)['peers_interested'] 
            self.logger.info('There are %s peers interested in RT %s'%(len(peers1), user_def_rt))
            ...
            if self.inputs.ext_routers[0][0] in peers1:
                ...
                sleep(10)
                peers2= self.cn_inspect[active_ctrl_node].get_cn_rtarget_group(user_def_rt)['peers_interested'] 
                self.logger.info('After disabling RT_filter, there are %s peers interested in RT %s'%(len(peers2), user_def_rt))
                ...
                sleep(10)
                peers3= self.cn_inspect[active_ctrl_node].get_cn_rtarget_group(user_def_rt)['peers_interested'] 
                self.logger.info('After re-enabling RT_filter, there are %s peers interested in RT %s'%(len(peers3), user_def_rt))
