...
class VerifyAlarms():
    ...
    def verify_alarm_config(self,exp_list=[], update_list=[], parent_type='project', alarm_type='vn_acl_rule', uve_keys=['virtual-network'],alarm_case='basic'):
        ...
        alarm_fix = self.create_alarm(exp_list,alarm_name,self.uve_keys, parent_type=self.parent_type)
        assert alarm_fix.verify_alarm_setup()
        if alarm_type=='vn_acl_rule':
            self.verify_vn_acl_alarm(alarm_fix,update_list,alarm_case,exp_list)
    ...
    def update_alarm_rules_and_verify(self,alarm_fix,update_list,multi_or_conditions=False):
        ...
        assert alarm_fix.set_alarm_rules(update_list,multi_or_conditions),'set alarm rules failed'
        assert alarm_fix.verify_alarm_setup()
        sleep(10)
    ...
    def create_and_verify_vn_acl_config(self,alarm_name,project_fixture=None,project_name=None):
        ...
        sleep(10)
        assert self.analytics_obj.verify_configured_alarm(alarm_name=alarm_name),'Alarm not raised'
        return vn_pol_dict
    ...
    def delete_and_verify_vn_acl_config(self,vn_pol_dict):
        ...
        assert self.analytics_obj.verify_configured_alarm(alarm_name=alarm_name,verify_alarm_cleared=True),'Alarm not cleared'
    
    def verify_vn_acl_alarm(self,alarm_fix,update_list,alarm_type,exp_list):
        if alarm_type=='basic':
            vn_pol_dict = self.create_and_verify_vn_acl_config(alarm_fix.alarm_name)
            vn_pol_dict['alarm_fix'] = alarm_fix
            assert self.update_alarm_rules_and_verify(alarm_fix,update_list),'Alarm not raised'
            self.delete_and_verify_vn_acl_config(vn_pol_dict)
            
        elif alarm_type=='multi_condition':
            self.logger.info('Created alarm_config rules with multiple And conditions')
            vn_pol_dict = self.create_and_verify_vn_acl_config(alarm_fix.alarm_name)
            vn_pol_dict['alarm_fix'] = alarm_fix
            ...
            assert not self.update_alarm_rules_and_verify(alarm_fix,update_list=update_list)
            self.logger.info('Update alarm_config rules with multiple Or conditions')
            assert self.update_alarm_rules_and_verify(alarm_fix,update_list=update_list,multi_or_conditions=True),'Alarm not raised'
            self.delete_and_verify_vn_acl_config(vn_pol_dict)
            ...
            assert not alarm_fix.set_alarm_rules(update_list),'Invalid operation should not be allowed'
            ...
            for i in range(0,scaling_factor):
                ...
                if self.parent_type == 'project':
                    alarm_fix = self.create_alarm(exp_list,alarm_name,self.uve_keys,self.parent_type,project_fixture)
                    assert alarm_fix.verify_alarm_setup()
                vn_pol_dict=self.create_and_verify_vn_acl_config(alarm_fix.alarm_name,project_fixture,project_name)
                vn_pol_dict['alarm_fix']=alarm_fix
