...
class TestLlgrBase(BaseNeutronTest):
    ...
    def setUpClass(cls):
        ...
        cls.set_xmpp_peering(compute_ip=cls.inputs.host_data[cls.host_list[0]]['host_control_ip'] , 
                                 ctrl_node=cls.inputs.bgp_ips[2],mode='disable')
        cls.set_xmpp_peering(compute_ip=cls.inputs.host_data[cls.host_list[1]]['host_control_ip'] , 
                                 ctrl_node=cls.inputs.bgp_ips[2],mode='disable')
    ...
    def tearDownClass(cls):
        ...
        cls.set_xmpp_peering(compute_ip=cls.inputs.host_data[cls.host_list[0]]['host_control_ip'] , 
                                 ctrl_node=cls.inputs.bgp_ips[0],mode='enable')
        cls.set_xmpp_peering(compute_ip=cls.inputs.host_data[cls.host_list[1]]['host_control_ip'] , 
                                 ctrl_node=cls.inputs.bgp_ips[1],mode='enable')
        cls.set_xmpp_peering(compute_ip=cls.inputs.host_data[cls.host_list[0]]['host_control_ip'] , 
                                 ctrl_node=cls.inputs.bgp_ips[2],mode='enable')
        cls.set_xmpp_peering(compute_ip=cls.inputs.host_data[cls.host_list[1]]['host_control_ip'] , 
                                 ctrl_node=cls.inputs.bgp_ips[2],mode='enable')
    ...
    def set_gr_llgr(self,**kwargs):
        ...
        if mode == 'enable':
             cmd = "/opt/contrail/utils/provision_control.py  --api_server_ip %s --api_server_port 8082 \
              --router_asn 64512 --admin_user admin --admin_password %s --admin_tenant_name admin \
              --set_graceful_restart_parameters --graceful_restart_time %s \
              --long_lived_graceful_restart_time %s --end_of_rib_timeout 60 --graceful_restart_enable \
              --graceful_restart_bgp_helper_enable"\
              %(self.inputs.bgp_ips[0],self.inputs.admin_password,gr_timeout,llgr_timeout)
        else :
            cmd = '/opt/contrail/utils/provision_control.py  --api_server_ip %s --api_server_port 8082 \
              --router_asn 64512 --admin_user admin --admin_password %s --admin_tenant_name admin \
              --set_graceful_restart_parameters --graceful_restart_time %s \
              --long_lived_graceful_restart_time %s --end_of_rib_timeout 60 \
              --graceful_restart_bgp_helper_enable'\
              %(self.inputs.bgp_ips[0],self.inputs.admin_password,gr_timeout,llgr_timeout)
        self.logger.info('%s gr and llgr : %s' %(mode,cmd))
    ...
    def set_bgp_peering(self,**kwargs):
        ...
        self.logger.info('%s bgp peering : %s' %(mode,cmd))
        self.inputs.run_cmd_on_server(self.inputs.bgp_ips[0],cmd)
        return True
    ...
    def verify_traffic_loss(self,**kwargs):
        ...
        self.logger.info('results %s' %(res))
        if not res[cmd]:
    ...
    def verify_gr_llgr_flags(self,**kwargs):
        ...
        if not rtbl :
            ...
            return False
        self.logger.info('prefix flags %s' %(rtbl['flags']))
        if flags != rtbl['flags'] :
    ...
    def set_xmpp_peering(self,**kwargs):
        ...
        if mode == 'enable':
            cmd = 'iptables -D INPUT -p tcp --dport 5269 ! -s %s -j DROP' \
                       %(compute_ip)
        else : 
             cmd = 'iptables -A INPUT -p tcp --dport 5269 ! -s %s -j DROP' \
                          %(compute_ip)
        self.logger.info('%s xmpp peering %s : %s' %(mode,ctrl_host,cmd))
        self.inputs.run_cmd_on_server(ctrl_host,cmd)
        return True
    ...
    def set_headless_mode(self,**kwargs):
        ...
        for host in self.host_list:
            self.logger.info('enable headless mode %s : %s' %(host,cmd))
