...
class Evpnt6base(BaseFabricTest):
    ...
    def configure_qfx1(self,vxlan_id,rt_value):
        ...
        #'+mx_lo0_ip+':100'
        cmd.append('set protocols evpn vni-options vni '+str(vxlan_id)+' vrf-target target:64512:'+str(rt_value)+'')
        cmd.append('set protocols evpn encapsulation vxlan')
        cmd.append('set protocols evpn multicast-mode ingress-replication')
        cmd.append('set protocols evpn extended-vni-list all')

        cmd.append('set protocols igmp-snooping vlan vxlan400 proxy')

        cmd.append('set switch-options vtep-source-interface lo0.0')
        cmd.append('set switch-options route-distinguisher 10.204.216.185:1')
        cmd.append('set switch-options vrf-import evpn1-import')
        cmd.append('set switch-options vrf-export evpn1-import')

        cmd.append('set vlans ' + vxlan + ' vlan-id '+str(vxlan_id)+'')
        cmd.append('set vlans '+ vxlan +' vxlan vni '+str(vxlan_id)+'')

        cmd.append('set interfaces xe-0/0/34:1 unit 0 family ethernet-switching interface-mode trunk')
        cmd.append('set interfaces xe-0/0/34:1 unit 0 family ethernet-switching vlan members '+str(vxlan_id)+'')

        cmd.append('set policy-options policy-statement evpn1-import term 2 from community '+vn)
        cmd.append('set policy-options policy-statement evpn1-import term 2 then accept')
        cmd.append('set policy-options community '+vn+' members target:64512:'+str(rt_value)+'')
        cmd.append('set protocols igmp-snooping vlan '+vxlan+' proxy')
        mx_handle = NetconfConnection(host = "10.204.216.185")
        mx_handle.connect()
        cli_output = mx_handle.config(stmts = cmd, timeout = 120)
        mx_handle.disconnect()
    ...
    def flap_bgp(self):
        import pdb;pdb.set_trace()
        cmd = []
        cmd.append('deactivate protocols bgp')
        cmd.append('activate interfaces')
        tors_info_list = self.get_available_devices('tor')
        tor_params = tors_info_list[0]
        mgmt_ip=tor_params['mgmt_ip']
        mx_handle = NetconfConnection(host = mgmt_ip)
        ...
        time.sleep(20)
        cmd = []
        cmd.append('activate protocols bgp')
        cmd.append('activate interfaces')
        time.sleep(10)
        cli_output = mx_handle.config(stmts = cmd, timeout = 120)
        time.sleep(60)
    ...
    def configure_tor(self):
        ...
        for i in range(0, 1):
            ...
            tor_fixture.setUp()
            phy_new_port = tor_fixture.add_physical_port('xe-0/0/34:1')
