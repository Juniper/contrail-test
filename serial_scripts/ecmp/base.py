...
class BaseECMPRestartTest(test_v1.BaseTestCase_v1):
    ...
    def config_basic(self):
        ...
        self.vm2_fixture = self.useFixture(VMFixture(
            project_name=self.inputs.project_name, connections=self.connections,
            vn_objs=[self.vn1_fixture.obj, self.vn2_fixture.obj], vm_name=vm2_name, node_name=None,
            image_name='ubuntu', flavor='m1.tiny'))
        ...
        self.vm5_fixture = self.useFixture(VMFixture(
            project_name=self.inputs.project_name, connections=self.connections,
            vn_objs=[self.vn1_fixture.obj, self.vn2_fixture.obj], vm_name=vm5_name, node_name=None,
            image_name='ubuntu', flavor='m1.tiny'))
        ...
        self.create_static_table(self.vn1_fixture, self.vn2_fixture, self.vm1_fixture, self.vm2_fixture, self.vm5_fixture)
    ...
    def create_static_table(self, src_vn_fix, dst_vn_fix, port1_fix, port2_fix, port3_fix):
    ...
    def send_traffic_and_update_hit_count(self, sender, receiver, sport= '8001', dport= '9001', protocol= 'udp', count = 2):

            self.check_all_ecmpinterfaces()
            self.verify_traffic(sender, receiver, protocol, sport=sport, dport=dport, count = count)
    ...
    def check_all_ecmpinterfaces(self):
        ...
        for vm in [self.vm1_fixture, self.vm2_fixture, self.vm5_fixture]:
            ...
            self.session_item, self.pcap_item = start_tcpdump_for_intf(vm_nodeip, compute_user, compute_password, tap_if_of_vm, filters=filters)
            self.session.append(self.session_item)
            self.pcap.append(self.pcap_item)
            i = i + 1

    #Stop tcpdump on the interface and from tcpdump output, get which ecmp path was taken
    ...
    def send_traffic_and_return_path_taken(self, sender, receiver, sport= '8001', dport= '9001', protocol= 'udp', count = 2):
    ...
    def ecmp_stats_with_hit_count_check(self):
        ...
        self.logger.info("Hit count on first ecmp path : %s" % self.available_ecmp_paths[self.current_tap[0]])
        self.logger.info("Hit count on second ecmp path : %s" % self.available_ecmp_paths[self.current_tap[1]])
        self.logger.info("Hit count on third ecmp path : %s" % self.available_ecmp_paths[self.current_tap[2]])

        assert (self.available_ecmp_paths[self.current_tap[0]] + self.available_ecmp_paths[self.current_tap[1]] + self.available_ecmp_paths[self.current_tap[2]] == 60), 'Traffic not distributed correctly across ecmp paths'
    
    #From introspect output, get the route entry for ECMP route and check for ECMP fields
    def verify_if_hash_changed(self, vn1_fixture, vn2_fixture, vm1_fixture, vm2_fixture, ecmp_hashing_include_fields):
        ...
        next_hops = inspect_h.get_vna_active_route(
                     vrf_id=vn_vrf_id, ip=str(vn2_fixture.get_cidrs()[0])[:-1][:-1][:-1], prefix='24')['path_list'][0]['nh']['mc_list'] 
        ...
        ecmp_field = inspect_h.get_vna_active_route(vrf_id=vn_vrf_id, ip=str(vn2_fixture.get_cidrs()[0])[:-1][:-1][:-1], prefix='24')['path_list'][0]['ecmp_hashing_fields']
        ...
        for node in self.inputs.bgp_ips:
            route_entry = self.cn_inspect[node]._get_if_map_table_entry(vm_uuid1)
