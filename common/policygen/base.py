...
class BasePolicyGenTest(BaseFirewallTest):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            cls.tearDownClass()
            raise
        cls.policy_gen = PolicyGeneratorClient(cls.connections)
        # TODO remove
        #cls.policy_gen = cls.connections.policy_gen
    ...
    def verify_not_in_untagged(self, untagged, traffics):
        # TODO: assumes flow contains local_ip & remote_ip
        if not untagged:
            return True
    ...
    def verify_in_untagged(self, untagged, query, traffics):
        # TODO: assumes flow contains local_ip & remote_ip
    ...
    def find_tag(self, tags, tag_type, tag_value=None):
        for tag in tags:
            if tag.tag_type_name == tag_type:
                if not tag_value:
                    return tag
                if tag.tag_value == tag_value:
                    return tag
        return None

    #TODO see if really needed, testcase can specify the untagged traffic
    def get_untagged_traffics(self, query, grouped_traffics):
        untagged = []
        for traffics in grouped_traffics:
            for traf in traffics:
                next_iter = False
                for vm in [traf.sender_vm, traf.receiver_vm]:
                    tags = self.get_tags(vm, query['tags'])
                    for tag_type in query['tags']:
                        if not self.find_tag(tags, tag_type=tag_type):
                            untagged.append(traf)
                            next_iter = True
                            break
                    if next_iter:
                        break
    ...
    def verify_not_in_aps(self, aps_list, query, not_expected):
    ...
    def verify_in_aps(self, aps_list, query, grouped_traffics):
        class found:
            pass
        for traffics in grouped_traffics:
            if not traffics:
    ...
    def verify_generated_rules(self, result, query, expected=[[]], not_expected=[], expected_untagged=[]):
        ...
        assert self.verify_not_in_untagged(untagged, not_expected), 'unexpected traffic seen in untagged flows'
        #TODO remove
        #assert self.verify_in_untagged(untagged, self.get_untagged_traffics(query, expected)), 'expected traffic not seen in untagged flows'
        assert self.verify_in_untagged(untagged, query, expected_untagged), 'expected traffic not seen in untagged flows'
        self.logger.info('verifying aps')
        assert self.verify_not_in_aps(result['application-policy-sets'], query, not_expected + expected_untagged), 'unexpected traffic seen in aps'
        assert self.verify_in_aps(result['application-policy-sets'], query, expected), 'expected traffic not seen in aps'
