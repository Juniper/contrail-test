...
class BaseVrouterTest(BaseNeutronTest, VerifySvcMirror):
    ...
    def get_cidr_mask(self, fix, ipv6=False):
    ...
    def fat_flow_with_prefix_aggr(self, prefix_length=29, prefix_length6=125,
        inter_node=False, inter_vn=False, ipv6=False, traffic_recvr=False,
        ...
        ipv6 : configures fat flow for both IPv4 and IPv6, use only_v6 flag to configure
        only for IPv6
        ...
        '''
        if resources: # Need to create new fixture objs if resources is set
        ...
        if policy_deny:
            vn_policy = True
        fat_flow = True # To test the scenario without fat flow
        ...
        cidr_mask_dict = self.get_cidr_mask(src_vn_fixture, ipv6=ipv6)
	...
	if vn_policy:
        ...
        if svc_chain:
            ...
            if ff_on_si_vmis:
                ...
                cidr_mask_dict_left = self.get_cidr_mask(src_vn_fixture)
                cidr_left, mask_left, subnet_left = cidr_mask_dict_left.get('v4')

                cidr_mask_dict_right = self.get_cidr_mask(dst_vn_fixture)
