...
class FabricUtils(object):
    ...
    def onboard_existing_fabric(self, fabric_dict, wait_for_finish=True,
        ...
        payload = {'fabric_fq_name': ["default-global-system-config", name],
                   ...
                   'loopback_subnets': [fabric_dict['namespaces']['loopback'][0]["cidr"]],
                   'pnf_servicechain_subnets': [fabric_dict['namespaces']['pnf_servicechain'][0]["cidr"]]
                   }
    ...
    def assign_roles(self, fabric, devices, rb_roles=None, wait_for_finish=True):
        ...
        for device, role in roles_dict.iteritems():
            if role == 'leaf':
                ...
                role = 'leaf'
                routing_bridging_role = rb_roles.get(
                    device.name, ['ERB-UCAST-Gateway'])
            elif role == 'border_leaf':
                role = 'leaf'
                routing_bridging_role = rb_roles.get(
                    device.name, ['CRB-MCAST-Gateway'])
            elif role == 'pnf':
                routing_bridging_role = rb_roles.get(
                    device.name, ['PNF-Servicechain'])
            elif role == 'spine':
                routing_bridging_role = rb_roles.get(
                    device.name, ['CRB-Gateway', 'Route-Reflector'])
