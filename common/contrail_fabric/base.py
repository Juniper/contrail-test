...
class FabricSingleton(FabricUtils, GenericTestBase):
    ...
    def create_ironic_provision_vn(self, admin_connections):
        ...
        self.mtu_list = []
        for node_ip in self.inputs.openstack_ips:
            device = self.inputs.control_data[node_ip]['device']
            output = self.inputs.run_cmd_on_server(node_ip,"ip addr show dev %s"%device)
            ret = re.search('mtu\s+(\d+)',output)
            if ret:
               self.mtu_list.append(ret.group(1))
            else:
               self.mtu_list.append("1500")
            self.inputs.run_cmd_on_server(node_ip,"ip link set mtu 9000 dev %s"%device)
    ...
    def cleanup(self):
        ...
        if self.invoked and getattr(self, 'fabric', None):
            ...
            if getattr(self,'mtu_list',None):
                for k,node_ip in enumerate(self.inputs.openstack_ips):
                    device = self.inputs.control_data[node_ip]['device']
                    self.inputs.run_cmd_on_server(node_ip,"ip link set mtu %s dev %s"%(self.mtu_list[k],device))
