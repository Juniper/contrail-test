...
#                self.leafs2.append(device)
    ...
    def config_dci(self, vn, vn1):
        ...
        vni = str(get_random_vxlan_id(min=10000))
        vni1 = str(get_random_vxlan_id(min=10000))
        lr = self.useFixture(LogicalRouterFixture(
            connections=self.connections,
            connected_networks=vn.uuid, vni=vni))
        lr1 = self.useFixture(LogicalRouterFixture(
            connections=self.connections,
            connected_networks=vn1.uuid, vni=vni1))
        lr.add_physical_router(self.vnc_h.read_physical_router(self.fabric1.devices[0]))
        lr1.add_physical_router(self.vnc_h.read_physical_router(self.fabric2.devices[0]))
