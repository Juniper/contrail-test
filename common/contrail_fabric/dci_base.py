...
#                self.leafs2.append(device)
    ...
    def setup_bare_metal(self, vn, vn1):
        self._interface1 = get_intf_name_from_mac('10.204.217.155','a0:42:3f:01:af:76',username='root',password='c0ntrail123')
        vcmd1 = 'vconfig add %s 100'%self._interface1
        upcmd1 = 'ifconfig %s.100 31.98.158.7 netmask 255.255.255.0 up'%self._interface1
        cmd1.append(vcmd1)
        cmd1.append(upcmd1)
        output = run_cmd_on_server(vcmd1,'10.204.217.155',username='root',password='c0ntrail123')
        output = run_cmd_on_server(upcmd1,'10.204.217.155',username='root',password='c0ntrail123')
        self._interface1 = get_intf_name_from_mac('10.204.217.165','00:e0:81:ce:80:60',username='root',password='c0ntrail123')
        vcmd1 = 'vconfig add %s 100'%self._interface1
        upcmd1 = 'ifconfig %s.100 31.98.158.7 netmask 255.255.255.0 up'%self._interface1
        cmd1.append(vcmd1)
        cmd1.append(upcmd1)
        output = run_cmd_on_server(vcmd1,'10.204.217.165',username='root',password='c0ntrail123')
        output = run_cmd_on_server(upcmd1,'10.204.217.165',username='root',password='c0ntrail123')
