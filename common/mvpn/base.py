...
MX_CONFIG = {
    ...
    'mcast_src': '30.30.30.1',      # Multicast source address behind MX
    'mcast_src_int': 'ge-2/3/9'     # Multicast source interface on MX
}
...
class MVPNTestBase(BaseVrouterTest):
    ...
    def provision_mx(self, mx_ip):
        ...
        mcast_src = MX_CONFIG.get('mcast_src', '0.0.0.0')

        # src interface on MX
        mcast_src_int = MX_CONFIG.get('mcast_src_int', 'ge-0/0/0')
        ...
        cmd.append('set groups mvpn routing-instances test interface '+mcast_src_int+'.0')
...
class IGMPTestBase(BaseVrouterTest):
    ...
    def start_tcpdump_mcast_rcvrs(self, vm_fixtures, traffic, expectation=True):
        ...
        for stream in traffic.values():
            ...
            src_ip = MX_CONFIG.get('src', '30.30.30.1')
            dst_ip = stream['maddr']
    ...
    def send_mcast_stream(self, src_vm_fixture, **kwargs):
        ...
        src = MX_CONFIG.get('src', '30.30.30.1')
