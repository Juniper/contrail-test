    ...
    def stop_traffic(self, traffic_obj, expectation=True, validate=True):
        ...
        if validate:
            if not expectation:
                ...
                if recv*100/float(sent) < 99:
                    assert False, "Packets not %s"%msg
        return (sent, recv)

    def verify_ping(self, src_vm, dst_vm, af=None, expectation=True, size=None):
        assert src_vm.ping_to_vn(dst_vm, af=af, expectation=expectation, size=size)

    def verify_traffic(self, src_vm, dst_vm, proto, sport=0, dport=0,
                       src_vn_fqname=None, dst_vn_fqname=None, af=None,
                       fip_ip=None, expectation=True, size=None):
        if proto == 'icmp':
