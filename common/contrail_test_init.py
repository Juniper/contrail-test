...
class TestInputs(object):
    ...
    def get_ctrl_data_ip(self, host):
        ctrldata = self.config.get('control_data')
        host_name = self.host_data[host]['name']
        ctrldata_ip = None
        if ctrldata and ctrldata.get(host_name):
            ctrldata_ip = ctrldata[host_name]['ctrldata_ip']
        if not ctrldata_ip:
            return
        if self.host_data[host].get('control_data_ip'):
            return self.host_data[host]['control_data_ip']
        self.host_data[host]['control_data_ip'] = ctrldata_ip
        return ctrldata_ip
    ...
    def parse_topo(self):
        ...
        for host, values  in (self.config.get('instances') or {}).iteritems():
            ...
            if control_data_ip:
                host_control_ip = control_data_ip
