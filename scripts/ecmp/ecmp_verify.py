...
class ECMPVerify():
    def get_rt_info_tap_intf_list(self, vn_fixture, vm_fixture, svm_ids):
        ...
        for svm_id in svm_ids:
            ...
            paths= inspect_h9.get_vna_active_route(vrf_id= vn_vrf_id9, ip=left_ip[svm_id], prefix='32')['path_list']
            self.logger.info('There are %s nexthops to %s on Agent %s'%(len(paths),left_ip[svm_id], vm_fixture.vm_node_ip))
            next_hops= inspect_h9.get_vna_active_route(vrf_id= vn_vrf_id9, ip= left_ip[svm_id], prefix='32')['path_list'][0]['nh']
            
            if not next_hops:
                result= False
                assert result, 'Route not found in the Agent %s'%vm_fixture.vm_node_ip
            else:
                self.logger.info('Route found in the Agent %s'%vm_fixture.vm_node_ip)

            if 'mc_list' in next_hops:
                self.logger.info('Composite Next Hops seen')
                multi_next_hops= inspect_h9.get_vna_active_route(vrf_id= vn_vrf_id9, ip= left_ip[svm_id], prefix='32')['path_list'][0]['nh']['mc_list']
                ...
                for nh in multi_next_hops:
                    ...
                    if nh['type'] == 'Tunnel':
                        ...
                        next_hops_in_tnl= inspect_hh.get_vna_active_route(vrf_id= fvn_vrf_id5, ip= left_ip[svm_id], prefix='32')['path_list'][0]['nh']['mc_list']
                ...
                if 'unnel' in next_hops['type']:
                    ...
                    agent_vrf_obj= vm_fixture.get_matching_vrf( agent_vrf_objs['vrf_list'],vn_fixture.vrf_name)
                    fvn_vrf_id5=agent_vrf_obj['ucindex']
                    next_hops_in_tnl= inspect_hh.get_vna_active_route(vrf_id= fvn_vrf_id5, ip= left_ip[svm_id], prefix='32')['path_list'][0]['nh']
                    ...
                    tap_intf_list.append(tap_intf)
            self.logger.info('The list of Tap interfaces from the agents are %s'%tap_intf_list)
