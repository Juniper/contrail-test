...
class TestECMPFeature(BaseECMPTest, VerifySvcFirewall, ECMPSolnSetup, ECMPTraffic, ECMPVerify):
    ...
    def test_ecmp_svc_v2_in_network_nat_with_3_instance(self):
        ...
        if self.inputs.orchestrator != 'vcenter':
            ...
            self.verify_svc_in_network_datapath(
                si_count=1, svc_scaling=True, max_inst=2, svc_mode='in-network-nat', st_version=2, svc_img_name='ubuntu-nat-fw')
    ...
    def test_three_stage_v2_SC(self):
        ...
        if self.inputs.orchestrator == 'vcenter':
            self.verify_multi_inline_svc(
                si_list=[('in-network', 1), ('in-network-nat', 1)], st_version=2, svc_img_name='ubuntu-in-net')
        else:
            self.verify_multi_inline_svc(
