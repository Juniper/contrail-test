...
class TestECMPFeature(BaseECMPTest, VerifySvcFirewall, ECMPSolnSetup, ECMPTraffic, ECMPVerify):
    ...
    def test_ecmp_svc_in_network_with_3_instance_add_flows(self):
        ...
        old_stream1 = Stream(
            proto="icmp", src=self.vm1_fixture.vm_ip,
            dst=self.vm2_fixture.vm_ip, sport=unicode(10000), dport=unicode(11000))
        old_stream2 = Stream(
            proto="udp", src=self.vm1_fixture.vm_ip,
            dst=self.vm2_fixture.vm_ip, sport=unicode(10000), dport=unicode(11000))
        old_stream3 = Stream(
            proto="tcp", src=self.vm1_fixture.vm_ip,
            dst=self.vm2_fixture.vm_ip, sport=unicode(10000), dport=unicode(11000))
...
class TestECMPConfigHashSanity(BaseECMPTest, VerifySvcFirewall, ECMPSolnSetup, ECMPTraffic, ECMPVerify):

    @classmethod
    def setUpClass(cls):
        super(TestECMPConfigHashSanity, cls).setUpClass()
...
class TestECMPConfigHashFeature(BaseECMPTest, VerifySvcFirewall, ECMPSolnSetup, ECMPTraffic, ECMPVerify):
    ...
    def test_ecmp_hash_src_ip(self):
        ...
        # Service VMs will be distributed evenly across all clusters
        max_inst = 3
        ...
        config_level = "vn"
        self.modify_ecmp_config_hash(ecmp_hash=ecmp_hash, config_level=config_level)
    ...
    def test_ecmp_hash_dest_ip(self):
        """
            ...
            Maintainer : cmallam@juniper.net
        """
        # Bringing up the basic service chain setup. i.e 2 VNs (left VN and
        # right VN), 2 VMs (left VM and right VM), service VMs, creating and attaching policy
        # In this case, all service VMs will be launched only on one cluster
        max_inst = 3
        svc_mode = 'in-network-nat'
        svc_img_name = 'tiny_nat_fw'
        st_version = 2
    ...
    def test_ecmp_hash_src_port(self):
        ...
        # Bringing up the basic service chain setup. i.e 2 VNs (left VN and
        # right VN), 2 VMs (left VM and right VM), service VMs, creating and attaching policy
        # In this case, service VMs will be launched as per Nova default behavior
        max_inst = 3
        svc_mode = 'in-network-nat'
        svc_img_name = 'tiny_nat_fw'
        st_version = 2
    ...
    def test_ecmp_hash_dest_port(self):
        ...
        # Bringing up the basic service chain setup. i.e 2 VNs (left VN and
        # right VN), 2 VMs (left VM and right VM), service VMs, creating and attaching policy
        # In this case, service VMs will be distributed evenly across all clusters
        max_inst = 3
        svc_mode = 'in-network-nat'
        svc_img_name = 'tiny_nat_fw'
        st_version = 2
