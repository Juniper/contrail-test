...
class TestVRRPwithSVM(BaseNeutronTest, ECMPTestBase, VerifySvcFirewall):
    ...
    def test_vrrp_with_svm_toggle_admin_port(self):
        ...
        5. Verify vrrp master and backup
        6. Verify ping to master using vIP
        7. Reduce priority of master, so that backup become new master
        8. Verify vrrp master and backup
        9. Verify ping to master using vIP
        Pass criteria: Verify steps 5,6,8 and 9 should pass
        Maintainer : kpatel@juniper.net
        """

        self.logger.info('Derive the virtual ip for vrrp (vIP)')
        left_vn_subnet=self.common_args['left_vn_subnets'][0]
        left_vn_subnet_ip = left_vn_subnet.split('/')
        digits = left_vn_subnet_ip[0].split('.')
        digits[3] = str(int(digits[3]) + 10)
        vIP = '.'.join(digits)
        ...
        self.logger.info('Verify master and backup roles configured as expected')
        assert self.vrrp_mas_bkp_chk_vsrx(src_vm=mgmt_vm_fixture, master_vm=svm0_fix, backup_vm=svm1_fix)
        ...
        self.logger.info('Verify master and backup roles after priority changed')
        assert self.vrrp_mas_bkp_chk_vsrx(src_vm=mgmt_vm_fixture, master_vm=svm1_fix, backup_vm=svm0_fix)
        self.logger.info('Verify ping to vIP responded by master only')
