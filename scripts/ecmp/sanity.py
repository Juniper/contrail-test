...
#            assert result, 'The Tap interface list across agents is incorrect'
    ...
    def test_ecmp_bw_three_vms_same_fip_incr_sip(self):
        ...
        fvn_vm2 = self.useFixture(
            VMFixture(
                ...
                vn_obj=self.res.fvn.obj, ram=4096, image_name='ubuntu-traffic', vm_name='fvn_vm2'))
        assert fvn_vm2.verify_on_setup()
        fvn_vm3 = self.useFixture(
            VMFixture(
                project_name=self.inputs.project_name, connections=self.connections,
                vn_obj=self.res.fvn.obj, ram=4096, image_name='ubuntu-traffic', vm_name='fvn_vm3'))
        ...
        for fvm in fvm_list:
            ...
            tx_local_host[fvm] = Host(
                tx_vm_node_ip[fvm], self.inputs.username, self.inputs.password)
            send_host[fvm] = Host(
    ...
    def test_ecmp_bw_two_vms_same_fip(self):
        ...
        vm1 = self.useFixture(
            VMFixture(
                project_name=self.inputs.project_name, connections=self.connections,
                vn_obj=vn1.obj, ram=4096, image_name='ubuntu-traffic', vm_name='vn_1_vm_1'))
        vm2 = self.useFixture(
            VMFixture(
                project_name=self.inputs.project_name, connections=self.connections,
                vn_obj=vn1.obj, ram=4096, image_name='ubuntu-traffic', vm_name='vn_1_vm_2'))
        fvn_vm1 = self.useFixture(
            VMFixture(
                project_name=self.inputs.project_name, connections=self.connections,
                vn_obj=fvn.obj, ram=4096, image_name='ubuntu-traffic', vm_name='fvn_1_vm1'))
