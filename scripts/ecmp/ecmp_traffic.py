...
class ECMPTraffic(ConfigSvcChain, VerifySvcChain):
    ...
    def verify_traffic_flow(self, src_vm, dst_vm):
        ...
        flow_rec1 = inspect_h100.get_vna_fetchflowrecord(
            vrf=vn_vrf_id, sip=src_vm.vm_ip, dip=dst_vm.vm_ip, sport=src_port, dport=dpi1, protocol='6')
        flow_rec2 = inspect_h100.get_vna_fetchflowrecord(
            vrf=vn_vrf_id, sip=src_vm.vm_ip, dip=dst_vm.vm_ip, sport=src_port, dport=dpi2, protocol='6')
        flow_rec3 = inspect_h100.get_vna_fetchflowrecord(
            vrf=vn_vrf_id, sip=src_vm.vm_ip, dip=dst_vm.vm_ip, sport=src_port, dport=dpi3, protocol='6')
        flow_recs = []
        ...
        for flow_rec in flow_recs:
            ...
            if flow_result is True:
                i += 1
        self.logger.info('%s Flows from %s to %s exist on Agent %s' %
                         (i, src_vm.vm_ip, dst_vm.vm_ip, src_vm.vm_node_ip))
        ...
        if i < 3:
            flow_result = False
        assert flow_result, 'Not all flows from %s to %s not seen on Agent %s' % (
            src_vm.vm_ip, dst_vm.vm_ip, src_vm.vm_node_ip)
