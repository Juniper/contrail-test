...
class IntrospectSslTest(BaseIntrospectSsl):
    ...
    def test_agent_introspect(self):
        ...
        agent_inspect = AgentInspect(host_ip, port, self.logger,
            inputs=self.inputs)
        compute_fixture = ComputeNodeFixture(self.connections, host_ip)

        url_http = 'http://%s:%s' % (host_name, port)
        output_http = self.get_url_and_verify(url_http, agent_inspect)

        self.create_agent_certs_and_update_on_compute(compute_fixture,
    ...
    def test_introspect_cert_without_cn(self):
        ...
        assert (self.create_agent_certs_and_update_on_compute(compute_fixture,
    ...
    def test_introspect_single_set_certs(self):
        ...
        self.inputs.copy_file_to_server(compute_fixture.ip, key, self.cert_location,
            key.split('/')[-1])
        self.inputs.copy_file_to_server(compute_fixture.ip, cert, self.cert_location,
            cert.split('/')[-1])
        self.inputs.copy_file_to_server(compute_fixture.ip, self.ca_cert, self.cert_location,
            self.ca_cert.split('/')[-1])

        self.update_agent_ssl_config(compute_fixture, ssl_enable, key,
            cert, self.ca_cert)
    ...
    def test_introspect_self_signed_cert(self):
        ...
        self.update_agent_ssl_config(compute_fixture, 'true', key,
            cert, cert)
    ...
    def test_analytics_introspect(self):
        ...
        for service in services:
            ...
            self.udpate_config_file_and_restart_service(host_ip,
    ...
    def test_introspect_ssl_negative_cases(self):
        ...
        self.udpate_config_file_and_restart_service(host_ip,
            CONTRAIL_CONF_FILES[service], ssl_enable, server_key,
            server_cert, ca2, service, container)
