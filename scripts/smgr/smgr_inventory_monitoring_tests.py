...
def inventory_tests(self, node_name=None):
    ...
    assertEqual(int(float(clock_speed)), int(inventory_data['ServerInventoryInfo']['cpu_info_state']['clock_speed_MHz']),
        "clock_speed mismatch for node %s = inventory_data - %s, proc-cpuinfo data - %s" 
            % (node_name,int(inventory_data['ServerInventoryInfo']['cpu_info_state']['clock_speed_MHz']),int(float(clock_speed))))
