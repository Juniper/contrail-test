    ...
    def test_diff_proj_same_vn_vm_add_delete(self):
        ...
        for new_tenant in projects:
            try:
                ...
                self.logger.error( 'ClientException:This is because the project info still remains in the API server ==> Bug 744')
...
#getting count of ping response from each vm
    ...
    def test_tcp_transfer_from_fip_vm(self):
        ...
        # backend VN
        vn2_fixture= self.res.vn2_fixture
        vn2_fixture.bind_policies([policy_fixture.policy_fq_name], vn2_fixture.vn_id)
    ...
    def test_control_node_switchover(self):
        ...
        self.logger.info('Active control node from the Agent %s is %s' %(vm1_fixture.vm_node_ip, active_controller_host_ip))
        ...
        self.logger.info('Active control node from the Agent %s is %s' %(vm1_fixture.vm_node_ip, new_active_controller_host_ip))
    ...
    def test_remove_policy_with_ref(self):
        ...
        for policy in pol_list['policys']:
            if policy['name']== policy_name:
                ...
                # In failure screnario clearing the Policy from the VN for further test case
                if policy_removal:
                    vn1_fixture.unbind_policies(vn1_fixture.vn_id,[policy_fixture.policy_fq_name])
                break
