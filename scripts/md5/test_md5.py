...
class TestMd5tests(Md5Base, VerifySecGroup, ConfigPolicy):
    ...
    def test_add_delete_md5(self):
        ...
        for host in list_uuid:
            ...
            self.config_md5( host=host, auth_data=auth_data )
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes should not be up because mx has md5 config" 
        host=list_uuid[1]
        auth_data={'key_items': [ { 'key':"7","key_id":0 } ], "key_type":"md5"}
        self.config_md5(host=host, auth_data=auth_data)
        sleep(95)
        ...
        self.config_md5(host=host, auth_data=auth_data)
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes 2 should not be up as mx still has md5 config"
        ...
        for host in list_uuid:
            ...
            self.config_md5( host=host, auth_data=auth_data )
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes should not be up 3 as mx still has md5 config"
        return True
    #end add_delete_md5
    ...
    def test_different_keys_md5(self):
        ...
        for host in list_uuid:
            ...
            self.config_md5( host=host, auth_data=auth_data )
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes should not be up as mx still has md5 config"
        return True
    ...
    def test_check_per_peer(self):
        ...
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes should not be up as mx still has md5"
        auth_data={'key_items': [ { 'key':"7","key_id":0 } ], "key_type":"md5"}
        host=list_uuid[1]
        notmx=0
        self.per_peer( host=host, auth_data=auth_data, notmx=notmx )
    ...
    def test_add_delete_per_peer(self):
        ...
        self.per_peer( host=host, auth_data=auth_data, notmx=notmx )
        sleep(95)
        assert (self.check_bgp_status()), "BGP between nodes not up after per peer with control node"        
        ...
        self.per_peer( host=host, auth_data=auth_data, notmx=notmx )
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes should not be up after removing per peer with mx"
    ...
    def test_diff_keys_per_peer(self):
        ...
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes not up as mx still has md5"        
        auth_data={'key_items': [ { 'key':"7","key_id":0 } ], "key_type":"md5"}
        host=list_uuid[1]
        notmx=0
        self.per_peer( host=host, auth_data=auth_data, notmx=notmx )
        ...
        self.per_peer( host=host, auth_data=auth_data, notmx=notmx )
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes should not be up key mismatch with mx"
        auth_data={'key_items': [ { 'key':"7","key_id":0 } ], "key_type":"md5"}
        host=list_uuid[1]
        notmx=0
        self.per_peer( host=host, auth_data=auth_data, notmx=notmx )
    ...
    def test_precedence_per_peer(self):
        ...
        self.per_peer( host=host, auth_data=auth_data, notmx=notmx )
        sleep(95)
        assert not (self.check_bgp_status()), "BGP between nodes should not be up after mismatch with mx"
    ...
    def test_iter_keys_per_peer(self):
        ...
        for i in range(1, 11):
            for host in list_uuid:
                ...
                self.config_md5( host=host, auth_data=auth_data )
            sleep(95)
            assert (self.check_bgp_status()), "BGP between nodes should be up as keys are the same everywhere"
            with settings(
                host_string='%s@%s' % (
