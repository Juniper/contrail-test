...
#    @classmethod
    ...
    def test_control_node_restart_and_validate_status_of_the_service(self):
        ...
        for elem in svc_lst:
            ...
            if (self.ds_obj.get_service_status(self.inputs.cfgm_ip,service_touple=elem) == 'up'):
                ...
                self.logger.info("Service %s is down even after service was started"%(elem,))
                result = result and False
                ...
                try:
                    ...
                    for k,v in in_use_after_stop.iteritems():
                        for k1,v1 in in_use_initial.iteritems():
                            if (k1 == k):
                                if (int(v1) - int(v) == 1):
                                    self.logger.info("in-use decremented for %s service-id after %s agent stopped"%(k1,ip))
                                    result=result and True
                                else: 
                                    self.logger.warn("in-use not decremented for %s service-id after %s agent stopped"%(k1,ip))
                                    result=result and False
                    ...
                    for k,v in in_use_after_start.iteritems():
                        for k1,v1 in in_use_after_stop.iteritems():
                            if (k1 == k):
                                if (int(v) - int(v1) == 1):
                                    self.logger.info("in-use incremented for %s service-id after %s agent started"%(k1,ip))
                                    result=result and True
                                else: 
                                    self.logger.warn("in-use not incremented for %s service-id after %s agent started"%(k1,ip))
                                    result=result and False
                    self.logger.info("************ END for %s *************"%(ip))
    ...
    def test_change_parameters_in_discovery_conf(self):
        ...
        try:
            ...
            for elem in svc_lst:
                ip=elem[0]
                if (self.ds_obj.get_service_status(self.inputs.cfgm_ip,service_touple=elem) == 'up'):
                    self.logger.warn("Service %s is still up after 30 secs"%(elem,))
                    result = result and False
                else:
                    self.logger.info("Service %s is down after 30 sec"%(elem,))
                    result = result and True
            #Starting the control node service
            for elem in svc_lst:
                ip=elem[0]
    ...
    def test_publish(self):
        ...
        '''
        self.logger.info("********TEST WILL FAIL IF RAN MORE THAN ONCE WITHOUT CLEARING THE ZOOKEEPER DATABASE*********")
        service='dummy_service23'
        port=658093 
        result = True
        try:
            #Changing the hc_max_miss=3000 and verifying that the services are down after 25 mins
            ...
            for ip in self.inputs.cfgm_ips:
                out_put=self.inputs.run_cmd_on_server(ip,cmd,username='root',password='c0ntrail123')
        ...
        #Bringing up services 
            ...
            if result1:
                self.logger.info("All services correctly received by subscriber")
...
#                svc = self.ds_obj.publish_service_to_discovery(service=service,ip=svc_ip,port=port)
            ...
            for elem in svc:
                ...
                if (ip in (base_ip + str(x) for x in range(1,101))):
                    ...
                    self.logger.warn("%s is NOT added to discovery service"%(elem,))
                    result=result and False
            ...
            if len(resp)< 100:
                result=result and False
                self.logger.warn("Not all services returned")
            self.logger.info ("Sending 100 subscription message to discovery..")
            subs_threads = []
            for i in range(100):
                cuuid= uuid.uuid4()
