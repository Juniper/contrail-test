...
#            assert result
    ...
    def test_zookeeper_states(self):
        ...
        try:
            ...
            if count_leader > 1:
                self.logger.warn(
                    "There are more than one zookeeper leader in this setup")
                result = result and False
            # Bringing down one follower and verifying leader still remains and
            ...
            self.inputs.run_cmd_on_server(
                follower_ip, 'service zookeeper stop', password='c0ntrail123')
            time.sleep(10)
            # Verifying leader is still up and running
            leader_zoo_keeper_status = self.ds_obj.get_zookeeper_status(
                ip=leader_ip)
...
#                        assert self.ds_obj.verify_registered_services_to_discovery_service()
                    ...
                    self.logger.warn("Leader NOT up : %s" %
                                     (str(leader_zoo_keeper_status)))
                    result = result and False
        except Exception as e:
            ...
            for ip in self.inputs.cfgm_ips:
                self.inputs.run_cmd_on_server(
                    ip, 'service zookeeper start', password='c0ntrail123')
            time.sleep(13)
            for ip in self.inputs.cfgm_ips:
                self.logger.info("Verifying for ip %s" % (ip))
                assert self.ds_obj.verify_registered_services_to_discovery_service(
