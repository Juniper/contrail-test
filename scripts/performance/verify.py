...
class PerformanceTest(ConfigPerformance, ConfigSvcChain):
    ...
    def test_ping_latency(self, no_of_vn=1, no_of_pkt=1):
        ...
        if no_of_vn == 2:
            ...
            if getattr(self, 'res', None):
                ...
                if len(host_list) > 1:
                    ...
                    compute_2 = host_list[1]
                self.vm1_fixture = self.config_vm(
                    self.vn1_fixture, self.vm1_name, node_name=compute_1, image_name='ubuntu-traffic')
                self.vm2_fixture = self.config_vm(
                    self.vn2_fixture, self.vm2_name, node_name=compute_2, image_name='ubuntu-traffic')
            ...
            if getattr(self, 'res', None):
                ...
                self.vm1_fixture = self.config_vm(
                    self.vn1_fixture, self.vm1_name, node_name=compute_1, image_name='ubuntu-traffic')
                self.vm2_fixture = self.config_vm(
                    self.vn1_fixture, self.vm2_name, node_name=compute_2, image_name='ubuntu-traffic')
        ...
        if errmsg:
            #assert False, errmsg
            self.logger.info(
                "This test wont fail; until we identify a number for ping latency.")
            self.logger.error(errmsg)

        return True

    def test_check_flow_setup_within_vn(
        self, dst_port_min, dst_port_max, src_port_min, src_port_max, no_of_vn=1,
        ...
        fr.write(
            (template.safe_substitute({
                '__pkt_size__': pkt_size, '__count__': no_of_flows, '__dst_ip__': vm2_ip, '__src_ip__': vm1_ip,
        ...
        with hide('everything'):
            with settings(
                ...
                self.logger.info(
                    "Start flow -r to monitor the flow setup rate")
                run('flow -r >> /tmp/flow_rate &', pty=False)
        # start the tcpdump on the tap interface of sender and receiver
        # computes.
        ...
        self.logger.info("sender_count: %s, rcvr_count: %s" %
                         (sender_count, rcvr_count))
