...
def preposttest_wrapper(function):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if vr_mem_stats != {}: 
            do_mem_check= True; log.info("vr_mem_stats: %s" %vr_mem_stats) 
        else: 
            do_mem_check= False
            log.error ("VR mem stats not available, skipping check, needs to be debugged...")
        if do_mem_check:
            vr_mem_pre_test= {}
            for i,j in vr_mem_stats.items(): vr_mem_pre_test[i]= j
        ...
        try:
            ...
            if do_mem_check:
                ...
                vr_mem_increase_pct= {}
                for i in vr_list:
                    vr_mem_increase_pct[i]= ((float(vr_mem_post_test[i] - vr_mem_pre_test[i]))/float(vr_mem_pre_test[i]))*100
                    vr_mem_increase_pct[i]= round(vr_mem_increase_pct[i],2)
                    log.info("vr_mem_increase_pct for %s is %.2f" %(i, vr_mem_increase_pct[i]))
                    if vr_mem_increase_pct[i] > 10: 
                        ...
                        log.warn(err_msg)
                    if vr_mem_increase_pct[i] > 30: log.error(err_msg); assert False, err_msg
            ...
            if crashes:
                ...
                #Preserve this msg format, it is used by tcutils.contrailtestrunner
                msg = "Contrail service crashed(%s): %s" % (crash_count, crashes)
                log.error(msg)
                errmsg.append(msg)
