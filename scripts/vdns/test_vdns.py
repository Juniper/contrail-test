    ...
    def test_agent_crash_dns_malformed_received(self):
        ...
        if cores:
                for node, corelist in cores.items():
                    new_core_count += len(corelist)
                err_msg = "New crashes has happenned during this test case"
                self.logger.error("The total number of new cores observed are %d" % new_core_count)
