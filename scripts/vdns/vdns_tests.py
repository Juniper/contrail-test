...
#    @classmethod
    ...
    def test_vdns_ping_same_vn(self):
        ...
        project_fixture = self.useFixture(ProjectFixture(vnc_lib_h= self.vnc_lib, project_name= self.inputs.project_name, connections=self.connections))
        proj_fixt = self.useFixture(ProjectTestFixtureGen(self.vnc_lib, project_name = 'admin'))
        dns_data = VirtualDnsType(domain_name=domain_name,dynamic_records_from_client = True,default_ttl_seconds=ttl,record_order='random')
        # Create VDNS server object.
        vdns_fixt1 = self.useFixture(VdnsFixture(self.inputs,self.connections,vdns_name=dns_server_name,dns_data = dns_data))
        result, msg = vdns_fixt1.verify_on_setup()
        self.assertTrue(result,msg)
        dns_server = IpamDnsAddressType(virtual_dns_server_name = vdns_fixt1.vdns_fq_name)
        ipam_mgmt_obj = IpamType(ipam_dns_method='virtual-dns-server',ipam_dns_server=dns_server)
        # Associate VDNS with IPAM.
        ipam_fixt1 = self.useFixture(NetworkIpamTestFixtureGen(self.vnc_lib,virtual_DNS_refs = [vdns_fixt1.obj],parent_fixt=proj_fixt,network_ipam_name=ipam_name,network_ipam_mgmt=ipam_mgmt_obj))
        vn_nets = {
                'vn1-vdns': [(ipam_fixt1.getObj(),VnSubnetsType([IpamSubnetType(subnet = SubnetType(vn1_ip, 24))]))],
        }
        # Launch VN with IPAM
        vn_fixt = self.useFixture(VirtualNetworkTestFixtureGen(self.vnc_lib,virtual_network_name=vn_name,network_ipam_ref_infos = vn_nets[vn_name],parent_fixt=proj_fixt,id_perms=IdPermsType(enable=True)))
        vm_fixture = {}
        # Launch  VM with VN Created above. This test verifies on launch of VM agent should updated DNS 'A' and 'PTR' records
        # The following code will verify the same. Also, we should be able ping with VM name.
        for vm_name in vm_list:
            vn_quantum_obj= self.quantum_fixture.get_vn_obj_if_present(vn_fixt._name)
            vm_fixture[vm_name] = self.useFixture(VMFixture(project_name= self.inputs.project_name,connections= self.connections, vn_obj= vn_quantum_obj,vm_name=vm_name))
            ...
            vm_ip = vm_fixture[vm_name].get_vm_ip_from_vm(vn_fq_name=vm_fixture[vm_name].vn_fq_name)
            vm_rev_ip = vm_ip.split('.')
            vm_rev_ip = '.'.join((vm_rev_ip[3],vm_rev_ip[2],vm_rev_ip[1],vm_rev_ip[0]))
            vm_rev_ip = vm_rev_ip + '.in-addr.arpa'
            msg = "Ping by using name %s is failed. Dns server should resolve VM name to IP" %(vm_name)
            self.assertTrue(vm_fixture[vm_name].ping_with_certainty(ip = vm_name),msg)
            # Frame the Expected DNS data for VM, one for 'A' record and another 'PTR' record.
            rec_name = vm_name + "."+ domain_name            
            vm_dns_exp_data = [ { 'rec_data': vm_ip,'rec_type':'A','rec_class': 'IN','rec_ttl': str(ttl),'rec_name':rec_name,'installed': 'yes','zone':domain_name }, { 'rec_data': rec_name,'rec_type': 'PTR','rec_class': 'IN','rec_ttl': str(ttl),'rec_name':vm_rev_ip,'installed': 'yes','zone': rev_zone } ]
            self.verify_vm_dns_data(vm_dns_exp_data)
            vm_dns_exp_data = []
        # ping between two vms which are in same subnets by using name.
        self.assertTrue(vm_fixture['vm1-test'].ping_with_certainty(ip =vm_list[1]))
        #delete VDNS with ipam as back refrence.
        self.logger.info("Try deleting the VDNS entry %s with back ref of ipam.", dns_server_name)
        try:
            self.vnc_lib.virtual_DNS_delete(fq_name=vdns_fixt1.obj.get_fq_name())
            errmsg = "VDNS entry deleted which is not expected, when it has back refrence of ipam."
            self.logger.error(errmsg)
            assert False, errmsg
        except Exception, msg:
            self.logger.info(msg)
            self.logger.info("Deletion of the vdns entry failed with back ref of ipam as expected")
        # Add VDNS record 'CNAME' and add it to VDNS and ping with alias for vm1-test
        self.logger.info('Add CNAME VDNS record for vm1-test and verify we able to ping by alias name')
        vdns_rec_data = VirtualDnsRecordType(cname_rec, 'CNAME', 'IN', 'vm1-test',ttl)
        vdns_rec_fix = self.useFixture(VdnsRecordFixture(self.inputs,self.connections,'test-rec',vdns_fixt1.vdns_fix,vdns_rec_data))
        result, msg = vdns_rec_fix.verify_on_setup()
        self.assertTrue(result,msg)
        self.assertTrue(vm_fixture['vm1-test'].ping_with_certainty(ip =cname_rec))        
        return True
    ...
    def test_vdns_ping_diff_vn(self):
        ...
        # create policy
        rules= {}
        rules[policy_name]= [ PolicyRuleType(direction='<>', protocol='icmp', dst_addresses= [AddressType(virtual_network ='any')],src_addresses=[AddressType(virtual_network = 'local')], action_list = ActionListType(simple_action='pass'), src_ports=[PortType(-1, -1)],dst_ports = [PortType(-1, -1)])]
        policy_fixt = self.useFixture( NetworkPolicyTestFixtureGen(self.vnc_lib, network_policy_name = policy_name,parent_fixt = proj_fixt,network_policy_entries=PolicyEntriesType(rules[policy_name])))
        policy_ref = [(policy_fixt.getObj(), VirtualNetworkPolicyType(sequence=SequenceType(major=0, minor=0)))]
        ...
        for vm_name in vm_list:
            ...
            vn_fixt[vm_name] = self.useFixture(VirtualNetworkTestFixtureGen(self.vnc_lib,virtual_network_name=vm_vn_list[vm_name],network_ipam_ref_infos = vn_nets[vn],parent_fixt=proj_fixt,id_perms=IdPermsType(enable=True),network_policy_ref_infos=policy_ref))
            ...
            self.assertTrue(self.verify_ns_lookup_data(vm_fixture[vm_name],cmd,exp_data),msg)
        ...
        self.logger.info('Add VDNS record and verify TTL value is set correctly using with dig command')
        ...
        for ttl_mod in ttl_list:
            ip_add = '1.1.1.'+ str(i)
            # Already configured TTL as a 100, so not configuring TTL value for first time
            if ttl_mod != 100:
                vdns_rec_data = VirtualDnsRecordType('rec1', 'A', 'IN',ip_add,ttl_mod)
                ...
                try:
                    ...
                    if ( ttl_mod == -1 or ttl_mod == 2147483648):
                        self.logger.info('Failed to configure invalid TTL values as expected')
                        continue
                    else:
            ...
            m_obj = re.search(r"rec1.juniper.net\.*\s*([0-9.]*)\s*IN\s*A\s*([0-9.]*)",result)
            if not m_obj:
                self.assertTrue(False,'record search is failed,please check syntax of regular expression')
            print ("\nTTL VALUE is %s ", m_obj.group(1))
            print ("\nrecord ip address is %s",m_obj.group(2))
            self.assertEqual(int(m_obj.group(1)),ttl_mod,'TTL value is not matching for static record after record modification')
            self.assertEqual(m_obj.group(2),ip_add,'IP Address is not matching for static record after record modification')
            i = i + 1
        return True
    ...
    def test_vdns_with_next_vdns(self):
        ...
        ttl = 100
        vm1_ping_list = [vm_list[0]+ "."+ domain_name1,vm_list[1]+ "."+ domain_name2,vm_list[2]+ "."+ domain_name3]
        vm2_ping_list = [vm_list[1]+ "."+ domain_name2,vm_list[0]+ "."+ domain_name1]
        vm3_ping_list = [vm_list[2]+ "."+ domain_name3,vm_list[0]+ "."+ domain_name1]
        vm_domain_list = {vm_list[0]:vm1_ping_list,vm_list[1]:vm2_ping_list,vm_list[2]:vm3_ping_list}

        project_fixture = self.useFixture(ProjectFixture(vnc_lib_h= self.vnc_lib, project_name= self.inputs.project_name, connections=self.connections))
        proj_fixt = self.useFixture(ProjectTestFixtureGen(self.vnc_lib, project_name = 'admin'))
        dns_server_name_list = ['vdns1','vdns2','vdns3']
        domain_name_list = {'vdns1':'juniper.net','vdns2':'bng.juniper.net','vdns3':'eng.juniper.net'}
        ...
        for dns_name in dns_server_name_list:
            # VDNS1
            if dns_name == 'vdns1':
                vdns_data[dns_name]= VirtualDnsType(domain_name=domain_name_list[dns_name],dynamic_records_from_client = True,default_ttl_seconds=ttl,record_order='random')
            else:
                # VDNS2 and VDNS3 need to point VDNS1 as next vdns server. 
                vdns_data[dns_name]= VirtualDnsType(domain_name=domain_name_list[dns_name],dynamic_records_from_client = True,default_ttl_seconds=ttl,record_order='random',next_virtual_DNS=vdns_fix['vdns1'].vdns_fq_name)
            vdns_fix[dns_name] = self.useFixture(VdnsFixture(self.inputs,self.connections,vdns_name=dns_name,dns_data = vdns_data[dns_name]))
            result, msg = vdns_fix[dns_name].verify_on_setup()
            self.assertTrue(result,msg)

        # Try to delete vdns entry which was referenced in other vdns entry, deletion should fail.
        self.logger.info("Try deleting the VDNS entry %s with back ref.", dns_server_name1)
        try:
            self.vnc_lib.virtual_DNS_delete(fq_name=vdns_fix[dns_server_name1].obj.get_fq_name())
            errmsg = "VDNS entry deleted which is not expected, when it is attached to a other vdns servers."
            self.logger.error(errmsg)
            assert False, errmsg
        except Exception, msg:
            self.logger.info(msg)
            self.logger.info("Not able to delete the vdns entry with back ref as expected")
        # In VDNS1 need to be added 'NS' records to delegate a subdomain to VDNS2 and VDNS3.
        for dns_name in dns_server_name_list:
            if dns_name != 'vdns1':
                vdns_rec_data = VirtualDnsRecordType(domain_name_list[dns_name], 'NS', 'IN',vdns_fix[dns_name].vdns_fq_name,ttl)
                vdns_rec[dns_name] = self.useFixture(VdnsRecordFixture(self.inputs,self.connections,rec_names[dns_name],vdns_fix['vdns1'].vdns_fix,vdns_rec_data))
        ...
        for ipam in ipam_dns_list:
            dns_server = IpamDnsAddressType(virtual_dns_server_name = vdns_fix[ipam].vdns_fq_name)
            ipam_mgmt_obj = IpamType(ipam_dns_method='virtual-dns-server',ipam_dns_server=dns_server)
            ipam_fixt[ipam] = self.useFixture(NetworkIpamTestFixtureGen(self.vnc_lib,virtual_DNS_refs = [vdns_fix[ipam].obj],parent_fixt=proj_fixt,network_ipam_name=ipam_dns_list[ipam],network_ipam_mgmt=ipam_mgmt_obj))
        
        rules= {}
        rules[policy_name]= [ PolicyRuleType(direction='<>', protocol='icmp', dst_addresses= [AddressType(virtual_network ='any')],src_addresses=[AddressType(virtual_network = 'any')], action_list = ActionListType(simple_action='pass'), src_ports=[PortType(-1, -1)],dst_ports = [PortType(-1, -1)]) ]
        policy_fixt = self.useFixture( NetworkPolicyTestFixtureGen(self.vnc_lib, network_policy_name = policy_name,parent_fixt = proj_fixt,network_policy_entries=PolicyEntriesType(rules[policy_name])))
        policy_ref = [(policy_fixt.getObj(), VirtualNetworkPolicyType(sequence=SequenceType(major=0, minor=0)))]
        ...
        for vm_name in vm_list:
            ...
            for cmd in vm_ping_list:
                self.assertTrue(vm_fixture[vm_name].ping_with_certainty(ip = cmd))

        # Try to delete vdns entry which was referenced in other vdns entry, deletion should fail.
        self.logger.info("Try deleting the VDNS entry %s with back ref of vdns records.", dns_server_name1)
        try:
            self.vnc_lib.virtual_DNS_delete(fq_name=vdns_fix[dns_server_name1].obj.get_fq_name())
            ...
            self.logger.info(msg)
            self.logger.info("Not able to delete the vdns entry with back ref of vdns records")
        return True 
    ...
    def vdns_with_cn_dns_agent_restart(self,restart_process):
        ...
        if restart_process == 'ControlNodeRestart':
            if len(set(self.inputs.bgp_ips)) < 2 :
                raise self.skipTest("Skiping Test. At least 2 control nodes required to run the control node switchover test")
        vn1_ip = '10.10.10.1'
        vm_list = ['vm1-test','vm2-test']
        vn_name ='vn1'
        dns_server_name = 'vdns1'
        ...
        for vm_name in vm_list:
            ...
            # Frame the Expected DNS data for VM, one for 'A' record and another 'PTR' record.
            rec_name = vm_name + "."+ domain_name 
            vm_dns_exp_data[vm_name] = [ { 'rec_data': vm_ip,'rec_type':'A','rec_class': 'IN','rec_ttl': str(ttl),'rec_name':rec_name,'installed': 'yes','zone':domain_name }, { 'rec_data': rec_name,'rec_type': 'PTR','rec_class': 'IN','rec_ttl': str(ttl),'rec_name':vm_rev_ip,'installed': 'yes','zone': rev_zone } ]
            self.verify_vm_dns_data(vm_dns_exp_data[vm_name])
        # ping between two vms which are in same subnets by using name.
        self.assertTrue(vm_fixture['vm1-test'].ping_with_certainty(ip =vm_list[1]))
        active_controller = vm_fixture['vm1-test'].get_active_controller()
        self.logger.info('Active control node from the Agent %s is %s' %(vm_fixture['vm1-test'].vm_node_ip, active_controller))
        # Control node restart/switchover.
        if restart_process == 'ControlNodeRestart':
            # restart the Active control node
            self.logger.info('restarting active control node')
            self.inputs.restart_service('contrail-control',[active_controller])
            sleep(5) 
            # Check the control node shifted to other control node
            new_active_controller = vm_fixture['vm1-test'].get_active_controller()
            self.logger.info('Active control node from the Agent %s is %s' %(vm_fixture['vm1-test'].vm_node_ip, new_active_controller))
            if new_active_controller == active_controller:
                self.logger.error('Control node switchover fail. Old Active controlnode was %s and new active control node is %s'%(active_controller, new_active_controller))
                return False
            self.inputs.restart_service('contrail-control',[new_active_controller])
        if restart_process == 'DnsRestart':
            # restart the dns process in the active control node
            self.logger.info('restart the dns process in the active control node')
        ...
        if restart_process == 'NamedRestart':
            ...
            self.logger.info('restart the named process in the active control node')
        ...
        # Verify after controlnode/dns/agent/named process restart ping vm's by using name.
        for vm_name in vm_list:
            msg = "Ping by using name %s is failed after controlnode/dns/agent/named process restart. Dns server should resolve VM name to IP" %(vm_name)
            self.assertTrue(vm_fixture[vm_name].ping_with_certainty(ip = vm_name),msg)
            self.verify_vm_dns_data(vm_dns_exp_data[vm_name])
        return True
    ...
    def verify_dns_record_order(self,record_order,test_type='test_record_order',record_num=10):
        ...
        project_fixture = self.useFixture(ProjectFixture(vnc_lib_h= self.vnc_lib, project_name= self.inputs.project_name, connections=self.connections))
        proj_fixt = self.useFixture(ProjectTestFixtureGen(self.vnc_lib, project_name = 'admin'))
        dns_data =VirtualDnsType(domain_name=domain_name,dynamic_records_from_client = True,default_ttl_seconds=ttl,record_order=record_order)
        # Create VDNS server object.
        vdns_fixt1 = self.useFixture(VdnsFixture(self.inputs,self.connections,vdns_name=dns_server_name,dns_data = dns_data))
        result, msg = vdns_fixt1.verify_on_setup()
        self.assertTrue(result,msg)
        ...
        if test_type == 'recordscaling':
            ...
            for rec in verify_rec_name_list:
                ...
                result = result.replace("\t", "")
                result = result.replace("\n", " ")
                m_obj = re.search(r"Address:[0-9.]*#[0-9]*\s*.*Name:(.*\.juniper\.net)\s*Address:\s*([0-9.]*)",result)
                if not m_obj:
                    #import pdb; pdb.set_trace()
                    self.assertTrue(False,'record search is failed,please check syntax of the regular expression/NSlookup is failed')
                print ('vm_name is ---> %s \t ip-address is ---> %s' %(m_obj.group(1),m_obj.group(2)))
        else:
            for num in range(1,record_num):
                rec = 'test-rec-' + str(j) +'-'+ str(i)
                rec_ip = '1.'+'1.'+ str(j) +'.'+ str(i)
                vdns_rec_data = VirtualDnsRecordType('test1','A','IN',rec_ip,ttl)
                vdns_rec_fix= self.useFixture(VdnsRecordFixture(self.inputs,self.connections,rec,vdns_fixt1.vdns_fix,vdns_rec_data))
                result, msg = vdns_rec_fix.verify_on_setup()
                i = i + 1 
                if i > 253:
            ...
            result = result.replace("\n", " ")
            import re
            m_obj = re.search(r"Address:[0-9.]*#[0-9]*\s*Name:test1.juniper.net\s*(Address:\s*[0-9.]*)",result)
            if not m_obj:
                self.assertTrue(False,'record search is failed,please check syntax of regular expression')
            print m_obj.group(1)
            ...
            for rec in rec_ip_list:
                ...
                if record_order == 'round-robin':
                    if next_ip != dns_record_ip1:
                        ...
                        self.assertTrue(False,'VDNS records are not sent in round-robin order')
                ...
                if record_order == 'random':
                    if dns_record_ip1 not in rec_ip_list:
                        ...
                        self.assertTrue(False,'VDNS records are not sent random order')
                if record_order == 'fixed':
                    if dns_record_ip != dns_record_ip1:
                        print "\n VDNS records are not sent fixed in fixed order"
                        self.assertTrue(False,'VDNS records are not sent fixed in fixed order')
        return True
    ...
    def test_vdns_with_fip(self):
        ...
        for vm_name in vm_list:
            vn = vm_vn_list[vm_name] 
            vn_fixt[vm_name]=self.useFixture( VNFixture(project_name= self.inputs.project_name, connections= self.connections,
                vn_name=vm_vn_list[vm_name], inputs= self.inputs, subnets=vn_nets[vn], ipam_fq_name = ipam_fixt1.getObj().get_fq_name()))
            vm_fixture[vm_name] = self.useFixture(VMFixture(project_name= self.inputs.project_name,connections= self.connections, vn_obj= vn_fixt[vm_name].obj,vm_name=vm_name))
            vm_fixture[vm_name].verify_vm_launched()
            vm_fixture[vm_name].verify_on_setup()
            self.nova_fixture.wait_till_vm_is_up( vm_fixture[vm_name].vm_obj )
        ...
        fip_id1= fip_fixture1.create_and_assoc_fip(vn_fixt['vm2-test'].vn_id, vm_fixture['vm1-test'].vm_id)
        self.addCleanup( fip_fixture1.disassoc_and_delete_fip, fip_id1)
        assert fip_fixture1.verify_fip( fip_id1, vm_fixture['vm1-test'], vn_fixt['vm2-test'])        
        # ping between two vms which are in different subnets by using name.
        self.assertTrue(vm_fixture['vm1-test'].ping_with_certainty(ip = vm_list[1]),"Ping with VM name failed for VDNS with floating ip")
        return True
    ...
    def test_vdns_with_diff_projs(self):
        ...
        vn_nets = {'project1':['10.10.10.0/24'],'project2':['20.20.20.0/24']}
        ...
        dns_server = IpamDnsAddressType(virtual_dns_server_name = vdns_fixt1.vdns_fq_name)
        ipam_mgmt_obj = IpamType(ipam_dns_method='virtual-dns-server',ipam_dns_server=dns_server)
        ipam_fixt = {};vn_fixt = {};vm_fix={};pol_fixt={}
        rules = {'project1':[{'direction': '<>', 'protocol': 'any', 'dest_network': 'default-domain:project2:vn2', 'source_network': 'any', 'dst_ports': 'any', 'simple_action': 'pass', 'src_ports': 'any'}],'project2': [{'direction': '<>', 'protocol': 'any', 'dest_network': 'default-domain:project1:vn1', 'source_network': 'any', 'dst_ports': 'any', 'simple_action': 'pass', 'src_ports': 'any'} ] }
        admin_ip = self.inputs
        admin_con = self.connections
        for proj in project_list:
            # Project creation
            project_fixture = self.useFixture(ProjectFixture(project_name = proj,vnc_lib_h= self.vnc_lib,username=proj_user[proj],
            ...
            # Ipam creation
            ipam_fixt[proj] = self.useFixture(NetworkIpamTestFixtureGen(self.vnc_lib,virtual_DNS_refs =[vdns_fixt1.obj],
                parent_fixt=proj_fixt,network_ipam_name=ipam_list[proj],network_ipam_mgmt=ipam_mgmt_obj))
            # VN Creation
            vn_fixt[proj] = self.useFixture( VNFixture(project_name= proj, connections= project_connections,
            ...
            # VM creation
            vm_fix[proj] = self.useFixture(VMFixture(project_name= proj,connections= project_connections, vn_obj=vn_fixt[proj].obj,
            ...
            msg = "Ping by using name %s is failed. Dns server should resolve VM name to IP" %(vm_list[proj])
            self.assertTrue(vm_fix[proj].ping_with_certainty(ip = vm_list[proj]),msg)
            vm_ip = vm_fix[proj].get_vm_ip_from_vm(vn_fq_name=vm_fix[proj].vn_fq_name)
            vm_rev_ip = vm_ip.split('.')
            vm_rev_ip = '.'.join((vm_rev_ip[3],vm_rev_ip[2],vm_rev_ip[1],vm_rev_ip[0]))
            vm_rev_ip = vm_rev_ip + '.in-addr.arpa'
            ...
            vm_dns_exp_data = []
        # ping between two vms which are in different subnets by using name.
        self.assertTrue(vm_fix['project1'].ping_with_certainty(ip=vm_list['project2']),"Ping with VM name failed for VDNS across the projects")
        return True
    @preposttest_wrapper
    def test_vdns_default_mode(self):
        ...
        # Associate VDNS with IPAM.
        ipam_fixt1 = self.useFixture(NetworkIpamTestFixtureGen(self.vnc_lib,parent_fixt=proj_fixt,network_ipam_name=ipam_name,network_ipam_mgmt=ipam_mgmt_obj))
        vn_fixt= self.useFixture( VNFixture(project_name= self.inputs.project_name, connections= self.connections,
            vn_name=vn_name, inputs= self.inputs, subnets=vn_nets[vn_name], ipam_fq_name = ipam_fixt1.getObj().get_fq_name()))
        vm_fix =self.useFixture(VMFixture(project_name=self.inputs.project_name,connections= self.connections,vn_obj=vn_fixt.obj,
            vm_name=vm_name))
        ...
        fip_id= fip_fixture.create_and_assoc_fip( fvn_fixture.vn_id, vm_fix.vm_id)
        ...
        for entry in self.inputs.bgp_ips:
            ...
            cn_fixture1= self.useFixture(CNFixture(connections= self.connections,router_name=hostname, router_ip= entry, router_type = 'contrail', inputs= self.inputs))
            cn_fixturemx= self.useFixture(CNFixture(connections= self.connections,router_name=router_name, router_ip= router_ip, router_type = 'mx', inputs= self.inputs))
        sleep(5)
        assert cn_fixturemx.verify_on_setup()
        #DNS methos configued is default, DNS should resolve for external DNS lookups.
        ...
        if not m_obj:
            self.assertTrue(False,'record search is failed,please check syntax of the regular expression/NSlookup is failed')
        print m_obj.group(1)
        # Ipam DNS mentod is set to default, so DNS resolution to external world needs to be resolved.
        self.assertTrue(vm_fix.ping_with_certainty(ip='juniper.net'),"DNS name resolution failed when vdns set to default DNS method")
    ...
    def test_vdns_tree_scaling(self):
        ...
        project_fixture = self.useFixture(ProjectFixture(vnc_lib_h= self.vnc_lib, project_name= self.inputs.project_name, connections=self.connections))
        proj_fixt = self.useFixture(ProjectTestFixtureGen(self.vnc_lib, project_name = 'admin'))
        dns_server_name_list = ['vdns1','vdns2','vdns3','vdns4','vdns5','vdns6','vdns7','vdns8','vdns9','vdns10','vdns11','vdns12','vdns13','vdns14','vdns15','vdns16']
        domain_name_list = {
        'vdns1':'juniper.net',
        ...
        next_vdns_list = {'vdns1':'vdns2','vdns2':'vdns3','vdns3':'vdns4','vdns4':'vdns5','vdns5':'vdns6','vdns6':'vdns7','vdns7':'vdns8','vdns8':'vdns9','vdns9':'vdns10','vdns10':'vdns11','vdns11':'vdns12','vdns12':'vdns13','vdns13':'vdns14','vdns14':'vdns15','vdns15':'vdns16','vdns16':'none'}  
        rec_names = {'vdns1':'test-rec1','vdns2':'test-rec2','vdns3':'test-rec3','vdns4':'test-rec4','vdns5':'test-rec5','vdns6':'test-rec6','vdns7':'test-rec7','vdns8':'test-rec8','vdns9':'test-rec9','vdns10':'test-rec10','vdns11':'test-rec11','vdns12':'test-rec12','vdns13':'test-rec13','vdns14':'test-rec14','vdns15':'test-rec15','vdns16':'test-rec16'}
        ...
        for dns_name in dns_server_name_list:
            ...
            if dns_name == 'vdns1':
                ...
                vdns_data[dns_name]= VirtualDnsType(domain_name=domain_name_list[dns_name],dynamic_records_from_client = True,default_ttl_seconds=ttl,record_order='random',next_virtual_DNS=next_dns.vdns_fq_name)
            vdns_fix[dns_name] = self.useFixture(VdnsFixture(self.inputs,self.connections,vdns_name=dns_name,dns_data = vdns_data[dns_name]))
            result, msg = vdns_fix[dns_name].verify_on_setup()
            self.assertTrue(result,msg)
        ...
        for dns_name in dns_server_name_list:
            if next_vdns_list[dns_name] != 'none':
                ...
                vdns_rec_data = VirtualDnsRecordType(domain_name_list[next_dns], 'NS', 'IN',vdns_fix[next_dns].vdns_fq_name,ttl)
                vdns_rec[dns_name] = self.useFixture(VdnsRecordFixture(self.inputs,self.connections,rec_names[dns_name],vdns_fix[dns_name].vdns_fix,vdns_rec_data))
                result, msg = vdns_rec[dns_name].verify_on_setup()
                self.assertTrue(result,msg)
        vn_fixt = {}
        vm_fixture = {}
        ...
        for dns_name in dns_server_name_list:
            dns_server = IpamDnsAddressType(virtual_dns_server_name = vdns_fix[dns_name].vdns_fq_name)
            ipam_mgmt_obj = IpamType(ipam_dns_method='virtual-dns-server',ipam_dns_server=dns_server)
            # Associate IPAM with VDNS server Object
            ipam_fixt[dns_name] = self.useFixture(NetworkIpamTestFixtureGen(self.vnc_lib,virtual_DNS_refs = [vdns_fix[dns_name].obj],parent_fixt=proj_fixt,network_ipam_name=ipam_dns_list[dns_name],network_ipam_mgmt=ipam_mgmt_obj))
            # Launch VN
            vn_fixt[dns_name] = self.useFixture( VNFixture(project_name= self.inputs.project_name, 
            ...
            # Launch VM
            vm_fixture[dns_name] = self.useFixture(VMFixture(project_name= self.inputs.project_name,connections= self.connections, vn_obj= vn_fixt[dns_name].obj,vm_name=vm_dns_list[dns_name]))
        ...
        for dns in dns_server_name_list:
            for dns_name in dns_server_name_list:
                cmd = 'nslookup ' + vm_dns_list[dns_name] +'.'+ domain_name_list[dns_name]
                self.logger.info('VM Name is ---> %s\t cmd is---> %s',vm_dns_list[dns],cmd)
                ...
                if not m_obj:
                    self.assertTrue(False,'record search is failed,please check syntax of the regular expression/NSlookup is failed')
                print ('vm_name is ---> %s \t ip-address is ---> %s' %(m_obj.group(1),m_obj.group(2)))
                vm_name_to_verify = vm_dns_list[dns_name] +'.'+ domain_name_list[dns_name]
                self.assertEqual(m_obj.group(1),vm_name_to_verify,'VM name is not matching with nslookup command output')
                self.assertEqual(m_obj.group(2),vm_ip_dns_list[dns_name],'IP Address is not matching with nslookup command output')
        return True
    ...
    def test_vdns_server_scaling(self):
        ...
        for num in range(1,vdns_scale+1):
            ...
            dns_data = VirtualDnsType(domain_name=domain_name,dynamic_records_from_client = True,default_ttl_seconds=ttl,record_order='random')
            vdns_fixt[vdnsName] = self.useFixture(VdnsFixture(self.inputs,self.connections,vdns_name=vdnsName,dns_data = dns_data))
            for rec_num in range(1,record_num+1):
                self.logger.info('Creating %s record for vdns server %s',rec_num , num )
                rec = 'test-rec-' + str(j) +'-'+ str(i)
                rec_ip = '1.'+'1.'+ str(j) +'.'+ str(i)
        ...
        for vdns in vdns_verify:
            ...
            ipam_mgmt_obj = IpamType(ipam_dns_method='virtual-dns-server',ipam_dns_server=dns_server)
            # Associate IPAM with VDNS server Object
            ipam_fixt = self.useFixture(NetworkIpamTestFixtureGen(self.vnc_lib,virtual_DNS_refs = [vdns_fixt[vdns].obj],parent_fixt=proj_fixt,network_ipam_name=ipam_name,network_ipam_mgmt=ipam_mgmt_obj))
            # Launch VN
            vn_fixt = self.useFixture( VNFixture(project_name= self.inputs.project_name,connections= self.connections,
                vn_name=vn_name, inputs= self.inputs, subnets=[subnet], ipam_fq_name = ipam_fixt.getObj().get_fq_name()))
            # Launch VM
            vm_fixture[vdns] = self.useFixture(VMFixture(project_name= self.inputs.project_name,connections= self.connections,
            ...
            result = result.replace("\t", "")
            result = result.replace("\n", " ")
            m_obj = re.search(r"Address:[0-9.]*#[0-9]*\s*.*Name:(.*\.vdns[0-9]*\.net)\s*Address:\s*([0-9.]*)",result)
            if not m_obj:
                self.assertTrue(False,'record search is failed,please check syntax of the regular expression/NSlookup is failed')
            print ('vm_name is ---> %s \t ip-address is ---> %s' %(m_obj.group(1),m_obj.group(2)))
            self.assertEqual(m_obj.group(1),vm_domain_name,'VM name is not matching with nslookup command output')
            self.assertEqual(m_obj.group(2),vm_ip,'IP Address is not matching with nslookup command output')
        return True
    ...
    def next_ip_in_list(self,iplist,item):
        ...
        # if the item is on end of list, the next element will be first element in the list
        else:
            next_item = iplist[0]
        return next_item
    ...
    def verify_ns_lookup_data(self,vm_fix,cmd,expectd_data):
        ...
        self.logger.info("cmd string is %s and  expected data %s for searching" %(cmd,expectd_data))
        vm_fix.run_cmd_on_vm(cmds=[cmd])
        result = vm_fix.return_output_cmd_dict[cmd]
        print ('\n result %s' %result)
    ...
    def verify_vm_dns_data(self,vm_dns_exp_data):
        ...
        for expected in vm_dns_exp_data:
            ...
            self.logger.info("actual record data %s ,\n expected record data %s" %(vm_dns_act_data,expected)) 
