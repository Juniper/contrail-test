...
class AnalyticsTestPerformance(testtools.TestCase, ConfigSvcChain , VerifySvcChain):
    ...
    def create_vms(self,vn_name = 'vn_analytics' ,vm_name = 'vm-analytics' , vn_count= 1, vm_count = 1,ram='8192'):
        ...
        try:
            ...
            self.logger.exception("Got exception  in create_vms function as %s"%(e))
    ...
    def run_query(self): 
        for ip in self.inputs.collector_ips:
            try:
                ...
                res1=self.analytics_obj.ops_inspect[ip].post_query('FlowSeriesTable',start_time=self.start_time,end_time='now'
    ...
    def create_svc_chains(self,st_name, si_prefix, si_count, max_inst,
        ...
        self.st_fixture, self.si_fixtures = self.config_st_si(st_name, si_prefix, si_count, 
    ...
    def create_policy(self,policy_name = 'policy_in_network',rules=[],src_vn_fixture=None,dest_vn_fixture=None):
        ...
        self.vn1_policy_fix = self.attach_policy_to_vn(self.policy_fixture, src_vn_fixture)
        self.vn2_policy_fix = self.attach_policy_to_vn(self.policy_fixture, dest_vn_fixture)
    ...
    def setup_service_instance(self,st_name = 'in_net_svc_template_1',si_prefix= 'in_net_svc_instance_',
                            si_count=1,svc_scaling=False,max_inst=1,left_vn='',right_vn='',svc_mode='in-network'):

        self.create_svc_chains(st_name, si_prefix, si_count, max_inst,svc_scaling=svc_scaling, 
    ...
    def setup_policy(self,policy_name='policy_in_network',policy_rules=[],src_vn_fixture=None,dest_vn_fixture=None):
        ...
        self.create_policy(policy_name = policy_name, rules = policy_rules,src_vn_fixture=src_vn_fixture,
    ...
    def restart_service(self,ip_list,service,command = 'restart'):
        ...
        for ip in ip_list:
            ...
            self.inputs.run_cmd_on_server(ip,cmd, username='root',password='c0ntrail123')
    ...
    def test_verify_analytics_scale(self):
        ...
        right_vn_fq_name = self.setup_fixture.vn_obj_dict.values()[1].vn_fq_name
        self.setup_service_instance(left_vn=left_vn_fq_name,right_vn=right_vn_fq_name)
        ...
        self.setup_policy(policy_rules=rules,src_vn_fixture=left_vn_fix,dest_vn_fixture=right_vn_fix)
        #Sending traffic
        prefix = '111.1.0.0/16'
        vm_uuid = self.setup_fixture.vm_valuelist[0].vm_obj.id
        vm_ip = self.setup_fixture.vm_valuelist[0].vm_ip
        self.provision_static_route(prefix = prefix, virtual_machine_id = vm_uuid,
        ...
        self.tx_vm_node_ip= self.inputs.host_data[self.nova_fixture.get_nova_host_of_vm(first_vm.vm_obj)]['host_ip'] 
        self.start_time=self.analytics_obj.getstarttime(self.tx_vm_node_ip)
        traffic_threads= []
        for vm in vm_list:
            t= threading.Thread(target=self.start_traffic, args=(first_vm,ips[0], ips[-1], vm.vm_ip,
...
#        print 'Waiting...'
        ...
        self.triggers(preference='agent restart',ip=self.inputs.compute_ips,command='restart',service='contrail-vrouter')
        time.sleep(20)
        ...
        self.triggers(preference='collector stop',ip=[self.inputs.collector_ips[0]],command='stop',service='supervisor-analytics')
        temp = self.inputs.collector_ips[:]
        self.inputs.collector_ips.remove(self.inputs.collector_ips[0])
        time.sleep(10)
        self.verifications(verify='uve')
        self.inputs.collector_ips = temp[:]
        self.triggers(preference='collector start',ip=[self.inputs.collector_ips[0]],command='start',service='supervisor-analytics')
        time.sleep(10)
        #collector reboot
        self.logger.info("Verifying collector reboot")
        self.triggers(preference='collector reboot',ip=[self.inputs.collector_ips[1]])
        temp = self.inputs.collector_ips[:]
        ...
        for vm in vm_list:
            dest_vm_node_list=[]
            dest_vm_node_ip= self.inputs.host_data[self.nova_fixture.get_nova_host_of_vm(vm.vm_obj)]['host_ip']
