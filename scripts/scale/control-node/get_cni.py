...
if __name__ == '__main__':
    ...
    #print "INST NAME:%s num xmpp peers:%s nprefixes:%s pending updates:%s" %(it, npeers, nprefixes, pending_updates)
    ...
    for j in range (1, nblocks + 1):
        for i in range (1, ninstances + 1):
            ...
            npeers = int(cn.get_cn_bgp_neighbor_stats_element('count', 'xmpp', 'up', instance_name))
            ...
            status, nprefixes = cn.get_cn_routing_instance_table_element (instance_name, family, 'prefixes')
            status, paths = cn.get_cn_routing_instance_table_element (instance_name, family, 'paths')
            status, primary_paths = cn.get_cn_routing_instance_table_element (instance_name, family, 'primary_paths')
            status, secondary_paths = cn.get_cn_routing_instance_table_element (instance_name, family, 'secondary_paths')
            status, infeasible_paths = cn.get_cn_routing_instance_table_element (instance_name, family, 'infeasible_paths')
            status, pending_updates = cn.get_cn_routing_instance_table_element (instance_name, family, 'pending_updates')
