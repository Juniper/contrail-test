...
class FlapAgentScaleInit (object):
    ...
    def __init__(self, args_str = None, pre_scale_setup = 0):
        ...
        # Print version
        #
        self._log_print ("INFO: bgp scale test flap_agent_scale_test version: %s" % self._args.version)
        ...
        #
        self.report_result_averages ("init", int(self._args.run_get_result_prior_run))
    ...
    def _log_print (self, line=''):
        ...
        if self.pre_scale_setup:
            ...
            msg = "%s %s%s %s" %(datetime.now(), who, self.current_run_count, line)
    ...
    def _get_linux_distribution (self):
        ...
        self._log_print ("INFO: linux distribution on api-server: %s release: %s" % (self._args.api_server_ip, self.api_linux_distribution))
        ...
        for i in range (len (self.cn_ips)):
            ...
            self._log_print ("INFO: linux distribution on control-node: %s release: %s" % (cn_ip, self.cn_linux_distribution[i]))
        ...
        for i in range (len (self.ts_ips)):
            ...
            self._log_print ("INFO: linux distribution on testserver: %s release: %s" % (ts_ip, self.ts_linux_distribution[i]))


        #
        # Print linux distribuition for localhost
        #
        self._log_print ("INFO: linux distribution on localhost testserver: %s release: %s" % (self.localhost_ip, self.linux_distribution))
    ...
    def _install_sshpass_if_missing (self):
        ...
        self._log_print ("INFO: checking for sshpass on localhost: %s" % self.localhost_ip)
        ...
        if status == None:
            self._log_print ("INFO: sshpass not found - installing on localhost: %s" % self.localhost_ip)
            ...
            if self.linux_distribution == self._args.linux_fedora and not self.pre_scale_setup:
                status, result = self._get_subprocess_info (self._args.mnt_shared_dir_cmd)
                status, result = self._get_subprocess_info (self._args.cp_cobbler_cmd)
                status, result = self._get_subprocess_info (self._args.install_sshpass_rpm_cmd)
            else: 
                status, result = self._get_subprocess_info (self._args.install_sshpass_rpm_cmd)

            self._log_print ("INFO: sshpass installed on localhost: %s" % self.localhost_ip)

        else:
            self._log_print ("INFO: sshpass found on localhost: %s result: %s" %(self.localhost_ip, result[:-1]))
    ...
    def _add_packages (self):
        ...
        # Add to local host
        #
        self._add_package_call ('local-host', self.localhost_ip, 0, self.linux_distribution, self._args.install_w_yum_cmd)
        ...
        for i in range (len (self.cn_ips)):
            cn_ssh_fd = self.cn_ssh_fds[i]
            cn_ip = self.cn_ips[i]
            self._add_package_call ('control-node', self.cn_ips[i], cn_ssh_fd, self.cn_linux_distribution[i], self._args.install_w_yum_cmd)
        ...
        for i in range (len (self.ts_ips)):
            ts_ssh_fd = self.ts_ssh_fds[i]
            ts_ip = self.ts_ips[i]
            self._add_package_call ('test-server', self.ts_ips[i], ts_ssh_fd, self.ts_linux_distribution[i], self._args.install_w_yum_cmd)

        #
        # Add packages to api server
        #
        self._add_package_call ('api-server', self._args.api_server_ip, self.api_ssh_fd, self.api_linux_distribution, self._args.install_w_yum_cmd)
    ...
    def _add_package_call (self, who, ip, fd, distro, cmd):
        ...
        if len(packages) == 1:
            self._log_print ("INFO: verifying package: %s exists on %s server: %s" % (packages, who, ip))
        elif len(packages) > 1:
            self._log_print ("INFO: verifying packages: %s exist on %s server: %s" % (packages, who, ip))
        else:
        ...
        for i in range (len(packages)):
            ...
            #
            if who == 'local-host':
                status, result = self._get_subprocess_info (which_cmd, print_err_msg_if_encountered=0)
                if status == None:
            ...
            if status != None:
                ...
                self._log_print ("INFO: package not found - installing: %s on %s server: %s" % (packages, who, ip))
        ...
        if len(new_packages_list):
            ...
            # Execute command
            #
            new_cmd = " ".join(re.split (" ", "".join(cmd))[0:3]) + " " + " ".join(new_packages_list)
            if who == 'local-host':
    ...
    def _setup (self):
        ...
        #
        #if (self.pre_scale_setup and self.set_general_vn_name_across_testservers) or (not self.pre_scale_setup and not self.set_general_vn_name_across_testservers):
    ...
    def _get_thread_cpu_from_params (self):
        ...
        if status:
            if re.search('TBB_THREAD_COUNT=', result):
                return_val = re.search('TBB_THREAD_COUNT=(\d+)', result).group(1)
    ...
    def _taskset_affinity_mask (self, ip = 0, fd = 0, cpu_threads = 0):
        ...
        self._log_print ("INFO: setting affinity mask on: %s number cpu_threads: %s cmd: %s" % (ip, cpu_threads, cmd))
    ...
    def _restart_cn (self, why=0, restart_anyway=0):
        ...
        for i in range (len (self.cn_ips)):
            ...
            #
            if type(why) == str:
                msg = "INFO: restarting control node: %s %s with cmd: %s" % (cn_ip, why, cmd)
            else:
                msg = "INFO: restarting control node: %s with cmd: %s" % (cn_ip, cmd)
            self._log_print ("INFO: restarting control node: %s with cmd: %s" % (cn_ip, cmd))
        ...
        self._log_print ("INFO: sleeping: %ss after restartng all control nodes result: %s" % (sleep_time, return_val))
    ...
    def check_if_swapping (self, result, who):
        ...
        if not re.search (' 0 kB', result):
            self._log_print ("WARNING: system swapping:{0} -> reboot device".format(who))
            return_val = True
    ...
    def _get_localhost_info (self):
        ...
        self._log_print ("INFO: localhost testserver ip: %ss" % self.localhost_ip)
    ...
    def _get_prog_fds (self):
        ...
        self.api_ssh_fd.execConnect(self._args.api_server_ip, username, password )
    ...
    def _open_logfile (self, who):
        ...
        if not os.path.exists(logdir):
            try:
                ...
                print ('ABORT: Failed to create logdir directory, since this is a scaling script, check that the number of file descriptors (max files) has not been exceeded:[lsof -n, and ulimit -n] or (2) check for directory name err for log in params (should be "<logdir>/<name>" found: %s aborting script..' %logdir)
                self.cleanup ()
                sys.exit()
        ...
        if who == 'init':
            if self.pre_scale_setup:
                self.fd_name = "%s/%s_%s.log" % (self._args.logdir_name, self._args.logfile_name_parent, self.pid)
            else:
                self.fd_name = "%s/%s_%s.log" % (self._args.logdir_name, self._args.logfile_name_flap_agent_scale_test, self.pid)
            ...
            self.summary_fd_name = "%s/%s_%s.txt" % (self._args.logdir_name, self._args.result_averages_logfilename, pid)
    ...
    def _open_file_for_write (self, filename):
        ...
        try:
            ...
            print ('ABORT: Failed to open file: %s, since this is a scaling script, check that the number of file descriptors (max files) has not been exceeded:[lsof -n, and ulimit -n]' %filename)
            self.cleanup ()
            sys.exit()
    ...
    def check_and_fix_connectivity (self, cn_self, src, dst, xmpp_src_net, test_server_ip, nagents, dev, index):
        ...
        if not self.standalone_mode:
            net = "%s/%s" % (IPNetwork(xmpp_src_net).network, IPNetwork(xmpp_src_net).prefixlen)
            route_add_cmd = "route add -net {0} gw {1}".format(net, test_server_ip)
            try:
        ...
        #
        self.check_and_fix_secondaries (dev, dst, nagents, xmpp_src_net, test_server_ip)
        ...
        ping_cmd = 'ping {0} -I {1} -c 1 -W 1 | grep -i "{2}"'.format(dst, src, pattern)
        ping_cmd2 = 'ping {0} -I {1} -c 1 -W 1 | grep -i "{2}"'.format(last_dst, src, pattern)
        result = cn_self.execCmd (ping_cmd)
        result2 = cn_self.execCmd (ping_cmd2)
        ...
        if result == None or (not re.search(pattern, result, re.IGNORECASE)):
            self._log_print ("WARNING: Pings not working to first xmpp_src address, check local secondaries are configured and up. Aborting script due to failure if this cmd: ping_cmd:{0}  result:{1}".format(ping_cmd, result))
            sys.exit()
        ...
        if result2 == None or (not re.search(pattern, result2, re.IGNORECASE)):
            self._log_print ("ERROR: Pings not working to last xmpp_src address, check local secondaries are configured and up. Aborting script due to failure if this cmd: ping_cmd:{0}  result:{1}".format(ping_cmd, result2))
            sys.exit()

        self._log_print ("INFO: Connectivity [ok] between cn:{0} and xmpp_server:{1}, result:{2}".format(src, dst, result))
    ...
    def check_and_fix_secondaries (self, dev, dst, nagents, xmpp_src_net, test_server):
        ...
        for i in range (1, nagents + 2):
            ...
            status, result = self._get_subprocess_info (cmd, print_err_msg_if_encountered = 0)
    ...
    def add_secondaries (self):
        ...
        status, result = self._get_subprocess_info (cmd)
        if status == None:
            self._log_print ("ABORT: problem configuring device: %s up on test server: %s" % (self.localhost_ip, cmd))
            self.cleanup ()
            sys.exit()
        self._log_print ("INFO: configured device: %s up on test server: %s" % (self.dev, self.localhost_ip))
        ...
        for i in range (cn_start_index, num_control_nodes):
            ...
            for j in range (self.nblocks_of_vns):
                for k in range (self.num_iterations):
                    ...
                    #
                    self.check_and_fix_connectivity (cn_ssh_fd, cn_ip, self.xmpp_src[kindex], self.xmpp_src_net[kindex], self.localhost_ip, self.nagents[k], self.dev, kindex)
    ...
    def start_bgp_scale (self, oper=''):
        ...
        self._log_print ("INFO: expected_prefixes: %s based on params: %s" % (self.total_expected_prefixes, self.test_param_name))

        #
        # Check if bgp_stress_test is there, abort with an error if not
        #
        status, result = self._get_subprocess_info ('ls -lt bgp_stress_test', print_err_msg_if_encountered=0)
        if status == None:
            self._log_print ("ABORT: no bgp_stress_test binary found on test server")
            self.cleanup ()
            sys.exit()
        else:
        ...
        #
        self._report_stats ("Stats Before BGP Scale {0}".format(oper), self.report_stats_before_and_after_tests)
        self._report_stat_during_test (self.report_stat_item_during_test, msg='stat before starting test', server_type='all', item='memory')
        ...
        for i in range (num_iterations):
            ...
            #
            sleeptime = self.get_sleeptime_seconds (i+1, num_iterations, oper)
            ...
            #
            self.run_bgp_scale (self.cn_index, block_index, i, test_id, oper, sleeptime)
            ...
            if (not background) and ((i + 1) != num_iterations):
                self._log_print ("INFO: Sleeping %s seconds in between runs" %self._args.sleeptime_between_runs)
        ...
        #
        self._report_stats ("Stats After BGP Scale {0}".format(oper), self.report_stats_before_and_after_tests)
    ...
    def _get_ifdev (self):
        ...
        status, result = self._get_subprocess_info (cmd, print_err_msg_if_encountered=0)
        if status == None:
            self._log_print ("ABORT: error getting ifconfig info on test server: %s cmd: %s" % (self.localhost_ip, cmd))
            self.cleanup ()
            sys.exit()
    ...
    def _wait_until_processes_finish (self):
        ...
        # Get stats
        #
        self._report_stats ("Stats During BGP Scale {0}".format(self._args.bgp_oper), self.report_stats_during_bgp_scale)
        self._report_stat_during_test (self.report_stat_item_during_test, msg='stat during test', server_type='all', item='memory')

        #
        # Start polling a bit later
        #
        self._log_print ("INFO: flap_parent sleeping %s seconds before checking if child processes are done yet" % int(self._args.sleeptime_before_polling_processes))
        ...
        while 1:
            ...
            for i in range (self.current_run_count):
                ...
                if self.process[i].is_alive():
                    ...
                    if timeout_status:
                        ...
                        num_terminated += 1
                        self._log_print ("DEBUG: killing bgp_scale_mock_agent processes %s num terminated: %s" % (i + 1, num_terminated))
                else:
                    still_kicking |= False
                    self._log_print ("DEBUG: bgp_scale_mock_agent processes %s ended naturally, num terminated: %s still_kicking: %s" % (i + 1, num_terminated, still_kicking))
            ...
            if timed_out:
                self._log_print ("WARNING: timeout waiting for child bgp_scale_mock_agent processes to complete, waited: %smin" % int(self._args.timeout_minutes_wait_process))
                break
            ...
            if not still_kicking:
                self._log_print ("INFO: all bgp_scale_mock_agent processes done")
                break
            ...
            #
            time_left = int(self._args.timeout_minutes_wait_process) - timed_minutes
            self._log_print ("INFO: flap_parent sleeping %s seconds before next child bgp_scale_mock_agent process status check, timeout in: %smin" % (self._args.sleeptime_between_polling_process, time_left))
        ...
        #
        self._log_print ("INFO: as an over-kill, kill all child bgp_scale_mock_agent processes")
        for i in range (self.current_run_count):
            self.process[i].terminate()

        self._log_print ("INFO: all child bgp_scale_mock_agent processes done, waited for ~%s min after launching all" % (timed_minutes))
        self._report_stat_during_test (self.report_stat_item_during_test, msg='stat after test', server_type='all', item='memory')
    ...
    def _check_timout_status (self, t1):
        ...
        # See if max time was 
        #
        status = delta_time_minutes > int (self._args.timeout_minutes_wait_process)
    ...
    def get_sleeptime_seconds (self, iteration_index, num_iterations, oper):
        ...
        if re.search('flap', oper, re.IGNORECASE):
            ...
            sleeptime_seconds = int(self._args.sleeptime_between_adding_and_deleting_routes)
    ...
    def run_bgp_scale (self, cn_index, block_index, iteration_index, test_id, oper, sleeptime):
        ...
        import_targets_per_instance = self.import_targets_per_instance[iteration_index]
        ...
        # Get derived vn name
        #
        self.ri_name = self._get_vn_name (block_index, cn_ip, ts_ip=self.localhost_ip)

        #
        # Get the rinstance names
        #
        ri_name = self.ri_name
        ri_domain_and_proj_name = "%s:%s" % (self._args.ri_domain_name, self._args.ri_proj_name)

        #
        # Misc
        #
        timeout_minutes_poll_prefixes = int(self._args.timeout_minutes_poll_prefixes)

        #
        # Get logfile name for bgp_stress_test and results log
        #
        logfile_name_bgp_stress = "%s/%s_%s.log.%s" % (self._args.logdir_name, self._args.logfile_name_bgp_stress, self.pid, run_id)
        logfile_name_results = "%s/%s_%s.log.%s" % (self._args.logdir_name, self._args.logfile_name_results, self.pid, run_id)

        #
        # Run call in the background
        #
        if background:
            process = multiprocessing.Process (target=bgp_scale_mock_agent, args=(cn_usr, cn_pw, rt_usr, rt_pw, cn_ip, rtr_ip, rtr_ip2, xmpp_source, ri_domain_and_proj_name, ri_name, ninstances, import_targets_per_instance, family, nh, test_id, nagents, nroutes, oper, sleeptime, logfile_name_bgp_stress, logfile_name_results, timeout_minutes_poll_prefixes, background, xmpp_start_prefix, xmpp_start_prefix_large, skip_krt_check, self.report_stats_during_bgp_scale, self.report_cpu_only_at_peak_bgp_scale, skip_rtr_check, self.bgp_env, no_verify_routes, self._args.logging_etc))
            process.start()
            self.process.append (process)
            self._log_print ("INFO: started background bgp_stress_test.%s, pid %d" % (run_id, process.pid))
            #time.sleep(1)
        ...
        #
            ...
            bgp_scale_mock_agent (cn_usr, cn_pw, rt_usr, rt_pw, cn_ip, rtr_ip, rtr_ip2, xmpp_source, ri_domain_and_proj_name, ri_name, ninstances, import_targets_per_instance, family, nh, test_id, nagents, nroutes, oper, sleeptime, logfile_name_bgp_stress, logfile_name_results, timeout_minutes_poll_prefixes, background, xmpp_start_prefix, xmpp_start_prefix_large, skip_krt_check, self.report_stats_during_bgp_scale, self.report_cpu_only_at_peak_bgp_scale, skip_rtr_check, self.bgp_env, no_verify_routes, self._args.logging_etc)
        ...
        if len(xmpp_src) >= (num_iterations + 1):
            ...
            if addr_type == 'xmpp_prefix':
                ...
                if self._args.ts_cn_one_to_one:
                    ...
                    local_ip_int = int(re.search('\d+$', self.localhost_ip).group())
    ...
    def _report_stat_during_test (self, report_stats, msg, server_type, item):
        ...
        self._log_print ("INFO: %s server_type: %s item: %s" % (msg, server_type, item))
        self._log_print ("INFO: --start-stat-get---------------------------------------------")
        ...
        self._log_print ("INFO: --end-stat-get---------------------------------------------")
        return
    ...
    def _derive_total_mem_used (self, result):
        ...
        #
        memory_vals = re.search ('MemTotal:\s+(\d+) kB\s+MemFree:\s+(\d+) kB\s+Cached:\s+(\d+) kB', result)
    ...
    def _report_stat_item (self, item, process, ip, fd):
        ...
        if item == "memory":
            ...
            if mem != None:
                ...
                self._log_print ("INFO: ip: %s total memory used (total-free-cached): %s kB - and total for %s: %s %s normalized: %s" % (ip, total_memory_used, process, process_mem, unit, normalized_process_mem))
            else:
                self._log_print ("INFO: ip: %s total memory used (total-free-cached): %s kB - and total for %s: %s %s" % (ip, total_memory_used, process, res))
    ...
    def _report_stats (self, msg, report_stats):
        ...
        #
        self._log_print ("INFO: ============================ <Begin Gather> {0} \t==============================".format(msg))
        ...
        # Localhost details
        #
        result1 = subprocess.check_output('ulimit -a',  stderr=subprocess.STDOUT, shell=True)
        result2 = subprocess.check_output('uname -a',  stderr=subprocess.STDOUT, shell=True)
        result3 = subprocess.check_output('cat /sys/block/sda/device/model',  stderr=subprocess.STDOUT, shell=True)
        self._log_print ("INFO: ip:{0} Localhost uname and ulimit Settings:".format(local_ip))
        self._log_print ("INFO: ip:{0} uname -a \n{1}".format(local_ip, result1))
        self._log_print ("INFO: ip:{0} ulimit -a \n{1}".format(local_ip, result2))
        self._log_print ("INFO: ip:{0} cat /sys/block/sda/device/model \n{1}".format(local_ip, result3))

        if re.search('Before', msg, re.IGNORECASE):
            result3 = subprocess.check_output('cat params.ini',  stderr=subprocess.STDOUT, shell=True)
            self._log_print ("INFO: cat params.ini\n{1}".format(local_ip, result3))
    
        #
        # Localhost memory info
        #
        mem_result1 = subprocess.check_output('egrep "Mem|Cache|Swap" /proc/meminfo', stderr=subprocess.STDOUT, shell=True)
        mem_result2 = subprocess.check_output('ps -e -orss=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20', stderr=subprocess.STDOUT, shell=True)
        mem_result3 = subprocess.check_output('ps -e -ovsz=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20', stderr=subprocess.STDOUT, shell=True)
        mem_result4 = subprocess.check_output('vmstat',  stderr=subprocess.STDOUT, shell=True)
        mem_result5 = subprocess.check_output('pmap {0} | grep -i total'.format(self.pid),  stderr=subprocess.STDOUT, shell=True)
        self._log_print ("INFO: ip:{0} Localhost Memory:".format(local_ip))
        self._log_print ('INFO: ip:{0} egrep "Mem|Cache|Swap" /proc/meminfo\n{1}'.format(local_ip, mem_result1))
        self._log_print ('INFO: ip:{0} ps -e -orss=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20\n{1}'.format(local_ip, mem_result2))
        self._log_print ('INFO: ip:{0} ps -e -ovsz=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20\n{1}'.format(local_ip, mem_result3))
        self._log_print ('INFO: ip:{0} vmstat\n{1}'.format(local_ip, mem_result4))
        self._log_print ('INFO: ip:{0} pmap {1} | grep -i total\n{2}'.format(local_ip, self.pid, mem_result5))
    
        #
        # Localhost file descriptprs
        #
        result1 = subprocess.check_output('lsof -n | wc -l', stderr=subprocess.STDOUT, shell=True)
        result2 = subprocess.check_output('lsof -n | grep -i tcp | wc -l', stderr=subprocess.STDOUT, shell=True)
        result3 = subprocess.check_output("lsof -p %s | wc -l" %self.pid, stderr=subprocess.STDOUT, shell=True)
        self._log_print ("INFO: ip:{0} Localhost File Descriptors (lsof -n, lsof -n | grep -i tcp, lsof -p {1} | wc -l):".format(local_ip, self.pid))
        self._log_print ("INFO: ip:{0} Total        fds: {1}".format(local_ip, result1))
        self._log_print ("INFO: ip:{0} TCP          fds: {1}".format(local_ip, result2))
        self._log_print ("INFO: ip:{0} This script  fds: {1}".format(local_ip, result3))
    
        #
        # Localhost cpu
        #
        result1 = subprocess.check_output('cat /proc/stat | grep -i cpu', stderr=subprocess.STDOUT, shell=True)
        result2 = subprocess.check_output('top -b | head -15', stderr=subprocess.STDOUT, shell=True)
        self._log_print ("INFO: ip:{0} Localhost CPU info (brief):".format(local_ip))
        self._log_print ("INFO: ip:{0} cat /proc/stat | grep -i cpu\n{1}".format(local_ip, result1))
        self._log_print ("INFO: ip:{0} top -b | head -15\n{1}".format(local_ip, result2))
        ...
        #
        status, localhost_crash_info = self._get_subprocess_info ('ls -lt /var/crashes; ls -lt /var/crash', print_err_msg_if_encountered=0)
        self._log_print ("INFO: ip:{0} Localhost Crash info:".format(local_ip))
        self._log_print ("INFO: ip:{0} ls -lt /var/crashes; ls -lt /var/crash\n{1}".format(local_ip, localhost_crash_info))
        self._check_for_crash (localhost_crash_info, "Localhost: %s running bgp_stress_test code" %local_ip)
        ...
        for i in range (cn_start_index, num_control_nodes):
            ...
            result2 = cnshell_self.execCmd ('contrail-status; contrail-version')
            result3 = cnshell_self.execCmd ('ls -lt %s' % self._args.control_node_binary_location)
            result4 = cnshell_self.execCmd ('cat /sys/block/sda/device/model')
            self._log_print ("INFO: ip:{0} Control Node ps info (with env vars) and version:".format(cn_ip))
            self._log_print ("INFO: ip:{0} ps e `pidof control-node`\n{1}".format(cn_ip, result1))
            self._log_print ("INFO: ip:{0} contrail-status; contrail-version\n{1}".format(cn_ip, result2))
            self._log_print ("INFO: ip:{0} ls -lt {1}\n{2}".format(cn_ip, self._args.control_node_binary_location, result3))
            self._log_print ("INFO: ip:{0} cat /sys/block/sda/device/model\n{1}".format(cn_ip, result4))

            #
            # Control node ulimit settings
            #
            result1 = cnshell_self.execCmd ('cat /proc/`pidof control-node`/limits')
            result2 = cnshell_self.execCmd ('uname -a')
            self._log_print ("INFO: ip:{0} Control Node uname and ulimit Settings:".format(cn_ip))
            self._log_print ("INFO: ip:{0} cat /proc/`pidof control-node`/limits\n{1}".format(cn_ip, result1))
            self._log_print ("INFO: ip:{0} ulimit -a\n{1}".format(cn_ip, result2))
    
            #
            # Control Node memory info
            #
            mem_result1 = cnshell_self.execCmd ('egrep "Mem|Cache|Swap" /proc/meminfo; echo " "; egrep "Active|MemTotal" /proc/meminfo')
            mem_result2 = cnshell_self.execCmd ('ps -e -orss=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20')
            mem_result3 = cnshell_self.execCmd ('ps -e -ovsz=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20')
            mem_result4 = cnshell_self.execCmd ('vmstat')
            mem_result5 = cnshell_self.execCmd ("pmap `pidof control-node` | grep -i total")
            self._log_print ('INFO: ip:{0} Control Node Memory:'.format(cn_ip))
            self._log_print ('INFO: ip:{0} egrep "Mem|Cache|Swap" /proc/meminfo; echo " "; egrep "Active" /proc/meminfo\n{1}'.format(cn_ip, mem_result1))
            self._log_print ('INFO: ip:{0} ps -e -orss=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20\n{1}'.format(cn_ip, mem_result2))
            self._log_print ('INFO: ip:{0} ps -e -ovsz=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20\n{1}'.format(cn_ip, mem_result3))
            self._log_print ('INFO: ip:{0} vmstat\n{1}'.format(cn_ip, mem_result4))
            self._log_print ('INFO: ip:{0} pmap `pidof control-node` | grep -i total\n{1}'.format(cn_ip, mem_result5))
    
            #
            # Check control node swap info
            #
            result = cnshell_self.execCmd ('egrep "SwapCached" /proc/meminfo')
            self.check_if_swapping (result, "control node: %s - %s" % (cn_ip, msg))
            ...
            result3 = cnshell_self.execCmd ('lsof -p `pidof control-node` | wc -l')
            #result3 = cnshell_self.execCmd ('lsof -i | wc -l') # can hang system...
            self._log_print ("INFO: ip:{0} Control Node File Descriptors (lsof -n, lsof -p `pidof control-node`, lsof -n | grep -i tcp | wc -l):".format(cn_ip))
            self._log_print ("INFO: ip:{0} Total        fds: {1}".format(cn_ip, result1))
            self._log_print ("INFO: ip:{0} TCP          fds: {1}".format(cn_ip, result2))
            self._log_print ("INFO: ip:{0} Control Node fds: {1}".format(cn_ip, result3))

            result1 = cnshell_self.execCmd ('netstat -vatn | wc -l')
            self._log_print ('INFO: ip:{0} Control Node Open Ports and Established TCP Sessions "netstat -vatn | wc -l"\n'.format(cn_ip, result1))
            ...
            self._log_print ("INFO: ip:{0} Control Node CPU info (brief):".format(cn_ip))
            self._log_print ("INFO: ip:{0} cat /proc/stat | grep -i cpu\n{1}".format(cn_ip, result1))
            self._log_print ("INFO: ip:{0} top -b | head -15\n{1}".format(cn_ip, result2))
            ...
            #
            if not self.standalone_mode:
                cn_crash_info = cnshell_self.execCmd ('ls -lt /var/crashes; ls -lt /var/crash')
                self._log_print ("INFO: ip:{0} Control Node Crash info:".format(cn_ip))
                self._log_print ("INFO: ip:{0} ls -lt /var/crashes; ls -lt /var/crash\n{1}".format(cn_ip, cn_crash_info))
                self._check_for_crash (cn_crash_info, "Control node: %s" %cn_ip)
        ...
        if not self.standalone_mode:
            ...
            mem_result1 = api_fd.execCmd ('egrep "Mem|Cache|Swap" /proc/meminfo; echo " "; egrep "Active|MemTotal" /proc/meminfo')
            self._log_print ('INFO: ip:{0} egrep "Mem|Cache|Swap" /proc/meminfo; echo " "; egrep "Active" /proc/meminfo\n{1}'.format(api_ip, mem_result1))
            mem_result2 = api_fd.execCmd ('ps -e -orss=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20')
            self._log_print ('INFO: ip:{0} ps -e -orss=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20\n{1}'.format(api_ip, mem_result2))
            mem_result3 = api_fd.execCmd ('ps -e -ovsz=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20')
            ...
            self._log_print ('INFO: ip:{0} du -skh /home/cassandra\n{1}'.format(api_ip, mem_result5))
            mem_result6 = api_fd.execCmd ("find / -name cassandra | gawk '\\''{print \"du -skh \" $1}'\\'' > /tmp/t.sh;sh /tmp/t.sh")
            self._log_print ('INFO: ip:{0} find and du all cassandra\n{1}'.format(api_ip, mem_result6))
            ...
            self._log_print ("INFO: ip:{0} cat /proc/`pidof vizd`/io\n{1}".format(api_ip, mem_result8))

            #
            # api server crash info
            #
            api_crash_info = api_fd.execCmd ('ls -lt /var/crashes; ls -lt /var/crash')
            self._log_print ("INFO: ip:{0} API Server Crash info:".format(api_ip))
            ...
            self._log_print ("INFO: ip:{0} API Server CPU info (brief):".format(api_ip))
            self._log_print ("INFO: ip:{0} cat /proc/stat | grep -i cpu\n{1}".format(api_ip, result1))
            self._log_print ("INFO: ip:{0} top -b | head -15\n{1}".format(api_ip, result2))
    
        self._log_print ("INFO: ============================ <End Gather> {0} \t==============================".format(msg))
    
        #
        # Get localhost swap info
        #
        result = subprocess.check_output('egrep "SwapCached" /proc/meminfo', stderr=subprocess.STDOUT, shell=True)
        ...
        if not self.standalone_mode:
            ...
            self.check_if_swapping (result, "api server: %s - %s" % (api_ip, msg))
        else: 
            pass
    ...
    def _check_for_crash (self, result, who):
        ...
        if re.search ('core', result):
            self._log_print ("WARNING: crash found on: {0} crash files:\n{1}".format(who, result))
            return_val = True
    ...
    def _get_bgp_start_vals (self):
        ...
        #
        self.cn_ulimit_n = self._get_ulimit (self._args.control_node_restart_cmd)
        ...
        xmpp_start_prefix = re.split (",", "".join(self._args.xmpp_start_prefix.split()))
        ...
        # Misc
        #
        self.set_general_vn_name_across_testservers = int(self._args.set_general_vn_name_across_testservers)
        self.run_bgp_scale_in_background = int(self._args.run_bgp_scale_in_background)
        self.skip_krt_check = int(self._args.skip_krt_check)
        self.report_stats_before_and_after_tests = int(self._args.report_stats_before_and_after_tests)
        self.report_stats_during_bgp_scale = int(self._args.report_stats_during_bgp_scale)
        self.report_cpu_only_at_peak_bgp_scale = int(self._args.report_cpu_only_at_peak_bgp_scale)
        self.report_stat_item_during_test = int(self._args.report_stat_item_during_test)

        try:
            self._args.kill_all_python_on_cleanup
            self.kill_all_python_on_cleanup = int (self._args.kill_all_python_on_cleanup)
        except:
        ...
        #
        self.check_param_has_enough_elements ('ninstances', ninstances_list, num_iterations)
        self.check_param_has_enough_elements ('nagents', nagents_list, num_iterations)
        self.check_param_has_enough_elements ('nroutes', nroutes_list, num_iterations)
        self.check_param_has_enough_elements ('import_targets_per_instance', import_targets_per_instance_list, num_iterations)
        ...
        for block_index in range (num_blocks):
            for index in range (num_iterations):
                ...
                #
                self.xmpp_src.append (self._derive_net (xmpp_src, block_index, kindex, num_iterations, addr_type='src_addr'))
                self.xmpp_src_net.append ("%s/%s" % (self.xmpp_src[kindex], xmpp_src_prefix_len))
                self.xmpp_start_prefix.append (self._derive_net (xmpp_start_prefix, block_index, kindex, num_iterations, addr_type='xmpp_prefix'))
                kindex += 1
        ...
        for index in range (num_iterations):
            ...
            self.import_targets_per_instance.append (int (import_targets_per_instance_list[index]))
        ...
        #
        self.standalone_mode = len (cn_ips) == 1 and (self.localhost_ip == self._args.api_server_ip) and (self.localhost_ip == self.cn_ips[self.cn_index])

        #
        # Use first val as for label
        #
        self.total_expected_prefixes = self.num_iterations * self.ninstances[0] * self.nagents[0] * self.nroutes[0]
        self.test_param_name = "%s_iterations_at_%sx%sx%s" %(self.num_iterations, self.ninstances[0], self.nagents[0], self.nroutes[0])
    ...
    def check_param_has_enough_elements (self, item_name, param_item, num_iterations):
       ...
       if len(param_item) < num_iterations:
           self._log_print ("ERROR: Stop and edit params file, number of elements in '%s' is %s (not enough for the num_iterations==%s)" %(item_name, len(param_item), num_iterations))
    ...
    def _parse_args(self, args_str):
        ...
        conf_parser.add_argument("-c", "--conf_file", help="Specify config file, e.g., prams.ini", metavar="FILE")
        ...
        parser.add_argument("-mu", "--multiple_testservers_running",  help = "override params to restart control nodes, use when stressfactr runs multiple test servers")
        parser.add_argument("-ts", "--ts_cn_one_to_one",              help = "each testserver maps to one control node")
        parser.add_argument("-nr", "--nroutes_per_all_agents",            help = "number prefixes for each case - overrides params file")
        ...
        if not args.conf_file:
            print ("ABORTing script no params file specified use: flap_agent_scale_test.py -c param_filename")
            self.cleanup ()
            sys.exit()
    ...
    def _overkill_lingering_processes (self):
        ...
        status, bgp_result = self._get_subprocess_info (cmd, print_err_msg_if_encountered=0)
        ...
        status, python_result = self._get_subprocess_info (cmd, print_err_msg_if_encountered=0)
        if status:
            python_lines = python_result.split('\n')[:-1] # chop newline at the end of the output
        ...
        if type(bgp_result) == str:
            ...
            for i in range (len (bgp_lines)):
                    ...
                    try:
                        ...
                        num_bgp_killed += 1
                        self._log_print ("DEBUG: sending SIGILL to bgp_stress pid: %s" % ppid)
                    except OSError:
                        pass
                        ...
                        for j in range (len (python_lines)):
                            ...
                            if re.search(str(ppid), python_lines[j]):
                                ...
                                try:
                                    ...
                                    num_python_killed += 1
                                    self._log_print ("DEBUG: SIGKILL python pid: %s" % python_pid)
                                except OSError:
                                    pass
        ...
        if num_bgp_killed > 0 or num_python_killed > 0:
            self._log_print ("WARNING: SIGKILL lingering processes bgp_stress: %s python: %s" % (num_bgp_killed, num_python_killed))
    ...
    def cleanup (self, oper=''):
        ...
        if int(self._args.run_bgp_scale):
            ...
            self._log_print ('INFO: grep -v egrep %s/*%s* | grep -v "abrt-watch-log" | egrep --color=auto "WARNING|ERROR"' % (self._args.logdir_name, self.pid))
            self._log_print ('INFO: grep -v egrep %s/*%s* | egrep --color=auto "Expected|Elapsed"' % (self._args.logdir_name, self.pid))
            self._log_print ('INFO: grep -v egrep %s/*%s* | egrep --color=auto "BGP Stress Test - CN|Expected|Elapsed"' % (self._args.logdir_name, self.pid))
    ...
    def _get_vn_subnet_addr (self, ip):
        ...
        #
        new_net = "%s.%s/%s" %(lowbyte, nextbyte, self._args.vn_net_prefix_len)
    ...
    def add_or_delete_vns (self, oper=''):
        ...
        if self.set_general_vn_name_across_testservers:
            self._log_print ("INFO: %s vns - num iterations: %s num blocks: %s num vns per block: %s total= %s cn_start_id: %s num_cns: %s" %(oper, num_iterations, nblocks_of_vns, num_vns, num_iterations*nblocks_of_vns*num_vns, cn_start_index, num_control_nodes))
        else:
            self._log_print ("INFO: %s vns - For each testserver: num iterations: %s num blocks: %s num vns per block: %s total= %s cn_start_id: %s num_cns: %s" %(oper, num_iterations, nblocks_of_vns, num_vns, num_iterations*nblocks_of_vns*num_vns, cn_start_index, num_control_nodes))
        ...
        for cn_index in range (cn_start_index, num_control_nodes):
            ...
            #
            self._report_stats ("Stats Before VN {0}".format(oper), report_stats_vn)
            ...
            for j in range(start_block_num, num_iterations+1):
                ...
                for i in range(1, num_vns+1):
                    ...
                    #
                    if oper == "add":
                        self.num_delete_polls_add += self._check_vn_deleted (vn_name, cn_ispec, cn_ip)

                    #
                    # Add or delete VN
                    #
                    cmd = '--api_server_ip {0} --api_server_port 8082 --public_subnet {1} --vn_name {2} --oper {3}'.format(self._args.api_server_ip, str(net), vn_name, oper)
                    ...
                    #
                    if oper == "del":
                        self.num_delete_polls_del += self._check_vn_deleted (vn_name, cn_ispec, cn_ip)
            ...
            #
            self._report_stats ("Stats After with VN {0}".format(oper), report_stats_vn)
        ...
        #
        if oper == "add":
            self._log_print ("INFO: number delete polls during vn add: %s" % self.num_delete_polls_add)
        else:
            self._log_print ("INFO: number delete polls during vn delete: %s" % self.num_delete_polls_del)
    ...
    def _check_vn_deleted (self, instance_name, cn, ip):
        ...
        while True:
            ...
            #
            self._abort_if_timeout (start_time, self._args.timeout_seconds_vn_del, "cn to verify on server: %s deleted instance: %s num_polls: %s" % (ip, instance_name, polls))
    ...
    def _check_vn_added (self, instance_name, cn):
        ...
        while True:
            ...
            # Check for timeout - abort if so..
            #
            self._abort_if_timeout (start_time, self._args.timeout_seconds_vn_add, "cn to verify added instance: %s num_polls: %s" % (instance_name, polls))
    ...
    def _abort_if_timeout (self, t1, timeout_seconds, msg=0):
        ...
        # Check for timeout
        #
        delta_time = self._get_time_diffs_seconds (t1, datetime.now(), decimal_places=0)
        if delta_time > float(timeout_seconds):
            if msg == 0:
                self._log_print ("ABORT: timed out - waited: %ss" % delta_time)
            else:
                self._log_print ("ABORT: timed out waiting for %s - waited: %ss" % (msg, delta_time))
            self.cleanup ()
            sys.exit()
        else:
            pass
    ...
    def _action_if_timeout (self, t1, timeout_seconds, action, msg=0):
        ...
        if delta_time > float(timeout_seconds):
            if msg == 0:
                self._log_print ("%s: timed out - waited: %ss" % (action, delta_time))
            else:
                self._log_print ("%s: timed out waiting for %s - waited: %ss" % (action, msg, delta_time))
            if action == "ABORT":
                self.cleanup ()
                sys.exit()
            else:
    ...
    def _check_vn_all (self):
        ...
        for i in range (cn_start_index, num_control_nodes):
            ...
            #
            total_expected_routing_instances = self._get_num_vns() * self.num_iterations
            if int(self._args.rule_ts_cn_one_to_one) or int(self._args.rule_ts_cn_many_to_one):
                ts_ip = self.ts_ips[i]

            vn_beginning_name = self._get_vn_beginning_name (cn_ip, ts_ip, self._args.vn_basename)

            self._log_print ("INFO: checking control node: %s for: %s vns present with names beginning with: '%s'" % (cn_ip, total_expected_routing_instances, vn_beginning_name))
            ...
            while True:
                ...
                if num_rinstances >= total_expected_routing_instances:
                    self._log_print ("INFO: found a total of %s vns on control node: %s" % (num_rinstances, cn_ip))
                    delta_time = self._get_time_diffs_seconds (start_time, datetime.now(), decimal_places=0)
                    break
                ...
                status, delta_time = self._action_if_timeout (start_time, timeout_sec, action, "control node: %s to verify all vns created - found: %s num_polls: %s" % (cn_ip, num_rinstances, num_polls))
                if status:
                     delta_time = 0
                     break
                ...
                self._log_print ("INFO: sleeping: %ss between vn present check on control node: %s - timeout in: %ss found: %s vns" % (sleep_time, cn_ip, timeout_sec - delta_time, num_rinstances))
                time.sleep (sleep_time)

            # end while loop

            if (i + 1) < num_control_nodes: 
                self._log_print ("DEBUG: checking next control node: %s" % self.cn_ips[i+1])
                pass
    ...
    def _get_vn_beginning_name (self, cn_ip, ts_ip, vn_basename):
        ...
        if self.set_general_vn_name_across_testservers:
            ...
            return_val = "c%s_t%s_%s" % ((re.search('\d+$', cn_ip).group()), (re.search('\d+$', ts_ip).group()), vn_basename)
    ...
    def _get_vn_name (self, block_index, cn_ip, ts_ip, n_index=0):
        ...
        if self.set_general_vn_name_across_testservers:
            ...
            # vn names
            vn_basename = "c%s_t%s_%s" % ((re.search('\d+$', cn_ip).group()), (re.search('\d+$', ts_ip).group()), self._args.vn_basename)
    ...
    def policy_config (self, oper=''):
        ...
        if num_bidir_policies == 0:
            self._log_print ("INFO: Only one instance, skipping instance-to-instance policy config")
            return

        #
        # Create multiple policies per block
        #
        self._log_print ("INFO: oper: %s %s bidr policies on: %s" %(oper, self.num_iterations * num_bidir_policies, self._args.api_server_ip))
        for block_index in range (1, self.num_iterations+1):
            for policy_index in range (1, num_bidir_policies+1):
                ...
                #
                if oper == "add":
                    self._config_policy (block_index, policy_index, num_bidir_policies, oper)
                elif oper == "del":
                    self._delete_policy (block_index, policy_index, num_bidir_policies)
    ...
    def _config_policy (self, block_index, policy_index, num_policies, oper=''):
        ...
        #
        policy1 = self._get_policy_name ('up', block_index, policy_index, second_ri_index)
        policy2 = self._get_policy_name ('dn', block_index, policy_index, second_ri_index)
        ...
        #
        cmd1 = 'cd test/scripts/scale/control-node; python policy.py add --vn_list {0} {1} {2}'.format(vn1, vn2, policy1)
        cmd2 = 'cd test/scripts/scale/control-node; python policy.py add --vn_list {0} {1} {2}'.format(vn2, vn1, policy2)
    ...
    def _get_policy_name (self, order, block_index, index, second_ri_index):
        ...
        try:
            ...
            pattern = "%sblock%s_" % (self._args.policy_name_prepend_pattern, block_index)
        except:
        ...
        #
        if order == 'up':
            new_policyname = "{0}n{1}_n{2}".format(pattern, index, second_ri_index)
        else:
            new_policyname = "{0}n{2}_n{1}".format(pattern, index, second_ri_index)
    ...
    def _get_event_per_second (self, number_of_events, seconds, decimal_places):
        ...
        if seconds == 0:
            ...
            return_val = round ((float(number_of_events)/seconds), decimal_places)
    ...
    def _get_event_per_usec (self, number_of_events, seconds, decimal_places):
        ...
        if seconds == 0:
            ...
            return_val = round ((float(number_of_events / seconds) * 1000000), decimal_places)
    ...
    def report_result_averages (self, called_by='', log_pid=''):
        ...
        self.delta_time_minutes = int(((self.test_end_time - self.test_start_time).seconds) / 60)
        ...
        self._print_summary_and_regular_log ("    Test start time: %s" % self.test_start_time)
        self._print_summary_and_regular_log ("    Test end time:   %s" % self.test_end_time)
        self._print_summary_and_regular_log ("    Number of iterations (running in %s): %s" % (seq_or_parallel, self.num_iterations))
        if self.run_cpu_test:
            self._print_summary_and_regular_log ("    Number of cpu_threads: %s affinity mask: %s" % (self.cpu_threads, self.affinity_mask))
        self._print_summary_and_regular_log ("    Test params: %s" %self.test_param_name)
        self._print_summary_and_regular_log ("    Test ran for ~%s min" % self.delta_time_minutes)
        self._print_summary_and_regular_log ("    Results file: %s" % self.summary_fd.name)
        ...
        trigger_state = "no"
        try:
            result = re.search('trigger', self._args.logging_etc, re.IGNORECASE)
            if result != None and result:
                trigger_state = "yes"
        except:
            pass

        self._print_summary_and_regular_log ("    Prefixes triggered after peers up: %s" % trigger_state)
    ...
    def _get_average_time_peer_bringup (self, pid):
        ...
        #
        self.timestamp_bgp_stress_started = self._get_log_event_timestamp (pid, self._args.logdir_name, self._args.logfile_name_results, 'notable_event', 'started bgp_stress');
        self.timestamp_last_peer_up = self._get_log_event_timestamp (pid, self._args.logdir_name, self._args.logfile_name_results, 'notable_event', 'peers up', 'most_recent');

        #
        # Gather other totals from log file
        #
        cmd = 'grep Elapsed %s/%s_%s.log* | grep peer' % (self._args.logdir_name, self._args.logfile_name_results, pid)
        ...
        if result:
            ...
            for i in range (len(lines)):
                result = re.search ('.* time to add (\d+) peers:(\d+\.\d+)', lines[i])
                if result != None:
                    num_peers = int(result.group(1))

                total_num_peers += num_peers
                total_time += float(re.search ('.* time to add (\d+) peers:(\d+\.\d+)s', lines[i]).group(2))
            ...
            #
            if self.run_bgp_scale_in_background:
                self.total_time_seconds_peer_bringup = self._get_time_diffs_seconds(self.timestamp_bgp_stress_started, self.timestamp_last_peer_up,  decimal_places=1)
                self.peer_bringup_per_second = self._get_event_per_second (total_num_peers, self.total_time_seconds_peer_bringup, decimal_places=2)
            else:
                self.peer_bringup_per_second = self._get_event_per_second (total_num_peers, self.total_agg_time_seconds_peer_bringup, decimal_places=2)
            ...
            self.average_time_peer_bringup = self._get_event_per_second (self.total_time_seconds_peer_bringup, total_num_peers, decimal_places=1)
            self.average_time_peer_bringup_per_iteration = self._get_event_per_second (self.total_time_seconds_peer_bringup, num_peer_results, decimal_places=1)
        ...
        #
        self._print_summary_and_regular_log (" ")
        self._print_summary_and_regular_log ("        Number of peer bringup iterations found: %s" % self.num_peer_results)
        if int(self.num_peer_results) == 0:
            self._print_summary_and_regular_log ("        No data found for peer bringup")

        else:
            self._print_summary_and_regular_log ("        Total number of XMPP peers: %s" % self.total_num_peers)
            if self.run_bgp_scale_in_background:
                self._print_summary_and_regular_log ("        Total wallclock time for peers brought up in parallel: %ss" % str(self.total_time_seconds_peer_bringup))

            self._print_summary_and_regular_log ("        Number of iterations found: %s" % self.num_peer_results)
            self._print_summary_and_regular_log ("        Total aggregate time to bringup peers (relevant for sequential runs): %ss" % self.total_agg_time_seconds_peer_bringup)
            self._print_summary_and_regular_log ("        Max number of peers per any one iterations: %s" % max_num_peers)
            self._print_summary_and_regular_log ("        Average time per iteration: %ss" % self.average_time_peer_bringup_per_iteration)
            self._print_summary_and_regular_log ("        Average time per peer bringup: %ss" % self.average_time_peer_bringup)
            self._print_summary_and_regular_log ("        Peers-up per second: %s" % self.peer_bringup_per_second)
    ...
    def _get_log_event_timestamp (self, pid, logdir, logname, pattern1, pattern2, order=''):
        ...
        cmd = 'grep %s %s/%s_%s.log* | grep "%s" | sort -n -k 2' % (pattern1, logdir, logname, pid, pattern2)
        ...
        if status:
            ...
            for i in range(len(lines)):
                ...
                if timestamp:
                    t1 = datetime.strptime(timestamp.group(1), '%Y-%m-%d %H:%M:%S.%f')
                    if i == 0:
                        tbest = t1
        ...
        if tbest == 0:
            self._log_print ("WARNING: no patterns pat1: %s, pat2: %s, pat:timestamp  matched for cmd: %s" % (pattern1, pattern2, cmd))
    ...
    def _get_average_time_prefix_add (self, pid, trigger_state):
        ...
        if trigger_state == "yes":
            self.timestamp_start_announcing_prefixes = self._get_log_event_timestamp (pid, self._args.logdir_name, self._args.logfile_name_results, 'notable_event', 'prefix adds triggered');
        else:
            self.timestamp_start_announcing_prefixes = self._get_log_event_timestamp (pid, self._args.logdir_name, self._args.logfile_name_results, 'notable_event', 'one peer up');

        #
        # Get the recorded timestamp for prefix add completion
        # Note that it's another notable event.
        #
        self.timestamp_last_prefix_add = self._get_log_event_timestamp (pid, self._args.logdir_name, self._args.logfile_name_results, 'notable_event', 'finished prefix add', 'most_recent');
 
        #
        # Derive average timings for prefix adds
        #
        cmd = 'grep Elapsed %s/%s_%s.log* | grep -v peer | grep add' % (self._args.logdir_name, self._args.logfile_name_results, pid)
        result = self._get_line_with_pattern_match (cmd)
        if result:
            ...
            #
            for i in range (len(lines)):
                result = re.search ('.* time to add (\d+) prefixes on_control_node:(\d+\.\d+)s', lines[i])
                if result != None:
            ...
            #
            if self.run_bgp_scale_in_background:
                self.total_time_seconds_prefix_adds = self._get_time_diffs_seconds(self.timestamp_start_announcing_prefixes, self.timestamp_last_prefix_add, decimal_places=2)
                self.prefix_adds_per_second = self._get_event_per_second (total_num_prefixes, self.total_time_seconds_prefix_adds, decimal_places=0)
                self.updates_adds_per_second = self._get_event_per_second (total_num_prefixes*self.nagents[0], self.total_time_seconds_prefix_adds, decimal_places=0)
            else:
                self.prefix_adds_per_second = self._get_event_per_second (total_num_prefixes, total_time, decimal_places=0)
                self.updates_adds_per_second = self._get_event_per_second (total_num_prefixes*self.nagents[0], total_time, decimal_places=0)
            ...
            # Derive averages
            #
            self.average_time_prefix_add_per_iteration = self._get_event_per_second (self.total_time_seconds_prefix_adds, num_prefix_results, decimal_places=2)
            self.average_time_prefix_add = self._get_event_per_usec (self.total_time_seconds_prefix_adds, total_num_prefixes, decimal_places=0)
            self.average_time_update_add = self._get_event_per_usec (self.total_time_seconds_prefix_adds, total_num_prefixes*self.nagents[0], decimal_places=0)
        ...
        #
        self._print_summary_and_regular_log (" ")
        self._print_summary_and_regular_log ("        Number of iterations found for adds: %s" % self.num_prefix_results_add)

        if int(self.num_prefix_results_add) == 0:
            self._print_summary_and_regular_log ( "        No data found for adds")

        else:
            self._print_summary_and_regular_log ("        Total prefixes added (all iterations): %s" % self.total_num_prefixes_add)
            if self.run_bgp_scale_in_background:
                self._print_summary_and_regular_log ("        Total wallclock time running in parallel for prefix adds: %ss" % str(self.total_time_seconds_prefix_adds))

            self._print_summary_and_regular_log ("        Total aggregate time (relevant for sequential runs) add: %ss" % self.total_agg_time_seconds_add)
            self._print_summary_and_regular_log ("        Average time per iteration for adds: %ss" % self.average_time_prefix_add_per_iteration)
            self._print_summary_and_regular_log ("        Average time per prefix add: %sus" % self.average_time_prefix_add)
            self._print_summary_and_regular_log ("        Average time per update add: %sus" % self.average_time_update_add)
            self._print_summary_and_regular_log ("        Prefix per second for adds: %s" % self.prefix_adds_per_second)
            self._print_summary_and_regular_log ("        Updates per second add (valid only if num agents same per iteration): %s" % self.updates_adds_per_second)
    ...
    def _get_subprocess_info (self, cmd = 0, print_err_msg_if_encountered = 1):
        ...
        if cmd == 0 or type(cmd) != str:
            if print_err_msg_if_encountered:
                self._log_print ("ERROR: improper cmd: %s string provided to function _get_subprocess_info, returning 0" % cmd)
            return None
        ...
        try:
            ...
            if print_err_msg_if_encountered:
                self._log_print ("WARNING: error executing subprocess shell cmd: '%s'" % cmd)
            status = None
            result = None
    ...
    def _get_time_diffs (self, t1, t2):
        ...
        if type(t1) == datetime and type(t2) == datetime:
            ...
            return_val = 0
            self._log_print ("ERROR: time1 or time2 not type datatime: t1 type: %s t2 type: %s" % (type(t1), type(t2)))
    ...
    def _get_time_diffs_seconds (self, t1, t2, decimal_places):
        ...
        if type(t1) == datetime and type(t2) == datetime:
            ...
            if delta_time.days < 0:
                self._log_print ("ERROR: time diff results in a past date t1: %s t2: %s" % (str(t1), str(t2)))
                return 0

            return_val = float("%s.%s" %(str(abs(delta_time).seconds), str(abs((delta_time)).microseconds)[:decimal_places]))
            if decimal_places == 0:
                return_val = int(return_val)
    ...
    def _get_average_time_prefix_del (self, pid):
        ...
        #
        self.timestamp_first_agent_del_done = self._get_log_event_timestamp (pid, self._args.logdir_name, self._args.logfile_name_results, 'notable_event', 'stopping prefix')
        self.timestamp_last_agent_del_done = self._get_log_event_timestamp (pid, self._args.logdir_name, self._args.logfile_name_results, 'notable_event', 'finished prefix del', 'most_recent')

        #
        # Get all lines with successful deletion completion
        #
        cmd = 'grep Elapsed %s/%s_%s.log* | grep -v peer | grep del' % (self._args.logdir_name, self._args.logfile_name_results, pid)
        result = self._get_line_with_pattern_match (cmd)
        if result:
            ...
            #
            for i in range (len(lines)):
                result = re.search ('.* time to del (\d+) prefixes on_control_node:(\d+\.\d+)s', lines[i])
                if result != None:
                    total_num_prefixes += int(result.group(1))
                    total_time += float(result.group(2))
                    num_prefix_results += 1
            ...
            #
            if self.run_bgp_scale_in_background:
                self.total_time_seconds_prefix_deletes = self._get_time_diffs_seconds (self.timestamp_first_agent_del_done, self.timestamp_last_agent_del_done, decimal_places=2)
                self.prefix_deletes_per_per_second = self._get_event_per_second (total_num_prefixes, self.total_time_seconds_prefix_deletes, decimal_places=0)
                self.updates_deletes_per_second = self._get_event_per_second (total_num_prefixes*self.nagents[0], self.total_time_seconds_prefix_deletes, decimal_places=0)
            else:
                self.prefix_deletes_per_per_second = self._get_event_per_second (total_num_prefixes, total_time, decimal_places=0)
                self.updates_deletes_per_second = self._get_event_per_second (total_num_prefixes*self.nagents[0], total_time, decimal_places=0)
            ...
            # Derive averages.
            #
            self.average_time_prefix_del_per_iteration = self._get_event_per_second (self.total_time_seconds_prefix_deletes, num_prefix_results, decimal_places=2)
            self.average_time_prefix_del = self._get_event_per_usec (self.total_time_seconds_prefix_deletes, total_num_prefixes, decimal_places=0)
            self.average_time_update_del = self._get_event_per_usec (self.total_time_seconds_prefix_deletes, total_num_prefixes*self.nagents[0], decimal_places=0)
        ...
        #
        self._print_summary_and_regular_log (" ")
        self._print_summary_and_regular_log ("        Number of iterations found for deletes: %s" % self.num_prefix_results_del)

        if int(self.num_prefix_results_del) == 0:
            self._print_summary_and_regular_log ("        No data found for deletes")

        else:
            self._print_summary_and_regular_log ("        Total prefixes deleted (all iterations): %s" % self.total_num_prefixes_del)
            if self.run_bgp_scale_in_background:
                self._print_summary_and_regular_log ("        Total wallclock time running in parallel for prefix deletes: %ss" % str(self.total_time_seconds_prefix_deletes))

            self._print_summary_and_regular_log ("        Total aggregate time (relevant for sequential runs) delete: %ss" % self.total_agg_time_seconds_del)
            self._print_summary_and_regular_log ("        Average time per iteration for deletes: %ss" % self.average_time_prefix_del_per_iteration)
            self._print_summary_and_regular_log ("        Average time per prefix delete: %sus" % self.average_time_prefix_del)
            self._print_summary_and_regular_log ("        Average time per update delete: %sus" % self.average_time_update_del)
            self._print_summary_and_regular_log ("        Prefix per second for deletes: %s" % self.prefix_deletes_per_per_second)
            self._print_summary_and_regular_log ("        Updates per second delete (valid only if num agents same per iteration): %s" % self.updates_deletes_per_second)
            self._print_summary_and_regular_log (" ")
    ...
    def _adjust_cn_ulimit (self):
        ...
        if self.cn_ulimit_n == None:
            self._log_print ("WARNING: no ulimit -n in command line of params file, skipping ulimit setting on file: %s" % self._args.control_node_supervisor_param_file)
        ...
        for i in range (len (self.cn_ips)):
            ...
            cmd ='sed -i.bak -re "s/(ulimit \-n )([0-9]+)/\\1%s/g" %s' % (self.cn_ulimit_n, self._args.control_node_supervisor_param_file)
            self._log_print ("INFO: setting ulimit -n to: %s in file: %s" % (self.cn_ulimit_n, self._args.control_node_supervisor_param_file))
    ...
    def _add_cn_env_vars (self):
        ...
        for i in range (len (self.cn_ips)):
            ...
            self._log_print ("INFO: adding env variables to control node: %s" % cn_ip)
            ...
            #
            cmd = "echo %s > %s_new" % (" ", self._args.control_node_param_file)
            ...
            for i in range (len(lines)):
                ...
                #
                cmd = "echo %s >> %s_new" % (line, self._args.control_node_param_file)
                cn_ssh_fd.execCmd (cmd)
            ...
            for i in range (len(self.cn_envs)):
                cmd = "echo \%s >> %s_new" % (self.cn_envs[i], self._args.control_node_param_file)
                cn_ssh_fd.execCmd (cmd)
    ...
    def _delete_policy (self, block_index, policy_index, num_policies):
        ...
        self._log_print ("INFO: deleting: %s policies on: %s" %(num_policies, self._args.api_server_ip))
