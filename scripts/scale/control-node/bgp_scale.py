...
def open_logfile (fname):
    ...
    try:
        ...
        print ('ABORT: Failed to open file:%s, since this is a scaling script, check that the number of file descriptors (max files) has not been exceeded:[lsof -n, and ulimit -n]', fname)
        sys.exit()
...
def check_if_swapping (result, who, fd):
    ...
    try:
        ...
        if not re.search (' 0 kB', result):
            log_print ("WARNING: system swapping:{0} -> reboot device".format(who), fd)
            return_val = True
    except:
        pass
...
def check_for_crash (result, who, fd):
    ...
    if re.search ('core', result):
         log_print ("WARNING: crash found on: {0} crash files:\n{1}".format(who, result), fd)
         return_val = True
...
def bgp_scale_mock_agent (cn_usr, cn_pw, rt_usr, rt_pw, cn_ip, rt_ip, rt_ip2, xmpp_src, ri_domain, ri_name, ninstances, import_targets_per_instance, family, nh, test_id, nagents, nroutes, oper, sleep_time, logfile_name_bgp_stress, logfile_name_results, timeout_minutes_poll_prefixes, background, xmpp_prefix, xmpp_prefix_large_option, skip_krt_check, report_stats_during_bgp_scale, report_cpu_only_at_peak_bgp_scale, skip_rtr_check, bgp_env, no_verify_routes, logging):
    ...
    #
    prefixes_per_instance, vpn_prefixes = get_total_prefix_expectations (ninstances, import_targets_per_instance, nagents, nroutes, overlapped_prefixes)
    ...
    log_print ("INFO: BGP Stress Test - CN:{0}  family:{1}  Operation:{2}  ninst X nagent X nroutes = {5}x{3}x{4} NumImportTargetsPerRinstance:{6}".format(cn_ip, family, oper, nagents, nroutes, ninstances, import_targets_per_instance), fd)
    ...
    #
    instance_name, full_instance_name = get_instance_name (ninstances, ri_name, 1, ri_domain)
    ...
    try:
        trigger_file = re.search('send-trigger(\s+|=)(.*)$', logging, re.IGNORECASE)
        if trigger_file != None:
            ...
            log_print ("DEBUG: found trigger file: %s new_trigger_file:%s" %(trigger_file, new_trigger_file), fd)
    except:
        trigger_file = 0
    ...
    #
    bgp_stress_test_command = '%s ./bgp_stress_test --no-multicast --xmpp-port=5269 --xmpp-server=%s --xmpp-source=%s --ninstances=%s --instance-name=%s --test-id=%s --nagents=%s --nroutes=%s --xmpp-nexthop=%s %s %s %s %s' %(bgp_env, cn_ip, xmpp_src, ninstances, instance_name, test_id, nagents, nroutes, nh, xmpp_start_prefix, xmpp_prefix_large, logging, logfile_name)

    #
    # Get stats before test run
    #
    if report_stats_during_bgp_scale:
        report_stats (cn_self, rt_self, cnshell_self, cn_ip, rt_ip, "Stats Before Test Run {0}".format(oper), report_cpu_only_at_peak_bgp_scale, fd)

    #
    # Log expected values and bgp_stress_test command
    #
    msg = "ninst X nagent X nroutes = {0}x{1}x{2}".format(ninstances, nagents, nroutes)
    log_print ("INFO: BGP Stress Test - {0} Total Prefixes Expected/all-instances-cn:{1} Expected/instance-cn:{2} VPN Prefixes-rtr:{3}".format(omsg, prefixes_per_instance*ninstances, prefixes_per_instance, vpn_prefixes), fd)
    ...
    if re.search('del', oper, re.IGNORECASE):
        ...
        get_prefix_install_or_delete_time (cn_self, rt_self, cn_ip, rt_ip, ri_domain, instance_name, ninstances,  prefixes_per_instance, vpn_prefixes, op, family, nagents, nroutes, timeout_minutes_poll_prefixes, skip_krt_check, skip_rtr_check, no_verify_routes, xmpp_src, del_start_time, fd)
    ...
    #
        ...
        #
        log_print ("INFO: Starting bgp_stress on localhost %s" % (localhost_ip), fd)
        BGP_STRESS = Command(bgp_stress_test_command)
        BGP_STRESS.start()
        bgp_start_time = datetime.now()
        log_print ("INFO: notable_event started bgp_stress at timestamp: %s" % str(bgp_start_time), fd)

        #
        # Time how long it takes for peers to come up - abort gracefully if timed out
        #
        tdelta, timestamp_all_peers_up, timestamp_at_least_one_peer_up  = get_agent_bringup_time (cn_self, xmpp_src, full_instance_name, nagents, "xmpp", op, False, bgp_start_time, fd)
        ...
        if trigger_file and len(trigger_file):
            log_print ("INFO: trigger file found: %s sleeping for %s seconds before starting prefix announcements" % (new_trigger_file, sleep_time), fd)
            time.sleep(sleep_time)
            cmd = 'touch %s' % new_trigger_file
            try:
                return_val = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True)
            ...
            log_print ("INFO: notable_event prefix adds triggered at timestamp: %s" % str(timestamp_trigger_prefix_announcements), fd)
        ...
        if re.search('agents_only', oper, re.IGNORECASE):
            ...
            #
            if not background:
                report_stats (cn_self, rt_self, cnshell_self, cn_ip, rt_ip, "Stats After Install - Peak period {0}".format(oper), report_cpu_only_at_peak_bgp_scale, fd)

            log_print ("INFO: sleeping a long time %s seconds or until ctrl-c..." %sleep_time, fd)
            time.sleep (sleep_time)
        ...
        #
        get_prefix_install_or_delete_time (cn_self, rt_self, cn_ip, rt_ip, ri_domain, instance_name, ninstances, prefixes_per_instance, vpn_prefixes, op, family, nagents, nroutes, timeout_minutes_poll_prefixes, skip_krt_check, skip_rtr_check, no_verify_routes, xmpp_src, timestamp_start_prefix_announcement, fd)

        #
        # Perform post-install tasks such as stats reporting and sleeping
        #
        post_install_tasks (cnshell_self, cn_self, rt_self, cn_ip, rt_ip, oper, sleep_time, msg, background, report_stats_during_bgp_scale, report_cpu_only_at_peak_bgp_scale, fd)
        ...
        #
        log_print ("DEBUG: stopping route announcements/agents.. stopping python first", fd)
        kill_bgp_stress_python_call ('bgp_stress_test', 'python', fd)
        del_start_time = datetime.now()
        log_print ("INFO: notable_event stopping prefix announcements at timestamp: %s" % (str(del_start_time)), fd)
        ...
        #
        get_prefix_install_or_delete_time (cn_self, rt_self, cn_ip, rt_ip, ri_domain, instance_name, ninstances,  prefixes_per_instance, vpn_prefixes, "del", family, nagents, nroutes, timeout_minutes_poll_prefixes, skip_krt_check, skip_rtr_check, no_verify_routes, xmpp_src, del_start_time, fd)

    #
    # Get stats after test run
    #
    if report_stats_during_bgp_scale:
        report_stats (cn_self, rt_self, cnshell_self, cn_ip, rt_ip, "Stats After Test Run {0}".format(oper), report_cpu_only_at_peak_bgp_scale, fd)
...
def kill_bgp_stress_python_call (child_name, child_of_child_name, fd, retry = 2, delay = 5):
    ...
    #
    child_ps_line = subprocess.check_output('ps -efww | \grep %s | grep " %s " | grep -v grep' % (child_name, parent_pid), stderr=subprocess.STDOUT, shell=True)
    log_print ("DEBUG: bgp_stress ps line:%s that matched this child:%s and parent pid:%s" % (child_ps_line, child_name, parent_pid), fd)
    bgp_stress_test_pid = 0
    try: 
        re.search('\d+', child_ps_line)
        bgp_stress_test_pid = int(re.search('\d+', child_ps_line).group())
        log_print ("DEBUG: bgp_stress_test pid found:%s" %bgp_stress_test_pid, fd)
    except:
        pass

    while True and bgp_stress_test_pid:
        cmd = 'ps -efww |  grep %s | grep " %s " | grep -v %s | grep -v grep' % (child_of_child_name, bgp_stress_test_pid, child_name)
        ...
        #
        try:
            child_of_child_ps_line = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True)
            python_pid = int(re.search('\d+', child_of_child_ps_line).group())
            log_print ("DEBUG: found python pid:%s ps of bgp_stress_test child python:%s" % (python_pid, child_of_child_ps_line), fd)
            ...
            try:
                ...
                log_print ("DEBUG: SIGKILL %s pid:%s" % (child_of_child_name, python_pid), fd)
                break
            except OSError:
                pass
            ...
            retry -= 1
            log_print ("WARNING: command %s failed: %s" % (cmd, error.output), fd)
            ...
            if retry > 0:
                ...
                if re.search('defunct', child_ps_line, re.IGNORECASE):
                    log_print ("ERROR: bgp_stress_test pid:%s state is defunct (check if it crashed), no child python process, sending it SIGKILL line:%s" % (bgp_stress_test_pid, child_ps_line), fd)
                    if type(bgp_stress_test_pid) == int:
                        try:
                            os.kill(bgp_stress_test_pid, signal.SIGKILL)
                            log_print ("INFO: sending SIGKILL %s pid:%s" % (child_name, bgp_stress_test_pid), fd)
                        except OSError:
                            pass

                    break

                log_print("WARNING: retry:%d Command %s after sleeping  %d seconds" % (retry, cmd, delay), fd)
                time.sleep(delay)
                continue
            else:
                log_print("ERROR: problem bringing down python (called by bgp_stress_test pid:%s) move'n on.." % bgp_stress_test_pid, fd)
        break
...
def post_install_tasks (cnshell_self, cn_self, rt_self, cn_ip, rt_ip, oper, sleep_time, test_info, background, report_stats_during_bgp_scale, report_cpu_only_at_peak_bgp_scale, fd):
    ...
    log_print ("INFO: sleeping for %s seconds after prefix installation of: %s" % (sleep_time, msg), fd)
    time.sleep(sleep_time)
...
def get_agent_bringup_time (self, ip, full_instance_name, num_peers, encoding, oper, log_details, start_time, fd):
    ...
    log_print ("INFO: polling for %s agent peers to come up instance:%s oper:%s at:%s" % (num_peers, full_instance_name, oper, str(start_time)), fd)

    #
    # Note that only one instance name is used
    #
    tdelta, timestamp_done, timestamp_at_least_one_peer_up = get_time_bringup_or_teardown_peers (self, ip, full_instance_name, num_peers, encoding, oper, log_details, start_time, fd)

    #
    # Abort this instance of bgp_stress if agent peers bring-up times out
    #
    if tdelta == 'TimeoutWaitingPeersToComeUp':
        log_print ("ERROR: agent peers not coming up, aborting this bgp_stress_test result:%s " % tdelta, fd)
        kill_bgp_stress_python_call ('bgp_stress_test', 'python', fd)
        rc, out, err = BGP_STRESS.stop()
        fd.close()
        sys.exit()
...
def get_agent_teardown_time (self, ip, instance_full_name, num_peers, encoding, oper, log_details, start_time, fd):
    ...
    log_print ("INFO: Waiting for {0} agent peers to go dn.. instance:{1}".format(num_peers, full_instance_name), fd)
    tdelta, timestamp_done = get_time_bringup_or_teardown_peers (self, ip, instance_full_name, num_peers, encoding, oper, log_details, start_time, fd)
...
def get_time_bringup_or_teardown_peers (self, ip, instance_full_name, num_peers, encoding, oper, log_details, start_time, fd):
    ...
    if re.search('del', oper, re.IGNORECASE):
        ...
        max_time = 15 # min
        while peers_up > 0:
            peers_up = self.get_cn_bgp_neighbor_stats_element ('count', 'xmpp', 'up', instance_full_name)
            if (peers_up == 0):
                delta_time, timestamp_done = get_delta_time (start_time)
                log_print ("INFO: notable_event peers down at timestamp: %s", str(timestamp_done), fd)
                continue
            ...
            if time_chk >= max_time:
                 log_print ("ERROR: timeout waiting for peers to go down... waited:%s minutes" % time_chk, fd)
        ...
        while peers_up < num_peers:
            peers_up = int(self.get_cn_bgp_neighbor_stats_element ('count', 'xmpp', 'up', instance_full_name))
            ...
            if peers_up > 0 and at_least_one_peer_up_noted == 0:
                ...
                t1, timestamp_at_least_one_peer_up = get_delta_time (start_time)
                log_print ("INFO: notable_event at least one peer up at timestamp: %s" % str(timestamp_at_least_one_peer_up), fd)
            ...
            if (peers_up == num_peers):
                delta_time, timestamp_done = get_delta_time (start_time)
                log_print ("INFO: notable_event peers up at timestamp: %s" % str(timestamp_done), fd)
                continue
            ...
            #
            if waited > 120:
                log_print ("DEBUG: sleeping %s sec in get_time_bringup_or_teardown_peers oper: %s peers_up:%s (out of: %s) timeout_in:%sm" % (sleeptime_between_introspect_polls*4, oper, peers_up, num_peers, max_time-time_chk), fd)
            ...
            time_chk, time_now = get_delta_time (start_time, "minutes")
            if time_chk >= max_time:
                 log_print ("ERROR: timeout waiting for peers to come up... inst:%s waited:%s minutes tnow:%s" % (instance_full_name, str(time_chk), str(time_now)), fd)
        ...
        log_print ("ERROR: invalid operation in timing peer bringup/teardown:%s" % oper, fd)
        return None
    ...
    log_print ("INFO:    Elapsed time to %s %s peers:%ss (total peers found:%s)" % (oper, num_peers, delta_time_str, peers_up), fd)
...
def get_time_diffs_seconds (t1, t2, decimal_places):
    ...
    if type(t1) == datetime and type(t2) == datetime:
        ...
        if delta_time.days < 0:
            log_print ("ERROR: time diff results in a past date t1:%s t2:%s" % (t1, t2))
            return 0

        return_val = float("%s.%s" %(str(abs(delta_time).seconds), str(abs((delta_time)).microseconds)[:decimal_places]))

    else:
        log_print ("ERROR: time1 or time2 not type datatime: t1 type:%s t2 type:%s" % (type(t1), type(t2)))
...
def timedelta_to_string (delta_time):
    ...
    if type(delta_time) == timedelta:
        return_val = float("%s.%s" %(str(abs(delta_time).seconds), str(abs((delta_time)).microseconds)[:3]))
        #seconds = (delta_time).seconds
        #microseconds = (delta_time).microseconds
        #return_val = float("%s.%s" % (seconds, str(microseconds)[:3]))
    else:
        log_print ("ERROR: delta_time wrong type, expecting timedelta type(%s) is:%s" % (delta_time, type(delta_time)))
...
def get_localhost_ip ():
    ...
    ip = subprocess.check_output('resolveip -s $HOSTNAME', stderr=subprocess.STDOUT, shell=True)
...
def get_peer_states (self, xmpp_src, nagents, instance, pending_updates, fd):
    ...
    for i in range(nagents):
        status, peer_state = self.get_cn_bgp_neighbor_element (str(ip), "state")
        log_print ("INFO: instance:%s xmpp_peer:%s pending_updates:%s STATE:%s" % (instance, str(ip), pending_updates, peer_state), fd)
        ip += 1
...
def check_peers_up (self, ip_start, num_peers, encoding, oper, print_peer_status, fd):
    ...
    for i in range(num_peers):
        ...
        t1 = datetime.now()
        status, peer_state = self.get_cn_bgp_neighbor_element (str(ip), "state")
        status, peer_encoding = self.get_cn_bgp_neighbor_element (str(ip), "encoding") 
        t2 = datetime.now()
        ...
        #
            ...
            if print_peer_status:
                log_print ("INFO: Peer:{0} status:{1} state:{2} encoding_param:{3} encoding_found:{4}".format(ip, status, peer_state, encoding, peer_encoding), fd)

        t3 = datetime.now()
        ip += 1

    log_print ("INFO: Total peers established: {0} (out ot {1})".format(number_peers_up, num_peers), fd)
...
def check_peer_error (self, ip, fd):
    ...
    status, last_error = self.get_cn_bgp_neighbor_element (str(ip), 'last_error')
    ...
    if status is True and last_error:
        status, peer_encoding = self.get_cn_bgp_neighbor_element (str(ip), 'encoding')
        log_print ("WARNING: {0} peer:{1} error notification:{2} peer found?:{3}".format(peer_encoding, ip, last_error, status), fd)
...
def check_peers_for_errors (self, ip_start, num_peers, print_logs, fd):
    ...
    if number_peers_with_errs:
        log_print ("INFO: Total peers with errors logged:{0} (out of {1} checked)".format(number_peers_with_errs, num_peers), fd)
...
def get_rtr_dram_pct_utlization (self, fd):
    ...
    #
    mem1 = self.execCmd ('cli -c "show chassis routing-engine | display xml | match memory-dram-size"')
    memory_dram_size = int(re.search('\d+', mem1).group())

    mem2 = self.execCmd ('cli -c "show chassis routing-engine | display xml | match memory-buffer-utilization"')
...
def get_rt_l3vpn_prefixes (self, instance_name, ninstances, nbr_ip, fd):
    ...
    #
    active_prefixes_resp = self.execCmd ('cli -c "show bgp neighbor {0} | display xml | grep active-prefix-count"'.format(nbr_ip))
...
def get_cn_introspect_elements (self, ninstances, ri_domain, ri_name, family, xmpp_src, oper, nagents, time_chk, fd):
    ...
    for i in range (ninstances):
        ...
        #
        instance_name, full_instance_name = get_instance_name (ninstances, ri_name, i+1, ri_domain)
        ...
        status, nprefixes = self.get_cn_routing_instance_table_element (full_instance_name, family, 'prefixes')
        ...
        #
        status, pending_updates = self.get_cn_routing_instance_table_element (full_instance_name, family, 'pending_updates')
        status, markers = self.get_cn_routing_instance_table_element (full_instance_name, family, 'markers')
        status, paths = self.get_cn_routing_instance_table_element (full_instance_name, family, 'paths')
        status, primary_paths = self.get_cn_routing_instance_table_element (full_instance_name, family, 'primary_paths')
        status, secondary_paths = self.get_cn_routing_instance_table_element (full_instance_name, family, 'secondary_paths')
        status, infeasible_paths = self.get_cn_routing_instance_table_element (full_instance_name, family, 'infeasible_paths')
        ...
        if oper == "add" and type(pending_updates) == int and pending_updates > 0 and time_chk > 1:
            get_peer_states (self, xmpp_src, nagents, full_instance_name, pending_updates, fd)

        #
        # Get integer return values, or 0 (log errors)
        #
        cn_prefixes += check_introspect_return_values (nprefixes, 'prefixes', full_instance_name, fd)
        cn_pending_updates += check_introspect_return_values (pending_updates, 'pending_updates', full_instance_name, fd)
        cn_markers += check_introspect_return_values (markers, 'markers', full_instance_name, fd)
        # New:paths, primary_paths, secondary_paths and infeasible_paths
        cn_paths += check_introspect_return_values (paths, 'paths', full_instance_name, fd)
        cn_primary_paths += check_introspect_return_values (primary_paths, 'primary_paths', full_instance_name, fd)
        cn_secondary_paths += check_introspect_return_values (secondary_paths, 'secondary_paths', full_instance_name, fd)
        cn_infeasible_paths += check_introspect_return_values (infeasible_paths, 'infeasible_paths', full_instance_name, fd)
...
def check_done_flags (cn_done, rt_done, skip_rtr_check, skip_krt_check, krt_clear, oper, timestamp_done_cn, timestamp_done_rt, fd):
    ...
    if (cn_done == True and rt_done == True and (skip_krt_check != 0 or krt_clear == True)):
        ...
        log_print ("INFO: notable_event finished prefix %s timestamp: %s" % (oper, str(timestamp_done)), fd)
...
def get_prefix_install_or_delete_time (cn_self, rt_self, cn_ip, rt_ip, ri_domain, ri_name, ninstances, prefixes_per_instance, vpn_prefixes, oper, family, nagents, nroutes, timeout_minutes_poll_prefixes, skip_krt_check, skip_rtr_check, no_verify_routes, xmpp_src, start_time, fd):
    ...
    while True:
        ...
        #
        if not cn_done:
            cn_prefixes, cn_pending_updates, cn_markers, cn_paths, cn_primary_paths, cn_secondary_paths, cn_infeasible_paths = get_cn_introspect_elements (cn_self, ninstances, ri_domain, ri_name, family, xmpp_src, oper, nagents, time_chk, fd)
        ...
        if not rt_done:
            if rt_self != None:
                rt_vpn_prefixes, rt_prefixes = get_rt_l3vpn_prefixes (rt_self, ri_name, ninstances, cn_ip, fd)

        #
        # Check if control node is done, but only if not already done in previous loop iteration
        #
        if not cn_done:
            cn_done = check_if_done_polling_for_prefixes (oper, cn_prefixes, expected_prefix_count, "cn", cn_pending_updates, fd)
            ...
            if cn_done: 
                ...
                cn_delta_seconds = get_time_diffs_seconds (start_time, timestamp_done_cn, decimal_places=2)

        #
        # Check if rotuer is done, note it's total is based on the bgp.l3vpn table.
        #
        if (rt_done == False or krt_clear == False):
            rt_done = check_if_done_polling_for_prefixes (oper, rt_vpn_prefixes, expected_prefix_count, "rt", cn_pending_updates, fd)
            krt_clear = skip_krt_check != 0 or check_krt_queue_empty (rt_self, oper, rt_vpn_prefixes, expected_prefix_count, fd)
            ...
            if (rt_done == True and krt_clear == True):
                rt_delta_seconds, timestamp_done_rt = get_delta_time (start_time, 'seconds')
        ...
        time_chk, time_now = get_delta_time (start_time, 'minutes')
        if time_chk >= max_time:
            log_print("ERROR: timeout waiting for route install, total expected prefixes:{0} cn had:{1} cn_pending_updates:{2} rtr had:{3} waited {4} min, expected_prefix_count {5}, rt_done {6}, cn_done {7}, krt_clear {8}".format(total_expected_prefixes, cn_prefixes, cn_pending_updates, rt_prefixes, max_time, expected_prefix_count, rt_done, cn_done, krt_clear), fd)
        ...
        #
        msg1 = "INFO: pfxes:%s pending:%s marker:%s paths:%s primry:%s secondry:%s infeasbl:%s" % (cn_prefixes, cn_pending_updates, cn_markers, cn_paths, cn_primary_paths, cn_secondary_paths, cn_infeasible_paths)
        msg_last = "ri:%s op:%s cdone:%s timeout-in:%sm pfx_expected:%s" % (ri_name, oper, cn_done, max_time-time_chk, expected_prefix_count)

        #
        # Optionally append router stats for log
        #
        if skip_rtr_check == 0:
            msg1 = "%s rt_vpn:%s rt:%s rdone:%s" % (msg1, rt_vpn_prefixes, rt_prefixes, rt_done)
    ...
    if not re.search('timeout', str(return_val), re.IGNORECASE):
        report_delta_times (oper, total_expected_prefixes, cn_delta_seconds, rt_delta_seconds, ninstances, nagents, nroutes, skip_rtr_check, fd)
...
def report_stats (cn_self, rt_self, cnshell_self, cn_ip, rt_ip, msg, report_cpu_only_at_peak_bgp_scale, fd):
    ...
    #
    log_print (" ", fd)
    log_print ("============================ <Begin> {0} ===============================".format(msg), fd)
    #
    # Control Node cpu
    #
    result1 = get_shell_cmd_output (cnshell_self, 'cat /proc/stat | grep -i cpu', fd)
    ...
    mem_result5 = subprocess.check_output('pmap {0} | grep -i total'.format(pid), stderr=subprocess.STDOUT, shell=True)
    log_print ("ip:{0} Localhost Memory:".format(local_ip), fd)
    log_print ('ip:{0} egrep "Mem|Cache|Swap" /proc/meminfo\n{1}'.format(local_ip, mem_result1), fd)
    log_print ('ip:{0} ps -e -orss=,args= | sort -b -k1,1n \n{1}'.format(local_ip, mem_result2), fd)
    log_print ('ip:{0} ps -e -ovsz=,args= | sort -b -k1,1n \n{1}'.format(local_ip, mem_result3), fd)
    log_print ('ip:{0} vmstat\n{1}'.format(local_ip, mem_result4), fd)
    log_print ('ip:{0} pmap {1} | grep -i total\n{2}'.format(local_ip, pid, mem_result5), fd)
    ...
    log_print ("ip:{0} Localhost File Descriptors (lsof -n):".format(local_ip), fd)
    ...
    if rt_self != None:
        ...
        log_print ("ip:{0} Memory utilization {1} percent".format(rt_ip, used), fd)
    ...
    log_print ("ip:{0} Control Node ps info with env variables:".format(cn_ip), fd)
    log_print ("ip:{0} ps e `pidof control-node`\n{1}".format(cn_ip, result1), fd)

    #
    # Control node  ulimit settings
    #
    result1 = get_shell_cmd_output (cnshell_self, 'cat /proc/`pidof control-node`/limits', fd)
    log_print ("ip:{0} Control Node Ulimit Settings:".format(cn_ip), fd)
    log_print ("ip:{0} cat /proc/`pidof control-node`/limits\n{1}".format(cn_ip, result1), fd)

    #
    # Control Node memory info
    #
    mem_result1 = get_shell_cmd_output (cnshell_self, 'egrep "Mem|Cache|Swap" /proc/meminfo', fd)
    mem_result2 = get_shell_cmd_output (cnshell_self, 'ps -e -orss=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20', fd)
    mem_result3 = get_shell_cmd_output (cnshell_self, 'ps -e -ovsz=,args= | sort -b -k1,1n | pr -TW195 | sort -rn | head -n 20', fd)
    mem_result4 = get_shell_cmd_output (cnshell_self, 'vmstat', fd)
    mem_result5 = get_shell_cmd_output (cnshell_self, 'pmap `pidof control-node` | grep -i total', fd)
    ...
    log_print ('ip:{0} pmap `pidof control-node` | grep -i total\n{1}'.format(cn_ip, mem_result5), fd)
    ...
    result2 = get_shell_cmd_output (cnshell_self, 'lsof -n | grep -i tcp | wc -l', fd)
    #result3 = get_shell_cmd_output (cnshell_self, 'lsof -i | wc -l', fd) # can hang system...
    log_print ("ip:{0} Control Node File Descriptors (lsof -n):".format(cn_ip), fd)
    ...
    log_print ('ip:{0} Control Node Open Ports and Established TCP Sessions "netstat -vatn | wc -l"\n{1}'.format(cn_ip, result1), fd)
    ...
    log_print ("============================ {0} <End> =======================".format(msg), fd)
    log_print (" ", fd)

    #
    # Get control node swap info
    #
    result = get_shell_cmd_output (cnshell_self, 'egrep "SwapCached" /proc/meminfo', fd)
    ...
    swap2 = check_if_swapping (result, "Localhost:%s running bgp_stress_test code" %local_ip, fd)

    if swap1 or swap2:
        log_print ("WARNING: control node swap status:{0}, localhost swap status:{1}".format(swap1, swap2), fd)
...
def get_delta_time (t1, units=''):
    ...
    if units == 'minutes':
        ...
        return_val = "%s.%s" % (int(((t2-t1).seconds)/60), ((t2-t1).microseconds))
...
def utc_to_tz (ut, time_zone=''):
    ...
    #
    if not time_zone: time_zone = 'US/Pacific'
    ...
    new_time = new_tz.normalize(d.astimezone(new_tz)).strftime('%Y-%b-%d %H:%M:%S.%f')
