...
class VnCfg(object):
    ...
    def _run(self, args_str = None, oper=''):
        ...
        if self._quantum == None:
            ...
            self._proj_obj = self._vnc_lib.project_read(fq_name = ['default-domain', 'demo'])
            self._ipam_obj = self._vnc_lib.network_ipam_read(fq_name = ['default-domain', 'default-project', 'default-network-ipam'])
