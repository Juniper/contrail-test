...
class Controller (object):
    ...
    def _edit_params (self, num_cpu_threads, ts_index, cn_index):
        ...
        self.fl._log_print ("INFO: changing number cpu_threads to: %s on control-node: %s" % (num_cpu_threads, self.fl.cn_ips[cn_index]))
        ts_fd = self.ts_ssh_fds[ts_index]
        cmd = 'sed -i.bak -re "s/(TBB_THREAD_COUNT=)([0-9]+)/\\1%s/g"  %s/params.ini' % (num_cpu_threads, self.fl._args.run_dir)
    ...
    def _adjust_contrail_services (self):
        ...
        fl._log_print ("INFO: stopping vrouter on api-server: %s cmd: %s" % (fl._args.api_server_ip, cmd))
        ...
        #
        if result == None:
            fl._log_print ("WARNING: trying again in %ss - problem stopping vrouter on api-server: %s result: %s" % (fl._args.sleeptime_between_polling_process, fl._args.api_server_ip, result))
            time.sleep (int(fl._args.sleeptime_between_polling_process))

            result = fl.api_ssh_fd.execCmd (cmd)
            if result == None:
                fl._log_print ("ERROR: failed to stop vrouter on api-server: %s result: %s move'n on.." % (fl._args.api_server_ip, result))
                #self._cleanup ()
                #sys.exit()
    ...
    def _stop_contrail_services_and_reboot (self, test_run=0):
        ...
        for i in range (len(fl.cn_ips)):
            ...
            cn_ip = fl.cn_ips[i]
            fl._log_print ("INFO: stopping services in preparation for rebooting control-node: %s testrun: %s" % (cn_ip, test_run))
        ...
        #
        fl._log_print ("INFO: stopping services in preparation for rebooting api-server: %s testrun: %s" % (fl._args.api_server_ip, test_run))
        ...
        #
        fl._log_print ("INFO: sleeping %ss after stopping contrail services and before reboot testrun: %s" % (fl._args.sleeptime_after_cn_restart, test_run))
        ...
        fl._log_print ("INFO: rebooting api server: %s testrun: %s" % (fl._args.api_server_ip, test_run))
        result1 = fl.api_ssh_fd.execCmd (cmd)
        sleep_time = int(fl._args.sleeptime_after_reboot_api)
        self._sleep_awhile (sleep_time, msg="sleeping %ss after reboot and before starting to poll for api server back up.. testrun: %s" % (sleep_time, test_run))
        ...
        for i in range (len(fl.cn_ips)):
            ...
            # Issue/log reboot
            #
            fl._log_print ("INFO: rebooting control node: %s testrun: %s" % (cn_ip, test_run))
        ...
        for i in range (self.num_test_servers):
            ...
            # Issue/log reboot
            #
            fl._log_print ("INFO: rebooting test server: %s testrun: %s" % (ts_ip, test_run))
            ...
            self._sleep_awhile (sleep_time, msg="sleeping %ss between rebooting testservers.. testrun: %s" % (sleep_time, test_run))
        ...
        self._sleep_awhile (sleep_time, msg="sleeping %ss after reboot and before starting to poll for servers back up.. testrun: %s" % (sleep_time, test_run))
    ...
    def _get_num_cpu_threads (self):
        ...
        for i in range (len(fl.cn_ips)):
            ...
            fl._log_print ("INFO: number of cpu threads available on control node: %s is: %s affinity mask: %s" % (cn_ip, cpu_threads, affinity_mask))

            #
            # Store cpu available, cpu running on cotrol node and affinity mask info
            #
            self.num_cpu_threads.append(cpu_threads)
            self.num_cpu_threads_running.append(self._get_num_cpu_threads_running(cnshell_self, cn_ip, cpu_threads))
    ...
    def _get_num_cpu_threads_running (self, fd, ip, cpu_trheads_available):
        ...
        if res == None:
            fl._log_print ("DEBUG: not able to get control-nodeI: %s env var cpu_threads process is not running, returning previous /proc num: %s" % (ip, cpu_trheads_available))
        ...
        if re.search('TBB_THREAD_COUNT=', res):
            ...
            if num_cpu_threads.isdigit():
                ...
                fl._log_print ("INFO: number of cpu threads set as env variable on control node process is: %s on cn: %s" % (return_val, ip))
        ...
        #
            ...
            fl._log_print ("INFO: number of cpu threads in use on control node: %s is not set as env variable - default from /proc is: %s" % (ip, cpu_trheads_available))
    ...
    def _check_servers_up_after_reboot (self, api_only=False):
        ...
        while True:
            ...
            if not status1:
                ...
                for i in range (len(fl.cn_ips)):
                    ...
                    if cn_up_status[i] == False:
                        fl._log_print ("WARNING: control node server: %s not responding.." % fl.cn_ips[i])
                    else:
                        pass
            ...
            if not status3:
                ...
                for i in range (self.num_test_servers):
                    ...
                    if ts_up_status[i] == False:
                        fl._log_print ("ERROR: test server: %s not responding.." % self.ts_ips[i])
                    else:
                        pass
            ...
            if status1 and status2 and status3:
                fl._log_print ("INFO: all servers that were online originally are back up")
                break

            delta_time = fl._get_time_diffs_seconds (start_time, datetime.now(), 0)
            time_left_sec = int(fl._args.timeout_seconds_server_up_after_reboot)-delta_time
            if delta_time > int(fl._args.timeout_seconds_server_up_after_reboot):
                fl._log_print ("ABORT: timed out waiting for servers to come back up - no time left- waited: %ss" % delta_time)
                self._cleanup ()
                sys.exit()
            else:
                time_left_sec = int(fl._args.timeout_seconds_server_up_after_reboot)-delta_time
                time_left_min = int(time_left_sec/60)
                sleep_time = int (fl._args.sleeptime_between_polling_servers_back_up)
                self._sleep_awhile (sleep_time, msg="sleeping %ss until next poll - timout in: %sm" % (sleep_time, time_left_min))
    ...
    def _check_bgp_up_to_mx (self):
        ...
        #
        cmd = "%s %s | grep %s" % (fl._args.rtr_cli_show_bgp_neighbor_cmd, cn_ip, fl._args.rtr_bgp_up_state)
        fl._log_print ("INFO: checking if bgp is up on router: %s cmd: %s" % (fl._args.rtr_ip, cmd))
        start_time = datetime.now()
        first_timeout = False
        while True:
            #
            ...
            if result != None and re.search(fl._args.rtr_bgp_up_state, result):
                fl._log_print ("INFO: bgp up between control-node: %s and mx: %s" % (cn_ip, fl._args.rtr_ip))
            ...
            # Check timeouts
            #
            timed_out, timed_minutes = self._check_timeout_minutes (start_time, int(fl._args.timeout_minutes_bgp_to_come_up))
            if timed_out:
                #
                # Try restarting control node process
                #
                if not first_timeout:
                    fl._log_print ("WARNING: starting to time out waiting for bgp up between control-node: %s and mx: %s waited: %smin, restarting cn" % (cn_ip, fl._args.rtr_ip, timed_minutes))
                    fl._restart_cn(why="bc peering with MX not coming up", restart_anyway=1)
                    first_timeout = True
                    continue
                else:
                    fl._log_print ("ABORT: timed out waiting for bgp up between control-node: %s and mx: %s waited: %smin" % (cn_ip, fl._args.rtr_ip, timed_minutes))
                    self._cleanup ()
                    sys.exit()
            else:
                pass
    ...
    def _get_remote_pid (self, remote_fd, remote_ip, parent_name, child_name):
        ...
        if remote_parent_pid.isdigit():
            cmd = "ps -efww | grep %s | grep ' %s ' | grep -v %s | grep -v grep |  gawk '\\''{print $2}'\\''" % (child_name, remote_parent_pid, parent_name)
            remote_child_pid = remote_fd.execCmd (cmd)[:-1]

            if remote_child_pid.isdigit():
                return_val = remote_child_pid
                fl._log_print ("INFO: found pid: %s for process: %s on node: %s" % (remote_child_pid, child_name, remote_ip))
            else:
                return_val = 0
                pass
    ...
    def _wait_until_done (self, process, num_cpu_threads=0):
        ...
        timed_out, timed_minutes = self._check_timeout_minutes (start_time, int(fl._args.timeout_minutes_wait_process))
        while not done:
            ...
            for i in range (self.num_test_servers):
                ...
                if self.ts_ip_status[i] == None and not self.ts_done_status[i]:
                    fl._log_print ("INFO: skip polling test server: %s for test completion server not up" % remote_ip)
                ...
                if remote_pid == None:
                    num_done += 1
                    fl._log_print ("INFO: skip polling test server: %s process was never found" % remote_ip)
                    self.ts_done_status[i] = True
                    continue

                fl._log_print ("INFO: polling test server: %s for process/pid: %s/%s" % (remote_ip, process, remote_pid))

                #
                # Check status of pid
                #
                cmd = "ps -efww | grep %s | grep %s | grep -v grep" % (process, remote_pid)
                ...
                if result == None or not re.search(remote_pid, result):
                    ...
                    if num_cpu_threads:
                        fl._log_print ("INFO: test server: %s done no process: %s with pid: %s found cpu_threads: %s" % (remote_ip, process, remote_pid, num_cpu_threads))
                    else:
                        fl._log_print ("INFO: test server: %s done no process: %s with pid: %s" % (remote_ip, process, remote_pid))
                ...
                timed_out, timed_minutes = self._check_timeout_minutes (start_time, int(fl._args.timeout_minutes_wait_process))
                if timed_out:
                    fl._log_print ("ERROR: timed out waiting for test server: %s to finish waited: %ss  - killing remote process pid: %s" % (remote_ip, timed_minutes, remote_pid))
            ...
            if done or num_done == self.num_test_servers:
                fl._log_print ("INFO: done checking for remote testserver status")
                break

            time_left = int(fl._args.timeout_minutes_wait_process)-timed_minutes
            fl._log_print ("INFO: sleeping: %ss until next poll - timeout in: %sm" % (fl._args.sleeptime_between_polling_process, time_left))
            time.sleep (int(fl._args.sleeptime_between_polling_process))
    ...
    def _execute_test_cmd (self, iteration_index=0, num_cpu_threads=0):
        ...
        for i in range (self.num_test_servers):
            ...
            if int(fl._args.rule_ts_cn_one_to_one):
                cmd = "%s %s %s" % (cmd, fl._args.rule_ts_cn_one_to_one_knob, i)

            #
            #  Add optinal knob for nroutes
            #
            if int(fl._args.rule_nroutes_per_all_agents):
                num_routes = self._get_num_routes_per_knobs_and_iteration (iteration_index)
                cmd = "%s %s %s" % (cmd, fl._args.rule_nroutes_per_all_agents_knob, num_routes)

            fl._log_print ("INFO: running on testserver: %s command: '%s'" % (ts_ip, cmd))
            ts_fd.execCmd (cmd)

            #
            # Get pid of parent flap process (all logs will have this pid)
            #
            pid = self._get_remote_pid (ts_fd, ts_ip, 'SCREEN', fl._args.bgp_test_script)
            ...
            #
            if pid == None:
                fl._log_print ("WARNING: problem getting pid from test server: %s'" % ts_ip)
    ...
    def _prepare_scripts (self, descr='pre-run'):
        ...
        for i in range (self.num_test_servers):
            ...
            fl._log_print ("INFO: untar script file on test server: %s - (assume tar'd dirs match run dir) cmd: %s" % (ts_ip, cmd))
    ...
    def _mv_rundir_log (self, ip, fd, descr):
        ...
        if result != None and len(result) > 0:
            cmd = 'mv {0}/log {0}_{1}_{2}_log'.format(fl._args.run_dir, descr, datetime.strftime(datetime.now(), '%m_%d_%Y-%H_%M_%S'))
            fl._log_print ("INFO: moving run dir log to named run dir on test server: %s cmd: %s" % (ip, cmd))
            result = fd.execCmd (cmd)
        else:
            fl._log_print ("INFO: did not find rundir log to move: %s/log" % script_dir)
    ...
    def _run_cpu_test (self):
        ...
        fl._log_print ("INFO: starting test '_run_cpu_test' on localhost: %s" % fl.localhost_ip)
    ...
    def _get_results (self):
        ...
        for i in range (self.num_test_servers):
            ...
            # Get logs
            #
            self._get_result_logs (ts_ip, ts_fd, fl._args.logfile_name_flap_agent_scale_test, fl._args.logdir_name, ts_pid)
    ...
    def _get_result_logs (self, dst, fd, name, logdir, pid):
        ...
        if result != None and len(result) > 0:
            ...
            #
            cmd = 'cd {0}; tar cvf {1}_{2}_{3}.tar *{3}*.*'.format(logdir, name, dst, pid)
            fl._log_print ("INFO: tar log files on test server: %s cmd: %s" % (dst, cmd))
            #result = fd.execCmd (cmd)
        else:
            fl._log_print ("ERROR: problem tarring remote logs - no log dir: %s found on testserver: %s" % (logdir, dst))
    ...
    def _run_test (self):
        ...
        fl._summary_print ("num_tests: %s num_testservers: %s num_control_nodes: %s" % (fl._args.rule_num_tests, len(fl.ts_ips), len(fl.cn_ips)))
        line = "%s\t%s\t%s\t%s\t" % ("ntest", "ip", "#iter", "#xmpp")
        line += "%s\t\t%s\t\t%s\t\t%s\t\t%s\t" % ("#iter_add", "#iter_del", "adds", "dels", "tpeers")
        line += "%s\t%s\t\t%s\t\t%s\t%s\t" % ("tadd", "tdel", "tpeer_ps", "updt_add_ps", "updt_del_ps")
        line += "%s\t%s\t%s\t%s\t" % ("tpeers_and_adds", "rts_all_ts", "rts_all_ts_M", "rts_all_per_ts")
        line += "%s\t%s" % ("total_updates", "total_updates_M")
        fl._summary_print (line)

        fl._log_print ("INFO: starting test '_run_test' %s times on localhost: %s" % (fl._args.rule_num_tests, fl.localhost_ip))
        ...
        for i in range (int(fl._args.rule_num_tests)):
            ...
            fl._log_print ("INFO: start test_n: %s test params: %s" % (i, descr))
            ...
            #
            #self._get_results()
            fl._log_print ("INFO: done test_n: %s test params: %s" % (i, descr))
            ...
            if (i+1) != int(fl._args.rule_num_tests):
                ...
                self._sleep_awhile (sleep_time, msg="sleeping %ss between top parent test runs %s" % (sleep_time, i))


        # end test loop

        fl._log_print ("INFO: done test '_run_test' on localhost: %s ran: %s tests" % (fl.localhost_ip, ntimes))
    ...
    def _get_total_routes(self, rindex):
        ...
        return_val =  fl.num_iterations * fl.ninstances[0] * fl.nagents[0] * self._get_num_routes_per_knobs_and_iteration(rindex)
    ...
    def _get_result_data(self, rindex, descr):
        ...
        for i in range (len(fl.ts_ips)):
            ...
            remote_pid = self.pid_parent_flap[i]
            remote_filename =  "%s/%s_%s.txt" % (fl._args.logdir_name, fl._args.result_averages_logfilename, remote_pid)
            ...
            fl._summary_print ("test %s.%s param descr: %s" % (rindex, i, log_descr))
            if descr != log_descr:
                fl._print_summary_and_regular_log ("WARNING: descr and log_descr don't match: %s vs %s ip: %s ts: %s.%s" % (descr, log_descr, ts_ip, rindex, i))

            fl._log_print ("INFO: getting summary datapoints results from ts: %s params: %s filename: %s total_expected_routes: %s" % (ts_ip, log_descr, remote_filename, total_expected_routes))
            ...
            #
            total_time_peers_up_and_adds = self._get_digit(tpeer) + self._get_digit(tadd)
            total_routes_all_servers = total_expected_routes * self.num_test_servers
            total_routes_all_servers_M = round(float(total_expected_routes * self.num_test_servers) / 1000000, 2)

            total_updates = total_expected_routes * fl.nagents[0]
            total_updates_M = round(float(total_expected_routes * fl.nagents[0]) / 1000000, 1)

            line = "%s\t%s\t%s\t%s\t%s\t\t" % (rindex, ts_ip, fl.num_iterations, xmpp_peers, add_iterations)
            line += "%s\t\t%s\t\t%s\t\t%s\t\t" % (del_iterations, add_prefixes, del_prefixes, tpeer)
            line += "%s\t%s\t\t%s\t\t%s\t%s\t" % (tadd, tdel, peer_per_sec, add_per_sec, del_per_sec)
            line += "%s\t%s\t\t%s\t%s\t" % (total_time_peers_up_and_adds, total_routes_all_servers, total_routes_all_servers_M, total_expected_routes)
            line += "%s\t%s" % (total_updates, total_updates_M)
            fl._summary_print (line)
            ...
            #
            remote_filename =  "%s/%s_%s.log" % (fl._args.logdir_name, fl._args.logfile_name_flap_agent_scale_test, remote_pid)
            fl._log_print ("INFO: getting memory stats from ts: %s filename: %s" % (ts_ip, remote_filename))

            #
            # Print headers in summary file - format targetsexcel import
            #
            fl._summary_print ("mem_stat_id\tts_ip\t\tcn_ip\t\ttot_mem_kB\t\tcn_mem unit\tnormalized_cn_mem")
            line_prepend = "mem_stat: %s\t%s\t" % (rindex, ts_ip)
            for i in range (len(fl.cn_ips)):
                cn_fd = fl.cn_ssh_fds[i]
                cn_ip = fl.cn_ips[i]

                cmd = "grep %s %s | grep total-free-cached | gawk '\\''{print $6 \"\t\" $11 \"\t\t\" $18 \" \" $19 \"\t\t\" $21}'\\'' " % (cn_ip, remote_filename)
                ...
                result = self._get_remote_cli_output(cmd, ts_fd)
                fl._log_print ("INFO: begin getting errors and warning from ts: %s \n\tcmd: %s \nbegin --------> result:\n%s\nend <--------" % (ts_ip, cmd, result))
                fl._log_print ("INFO: end getting errors and warning from ts: %s result from cmd: %s" % (ts_ip, cmd))
        ...
        if int(fl._args.rule_nroutes_per_all_agents):
            if int(fl._args.rule_nroutes_start_add_val_per_call) > 0:
                if iteration_index == 0:
                    ...
                    num_routes = int(fl._args.rule_nroutes_start_add_val_per_call) + int(fl._args.rule_nroutes_add_val_per_call) * iteration_index
            else:
                num_routes = int(fl._args.rule_nroutes_add_val_per_call) * (iteration_index + 1)
        else:
    ...
    def _get_params_descr (self, iteration_index):
        ...
        nroutes = self._get_num_routes_per_knobs_and_iteration (iteration_index)
        return_val = "%s_iterations_at_%sx%sx%s" %(fl.num_iterations, fl.ninstances[0], fl.nagents[0], nroutes)
    ...
    def _order_summary_lines (self, sum_fname, sorted_sum_fname):
        ...
        #
        cmd = 'head -3 {0} | /bin/egrep -v "Summary|num" > {1}; cat {0} | /bin/egrep -v "Summary|WARNING|param|test|mem_stat" | sort -k2,2d -k1,1d >> {1}; grep -m 1 mem_stat_id {0} >> {1};grep "mem_stat\:" {0} >> {1}'.format(sum_fname, sorted_sum_fname)
        status, result = fl._get_subprocess_info (cmd)
    ...
    def _cleanup (self):
        ...
        try:
            if fl.summary_fd:
                sorted_filename =  "%s/sorted_%s_%s.txt" % (fl._args.logdir_name, fl._args.result_averages_logfilename, fl.pid)
                self._order_summary_lines (fl.summary_fd.name, sorted_filename)
                fl._log_print ("INFO: result file: %s" % fl.summary_fd.name)
                fl._log_print ("INFO: sorted result file: %s" % sorted_filename)
        except:
            pass
        ...
        fl._log_print ('INFO: grep -v egrep %s/*%s* | grep -v "abrt-watch-log" | egrep --color=auto "WARNING|ERROR"' % (fl._args.logdir_name, fl.pid))
        fl._log_print ('INFO: grep -v egrep %s/*%s* | grep -v "abrt-watch-log" | egrep --color=auto "done no process"' % (fl._args.logdir_name, fl.pid))
