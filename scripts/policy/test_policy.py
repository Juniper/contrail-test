...
class TestBasicPolicy0(BasePolicyTest):
    ...
    def test_policy_with_multi_vn_in_vm(self):
        ...
        policy1_fixture= self.useFixture( PolicyFixture( policy_name= policy1_name, rules_list= rules1, inputs= self.inputs,\
            connections= self.connections))
        policy2_fixture= self.useFixture( PolicyFixture( policy_name= policy2_name, rules_list= rules2, inputs= self.inputs,\
            connections= self.connections))
        ...
        for ip in multivn_vm_ip_list:
            if ip not in output:
                self.logger.error("IP %s not assigned to any eth intf of %s"%(ip,vm1_fixture.vm_name))
                assert False
    ...
    def test_remove_policy_with_ref(self):
        ...
        for policy in pol_list['policys']:
            if policy['name']== policy_name:
                pol_id = policy['id']
                policy_removal = self.quantum_fixture.delete_policy(policy['id']);
                break
        self.assertFalse(policy_removal,'Policy removal succeed as not expected since policy is referenced with VN')
        assert vn1_fixture.verify_on_setup()
        policy_fixture.verify_policy_in_api_server()

        if vn1_fixture.policy_objs:
            policy_fq_names= [ self.quantum_fixture.get_policy_fq_name( x ) for x in vn1_fixture.policy_objs]

        self.assertTrue(vn1_fixture.verify_vn_policy_in_vn_uve(),"Policy information is not found in VN UVE Analytics after binding policy to VN")
        # unbind the policy from VN
        vn1_fixture.unbind_policies(vn1_fixture.vn_id,policy_fq_names)
        self.assertTrue(vn1_fixture.verify_vn_policy_not_in_vn_uve(),"Policy information is not removed from VN UVE Analytics after policy un binded from VN")
        # Verify policy ref is removed from VN
        vn_pol_found = vn1_fixture.verify_vn_policy_not_in_api_server(policy_name)
        self.assertFalse(vn_pol_found,'policy not removed from VN after policy unbind from VN')
...
class TestBasicPolicy2(BasePolicyTest):
    ...
    def test_policy_protocol_summary(self):
        ...
        policy1_fixture= self.useFixture( PolicyFixture( policy_name= policy1_name, rules_list= rules1, inputs= self.inputs,connections= self.connections ))
        policy2_fixture= self.useFixture( PolicyFixture( policy_name= policy2_name, rules_list= rules2, inputs= self.inputs,connections= self.connections ))

        vn1_fixture = self.useFixture(VNFixture(project_name= self.inputs.project_name, connections= self.connections,vn_name=vn1_name, inputs= self.inputs, subnets= vn1_subnets, policy_objs=[policy1_fixture.policy_obj]))
        assert vn1_fixture.verify_on_setup()
        ...
        vn_fq_name= inspect_h.get_vna_vn(domain='default-domain', project=proj_name, vn_name= vn1_name)['name']
        ...
        policy_fq_name2 = self.quantum_fixture.get_policy_fq_name(policy2_fixture.policy_obj)
        ...
        out= policy_test_utils.compare_args('policy_rules',vna_acl1['entries'],vna_acl2['entries'])

        if out :
            self.logger.info("policy rules are not matching with expected %s  and actual %s" %(vna_acl1['entries'] ,vna_acl2['entries']))
...
class TestBasicPolicy3(BasePolicyTest):
    ...
    def test_policy_RT_import_export(self):
        ...
        rules = [
            { 'direction': '<>', 'simple_action': 'pass', 'protocol': 'icmp', 'source_network': vn1_name, 'dest_network': vn2_name },
            { 'direction': '<>', 'simple_action': 'deny', 'protocol': 'icmp', 'source_network': vn1_name, 'dest_network': vn3_name },
            { 'direction': '<>', 'simple_action': 'pass', 'protocol': 'icmp', 'source_network': vn2_name, 'dest_network': vn3_name },
            { 'direction': '<>', 'simple_action': 'pass', 'protocol': 'icmp', 'source_network': 'any', 'dest_network': vn3_name },
             ]
        rev_rules2= [
            { 
        ...
        policy2_fixture= self.useFixture( PolicyFixture( policy_name= policy2_name, rules_list= rev_rules1, inputs= self.inputs,connections= self.connections ))
        policy3_fixture= self.useFixture( PolicyFixture( policy_name= policy3_name, rules_list= rules2, inputs= self.inputs,connections= self.connections ))
        policy4_fixture= self.useFixture( PolicyFixture( policy_name= policy4_name, rules_list= rev_rules2,inputs= self.inputs,connections= self.connections ))
        ...
        assert vn2_fixture.verify_on_setup()
        vn3_fixture=  self.useFixture(VNFixture(project_name= self.inputs.project_name, connections= self.connections,vn_name=vn3_name,inputs= self.inputs, subnets= vn3_subnets))
        assert vn3_fixture.verify_on_setup()

        self.logger.info("TEST STEP: End of setup")
        vn_fixture= {vn1_name: vn1_fixture, vn2_name: vn2_fixture, vn3_name: vn3_fixture}
        vnet_list= [vn1_name, vn2_name, vn3_name]

        self.logger.info("TEST STEP: Route verification for VN after setup")
        actual_peer_vns_by_policy= policy_test_utils.get_policy_peer_vns(self, vnet_list, vn_fixture)
        for vn in vnet_list:
            err_msg_on_fail= "route verification failed for vn %s" %(vn)
            assert (vn_fixture[vn].verify_vn_route_target(policy_peer_vns= actual_peer_vns_by_policy[vn]) == True), err_msg_on_fail
        ...
        vn3_fixture.bind_policies( [policy3_fixture.policy_fq_name],vn3_fixture.vn_id)
        ...
        for vn in vnet_list:
            err_msg_on_fail= "route verification failed for vn %s" %(vn)
            out = vn_fixture[vn].verify_vn_route_target(policy_peer_vns= actual_peer_vns_by_policy[vn])
            ...
            if not out:
                ...
                time.sleep(5)
                out = vn_fixture[vn].verify_vn_route_target(policy_peer_vns= actual_peer_vns_by_policy[vn])
            assert (out == True), err_msg_on_fail

        self.logger.info(
            "TEST STEP: Bind one more policy to VN and verify RT import values updated")
        vn1_fixture.bind_policies( [policy1_fixture.policy_fq_name,policy4_fixture.policy_fq_name],vn1_fixture.vn_id)
        ...
        vn1_fixture.unbind_policies(vn1_fixture.vn_id,[policy4_fixture.policy_fq_name])
        vn3_fixture.unbind_policies(vn3_fixture.vn_id,[policy3_fixture.policy_fq_name])
...
class TestBasicPolicy4(BasePolicyTest):
    ...
    def test_policy_modify_vn_policy(self):
        ...
        config_topo['policy'][new_policy_to_add]= self.useFixture( PolicyFixture( policy_name= new_policy_to_add,
