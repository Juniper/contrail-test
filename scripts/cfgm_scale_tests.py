...
class CfgmScaleTest(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def setUp(self):
        ...
        self.tid = str(self.vnc_lib.project_read(fq_name=[u'default-domain', u'demo']).uuid)
    ...
    def test_cfgm_scale(self):
        ...
        self.logger.info(str(len(self.vnc_lib.virtual_networks_list()['virtual-networks'])))
        #import pdb; pdb.set_trace()
        self.logger.info("Reading VMs at %d" % time.time())
        self.logger.info(str(len(self.vnc_lib.virtual_machines_list()['virtual-machines'])))
        self.logger.info("Reading VN %s from API at %d ..." % (cfgm_obj._vns[0].get_fq_name_str(), time.time()))
        bb = requests.get('http://localhost:8082/virtual-network/%s' % cfgm_obj._vns[0].uuid)
        self.logger.info(len(str(bb.content)))
        self.logger.info("Reading VN %s from LIB at %d ..." % (cfgm_obj._vns[0].get_fq_name_str(), time.time()))
    ...
    def test_quantum_scale(self):
        ...
        #import pdb; pdb.set_trace()
        q_obj = self.useFixture(QuantumFixture(connections = self.connections, tid=self.tid,
                    inputs = self.inputs, vn_count = 1, vms_per_vn = 100))
        self.logger.info("Reading VNs at %d" % time.time())
        self.logger.info(str(len(self.vnc_lib.virtual_networks_list()['virtual-networks'])))
        #import pdb; pdb.set_trace()
        self.logger.info("Reading VMs at %d" % time.time())
        self.logger.info(str(len(self.vnc_lib.virtual_machines_list()['virtual-machines'])))
        self.logger.info("Reading VN %s from API at %d ..." % (q_obj.topVN(),time.time())) 
        bb = requests.get('http://localhost:8082/virtual-network/%s' % q_obj.topVN())
        self.logger.info(len(str(bb.content)))
        self.logger.info("Reading VN %s from LIB at %d ..." % (q_obj.topVN(),time.time()))
        self.logger.info(self.vnc_lib.virtual_network_read(id = q_obj.topVN()))
        self.logger.info("Reading VN %s from Quantum at %d ..." % (q_obj.topVN(),time.time()))
