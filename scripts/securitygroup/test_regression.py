...
class SecurityGroupRegressionTests4(BaseSGTest, VerifySecGroup, ConfigPolicy):
    ...
    @preposttest_wrapper
    def test_vn_compute_sg_comb_cidr(self):
        """
        Description: Verify traffic between intra/inter VN,intra/inter compute where SG rules usage cidr as address
        Steps:
            1. define the topology for intra/inter VN,intra/inter compute and SG rules
            2. create the resources as defined in the topo
            3. verify the traffic
        Pass criteria: step 3 should pass
        """

        topology_class_name = sdn_sg_test_topo.sdn_2vn_5vm_config_sg_subnet
        try:
            # provided by wrapper module if run in parallel test env
            topo = topology_class_name(
                project=self.project.project_name,
        ...
        return True
    # end test_vn_compute_sg_comb_cidr
...
class SecurityGroupRegressionTests5(BaseSGTest, VerifySecGroup, ConfigPolicy):
    ...
    def test_default_sg(self):
        ...
        secgrp_rules = self.sg1_fix.create_sg_rule_quantum(
            sg_id,
            secgrp_rules=rule)
        assert secgrp_rules
...
class SecurityGroupRegressionTests6(BaseSGTest, VerifySecGroup, ConfigPolicy):
    ...
    def test_sg_stateful(self):
        ...
        errmsg = "Ping to VM ip %s from VM ip %s failed" % (dst_vm_fix.vm_ip,
                                                            src_vm_fix.vm_ip)
...
class SecurityGroupRegressionTests9(BaseSGTest, VerifySecGroup, ConfigPolicy):
    ...
    def test_add_remove_default_sg_active_flow(self):
        """
        Description: add/remove default SG from VM when flow is active and traffic from both ends
        Steps:
            1. create resources as defined in topology
            2. start traffic for tcp/udp protocol and add/remove SG to/from VM
            3. verify traffic after each add remove of SG
        Pass criteria:
            step 3 should PASS
        """
    ...
    def test_add_remove_sg_active_flow1(self):
        """
        Description:
            1. add/remove custom SG from VM when flow is active and traffic from both ends.
            2. custom SG rules allow ingress/egress traffic from same SG
        Steps:
            1. create resources as defined in topology
            2. start traffic for udp protocol and add/remove SG to/from VM
            3. verify traffic after each add remove of SG
        Pass criteria:
            step 3 should PASS
        """
    ...
    def test_add_remove_sg_active_flow2(self):
        ...
        Description:
            1. add/remove custom SG from VM when flow is active and traffic from both ends.
            2. custom SG rules allow ingress traffic from same SG and egress from all IP
        Steps:
            1. create resources as defined in topology
            2. start traffic for udp protocol and add/remove SG to/from VM
            3. verify traffic after each add remove of SG
    ...
    def test_add_remove_sg_active_flow3(self):
        ...
        Description:
            1. add/remove custom SG from VM when flow is active and traffic from both ends.
            2. custom SG rules allow egress traffic from same SG and ingress from all IP
        Steps:
            1. create resources as defined in topology
            2. start traffic for udp protocol and add/remove SG to/from VM
            3. verify traffic after each add remove of SG
    ...
    def test_add_remove_sg_active_flow4(self):
        ...
        Description:
            1. add/remove custom SG from VM when flow is active and traffic from both ends.
            2. custom SG rules allow ingress/egress from all IP
        Steps:
            1. create resources as defined in topology
            2. start traffic for udp protocol and add/remove SG to/from VM
            3. verify traffic after each add remove of SG
...
class SecurityGroupRegressionTests10(BaseSGTest, VerifySecGroup, ConfigPolicy):

    @classmethod
    def setUpClass(cls):
        super(SecurityGroupRegressionTests10, cls).setUpClass()
        cls.option = 'openstack'
    ...
    def test_sg_traffic_via_mx(self):
        """
        Description: Test for traffic via MX, private vn <-> public vn, uses ping for verification
            ...
            4. test without sg, should not allow icmp from outside
        Steps:
            1. create resources as defined in topology
            2. verify traffic using ping for each of the scenarios
        Pass criteria:
            step 2 should PASS
        """
        ...
        errmsg = "Ping to external ip %s from VM failed" % external_ip
        errmsg2 = "Ping to external ip %s from VM passed, expected to fail" \
            % external_ip

        assert src_vm_fix.ping_with_certainty(external_ip), errmsg
        ...
        src_vm_name = 'vm1'
        src_vm_fix = config_topo['vm'][src_vm_name]
        assert src_vm_fix.ping_with_certainty(external_ip,
                                              expectation=False), errmsg2

        # test without sg, should not allow icmp from outside
        config_topo['vm']['vm2'].remove_security_group(secgrp=default_sg_id)
        src_vm_name = 'vm2'
        src_vm_fix = config_topo['vm'][src_vm_name]
        assert src_vm_fix.ping_with_certainty(external_ip,
                                              expectation=False), errmsg2
    ...
    def test_sg_traffic_via_firewall_in_net(self):
        """
        Description: Test with in-network firewall(default SG is attached to in-net SI when its launched):
            1. default sg in traffic vms as well as SI
            2. default security group in traffic VMs only
            3. default security group in SI only
            4. user-defined sg only in traffic VMs
            5. user-defined sg in traffic VMs as well as SI
        Steps:
            1. create resources as defined in topology
            2. verify traffic using ping for each of the scenarios
        Pass criteria:
        ...
        errmsg2 = "Ping to right VM ip %s from left VM passed, expected to fail"  \
            % dst_vm_fix.vm_ip
        # 1. default sg in traffic vms as well as SI
        assert src_vm_fix.ping_with_certainty(dst_vm_fix.vm_ip), errmsg
