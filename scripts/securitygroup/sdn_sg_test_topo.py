...
class sdn_topo_public_vn():
    def build_topo(self, domain= 'default-domain', project= 'admin',
        ...
        ##
        # Define VN's in the project:
        self.vnet_list=  ['public']
        ##
        # Define network info for each VN:
        ip_prefix = public_vn_info['subnet'][0].split('/')[0]
        ip_prefix_len = public_vn_info['subnet'][0].split('/')[1]

        if config_option == 'openstack':
            self.vn_nets=  {'public': public_vn_info['subnet']}
        else:
            self.vn_nets = {
            'public': [(NetworkIpam(), VnSubnetsType(
                                       [IpamSubnetType(
                                         subnet=SubnetType(
                                                  ip_prefix,
                                                  int(ip_prefix_len)))]))]
        ...
        # Define network policies
        self.policy_list=  ['policy0']
        self.vn_policy=  {self.vnet_list[0]: ['policy0']}

        self.vn_of_vm= {'vm1': 'public', 'vm2': 'public'}
        ...
        if config_option == 'openstack':
            self.rules['policy0']= [{'direction': '<>', 'protocol': 'any', 'dest_network': 'any',
                                    'source_network': 'any', 'dst_ports': 'any',
                                    'simple_action': 'pass', 'src_ports': 'any'
                                    }]
        else:
...
class sdn_topo_with_si_firewall():
    def build_topo(self, domain= 'default-domain', project= 'admin',
                        compute_node_list= None, username= None,
                        password= None, svc_mode=None,
                        config_option='openstack'):
        ##
        # Domain and project defaults: Do not change until support for non-default is tested!
        self.domain= domain; self.project= project; self.username= username; self.password= password
        ...
        # Define network info for each VN:
        if config_option == 'openstack':
            self.vn_nets=  {'vnet1': ['9.9.1.0/24'], 'vnet2': ['9.9.2.0/24']}
        else:
            self.vn_nets = {
            'vnet1': [(NetworkIpam(), VnSubnetsType(
                                        [IpamSubnetType(
        ...
        if (svc_mode  == 'in-network') or (svc_mode == 'in-network-nat'):
            ...
            right_vn = ':'.join([self.domain,
                                self.project,
                                self.vnet_list[1]])
        self.st_params = {self.st_list[0]: {'svc_img_name': svc_img_name,
                                            'svc_type': 'firewall', 'if_list':
        ...
        self.si_params = {
            self.si_list[0]: {'if_list': [['management', False, False],
                                         ['left', False, False],
                              ['right', False, False]],
        ...
        #
        # Define network policies
        self.policy_list=  ['pol-si']
        self.vn_policy=  {self.vnet_list[0]: ['pol-si'], self.vnet_list[1]: ['pol-si']}
        ...
        # Define network policy rules
        self.rules= {}
        self.policy_test_order= ['pol-si']

        if config_option == 'openstack':
            self.rules['pol-si']= [{'direction': '<>', 'protocol': 'any',
                                    'dest_network': self.vnet_list[1],
                                    'source_network': self.vnet_list[0],
                                    'dst_ports': 'any',
                                    'simple_action': 'pass', 'src_ports':'any',
                                    'action_list': {'simple_action':'pass',
                                    'apply_service': [':'.join([self.domain,
                                                           self.project,
                                                           self.si_list[0]])
                                                                 ]}
                                    }]
            ...
            self.rules['pol-si'] = [
            PolicyRuleType(direction='<>', protocol='any',
             dst_addresses=[AddressType(virtual_network=self.vnet_list[1])],
             src_addresses=[AddressType(virtual_network=self.vnet_list[0])],
             dst_ports=[PortType(-1, -1)],
             action_list=ActionListType(simple_action='pass',
                                    apply_service= [':'.join([self.domain,
                                                           self.project,
...
class sdn_2vn_5vm_config_sg_subnet ():
    def __init__(self, domain= 'default-domain', project= 'admin',
                 compute_node_list= None, username= None, password= None,
                 config_option='openstack'):
        ##
        # Domain and project defaults: Do not change until support for non-default is tested!
        self.domain= domain; self.project= project; self.username= username;
        self.password= password
        ##
        # Define VN's in the project:
        self.vnet_list=  ['vnet1','vnet2','vnet3']
        ##
        # Define network info for each VN:
        if config_option == 'openstack':
            self.vn_nets=  {'vnet1': ['10.1.1.0/24', '11.1.1.0/24'],
                            'vnet2': ['10.1.2.0/24', '11.1.2.0/24'],
                            'vnet3': ['10.1.3.0/24']}
        else:
            self.vn_nets = {
            'vnet1': [(NetworkIpam(), VnSubnetsType(
            ...
            'vnet2': [(NetworkIpam(), VnSubnetsType(
                                        ...
                                        IpamSubnetType(
                                          subnet=SubnetType(
                                                  '11.1.2.0',
                                                  24))]))],
            'vnet3': [(NetworkIpam(), VnSubnetsType(
                                       [IpamSubnetType(
                                         subnet=SubnetType(
                                                  '10.1.3.0',
                                                  24))]))]
        ...
        self.vn_of_vm= {'vm1': 'vnet1', 'vm2': 'vnet1', 'vm3': 'vnet1',
                        'vm4': 'vnet2', 'vm5': 'vnet2', 'vm6': 'vnet3'}
        ...
        if compute_node_list is not None:
            if len(compute_node_list) == 2:
                self.vm_node_map = {'vm1':'CN0', 'vm2':'CN0', 'vm3':'CN1',
                                    'vm4':'CN0', 'vm5':'CN1', 'vm6':'CN0'}
            elif len(compute_node_list) > 2:
               self.vm_node_map = {'vm1':'CN0', 'vm2':'CN0', 'vm3':'CN2',
                                   'vm4':'CN0', 'vm5':'CN1','vm6':'CN0'}
        ...
        self.policy_test_order= ['policy0']
        if config_option == 'openstack':
            self.rules['policy0']= [{'direction': '<>', 'protocol': 'any',
                                  'dest_network': 'any','source_network': 'any',
                                  'dst_ports': 'any', 'simple_action': 'pass',
                                  'src_ports': 'any'
                                    }]
        else:
            self.rules['policy0'] = [
        ...
        for key in self.vn_of_vm:
           self.sg_of_vm[key] = []
        self.sg_of_vm['vm1'] = [self.sg_list[0]];
        self.sg_of_vm['vm2'] = [self.sg_list[0]];
        self.sg_of_vm['vm3'] = [self.sg_list[0]];
        self.sg_of_vm['vm4'] = [self.sg_list[0]];
        self.sg_of_vm['vm5'] = [self.sg_list[0]];
        ##Define the security group rules
        import uuid
        uuid_1= uuid.uuid1().urn.split(':')[2]
        uuid_2= uuid.uuid1().urn.split(':')[2]
        ...
        self.sg_rules[self.sg_list[0]]=[
               {'direction' : '>',
                 ...
                 'src_addresses': [{'subnet': {'ip_prefix': '10.1.1.0', 'ip_prefix_len': 24}},
                                   {'subnet': {'ip_prefix': '11.1.1.0', 'ip_prefix_len': 24}},
                                   {'subnet': {'ip_prefix': '10.1.2.0', 'ip_prefix_len': 24}},
                                   {'subnet': {'ip_prefix': '11.1.2.0', 'ip_prefix_len': 24}}],
                 'rule_uuid': uuid_1
               },{'direction' : '>',
                 'protocol' : 'any',
                 'src_addresses': [{'security_group': 'local', 'subnet' : None}],
        ...
        ##
        # Define traffic profile.
        self.traffic_profile= [{'src_vm':'vm1', 'dst_vm':'vm2', 'proto':'udp',
                                   'sport':8000, 'dport':9000, 'exp':'pass'},# intra VN, intra compute
                               {'src_vm':'vm1', 'dst_vm':'vm3', 'proto':'udp',
                                   'sport':8000, 'dport':9000, 'exp':'pass'},# intra VN, inter compute
                               {'src_vm':'vm1', 'dst_vm':'vm5', 'proto':'udp',
                                   'sport':8000, 'dport':9000, 'exp':'pass'},# inter VN, inter compute
                               {'src_vm':'vm1', 'dst_vm':'vm4', 'proto':'udp',
                                   'sport':8000, 'dport':9000, 'exp':'pass'},# inter VN, intra compute
                               {'src_vm':'vm6', 'dst_vm':'vm4', 'proto':'udp',
                                   'sport':8000, 'dport':9000, 'exp':'fail'}]

        # end __init__
# end class sdn_2vn_5vm_config_sg_subnet
