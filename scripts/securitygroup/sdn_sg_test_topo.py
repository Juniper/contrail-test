...
class sdn_topo_flow_to_sg_rule_mapping():
        #2 VN and 2 VM
    def build_topo(self, domain= 'default-domain', project= 'admin', compute_node_list= None, username= None, password= None,no_of_vm=2):
    ...
    def build_topo2(self, domain= 'default-domain', project= 'admin', compute_node_list= None, username= None, password= None,no_of_vm=2):
        #no_of_vm must be 2 or 3
        print "building dynamic topo"
        ##
        # Domain and project defaults: Do not change until support for non-default is tested!
...
class sdn_topo_with_si_firewall():
	...
	if (svc_mode  == 'in-network') or (svc_mode == 'in-network-nat'):
        ...
        self.rules['pol-si']= [{'direction': '<>', 'protocol': 'any', 'dest_network': self.vnet_list[1],
                                'source_network': self.vnet_list[0], 'dst_ports': 'any',
                                'simple_action': 'pass', 'src_ports': 'any',
                                'action_list': {'simple_action':'pass', 'apply_service': [':'.join([self.domain,
                                                                      self.project,
                                                                      self.si_list[0]])
