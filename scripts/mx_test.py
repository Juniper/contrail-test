...
#    @classmethod
    ...
    def test_change_of_rt_in_vn (self):
        ...
        if (('MX_GW_TEST' in os.environ) and (os.environ.get('MX_GW_TEST') == '1')):
            ...
            if not vm1_fixture.ping_to_ip('www-int.juniper.net'):
                self.logger.info( "Here ping should fail as VN %s is configured with wrong RT values" %fvn_name)
            else :
                self.logger.error("Ping should fail. But ping is successful even with wrong RT values")
                result = result and False

            # Change the RT value to correct one.   
            fvn_fixture.del_route_target(routing_instance, self.inputs.router_asn,mx_rt_wrong) 
            sleep(2)
            fvn_fixture.add_route_target(routing_instance, self.inputs.router_asn,mx_rt)
            sleep(10)
            ...
            fvn_fixture.del_route_target(routing_instance, self.inputs.router_asn,mx_rt)
            sleep(2)
            fvn_fixture.add_route_target(routing_instance, self.inputs.router_asn,mx_rt_wrong)
    ...
    def test_ftp_http_with_public_ip (self):
        ...
        if (('MX_GW_TEST' in os.environ) and (os.environ.get('MX_GW_TEST') == '1')):
            ...
            if 'saved' not in output:
                self.logger.error("FTP failed from VM %s" %(vm1_fixture.vm_name))
                result = result and False
            else:
                self.logger.info("FTP successful from VM %s via FIP" %(vm1_fixture.vm_name))
          

            self.logger.info( 'Testing HTTP...Trying to access www-int.juniper.net')
            run_cmd = "wget http://www-int.juniper.net"
            vm1_fixture.run_cmd_on_vm(cmds=[run_cmd])
            output = vm1_fixture.return_output_values_list[0]
            if 'saved' not in output:
                self.logger.error("HTTP failed from VM %s" %(vm1_fixture.vm_name))
                result = result and False
            else:
                self.logger.info("HTTP successful from VM %s via FIP" %(vm1_fixture.vm_name))

            self.logger.info( 'Testing bug 1336. Trying wget with www.google.com')
            run_cmd = "wget http://www.google.com --timeout 5 --tries 12"
            output = None
            vm1_fixture.run_cmd_on_vm(cmds=[run_cmd])
            output = vm1_fixture.return_output_values_list[0]
            if 'saved' not in output:
                self.logger.error("HTTP failed from VM %s to google.com. Bug 1336" %(vm1_fixture.vm_name))
                result = result and False
            ...
            if not result :
                self.logger.error('Test FTP and HTTP traffic from public network Failed.')
                assert result
        else:
            self.logger.info('Testcase test_mx_gateway for now only need to be run in BLR Sanity Setup')
    ...
    def test_fip_with_vm_in_2_vns(self):
        ...
        # FIP public
        self.logger.info( "Configuring FLoating IP in VM %s to communicate public network" %(vm1_name))
        ...
        # FIP internal 
        self.logger.info( "Configuring FLoating IP in VM %s to communicate inside VNS to other network" %(vm1_name))
