...
class ContrailConnections():
    ...
    def start_virtual_display(self):
        ...
        if self.display:
            self.inputs.logger.info(
                "Virtual Display Started..Going to run Selenium Headless tests....")
    ...
    def login_webui(self, project_name, username, password):
        ...
        self.browser.get('http://' + self.inputs.openstack_host_name +
                         '.englab.juniper.net:8080')
    ...
    def login_openstack(self, project_name, username, password):
        ...
        if self.os_name == 'ubuntu':
            self.inputs.logger.info(
                "Opening http://" + self.inputs.openstack_host_name + ".englab.juniper.net/horizon")
            self.browser_openstack.get(
                'http://' + self.inputs.openstack_host_name + '.englab.juniper.net/horizon')
        else:
            self.inputs.logger.info(
                "Opening http://" + self.inputs.openstack_host_name + ".englab.juniper.net")
            self.browser_openstack.get(
                'http://' + self.inputs.openstack_host_name + '.englab.juniper.net')
