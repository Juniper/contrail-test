...
class NewPolicyTestFixture(NewPolicyTestsBase):
    ...
    def itest_policy_with_duplicate_rules(self):
        ...
        if out :
            self.logger.info("policy rules are not matching with expected %s \n and actual %s" %(vna_acl1['entries'] ,vna_acl2['entries']))
            self.assertIsNone(out,"policy compare failed")
    ...
    def test_policy_RT_import_export(self):
        ...
        actual_peer_vns_by_policy= policy_test_utils.get_policy_peer_vns(vnet_list, vn_fixture)
        for vn in vnet_list:
