...
class TestQos(QosTestExtendedBase):
    ...
    def test_invalid_range_for_qc(self):
        ...
        maps = [{'dscp_map' : { 64: 4 }},
               {'dot1p_map' : { 8: 4 }},
               {'exp_map' : { 8: 4 }},
               {'dscp_map':{-1:4},'dot1p_map':{-1:4},'exp_map':{-1:4}},
               {'dscp_map':{"x":4},'dot1p_map':{"y":4},'exp_map':{"z":4}},
               {'dscp_map':{63:-1},'dot1p_map':{7:-1},'exp_map':{7:-1}},
               {'dscp_map':{63:"x"},'dot1p_map':{7:"y"},'exp_map':{7:"z"}},
               {'dscp_map' : { 64: 4 }}]
        for elem in maps:
            try:
...
class TestQosPolicy(TestQosPolicyBase):
    ...
    def test_qos_remark_dscp_on_particular_rule_of_policy(self):
        ...
        assert self.validate_packet_qos_marking(protocol = "udp",
                                                **validate_method_args)
        validate_method_args['expected_dscp']=62
        validate_method_args['expected_exp']=6
        validate_method_args['expected_dot1p']=6
