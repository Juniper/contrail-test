...
class TestBasicVMVN0(BaseVnVmTest):
    ...
    def test_broadcast_udp_w_chksum(self):
        ...
        for ips in list_of_ips:
            ...
            receiver2.stop()
            receiver3.stop()
            self.logger.info('%s sent %s packets to %s. %s received %s packets out of \
                        which %s are Corrupted'%(vm1_fixture.vm_name, sender.sent, 
                       ips, vm2_fixture.vm_name, receiver1.recv, receiver1.corrupt))
            self.logger.info('%s sent %s packets to %s. %s received %s packets out of \
                           which %s are Corrupted'%(vm1_fixture.vm_name, sender.sent, 
                           ips, vm3_fixture.vm_name, receiver2.recv, receiver2.corrupt))
            self.logger.info('%s sent %s packets to %s. %s received %s packets out of \
                          which %s are Corrupted'%(vm1_fixture.vm_name, sender.sent, ips, 
                          vm4_fixture.vm_name, receiver3.recv, receiver3.corrupt))
    ...
    def test_bulk_add_delete(self):
        ...
        vn1_fixture= self.useFixture(VNFixture(project_name= self.project.project_name,
        ...
        vm_count = 15
        vmx_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                        connections= self.connections,vn_obj= vn1_fixture.obj,
                        vm_name= vn1_name, count=vm_count,image_name='ubuntu'))
    ...
    def test_host_route_add_delete(self):
        ...
        vn_obj= vn_fixture.obj
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn_obj, vm_name= vm1_name, project_name= self.project.project_name,ram = '4096',image_name= 'ubuntu-traffic'))
        assert vm1_fixture.verify_on_setup()
        self.nova_fixture.wait_till_vm_is_up(vm1_fixture.vm_obj)
        route_cmd= 'route -n'
        ...
        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= vm2_name, project_name= self.project.project_name,ram = '4096',image_name= 'ubuntu-traffic'))
        assert vm2_fixture.verify_on_setup()
        self.nova_fixture.wait_till_vm_is_up(vm2_fixture.vm_obj)
...
class TestBasicVMVN1(BaseVnVmTest):
    ...
    def test_multistep_vm_add_delete_with_stop_start_service(self):
        ...
        vm1_fixture= VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= 'vm1', project_name= self.project.project_name)
        vm1_fixture.setUp()
        vm1_fixture.verify_vm_launched()
...
#            self.inputs.stop_service('contrail-vrouter',[compute_ip])
        ...
        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= 'vm2', project_name= self.project.project_name))
        vm2_fixture.verify_vm_launched()
...
#class TestBasicVMVN1XML(TestBasicVMVN1):
#    _interface = 'xml'
#    pass
    ...
    def test_release_ipam(self):
        ...
        try:
            self.vnc_lib.network_ipam_delete(vn_fixture.ipam_fq_name)
             ...
             self.logger.info( 'RefsExistError:Check passed that the IPAM cannot be \
                            released when the VN is associated to it, which has VMs on it.')
...
#class TestBasicVMVN2XML(TestBasicVMVN2):
#    _interface = 'xml'
#    pass
...
class TestBasicVMVN3(BaseVnVmTest):
    ...
    def test_traffic_bw_vms_diff_pkt_size(self):
        ...
        if len(set(self.inputs.compute_ips)) > 1 :
            ...
            vm1_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                          connections= self.connections, vn_obj= vn_fixture.obj,ram= 4096, image_name= 'ubuntu-traffic', 
                          vm_name= 'vm1', node_name=  host_list[1]))
            assert vm1_fixture.verify_on_setup()
            vm2_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                            connections= self.connections, vn_obj= vn_fixture.obj,ram= 4096, image_name= 'ubuntu-traffic', 
                            vm_name= 'vm2', node_name=  host_list[0]))
            assert vm2_fixture.verify_on_setup()
        else :
            self.logger.info("Single-Node Setup")
            vm1_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                             connections= self.connections, vn_obj= vn_fixture.obj,ram= 4096, 
                             image_name= 'ubuntu-traffic', vm_name= 'vm1'))
            vm2_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                             connections= self.connections, vn_obj= vn_fixture.obj,ram= 4096, 
                             image_name= 'ubuntu-traffic', vm_name= 'vm2'))
            assert vm1_fixture.verify_on_setup()
            assert vm2_fixture.verify_on_setup()
...
#class TestBasicVMVN3XML(TestBasicVMVN3):
#    _interface = 'xml'
...
class TestBasicVMVN4(BaseVnVmTest):
    ...
    def test_traffic_bw_vms_diff_pkt_size_w_chksum(self):
        ...
        if len(set(self.inputs.compute_ips)) > 1 :
            ...
            vm1_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                          connections= self.connections, vn_obj= vn_fixture.obj,ram= 4096, 
                          image_name= 'ubuntu-traffic', vm_name= 'vm1', node_name=  host_list[1]))
            assert vm1_fixture.verify_on_setup()
            vm2_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                           connections= self.connections, vn_obj= vn_fixture.obj,ram= 4096, 
                           image_name= 'ubuntu-traffic', vm_name= 'vm2', node_name=  host_list[0]))
            assert vm2_fixture.verify_on_setup()
        else :
            self.logger.info("Single-Node Setup")
            vm1_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                         connections= self.connections, vn_obj= vn_fixture.obj,
                         ram= 4096, image_name= 'ubuntu-traffic', vm_name= 'vm1'))
            vm2_fixture= self.useFixture(VMFixture(project_name= self.project.project_name,
                         connections= self.connections, vn_obj= vn_fixture.obj,
                         ram= 4096, image_name= 'ubuntu-traffic', vm_name= 'vm2'))
            assert vm1_fixture.verify_on_setup()
            assert vm2_fixture.verify_on_setup()
    ...
    def test_vm_add_delete(self):
        ...
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                     vn_obj=vn_obj, vm_name= vm1_name, project_name= self.project.project_name,
                      flavor='contrail_flavor_medium',image_name= 'ubuntu-traffic'))
        assert vm1_fixture.verify_on_setup()
        return True
    ...
    def test_vm_add_delete_in_2_vns(self):
        ...
        assert vn2_fixture.verify_on_setup()
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_objs=[ vn1_fixture.obj, vn2_fixture.obj ], vm_name= vm1_name, project_name= self.project.project_name))
        assert vm1_fixture.verify_on_setup()
        return True
    ...
    def test_vm_add_delete_in_2_vns_chk_ips(self):
        ...
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                     vn_objs=[ vn1_fixture.obj, vn2_fixture.obj ], vm_name= vm1_name, 
                     project_name= self.project.project_name,image_name='ubuntu',
    ...
    def test_vm_arp(self):
        ...
        vn_obj= vn_fixture.obj
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn_obj, image_name= 'ubuntu-arping', vm_name= vm1_name, project_name= self.project.project_name))
        assert vm1_fixture.verify_on_setup()
        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn_obj, image_name= 'ubuntu-arping', vm_name= vm2_name, project_name= self.project.project_name))
        assert vm2_fixture.verify_on_setup()
    ...
    def test_vm_file_trf_scp_tests(self):
        ...
        assert vm1_fixture.verify_on_setup()
        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn_obj, vm_name= vm2_name, project_name= self.project.project_name))
        assert vm2_fixture.verify_on_setup()
        self.nova_fixture.wait_till_vm_is_up(vm2_fixture.vm_obj)
        self.nova_fixture.wait_till_vm_is_up(vm1_fixture.vm_obj)
    ...
    def test_vm_file_trf_tftp_tests(self):
        ...
        vn_obj= vn_fixture.obj
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn_obj, ram = '4096', image_name= 'ubuntu-tftp', vm_name= vm1_name, project_name= self.project.project_name))
        assert vm1_fixture.verify_on_setup()
        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn_obj, ram = '4096', image_name= 'ubuntu-tftp', vm_name= vm2_name, project_name= self.project.project_name))
        assert vm2_fixture.verify_on_setup()
        self.nova_fixture.wait_till_vm_is_up(vm2_fixture.vm_obj)
        self.nova_fixture.wait_till_vm_is_up(vm1_fixture.vm_obj)
        #ssh and tftp taking sometime to be up and runnning
...
#class TestBasicVMVN4XML(TestBasicVMVN4):
#    _interface = 'xml'
#    pass
...
class TestBasicVMVN5(BaseVnVmTest):
    ...
    def test_vm_in_2_vns_chk_ping(self):
        ...
        assert vm1_fixture.verify_on_setup()
        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn1_fixture.obj, vm_name= vm2_name, project_name= self.project.project_name))
        assert vm2_fixture.verify_on_setup()
        vm3_fixture= self.useFixture(VMFixture(connections= self.connections,
                vn_obj=vn2_fixture.obj, vm_name= vm3_name, project_name= self.project.project_name))
        assert vm3_fixture.verify_on_setup()
        list_of_ips= vm1_fixture.vm_ips
        ...
        self.logger.info('-'*80)
        self.logger.info('Will shut down eth1 and hence ping to the second n/w should \
                        fail,while the ping to the first n/w is unaffected. The same\
                          is not done for eth0 as it points to the default GW') 
        self.logger.info('-'*80)
        vm1_fixture.run_cmd_on_vm( cmds= cmd_to_add_file);
        cmd_to_add_cmd_to_file=["echo 'ifconfig -a; sudo ifconfig eth1 down; ifconfig -a; \
                                ping -c 5 %s > ping_output_after_shutdown.log;\
                                sudo ifconfig eth1 up; ifconfig -a ' > batchfile"%vm3_fixture.vm_ip]
        ...
        self.logger.info('-'*80)
        self.logger.info('Will unshut eth1 and hence ping to the second n/w should \
                         pass, while the ping to the first n/w is still unaffected.\
                         The same while the ping to the first n/w is still unaffected.\
                         The same is not done for eth0 as it points to the default GW')

        self.logger.info('-'*80)
        vm1_fixture.run_cmd_on_vm( cmds= cmd_to_add_file);
        cmd_to_add_cmd_to_file=["echo 'ifconfig -a; sudo ifconfig eth1 up; sleep 10; sudo ifconfig -a;\
                                ping -c 5 %s > ping_output.log' > batchfile"%vm3_fixture.vm_ip]
        vm1_fixture.run_cmd_on_vm( cmds= cmd_to_add_cmd_to_file)
        vm1_fixture.run_cmd_on_vm( cmds= cmd_to_exec_file)
        vm1_fixture.run_cmd_on_vm( cmds= cmd_to_delete_file)
    ...
    def itest_vm_multi_intf_in_same_vn_chk_ping(self):
        ...
        assert vn1_fixture.verify_on_setup()
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                     vn_objs=[vn1_fixture.obj, vn1_fixture.obj, vn1_fixture.obj, vn1_fixture.obj, 
                     vn1_fixture.obj], vm_name= vm1_name, project_name= self.project.project_name))
        assert vm1_fixture.verify_on_setup()
        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
                     vn_objs=[vn1_fixture.obj, vn1_fixture.obj, vn1_fixture.obj, vn1_fixture.obj, 
                    vn1_fixture.obj], vm_name= vm2_name, project_name= self.project.project_name))
        assert vm2_fixture.verify_on_setup()
    ...
    def test_vm_multiple_flavors(self):
        ...
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= 'vm_tiny', ram = '4096', project_name= self.project.project_name, image_name='ubuntu'))

        vm2_fixture= self.useFixture(VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= 'vm_small', ram = '4096', project_name= self.project.project_name))

        vm3_fixture= self.useFixture(VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= 'vm_medium', ram = '4096', project_name= self.project.project_name))

        vm4_fixture= self.useFixture(VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= 'vm_large', ram = '8192', project_name= self.project.project_name))

        vm5_fixture= self.useFixture(VMFixture(connections= self.connections,
            vn_obj=vn_obj, vm_name= 'vm_xlarge', ram = '16384', project_name= self.project.project_name))
        assert vm1_fixture.verify_on_setup()
        assert vm2_fixture.verify_on_setup()
        assert vm3_fixture.verify_on_setup()
        assert vm4_fixture.verify_on_setup()
    ...
    def test_vm_static_ip_tests(self):
        ...
        cmd_to_add_cmd_to_file=["echo 'ifconfig; route; sudo ifconfig eth0 10.10.10.10 netmask 255.255.255.0;\
                               ifconfig; route; sudo ifconfig eth0 11.1.1.253 netmask 255.255.255.0;\
                                ifconfig;route; sudo ifdown eth0; sleep 5; sudo ifup eth0; ifconfig; route' > batchfile"]
    ...
    def test_vm_vn_block_exhaustion(self):
        ...
        vm3_fixture= self.useFixture(VMFixture(connections= self.connections,
                    vn_obj=vn_obj, vm_name= 'vm3', project_name= self.project.project_name))

        vm4_fixture= self.useFixture(VMFixture(connections= self.connections,
                    vn_obj=vn_obj, vm_name= 'vm4', project_name= self.project.project_name))

        vm5_fixture= self.useFixture(VMFixture(connections= self.connections,
                    vn_obj=vn_obj, vm_name= 'vm5', project_name= self.project.project_name))
        ...
        vm6_fixture= self.useFixture(VMFixture(connections= self.connections,
                    vn_obj=vn_obj, vm_name= 'vm6', project_name= self.project.project_name))
        assert vm1_fixture.verify_on_setup()
        assert vm2_fixture.verify_on_setup()
        assert vm3_fixture.verify_on_setup()
        assert vm4_fixture.verify_on_setup()
    ...
    def test_vn_add_delete(self):
        ...
        vn_obj=self.useFixture( VNFixture(project_name= self.project.project_name,
                                connections= self.connections,
                               vn_name=vn_name, inputs= self.inputs
                               , subnets=['22.1.1.0/24'] ))
...
#class TestBasicVMVN5XML(TestBasicVMVN5):
#    _interface = 'xml'
...
class TestBasicVMVN6(BaseVnVmTest):
    ...
    def test_vn_subnet_types(self):
        ...
        res_vn_fixture = self.useFixture(MultipleVNFixture( connections=self.connections, 
                        inputs=self.inputs, subnet_count=2,vn_name_net=reserved_ip_vns,  
                        project_name=self.project.project_name))
        ovlap_vn_fixture = self.useFixture(MultipleVNFixture( connections=self.connections, 
                           inputs=self.inputs, subnet_count=2,vn_name_net=overlapping_vns,  
                           project_name=self.project.project_name))
        try:
            non_usable_vn_fixture = self.useFixture(MultipleVNFixture( connections=self.connections, 
                                    inputs=self.inputs, subnet_count=2,vn_name_net=non_usable_block_vns,  
                                    project_name=self.project.project_name))
    ...
    def test_vn_vm_no_ip(self):
        ...
        ts = time.time()
        vn_name = '%s_%s'%(inspect.stack()[0][3],str(ts))
        vn_obj=self.useFixture( VNFixture(project_name= self.project.project_name, connections= self.connections,
                     vn_name= vn_name, inputs= self.inputs ))
        #assert vn_obj.verify_on_setup()
        assert vn_obj
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                         vn_obj=vn_obj.obj, vm_name= 'vm222', project_name= self.project.project_name))
    ...
    def test_vn_vm_no_ip_assign(self):
        ...
        vm1_fixture= self.useFixture(VMFixture(connections= self.connections,
                         vn_obj=vn_fixture.obj, vm_name= 'vm222', project_name= self.project.project_name))
        self.project_fixture = self.useFixture(ProjectFixture(vnc_lib_h= self.vnc_lib, 
                               project_name= self.project.project_name, connections=self.connections))
    ...
    def test_multiple_vn_vm(self):
        ...
        multi_vn_fixture = self.useFixture(MultipleVNFixture(
            ...
            vn_name_net=vn_s,  project_name=self.project.project_name))
        ...
        multi_vm_fixture = self.useFixture(MultipleVMFixture(
            project_name=self.project.project_name, connections=self.connections,
            vm_count_per_vn=4,vn_objs=vn_objs,image_name='ubuntu'))
