...
#    @preposttest_wrapper
    def test_rt_table_scenario_3(self):
        ...
        later_route = self.get_vrouter_route(prefix,
            vn_fixture=vn1_fixture, node_ip=vm1_fixture.vm_node_ip)
        assert later_route, 'No vrouter route for prefix %s found' % (prefix)
        # This route should be pointing to a discard nh and not vm2's nh
        msg = 'Discard route not added even after deleting the route'
        assert later_route['label'] == '0' and later_route['nh_id'] == '1', msg
