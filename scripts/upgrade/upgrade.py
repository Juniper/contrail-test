...
class Upgrade(ResourcedTestCase, testtools.TestCase, ConfigSecGroup):
    ...
    def test_traffic_after_upgrade(self):
        ...
        self.secgrp_fixture1 = self.config_sec_group(
            name=self.sg2_name, entries=rule)
    ...
    def test_fiptraffic_before_upgrade(self):
        ...
        self.secgrp_fixture = self.config_sec_group(
            name=self.sg1_name, entries=rules)
        ...
        if vn11_vm3_fixture.ping_to_ip(vn11_vm1_fixture.vm_ip):
            result = result and False
            self.logger.error(
                'Test to ping between VMs expected to FAIL problem with security group \n')
            assert result
        self.logger.info(
            "Test to ping from vn11_vm3 was expected to fail since security allows only 'tcp' traffic")
    ...
    def vm_add_delete(self):
        ...
        self.res.vm1_fixture = self.useFixture(
        ...
        self.res.vm2_fixture = self.useFixture(
            VMFixture(connections=self.connections,
        ...
        self.res.vm3_fixture = self.useFixture(
            VMFixture(connections=self.connections,
