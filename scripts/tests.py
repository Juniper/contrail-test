    ...
    def test_control_node_switchover(self):
        ...
        if new_active_controller == active_controller:
            self.logger.error(
                ...
                (self.active_controller, new_active_controller))
            result = False
...
echo "Hello World.  The time is now $(date -R)!" | tee /tmp/output.txt
    ...
    def test_generic_link_local_service(self):
        ...
        link_local_base_args = "--admin_user %s \
             --admin_password %s\
             --linklocal_service_ip 169.254.1.2\
             --linklocal_service_port 80\
             --ipfabric_service_port 80" %(self.inputs.connections.ks_client.username,
                                           self.inputs.connections.ks_client.password)
