...
class KeystoneCommands():
    ...
    def __init__(self,username=None,password=None,tenant=None,auth_url=None,token=None,endpoint=None):
        ...
        if token:
            self.keystone=keystoneclient.Client(token=token,endpoint=endpoint)
        else:
...
#        return user
    ...
    def update_tenant(self,tenant_id, tenant_name=None, description=None,
       ...
       self.keystone.tenants.update(tenant_id,tenant_name=tenant_name, description=description, enabled=enabled)
