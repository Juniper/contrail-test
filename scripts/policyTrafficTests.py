...
class policyTrafficTestFixture(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def all_policy_verify(self, config_topo, topo, state='unspecified', fixture_only='no'):
        ...
        if fixture_only == 'no':
            ...
            policy_fixt= config_topo['policy'][str(topo.policy_list[0])]    #pick any policy configured
    ...
    def check_exception_flows_in_kernel(self, compNode):
        ...
        '''
        self.logger.info ("Inspecting kernel for exception flows - Null flows with src/dest address 0.0.0.0 & HOLD flows with action as HOLD")
        status= True; msg= []
        inspect_h= self.agent_inspect[compNode]
        try:
            kflows= inspect_h.get_vna_kflowresp()
        except Exception, e:
            err= "Compute: %s, hit error in kernel introspect, check topology for expected result" %(compNode)
            self.logger.warn (err)
            et, ei, tb = sys.exc_info()
            formatted_traceback = ''.join(traceback.format_tb(tb))
            fail_trace = '\n{0}\n{1}:\n{2}'.format(formatted_traceback, et.__name__, ei.message)
            self.logger.warn ("Exception happened while collecting kernel flow, failure trace as follows:")
            self.logger.warn (fail_trace)
        if kflows == None:
            msg.append ("Not getting kernel flow info from agent %s..Check detailed error printed by introspect" %compNode)
            self.logger.error (msg)
            return {'status': False, 'msg': msg}
        self.logger.info ("Info on total kernel flows in compute %s: %s" %(compNode, len(kflows)))
        #self.logger.info ("all kernel flows in compute %s: %s" %(compNode, kflows))
        hold_flow_l= []; null_flow_l= []
        for flow in kflows:
            if flow['action'] == 'HOLD': hold_flow_l.append(flow)
            ...
            # we will just do partial check and see if other variants show up...
            elif flow['sip'] == '0.0.0.0': null_flow_l.append(flow)
        self.logger.info ("Info on kernel flows in HOLD state in compute %s: %s" %(compNode, hold_flow_l))
        self.logger.info ("Info on kernel Null flows in compute %s: %s" %(compNode, null_flow_l))
        # end checking all test flows
        if hold_flow_l: status= False; msg.extend(["Hold flows found in compute: ", hold_flow_l])
        if null_flow_l: status= False; msg.extend(["Null flows found in compute: ", null_flow_l])
    ...
    def validate_flow_in_kernel(self, test_flow_list, test_vn):
        ...
        #self.logger.info ("all kernel flows in compute %s: %s" %(compNode, kflows))
        if kflows == None: 
            msg.append ("No kernel flows in agent %s..Check expected behavior." %compNode); self.logger.warn (msg)
            return {'status': False, 'msg': msg}
        non_nat_flow_l= []
        for flow in kflows:
            if flow['action'] != 'NAT': non_nat_flow_l.append(flow)
        self.logger.info ("non-NAT kernel flows in compute %s: %s" %(compNode, non_nat_flow_l))
        self.logger.info ("test flow list recd. for checking: %s" %test_flow_list)
        # Look specifically for flows generated by test in kernel..
        # if values for these keys match, we assume we have the flow in system and the flow can be in any state
        flow_id_keys= ['src', 'dst', 'src_port', 'dst_port', 'protocol']
        # ex. action can be different due to system conditions, which is what we are testing..
        keys_to_verify= ['action_l']
        map_test_flow_key= {'src':'sip', 'dst':'dip', 'src_port':'sport', 'dst_port':'dport', 'protocol':'proto', 'action_l':'action'}
        for f in test_flow_list:
            flow= f['flow_entries']
            flow['num_match']= 0    #to track num flows matching this flow in case of bug [valid flow can be only one; there can be invalid flows]
            self.logger.info ("-" * 80)
            ...
            for kflow in non_nat_flow_l:
                ...
                for k in flow_id_keys:
                    if flow[k] != kflow[map_test_flow_key[k]]:
                        keys_miss= k
                        #print "For key: %s, looking for %s, this flow has %s, check further..." %(k, flow[k], kflow[map_test_flow_key[k]])
                        match= False; break
                    else: print "For key: %s, looking for %s, found.. %s" %(k, flow[k], kflow)
                    # end if
                # end checking all keys
                # For this flow, check action to see if they are intact
                for k in keys_to_verify:
                    if k == 'action_l': 
                        expected_act= 'FORWARD' if flow[k] == ['pass'] else 'DROP'
                    else: expected_act= flow[k]
                    ...
                    # end if
                if match == True:
                    self.logger.info ("--->Matching flow has all fields intact, flow: %s " %flow)
                    flow['num_match'] += 1    
            ...
            if flow['num_match'] == 0:
                msg1= "Agent %s: Having issue with following test flow in kernel validation - %s" %(compNode, flow)
                print msg1; msg.append(msg1); status= False
                self.logger.error (msg)
        # end checking all test flows
        return {'status': status, 'msg': msg}
    ...
    def check_policy_route_available (self, vnet_list, vn_fixture):
        ...
        vn_policys_peer_vns= {}; vns= []; fqns= {}; pvns= {}
        for v in vnet_list:
            m= re.search(r"(\S+):(\S+):(\S+)", v); vns.append(m.group(3))
            fqns[m.group(3)]= v

        actual_peer_vns_by_policy= policy_test_utils.get_policy_peer_vns(vns, vn_fixture)
        print "actual_peer_vns_by_policy: ", actual_peer_vns_by_policy
        # actual_peer_vns_by_policy format: {'vnet0':[u'default-domain:admin:vnet1', u'default-domain:admin:vnet2'], ..}]

        pvns[0]= actual_peer_vns_by_policy[vns[0]]
        # find if vn1 in the given pair is in vn0's peer list   
        if fqns[vns[1]] not in pvns[0]:        
            self.logger.info("vn %s not in peer list of %s, actual_peer_vns_by_policy %s" %(vns[1], vns[0], pvns[0]))
            result= False 
        else:
            # Find if vn0 is in vn1's peer list
            pvns[1]= actual_peer_vns_by_policy[vns[1]]
            if fqns[vns[0]] not in pvns[1]:
                self.logger.info("vn %s not in peer list of %s, actual_peer_vns_by_policy %s" %(vns[0], vns[1], pvns[1]))
                result= False 
            else: result= True
    ...
    def validate_flow_in_vna(self, test_flow_list, test_vn, vn_fixture):
        ...
        compNode= test_flow_list[0]['agent_inspect_ip']
        inspect_h= self.agent_inspect[compNode]
        flows_found= []; flows_not_found= []
        for f in test_flow_list:
            flow= f['flow_entries']
            self.logger.info ("--->VNA-Flow check: Looking for following test flow: %s" %(json.dumps(flow, sort_keys=True)))
            vnet_list= [flow['source_vn'], flow['dst_vn']]
            policy_route_state= self.check_policy_route_available (vnet_list, vn_fixture)
            try:
                mflow= inspect_h.get_vna_fetchflowrecord(vrf= flow['vrf_id'], sip= flow['src'], dip= flow['dst'], sport= flow['src_port'], dport= flow['dst_port'], protocol= flow['protocol'])
            except:
                msg.append("Agent: %s, VN: %s, hit error in vna flow inspect, check topology for expected result" %(compNode, test_vn))
                self.logger.warn (msg)
                return {'status': False, 'msg': msg}
    
            # maintain 2 variables of match - few_flows_found [to track none or few found] & all_flows_found
            few_flows_found= False; all_flows_found= True
            if mflow == None:
                if policy_route_state == True:
                    msg.append("test_flow not seen in agent %s, flow: %s" %(compNode, json.dumps(flow, sort_keys=True)))
                    ...
                    self.logger.info ("Flow not found, which is expected when routes not available")
                    continue

            # proceed with flow inspection..
            agent_flow= {}
            for i in mflow: agent_flow.update(i)
            self.logger.info ("Matching flow from agent: %s" %(json.dumps(agent_flow, sort_keys=True)))

            # For a matching flow, check following key values
            keys_to_verify= ['dst_vn', 'action']

            # For matching flow, check dest_vn and action to see if they are intact
            for k in keys_to_verify:
                err_msg= None; match= True
                if k == 'action':
                    if flow[k][0] == 'pass':
                        if agent_flow[k] == 'pass' or agent_flow[k] == '32': match= match and True
                        else:    
                            err_msg= ("For the matching flow, data for key %s not matching, test has %s, agent flow has %s" %(k, flow[k], agent_flow[k]))
                            match= match and False; break     
                    if flow[k][0] == 'deny':  
                        if agent_flow[k][0] == 'drop' or agent_flow[k] == '4': match= match and True
                        else: 
                            err_msg= ("For the matching flow, data for key %s not matching, test has %s, agent flow has %s" %(k, expected, agent_flow[k]))
                            match= match and False; break
                elif k == 'dst_vn':
                    expected_vn= "__UNKNOWN__" if policy_route_state == False else flow[k]
                    if expected_vn == agent_flow[k]: match= match and True
                    else:    
                        err_msg= ("For the matching flow, data for key %s not matching, test has %s, agent flow has %s" %(k, flow[k], agent_flow[k]))
                        match= match and False; break
            ...
            if match == True: 
                self.logger.info ("--->VNA-Flow check: Matching flow has all fields intact: %s" %agent_flow['uuid'])
            ...
            if match == False:
                ...
                try: err_msg
                except: msg.append(msg1)
                else: msg.extend([msg1,err_msg])
        # end checking all test flows

        status= True if all_flows_found == True else False
        rdata= {'status': status, 'msg': msg, 'few_flows_found': few_flows_found, 'flows_found': flows_found, 'flows_not_found': flows_not_found}
    ...
    def build_test_flow(self,policy,test_vm1,test_vm2,topo,config_topo,traffic_proto_l,total_streams,dpi=9100):    
        ...
        test_vn_vm1= topo.vn_of_vm[test_vm1]; test_vn_vm1_fix= config_topo['vn'][test_vn_vm1]
        test_vn_vm2= topo.vn_of_vm[test_vm2]; test_vn_vm2_fix= config_topo['vn'][test_vn_vm2]
        for proto in traffic_proto_l:
            for i in range(total_streams[proto]):
                ...
                f['src']= test_vm1_fixture.vm_ip; f['dst']= test_vm2_fixture.vm_ip
                f['source_vn']= test_vn_vm1_fix.vn_fq_name; f['dst_vn']= test_vn_vm2_fix.vn_fq_name
                f['vrf_id']= test_vm1_fixture.get_vrf_id (test_vn_vm1_fix.vn_fq_name, test_vn_vm1_fix.vrf_name)
                if proto == 'icmp': 
                    f['protocol']= '1'; 
                    f['src_port']= '0'; f['dst_port']= '0'
                if proto == 'udp' or proto == 'tcp': 
                    if proto == 'udp': f['protocol']= '17'; 
                    if proto == 'tcp': f['protocol']= '6'; 
                    f['dst_port']= str(dpi+i); f['src_port']= '8000'
        ...
        for i in range(num_rules):
            ...
            matching_rule_action[proto]= topo.rules[policy][i]['simple_action']
        if num_rules == 0: 
            for proto in traffic_proto_l: matching_rule_action[proto]= 'deny'
        ...
        self.logger.info ("return data from build test-flow,test_flow_list :%s and matching_rule_action:%s" %(test_flow_list,matching_rule_action))
    ...
    def policy_scale_test_with_ping(self, topo):
        ...
        for vmi in range(1, len(topo.vn_of_vm.items())):
            ...
            expectedResult= True if matching_rule_action[test_proto]== 'pass' else False
            ret= test_vm1_fixture.ping_with_certainty( test_vm2_fixture.vm_ip, expectation=expectedResult )
            result_msg= "vm ping test result to vm %s is: %s" %(vmi, ret)
            self.logger.info (result_msg)
            if ret != True : result= False; msg.extend([result_msg, policy_info])
    ...
    def repeated_policy_update_test_with_ping(self, topo):
        ...
        for policy in topo.policy_test_order:
            ...
            for i in range(num_rules):
                proto= topo.rules[policy][i]['protocol']
                matching_rule_action[proto]= topo.rules[policy][i]['simple_action']
            if num_rules == 0: matching_rule_action[test_proto]= 'deny'
            self.logger.info ("matching_rule_action: %s" %matching_rule_action)
            # 4. Test with ping
            expectedResult= True if matching_rule_action[test_proto] == 'pass' else False
            ret= test_vm1_fixture.ping_with_certainty( test_vm2_fixture.vm_ip, expectation=expectedResult )
            result_msg= "vm ping test result after " + state + " is: " + str(ret)
            self.logger.info (result_msg)
            if ret != True : result= False; msg.extend([result_msg, policy_info])
            if ret != True : self.all_policy_verify (config_topo, updated_topo, state, fixture_only='yes')
        self.assertEqual(result, True, msg)
        return result
    ...
    def policy_test_with_multi_proto_traffic(self, topo):
        ...
        total_streams['udp']= 1; total_streams['tcp']= 1; dpi= 9100
        # 2. set expectation to verify..
        matching_rule_action= {}
        ...
        for proto in traffic_proto_l:
            expectedResult[proto]= True if matching_rule_action[proto] == 'pass' else False
            ...
            msg1= "Status of start traffic : %s, %s, %s" %(proto, test_vm1_fixture.vm_ip, startStatus[proto]['status'])
            if startStatus[proto]['status'] == False:
                self.logger.error (msg1); msg.extend([msg1, 'More info on failure: ', startStatus[proto]['msg']])
            else:
                self.logger.info (msg1)
            self.assertEqual(startStatus[proto]['status'], True, msg)
        self.logger.info ("-"*80)
        # 4. Poll live traffic
        # poll traffic and get status - traffic_stats['msg'], traffic_stats['status']
        ...
        for proto in traffic_proto_l:
            ...
            self.logger.info (" --> , flow proto: %s, expected: %s, got: %s" %(proto, expectedResult[proto], traffic_stats['status']))
            self.assertEqual(traffic_stats['status'], expectedResult[proto], err_msg)
        self.logger.info ("-"*80)
        # 4.a Opserver verification
        self.logger.info ("Verfiy Policy info in Opserver")
        self.logger.info ("-"*80)
        exp_flow_count = total_streams['icmp'] + total_streams['tcp'] + total_streams['udp']
        self.verify_policy_opserver_data(vn_name=test_vn,num_of_flows=exp_flow_count)
        self.logger.info ("-"*80)

        src_vn='default-domain'+':'+self.inputs.project_name+':'+test_vn
        dst_vn='default-domain'+':'+self.inputs.project_name+':'+test_vn1
        query = {};
        query['udp']='('+'sourcevn='+src_vn+') AND (destvn='+dst_vn+') AND (protocol =17) AND (sourceip='+test_vm1_fixture.vm_ip+') AND (destip='+test_vm2_fixture.vm_ip+')'
        query['tcp']='('+'sourcevn='+src_vn+') AND (destvn='+dst_vn+') AND (protocol =6) AND (sourceip='+test_vm1_fixture.vm_ip+') AND (destip='+test_vm2_fixture.vm_ip+')' 
        query['icmp']='('+'sourcevn='+src_vn+') AND (destvn='+dst_vn+') AND (protocol =1) AND (sourceip='+test_vm1_fixture.vm_ip+') AND (destip='+test_vm2_fixture.vm_ip+')'
        flow_record_data = {}; flow_series_data= {};
        expected_flow_count= {}
        for proto in traffic_proto_l:
            flow_record_data[proto] = self.ops_inspect.post_query('FlowRecordTable',start_time=start_time,end_time='now',select_fields=['sourcevn', 'sourceip', 'destvn', 'destip','setup_time','teardown_time','agg-packets', 'agg-bytes','protocol'],where_clause=query[proto])
            flow_series_data[proto] = self.ops_inspect.post_query('FlowSeriesTable',start_time=start_time,end_time='now',select_fields=['sourcevn', 'sourceip', 'destvn', 'destip','sum(packets)','flow_count','sum(bytes)','sum(bytes)'],where_clause= query[proto])
            msg1 = proto + " Flow count info is not matching with opserver flow series record"
            # initialize expected_flow_count to num streams generated for the proto
            expected_flow_count[proto]= total_streams[proto]
            self.logger.info(flow_series_data[proto])
            self.assertEqual(flow_series_data[proto][0]['flow_count'],expected_flow_count[proto],msg1)
        # 5. Stop Traffic
        ...
        for proto in traffic_proto_l:
            ...
            if status != expectedResult[proto]:
                msg.append(stopStatus[proto]); result= False
        ...
        self.logger.info ("-"*80)
        self.logger.info ("***Match traffic stats against Analytics flow series data***")
        self.logger.info ("-"*80)
        msg = {}
        for proto in traffic_proto_l:
            self.logger.info (" verify %s traffic status against Analytics flow series data" %(proto))
            msg[proto] = proto + " Traffic Stats is not matching with opServer flow series data"
            self.logger.info ("***Actual Traffic sent by agent %s \n\n stats shown by Analytics flow series%s" %(traffic_stats[proto],flow_series_data[proto] ))
            self.assertGreaterEqual(flow_series_data[proto][0]['sum(packets)'],traffic_stats[proto]['total_pkt_sent'],msg[proto])

        # 6.a Let flows age out and verify analytics still shows the data 
        self.logger.info ("-"*80)
        self.logger.info ("***Let flows age out and verify analytics still shows the data in the history***")
        self.logger.info ("-"*80)
        time.sleep(180)
        for proto in traffic_proto_l:
            self.logger.info (" verify %s traffic status against Analytics flow series data after flow age out" %(proto))
            flow_series_data[proto] = self.ops_inspect.post_query('FlowSeriesTable',start_time='now',end_time='now',select_fields=['sourcevn', 'sourceip', 'destvn', 'destip','sum(packets)','flow_count','sum(bytes)','sum(bytes)'],where_clause= query[proto])
            msg = proto + " Flow count info is not matching with opserver flow series record after flow age out in kernel"
            ...
            flow_series_data[proto] = self.ops_inspect.post_query('FlowSeriesTable',start_time=start_time,end_time='now',select_fields=['sourcevn', 'sourceip', 'destvn', 'destip','sum(packets)','flow_count','sum(bytes)','sum(bytes)'],where_clause= query[proto])
            msg = proto + " Traffic Stats is not matching with opServer flow series data after flow age out in kernel"
            # Historical data should be present in the Analytics, even if flows age out in kernel
            self.assertGreaterEqual(flow_series_data[proto][0]['sum(packets)'],traffic_stats[proto]['total_pkt_sent'],msg)
        return result
    ...
    def verify_policy_opserver_data(self,vn_name,num_of_flows):
        ...
        policy_attached = opserver_data['UveVirtualNetworkConfig']['attached_policies']
        ...
        self.assertTrue(policy_found_opserver,"Policy information not matching with the Opserver config data");
        self.assertEqual(num_rules_exp,opserver_data['UveVirtualNetworkConfig']['total_acl_rules'],"number of ACL rules are not matching with opserver config data")
        status = True
        retry = 6
        while retry:
            # Expecting atleast number of flows in analytics, since flows are dynamic can't be match for exact count.
        ...
        self.assertGreaterEqual(opserver_data['UveVirtualNetworkAgent']['ingress_flow_count'],num_of_flows,"Flow count is not matching with opeserver uve agent data")
        self.assertEqual(num_rules_exp,opserver_data['UveVirtualNetworkAgent']['total_acl_rules'],"total acl rules in VN Uve is not matching with opeserver uve agent data")
    ...
    def policy_test_modify_rules_of_live_flows(self, topo, update_mode='modify'):
        ...
        test_vm1= topo.vmc_list[0]; test_vm1_fixture= config_topo['vm'][test_vm1]
        test_vm2= topo.vmc_list[1]; test_vm2_fixture= config_topo['vm'][test_vm2]
        test_vn= topo.vn_of_vm[test_vm1]; test_vn_fix= config_topo['vn'][test_vn]
        test_vn_id= test_vn_fix.vn_id
        test_vn_vm1= topo.vn_of_vm[test_vm1]; test_vn_vm1_fix= config_topo['vn'][test_vn_vm1]
        test_vn_vm2= topo.vn_of_vm[test_vm2]; test_vn_vm2_fix= config_topo['vn'][test_vn_vm2]
        traffic_obj= {}; startStatus= {}; stopStatus= {}
        traffic_proto_l= ['icmp', 'udp', 'tcp']; total_streams= {} 
        total_streams['icmp']= 1; total_streams['udp']= 1; total_streams['tcp']= 1
        ...
        self.logger.info ("TEST STEP -3a: build test_flow_list and validate in VN Agent")
        test_flow_list,matching_rule_action=self.build_test_flow(policy,test_vm1,test_vm2,topo,config_topo,traffic_proto_l,total_streams,dpi)
        self.logger.info ("TEST STEP -3b: Validate Agent/Kernel for flows and Poll traffic ..")
        ...
        time.sleep (30)    # waiting for short flows to settle down after setup/starting traffic..
        out= self.validate_flow_in_vna (test_flow_list, test_vn, config_topo['vn'])
        if out['status'] != True: 
            err_msg.extend (["Flows not programmed as expected in computes after startTraffic - ", out['msg']])
            self.logger.error (err_msg)
        self.assertEqual(out['status'], True, err_msg)
        expectedResult= {}
        for proto in traffic_proto_l:
        ...
        for proto in traffic_proto_l:
            traffic_stats= traffic_obj[proto].getLiveTrafficStats()
            err_msg= [policy_info] + traffic_stats['msg']
            self.logger.info (" --> expected: %s, got: %s" %(expectedResult[proto], traffic_stats['status']))
            ## ---> Verify & return here on failure
        self.assertEqual(traffic_stats['status'], expectedResult[proto], err_msg)
        self.logger.info ("-"*80)
        ...
        for policy in topo.policy_test_order:
            if update_mode == 'replace':
                ...
                new_policy_entries= config_topo['policy'][policy].policy_obj['policy']['entries']
                ...
                topo.rules[starting_policy_name]= copy.copy(new_rules)    #update new rules in reference topology
            # wait for tables update before checking after making changes to system
            time.sleep(5)
            state= "policy for " + test_vn + " updated to rules of policy" + policy + "by mode" + update_mode
            self.logger.info ("new policy list of vn %s is %s" %(test_vn, policy))
            # update expected topology with this new info for verification
            updated_topo= policy_test_utils.update_topo(topo, test_vn, policy)
            self.logger.info ("Starting Verifications after %s" %(state))
            ...
            # if action = 'pass', expectedResult= True, else Fail;
            test_flow_list,matching_rule_action=self.build_test_flow(policy,test_vm1,test_vm2,topo,config_topo,traffic_proto_l,total_streams,dpi)
            self.logger.info ("test_flow_list after setting action as follows: %s" %test_flow_list)
            ...
            if out['status'] != expected_status: 
                err_msg.extend (["Flows not programmed as expected in computes after policy update - ", out['msg']])
                self.logger.error (err_msg)
            ...
            for proto in traffic_proto_l:
                traffic_stats= traffic_obj[proto].getLiveTrafficStats()
                m= (" --> flow for proto %s, traffic result expected: %s, got: %s" %(proto, expectedResult[proto], traffic_stats['status']))
                self.logger.info (m)
        ...
        for proto in traffic_proto_l:
            stopStatus[proto]= {}
            stopStatus[proto]= traffic_obj[proto].stopTraffic()
        # 6. Verify flow aging(expect flows to exist for 180s after stopping traffic)
        self.logger.info ("TEST STEP -6: Check for flow existence after stopping traffic")
        wait_time= 150; time.sleep (wait_time)
        self.logger.info ("Checking for flows after %s secs after stopping traffic" %wait_time)
        ...
        while recheck > 0: 
            ...
            if out['few_flows_found'] != expected_status: 
                err_msg.extend (["In flow aging check, time_elapsed_since_stop_traffic: ", wait_time, \
                    "Flows have not aged out as expected in computes, retry if allowed.. error info: ", out['flows_found']])
                self.logger.warn (err_msg)
                time.sleep (30); recheck= recheck-1; wait_time +=30
            else: self.logger.info ("no flows found as expected.."); recheck= 0
        self.assertEqual(out['status'], expected_status, err_msg)
        self.logger.info ("-"*80)
        return result
    ...
    def test_controlnode_switchover_policy_between_vns_traffic(self):
        ...
        policy2_fixture= self.useFixture( PolicyFixture( policy_name= policy2_name, rules_list= rev_rules, inputs= self.inputs,
                                    connections= self.connections ))
        policy3_fixture= self.useFixture( PolicyFixture( policy_name= policy3_name, rules_list= rules1, inputs= self.inputs,
                                    connections= self.connections ))
        policy4_fixture= self.useFixture( PolicyFixture( policy_name= policy4_name, rules_list= rev_rules1, inputs= self.inputs,
                                    connections= self.connections ))
        ...
        expectedResult = {}
        for proto in traffic_proto_l:
            expectedResult[proto]= True if rules[0]['simple_action'] == 'pass' else False
            traffic_obj[proto]= {}; startStatus[proto]= {}
            traffic_obj[proto]= self.useFixture(traffic_tests.trafficTestFixture(self.connections))
            # def startTraffic (self, name, num_streams= 1, start_port= 9100, tx_vm_fixture= None, rx_vm_fixture= None, stream_proto= 'udp', \
            # packet_size= 100, start_sport= 8000, total_single_instance_streams= 20):
            ...
            msg1= "Status of start traffic : %s, %s, %s" %(proto, vm1_fixture.vm_ip, startStatus[proto]['status'])
            if startStatus[proto]['status'] == False:
                self.logger.error (msg1); msg.extend([msg1, 'More info on failure: ', startStatus[proto]['msg']])
            else:
                self.logger.info (msg1)
        ...
        self.addCleanup(self.inputs.start_service , 'contrail-control' , [active_controller])
        sleep(5)
	...
	# policy deny applied traffic should fail
        self.logger.info('Checking the ping between the VM with new policy(deny)')
        self.logger.info ("Verify ping to vm %s" %(vn1_vm2_name))
        ret= vm1_fixture.ping_with_certainty( vm2_fixture.vm_ip, expectation=False )
        result_msg= "vm ping test result to vm %s is: %s" %(vn1_vm2_name, ret)
        self.logger.info (result_msg)
        if ret != True : result= False; msg.extend([result_msg, policy3_name])
        self.assertEqual(result, True, msg)

        self.logger.info ("Verify ping to vm %s" %(vn1_vm1_name))
        ret= vm1_fixture.ping_with_certainty( vm2_fixture.vm_ip, expectation=False )
        result_msg= "vm ping test result to vm %s is: %s" %(vn1_vm1_name, ret)
        self.logger.info (result_msg)
        if ret != True : result= False; msg.extend([result_msg, policy4_name])
        self.assertEqual(result, True, msg)
        ...
        self.logger.info('Stoping the Control service in  %s' %(new_active_controller))
        self.inputs.stop_service('contrail-control',[new_active_controller])
        self.addCleanup(self.inputs.start_service , 'contrail-control' , [new_active_controller])
        sleep(5)
        ...
        self.logger.info('Active control node from the Agent %s is %s' %(vm1_fixture.vm_node_ip, orig_active_controller))
        ...
        # Check the ping 
        self.logger.info('Checking the ping between the VM again with new policy deny..')
        self.logger.info ("Verify ping to vm %s" %(vn1_vm2_name))
        ret= vm1_fixture.ping_with_certainty( vm2_fixture.vm_ip, expectation=False )
        result_msg= "vm ping test result to vm %s is: %s" %(vn1_vm2_name, ret)
        self.logger.info (result_msg)
        ...
        # Start the control node service again
        self.logger.info('Starting the Control service in  %s' %(new_active_controller))
    ...
    def test_policy_rules_scaling(self):
        ...
        self.logger.info('Creating %d policy and %d rules to test policy scalability' %(number_of_policy, number_of_dummy_rules+len(valid_rules)))
        # for now we are creating limited number of policy and rules
        policy_objs_list = policy_test_utils._create_n_policy_n_rules(self, number_of_policy, valid_rules, number_of_dummy_rules)
        return True
    ...
    def test_policy_rules_scaling_with_ping(self):
        ...
        result_msg= "vm ping test result to vm %s is: %s" %(vn1_vm2_name, ret)
        self.logger.info (result_msg)
        if ret != True : result= False; msg.extend(["ping failure with scaled policy and rules:", result_msg])
        self.assertEqual(result, True, msg)
        return True
    ...
    def itest_policy_stress_agent_with_flow_aging (self):
        ...
        #return self.policy_test_with_scaled_udp_flows (topo, num_udp_streams= 5400, pps= 15, wait_time_after_start_traffic= 300)
    ...
    def test_policy_with_scaled_udp_flows(self):
        ...
        """
        computes= len(self.inputs.compute_ips)
        vms_per_compute= 2; num_streams= 800
        topo_obj= sdn_policy_traffic_test_topo.sdn_2vn_xvm_config()
        topo= topo_obj.build_topo(num_compute=computes, num_vm_per_compute= vms_per_compute)
        ...
        self.logger.info("Total streams to be generated per compute: %s" %planned_per_compute_num_streams)
        self.logger.info("Total streams to be generated : %s" %total_streams_generated)
    ...
    def policy_test_with_scaled_udp_flows(self, topo, num_udp_streams= 100, pps= 100, wait_time_after_start_traffic= 300, vms_on_single_compute= False, setup_only= False):
        ...
        # Setup/Verify Traffic ---         
        # 1. Define Traffic Params
        test_vn= topo.vnet_list[0]          # This will be source_vn for traffic test
        ...
        self.logger.info("----"*20)
        self.logger.info("num_udp_streams: %s, pps: %s, src_vms: %s" %(num_udp_streams, pps, len(src_vms) ))
        self.logger.info("----"*20)
        ...
        if num_rules == 0: matching_rule_action[dvn]= 'deny'
        self.logger.info ("matching_rule_action: %s" %matching_rule_action)
        # 3. Start Traffic 
        traffic_obj= {}; startStatus= {}; stopStatus= {}; expectedResult= {}
        for i in range(len(src_vms)):
            test_vm1= src_vms[i]
            test_vm2= dest_vms[i]
            test_vm1_fixture= config_topo['vm'][test_vm1]
            test_vm2_fixture= config_topo['vm'][test_vm2]
            expectedResult[i]= True if matching_rule_action[dest_vn] == 'pass' else False
            startStatus[i]= {}
            traffic_obj[i]= self.useFixture(traffic_tests.trafficTestFixture(self.connections))
            # def startTraffic (self, name=name, num_streams= 1, start_port= 9100, tx_vm_fixture, rx_vm_fixture, \
            #   stream_proto, packet_size= 100, start_sport= 8000, total_single_instance_streams= 20)
            startStatus[i]= traffic_obj[i].startTraffic(tx_vm_fixture=test_vm1_fixture, \
            ...
            msg1= "Status of start traffic : %s, %s, %s" %(i, test_vm1_fixture.vm_ip, startStatus[i]['status'])
            if startStatus[i]['status'] == False:
                self.logger.error (msg1); msg.extend([msg1, 'More info on failure: ', startStatus[i]['msg']])
            else:
                self.logger.info (msg1)
        ...
        if setup_only: 
            ...
            time.sleep(wait_time_after_start_traffic) # Should be more than 3 mins, aging time, for flows to peak and stabilise
        ...
        for i in range(len(src_vms)):
            ...
            if status != expectedResult[i]: msg.append(stopStatus[i])
            self.logger.info ("Status of stop traffic for instance %s is %s" %(i, stopStatus[i]))
        if msg != []: result= False
        self.assertEqual(result, True, msg)
        self.logger.info ("-"*80)
        # 5. verify kernel flows after stopping traffic.. we dont expect any stuck HOLD flows.
        for compNode in self.inputs.compute_ips:
            ...
            while retry < 5:
                ...
                if len(kflows) == 0: break
                else:
                    self.logger.info("Waiting for Kernel flows to drain in Compute %s, attempt %s, num_flows %s" %(compNode, retry, len(kflows)))
    ...
    def policy_test_verify_flows_unaffected_by_policy_update(self, topo):
        ...
        # vnet0 to vnet2 to be affected and stats to be ignored after stopping traffic
        ...
        test_vn_fix= config_topo['vn'][test_vn]
        test_vn_id= test_vn_fix.vn_id
        dest_vn_vm_dict= {topo.vn_of_vm[test_vm2]: test_vm2_fixture, topo.vn_of_vm[test_vm3]: test_vm3_fixture}
        # 2. set expectation to verify..
        matching_rule_action= {}
        # Assumption made here: test_vn, which is traffic source, has one policy, with one rule for each dest vn.. 
        policy= topo.vn_policy[test_vn][0]
        policy_info= "policy in effect is : " + str(topo.rules[policy])
        num_rules= len(topo.rules[policy])
        for dest_vn, dest_vm_fixture in dest_vn_vm_dict.items():
            ...
            for i in range(num_rules):
                ...
                if vn == dest_vn: matching_rule_action[dest_vn]= topo.rules[policy][i]['simple_action']; break
        self.logger.info ("matching_rule_action: %s" %matching_rule_action)
        # 3. Start Traffic 
        traffic_obj= {}; startStatus= {}; stopStatus= {}; expectedResult= {}
        for dest_vn, dest_vm_fixture in dest_vn_vm_dict.items():
            expectedResult[dest_vn]= True if matching_rule_action[dest_vn] == 'pass' else False
            traffic_obj[dest_vn]= self.useFixture(traffic_tests.trafficTestFixture(self.connections))
            startStatus[dest_vn]= traffic_obj[dest_vn].startTraffic(name=dest_vn, tx_vm_fixture=test_vm1_fixture, \
                rx_vm_fixture=dest_vm_fixture, cfg_profile= 'ContinuousSportRange')
            msg1= "Status of start traffic : %s, %s, %s" %(dest_vn, test_vm1_fixture.vm_ip, startStatus[dest_vn]['status'])
            if startStatus[dest_vn]['status'] == False:
                self.logger.error (msg1); msg.extend([msg1, 'More info on failure: ', startStatus[dest_vn]['msg']])
            else:
                self.logger.info (msg1)
        ...
        for policy in topo.policy_test_order:
            ...
            new_policy_entries= config_topo['policy'][policy].policy_obj['policy']['entries']
            data= {'policy': {'entries': new_policy_entries}}
            update_status= starting_policy_fixture.update_policy(starting_policy_id, data)
            new_rules= topo.rules[policy]
            #policy= current_policy_name     #policy name is still old one
            topo.rules[starting_policy_name]= copy.copy(new_rules)    #update new rules in reference topology
        ...
        for dest_vn, dest_vm_fixture in dest_vn_vm_dict.items():
            ...
            # Check the status only if the stream is unaffected by policy change
            if dest_vn == vm_vn_to_check:
                self.logger.info ("Status of stop traffic for dest_vn %s is %s" %(dest_vn, stopStatus[dest_vn]))
                status= True if stopStatus[dest_vn] == [] else False
                if status != True: msg.append(stopStatus[dest_vn]); result= False
        self.assertEqual(result, True, msg)
        self.logger.info ("-"*80)
        return result
    ...
    def test_policy_across_projects(self):
        ...
        for proto in traffic_proto_l:
            ...
            traffic_obj[proto]= {}; startStatus[proto]= {}
            if proto == 'icmp':
                tx_vm_fixt= config_topo[p_lst[0]]['vm'][p1vm1]; rx_vm_fixt= config_topo[p_lst[1]]['vm'][p2vm2]
            else:
                tx_vm_fixt= config_topo[p_lst[0]]['vm'][p1vm1]; rx_vm_fixt= config_topo[p_lst[2]]['vm'][p3vm3]
            ...
            msg1= "Status of start traffic : %s, %s, %s" %(proto, tx_vm_fixt.vm_ip, startStatus[proto]['status'])
            if startStatus[proto]['status'] == False:
                self.logger.error (msg1); msg.extend([msg1, 'More info on failure: ', startStatus[proto]['msg']])
            else:
                self.logger.info (msg1)
        ...
        self.logger.info ("Poll live traffic and get status..")
        for proto in traffic_proto_l:
            self.logger.info ("Poll live traffic %s and get status.." %(proto))
            traffic_stats= traffic_obj[proto].getLiveTrafficStats()
            if not traffic_stats['status']:
                err_msg.extend(["Traffic disruption is seen:", traffic_stats['msg']])
        self.assertEqual(traffic_stats['status'], expectedResult[proto], err_msg)
        self.logger.info ("-"*80)
        ...
        self.logger.info("With proto tcp allowed between %s and %s, trying to send icmp traffic" %(p1vm1, p3vm3))
        expectedResult= False
        self.logger.info ("Verify ping to vm %s from vm %s, expecting it to fail" %(dst_vm, src_vm))
        ret= src_vm_fixture.ping_with_certainty( dst_vm_ip, expectation=expectedResult )
        result_msg= "vm ping test result to vm %s is: %s" %(dst_vm, ret)
        self.logger.info (result_msg)
        if ret != True : result= False; msg.extend(["icmp traffic passed with deny rule:", result_msg])
        self.assertEqual(result, True, msg)

        result= True; msg=[]; expectedResult= True
        dst_vm= p2vm2                                        #'vmc2'
        dst_vm_fixture= config_topo[p_lst[1]]['vm'][p2vm2]
        dst_vm_ip = dst_vm_fixture.vm_ip
        src_vm_fixture= config_topo[p_lst[3]]['vm'][adminvm] #'vmc-admin'
        self.logger.info("Now will test ICMP traffic between admin VM %s and non-default project VM %s" %(adminvm, p2vm2))
        ret= src_vm_fixture.ping_with_certainty( dst_vm_ip, expectation=expectedResult )
        result_msg= "vm ping test result to vm %s is: %s" %(dst_vm, ret)
        self.logger.info (result_msg)
        if ret != True : result= False; msg.extend(["ICMP traffic failed between default and non-default project with policy:", result_msg])
        self.assertEqual(result, True, msg)
        return True
    ...
    def policy_test_with_implicit_rule_proto_traffic(self, topo):
        ...
        for policy in topo.policy_test_order:
            ...
            same_ntw_im_rule,diff_ntw_im_rule=self.parse_and_build_implicit_rule(policy,topo)
            # 4. Traslation of flow parameters (5-tuples) and run the traffic for same network implicit rule
            if (same_ntw_im_rule.has_key('tcp') or same_ntw_im_rule.has_key('udp') or same_ntw_im_rule.has_key('icmp')): 
               proto_list, source_fixture, dest_fixture, dpi=self.translate_of_implicit_rule_into_flow_params(same_ntw_im_rule,config_topo,topo)
               self.logger.info("For applied policy :%s ,Generated implicit rule for same network :%s"%(policy,same_ntw_im_rule)) 
               self.logger.info("Generating the traffic flow for same network with combination of protocol list: %s ,source ip :%s , dest-ip:%s,port-id:%d"%(proto_list, source_fixture.vm_ip, dest_fixture.vm_ip, dpi)) 
               im_flow_result=self.traffic_generator_for_proto_list(proto_list,source_fixture,dest_fixture,dpi,policy_info,topo)
               self.logger.info("Traffic flow generation is done for same network implicit rule :%s",same_ntw_im_rule)   
            else:
                 pass
            # 5.Translation of implicit rule into flow parameters(5-tuples) and run the traffic for different network implicit rule
            if (diff_ntw_im_rule.has_key('tcp') or  diff_ntw_im_rule.has_key('udp') or diff_ntw_im_rule.has_key('icmp')):
               proto_list, source_fixture, dest_fixture, dpi=self.translate_of_implicit_rule_into_flow_params(diff_ntw_im_rule,config_topo,topo)
               self.logger.info("For applied policy :%s ,Generated implicit rule for differnt network :%s"%(policy,diff_ntw_im_rule)) 
               self.logger.info("Genarating the traffic flow for different network with combination of protocol list: %s ,source ip :%s , dest-ip:%s,port-id:%d"%(proto_list, source_fixture.vm_ip, dest_fixture.vm_ip, dpi)) 
               im_flow_result=self.traffic_generator_for_proto_list(proto_list,source_fixture,dest_fixture,dpi,policy_info,topo)
               self.logger.info("Traffic flow generation is done for different network implicit rule :%s",diff_ntw_im_rule) 
            else :
                 pass  
        return result 
    ...
    def traffic_generator_for_proto_list(self,proto_list,source_fixture,dest_fixture,dpi,policy_info,topo):
        ...
        traffic_obj= {}; startStatus= {}; stopStatus= {}
        total_streams= {}; total_streams['icmp']= 1
        total_streams['udp']= 1; total_streams['tcp']= 1; 
        traffic_proto_l=proto_list.keys()
        expectedResult= {}
        start_time = self.analytics_obj.getstarttime(self.inputs.compute_ips[0])
        for proto in traffic_proto_l:
            expectedResult[proto]= True if proto_list[proto] == 'pass'   else False
            traffic_obj[proto]= {}; startStatus[proto]= {}
            traffic_obj[proto]= self.useFixture(traffic_tests.trafficTestFixture(self.connections))
            # def startTraffic (self, name, num_streams= 1, start_port= 9100, tx_vm_fixture= None, rx_vm_fixture= None, stream_proto= 'udp', \
            # packet_size= 100, start_sport= 8000, total_single_instance_streams= 20):
            ...
            msg1= "Status of start traffic : %s, %s, %s" %(proto,source_fixture.vm_ip, startStatus[proto]['status'])
            if startStatus[proto]['status'] == False:
               self.logger.error (msg1); msg.extend([msg1, 'More info on failure: ', startStatus[proto]['msg']])
            else:
               self.logger.info(msg1)
    ...
    def parse_and_build_implicit_rule(self,policy,topo):
        #1 Get the implicit rule for both same and different network from the policy
        ...
        min=1
        max=65535
        #Getting the explicit protocol list and port number for selected explicit rule.
        for i in range(num_rules):
            if (chk_policy[i]['dest_network'] == chk_policy[i]['source_network']):
                  if  ((chk_policy[i]['src_ports'] != chk_policy[i]['dst_ports']) and (chk_policy[i]['protocol'] == 'tcp' or 'udp'or 'any')):
                       if (chk_policy[i]['src_ports'] != 'any'):
                            ...
                            intra_vn_ntw['src_ntw']=chk_policy[i]['source_network'];intra_vn_ntw['dst_ntw']=chk_policy[i]['dest_network']
                       else:
                       ...
                       intra_vn_explicit_proto[chk_policy[i]['protocol']] = chk_policy[i]['simple_action']
                       intra_vn_ntw['dst_ntw']=chk_policy[i]['dest_network']; intra_vn_ntw['src_ntw']=chk_policy[i]['source_network']
                       intra_vn_ntw['src_port']=new_port;intra_vn_ntw['dst_port']=new_port
                       self.logger.info ("Defined Explicit rule#:%d  for same network :src_net= %s, dst_net=%s, src_port= %s, dst_port=%s, protocol:%s,action= %s "%(i,chk_policy[i]['source_network'],chk_policy[i]['dest_network'],chk_policy[i]['src_ports'],chk_policy[i]['dst_ports'],chk_policy[i]['protocol'],chk_policy[i]['simple_action']))
                       break
                  ...
                  if ((chk_policy[i]['src_ports'] != chk_policy[i]['dst_ports']) and (chk_policy[i]['protocol'] == 'tcp' or 'udp' or 'any')):
                       if (chk_policy[i]['src_ports'] != 'any'):
                            ...
                            inter_vn_ntw['src_ntw']=chk_policy[i]['source_network'];inter_vn_ntw['dst_ntw']=chk_policy[i]['dest_network']
                       else:
                            port_range=chk_policy[i]['dst_ports']
                            start_port=port_range[0]
                            end_port=port_range[1]
                       ...
                       inter_vn_explicit_proto[chk_policy[i]['protocol']] = chk_policy[i]['simple_action']
                       inter_vn_ntw['dst_ntw']=chk_policy[i]['dest_network']; inter_vn_ntw['src_ntw']=chk_policy[i]['source_network']
                       inter_vn_ntw['src_port']=new_port;inter_vn_ntw['dst_port']=new_port
                       self.logger.info ("Defined Explicit rule#:%d for two different network  :src_net= %s, dst_net=%s, src_port= %s, dst_port=%s, protocol:%s,action= %s "%(i,chk_policy[i]['source_network'],chk_policy[i]['dest_network'],chk_policy[i]['src_ports'],chk_policy[i]['dst_ports'],chk_policy[i]['protocol'],chk_policy[i]['simple_action']))
                       break
                  elif ((chk_policy[i]['src_ports'] == chk_policy[i]['dst_ports']) and (chk_policy[i]['protocol'] == 'tcp' or 'udp' or 'icmp')):
                       inter_vn_explicit_proto[chk_policy[i]['protocol']] = chk_policy[i]['simple_action']
                       inter_vn_ntw['dst_ntw']=chk_policy[i]['dest_network']; inter_vn_ntw['src_ntw']=chk_policy[i]['source_network']
                       inter_vn_ntw['src_port']=new_port;inter_vn_ntw['dst_port']=new_port
                  self.logger.info ("Defined Explicit rule#:%d for two different network  :src_net= %s, dst_net=%s, src_port= %s, dst_port=%s, protocol:%s,action= %s "%(i,chk_policy[i]['source_network'],chk_policy[i]['dest_network'],chk_policy[i]['src_ports'],chk_policy[i]['dst_ports'],chk_policy[i]['protocol'],chk_policy[i]['simple_action']))
        #2 Building the implicit rule based selected explicit protocols and  Insert the remaing implicit protocols 
        ...
        if   proto_intra_vn_len == 1 :
             if   intra_vn_explicit_proto.has_key('tcp'):
                  ...
                  final_intra_vn_implicit_rule={'tcp':intra_vn_explicit_proto['any'],'udp':intra_vn_explicit_proto['any'],'icmp':intra_vn_explicit_proto['any']}
             else :
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for same network, Please define the proper protocol rules in policy")
                  sys.exit(0)
        elif (proto_intra_vn_len == 0 and (intra_vn_ntw['dst_port'] != 9100 or intra_vn_ntw['src_port'] != 9100)):
                  final_intra_vn_implicit_rule={'udp':'pass','icmp':'pass','tcp':'pass'} 
             ...
             if   (intra_vn_explicit_proto.has_key('tcp') and intra_vn_explicit_proto.has_key('udp')):
                  ...
                  final_intra_vn_implicit_rule={'udp':intra_vn_explicit_proto['any'],'icmp':intra_vn_explicit_proto['any']}
             elif (intra_vn_explicit_proto.has_key('any') and intra_vn_explicit_proto.has_key('udp')):
                  final_intra_vn_implicit_rule={'tcp':intra_vn_explicit_proto['any'],'icmp':intra_vn_explicit_proto['any']}
             elif (intra_vn_explicit_proto.has_key('any') and intra_vn_explicit_proto.has_key('icmp')):
                  final_intra_vn_implicit_rule={'tcp':intra_vn_explicit_proto['any'],'udp':intra_vn_explicit_proto['any']}
             else :
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for same network, Please define the proper protocol rules in policy")
                  sys.exit(0)
             ...
             if   (intra_vn_explicit_proto.has_key('tcp') and intra_vn_explicit_proto.has_key('udp') and intra_vn_explicit_proto.has_key('icmp')):
                  self.logger.error(("No need to run the implicit rule for defined explicit rules "))
             elif (intra_vn_explicit_proto.has_key('tcp') and intra_vn_explicit_proto.has_key('udp') and intra_vn_explicit_proto.has_key('any')):
                  final_intra_vn_implicit_rule={'icmp':intra_vn_explicit_proto['any']}
                  ...
                  final_intra_vn_implicit_rule={'udp':intra_vn_explicit_proto['any']}
                  ...
                  final_intra_vn_implicit_rule={'tcp':intra_vn_explicit_proto['any']}
             else :
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for same network, Please define the proper protocol rules in policy")
                  sys.exit(0)
             ...
             if   (intra_vn_explicit_proto.has_key('tcp') and intra_vn_explicit_proto.has_key('udp') and intra_vn_explicit_proto.has_key('icmp')and
                  ...
                  self.logger.error("All three protocols are not defined properly in policy for same network combination.")
                  sys.exit(0)
        elif proto_intra_vn_len > 4 :
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for same network, Please define the proper protocol rules in policy")
                  sys.exit(0)
        ...
        if   proto_inter_vn_len == 1 :
             if   inter_vn_explicit_proto.has_key('tcp'):
                  ...
                  final_inter_vn_implicit_rule={'tcp':inter_vn_explicit_proto['any'],'udp':inter_vn_explicit_proto['any'],'icmp':inter_vn_explicit_proto['any']}
             else:
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for different network, Please define the proper protocol rules in policy")
                  sys.exit(0)
        elif (proto_inter_vn_len == 0 and (inter_vn_ntw['dst_port'] != 9100 or inter_vn_ntw['src_port'] != 9100)):
                  final_inter_vn_implicit_rule={'udp':'deny','icmp':'deny','tcp':'deny'} 
             ...
             if   (inter_vn_explicit_proto.has_key('tcp') and inter_vn_explicit_proto.has_key('udp')):
                  ...
                  final_inter_vn_implicit_rule={'udp':inter_vn_explicit_proto['any'],'icmp':inter_vn_explicit_proto['any']}
             elif (inter_vn_explicit_proto.has_key('any') and inter_vn_explicit_proto.has_key('udp')):
                  final_inter_vn_implicit_rule={'tcp':inter_vn_explicit_proto['any'],'icmp':inter_vn_explicit_proto['any']}
             elif (inter_vn_explicit_proto.has_key('any') and inter_vn_explicit_proto.has_key('icmp')):
                  final_inter_vn_implicit_rule={'tcp':inter_vn_explicit_proto['any'],'udp':inter_vn_explicit_proto['any']}
             else:
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for different network, Please define the proper protocol rules in policy")
                  sys.exit(0)
             ...
             if   (inter_vn_explicit_proto.has_key('tcp') and inter_vn_explicit_proto.has_key('udp') and inter_vn_explicit_proto.has_key('icmp')):
                   self.logger.info("No need to run the implicit rule for defined explicit rules ")
             elif (inter_vn_explicit_proto.has_key('tcp') and inter_vn_explicit_proto.has_key('udp') and inter_vn_explicit_proto.has_key('any')):
                  final_inter_vn_implicit_rule={ 'icmp':inter_vn_explicit_proto['any']}
                  ...
                  final_inter_vn_implicit_rule={'udp':inter_vn_explicit_proto['any']}
                  ...
                  final_inter_vn_implicit_rule={'tcp':inter_vn_explicit_proto['any']}
             else:
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for different network, Please define the proper protocol rules in policy")
                  sys.exit(0)
             ...
             if   (inter_vn_explicit_proto.has_key('tcp') and inter_vn_explicit_proto.has_key('udp') and inter_vn_explicit_proto.has_key('icmp')and
                  ...
                  self.logger.error("All three protocols are not defined properly in policy for different network combination.")
        elif proto_inter_vn_len > 4:
                  self.logger.error("Only TCP,UDP and ICMP protocols are applicable for different network, Please define the proper protocol rules in policy")
                  sys.exit(0)
        else : 
             pass
        final_intra_vn_implicit_rule=dict(final_intra_vn_implicit_rule,**intra_vn_ntw)
        final_inter_vn_implicit_rule=dict(final_inter_vn_implicit_rule,**inter_vn_ntw)  
        #sending the 5 tuple list of same and different network implicit rules like implicit proto_list,src/dst ntws and ports 
    ...
    def translate_of_implicit_rule_into_flow_params(self,all_implicit_rule,config_topo,topo):
        #Defining the source network ,destination network ,src/dst ports and protocol list to run the traffic  
        if all_implicit_rule:
           ...
           if  (parse_im_rule.has_key('tcp') and parse_im_rule.has_key('udp') and parse_im_rule.has_key('icmp')):
                proto_list={'tcp':parse_im_rule['tcp'],'udp':parse_im_rule['udp'],'icmp':parse_im_rule['icmp']}
           elif (parse_im_rule.has_key('tcp') and parse_im_rule.has_key('udp')):
                proto_list={'tcp':parse_im_rule['tcp'],'udp':parse_im_rule['udp']}
           elif (parse_im_rule.has_key('tcp') and parse_im_rule.has_key('icmp')):
                proto_list={'tcp':parse_im_rule['tcp'],'icmp':parse_im_rule['icmp']}
           elif (parse_im_rule.has_key('udp') and parse_im_rule.has_key('icmp')):
                proto_list={'udp':parse_im_rule['udp'],'icmp':parse_im_rule['icmp']}
             ...
             self.logger.error("Implicit rules are not exist for defined policy")
