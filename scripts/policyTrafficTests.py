...
class policyTrafficTestFixture(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def policy_test_with_multi_proto_traffic(self, topo):
        ...
        self.verify_policy_opserver_data(
            vn_name=test_vn, num_of_flows=exp_flow_count)
        self.logger.info("-" * 80)
    ...
    def test_policy_across_projects(self):
        ...
        # For communication across projects to work we need to have default SG
        # with allow rule in ingress/egress
        for project in topo_obj.project_list:
            self.logger.info(
                'Default SG to be edited for allow all on project: %s' % project)
            config_topo[project]['project'][
                project].set_sec_group_for_allow_all(project, 'default')
