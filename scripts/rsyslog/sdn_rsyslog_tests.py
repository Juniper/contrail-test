...
class sdnRsyslog(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def test_rsyslog_messages_in_db_through_contrail_logs(self):
        ...
        try:
            ...
            self.logger.error("Colud not get a collector less compute node for the test.")
            self.logger.exception(
                "Got exception as %s" %(e))

        # bring up rsyslog client-server connection with udp protocol.
        restart_collector_to_listen_on_35999(self, self.inputs.collector_ips[0])
        restart_rsyslog_client_to_send_on_35999(self, comp_node_ip, self.inputs.collector_ips[0])
        ...
        with settings(host_string='%s@%s' % (self.inputs.username,self.inputs.cfgm_ips[0]),
            ...
            output = fab_put_file_to_vm(host_string='%s@%s' %(self.inputs.username, comp_node_ip),
                                        password=self.inputs.password, src = path, dest='~/')
        ...
        with settings(host_string='%s@%s' % (self.inputs.username, self.inputs.collector_ips[0]),
            ...
            if int(output) == 0:
                self.logger.error("No syslog messages were through contrail-logs. Seems to be an issue")
                return False
            elif int(output) < 7:
                self.logger.info("Remote syslog message test connection setup passed.")
                self.logger.warn("There is 30% message loss. There might be an issue.")
            else:
                self.logger.info("Remote syslog message test connection setup passed.")
                self.logger.info("Remote syslog message test over UDP connection passed.")

        # change rsyslog client server connection to tcp.
        update_rsyslog_client_connection_details(self,
        ...
        with settings(host_string='%s@%s' % (self.inputs.username, self.inputs.collector_ips[0]),
            ...
            output = run('%s' % (cmd), pty=True)
            if int(output) != 10:
                self.logger.error("There was a message loss in a tcp connection which is unexpected.")
                return False
            else:
                self.logger.info("Remote syslog message test over TCP connection passed.")
        ...
        # verify 'category' query of contrail logs.
            ...
            output = run('%s' % (cmd), pty=True)
            if int(output) != 10:
                self.logger.error("Unable to retrieve messages from the database using the 'category' query.")
                return False
            else:
                self.logger.info("Succesfully retrived messages from the database using the 'category' query.")
        ...
        with settings(host_string='%s@%s' % (self.inputs.username, self.inputs.collector_ips[0]),
            ...
            for each_facility in list_of_facility:
                for each_severity in list_of_severity:
                    ...
                    output = run('%s' % (cmd), pty=True)
                    if int(output) != 1:
                        self.logger.error("Syslog message with facility %s and severity %s was not received" %(each_facility, each_severity))
                        result_flag = 1
                    else:
                        self.logger.info("Syslog message with facility %s and severity %s was received" %(each_facility, each_severity))

        if result_flag != 0:
            self.logger.error("Error in transmitting or receiving some syslog facilities and severities")
            return False

        
        # verify 'level' query of contrail logs.
        bug_1353624_fix = False
        if bug_1353624_fix == True:
            with settings(host_string='%s@%s' % (self.inputs.username, self.inputs.collector_ips[0]),
                          password=self.inputs.password, warn_only=True,abort_on_prompts= False):
                for each_severity in list_of_severity:
                    ...
                    output = run('%s' % (cmd), pty=True)
                    if int(output) < 1:
                        self.logger.error("Syslog message with severity %s was not found." %(each_severity))
                        result_flag = 1
                    else:
                        self.logger.info("Syslog message with severity %s was found." %(each_severity))

            if result_flag != 0:
                self.logger.error("Error in transmitting or receiving some syslog severities.")
                return False
        ...
        with settings(host_string='%s@%s' % (self.inputs.username, self.inputs.collector_ips[0]),
            ...
            output = run('%s' % (cmd), pty=True)
            if int(output) != 100:
                self.logger.error("Failed to receive all the messages greater than 1024 bytes over a tcp connection.")
                return False
            else:
                self.logger.info("Successfully received all the messages greater than 1024 bytes over a tcp connection.")
