...
class SolnSetup( fixtures.Fixture ):
    ...
    def setup_common_objects(self):
        ...
        (self.fvn_public_name, self.fvn_public_subnets)= ("fip_vn_public", ['10.204.219.16/28'])
        ...
        self.fvn_public_fixture=self.useFixture( VNFixture(project_name= self.inputs.project_name, connections= self.connections, inputs= self.inputs, vn_name= self.fvn_public_name, subnets= self.fvn_public_subnets))
        self.fvn1_fixture=self.useFixture( VNFixture(project_name= self.inputs.project_name, connections= self.connections, inputs= self.inputs, vn_name= self.fvn1_name, subnets= self.fvn1_subnets))
        self.fvn2_fixture=self.useFixture( VNFixture(project_name= self.inputs.project_name, connections= self.connections, inputs= self.inputs, vn_name= self.fvn2_name, subnets= self.fvn2_subnets))
        self.fvn3_fixture=self.useFixture( VNFixture(project_name= self.inputs.project_name, connections= self.connections, inputs= self.inputs, vn_name= self.fvn3_name, subnets= self.fvn3_subnets))

        # Configure 2 VMs in VN1, 2 VMs in VN2, 1 VM in FVN_PUBLIC, 1 VM in FVN1,FVN2 and FVN3 each
        self.vn1_vm1_fixture=self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections, vn_obj= self.vn1_fixture.obj, vm_name= self.vn1_vm1_name,node_name= compute_1))
        self.vn1_vm2_fixture=self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections, vn_obj= self.vn1_fixture.obj, vm_name= self.vn1_vm2_name,node_name= compute_2))
        self.vn2_vm1_fixture=self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections, vn_obj= self.vn2_fixture.obj, vm_name= self.vn2_vm1_name,node_name= compute_2))
        self.vn2_vm2_fixture=self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections, vn_obj= self.vn2_fixture.obj, vm_name= self.vn2_vm2_name,node_name= compute_1))
        self.fvn_public_vm1_fixture=self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections, vn_obj= self.fvn_public_fixture.obj, vm_name= self.fvn_public_vm1_name))
