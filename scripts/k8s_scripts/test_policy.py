...
class TestNetworkPolicy(BaseK8sTest):
    ...
    def setUpClass(cls):
        ...
        busybox_spec_4 = {'containers': [{'image': 'busybox','command': ['sleep', '1000000'],
                                          'image_pull_policy': 'IfNotPresent',}],
                                    'restart_policy': 'Always'}
        busybox_spec_5 = {'containers': [{'image': 'busybox','command': ['sleep', '1000000'],
                                          'image_pull_policy': 'IfNotPresent',}],
                                    'restart_policy': 'Always'}
        busybox_spec_6 = {'containers': [{'image': 'busybox','command': ['sleep', '1000000'],
                                          'image_pull_policy': 'IfNotPresent',}],
                                    'restart_policy': 'Always'}
        busybox_spec_7 = {'containers': [{'image': 'busybox','command': ['sleep', '1000000'],
                                          'image_pull_policy': 'IfNotPresent',}],
                                    'restart_policy': 'Always'}
    ...
    def test_ingress_egress_on_pod(self):
        ...
        policy1 = self.setup_update_policy(pod_selector = self.client1_pod_ns3.labels,
                                   name="igress-egress-policy-on-pod",
        ...
        assert self.validate_wget(self.client1_pod_ns3, url2, expectation=False)
    ...
    def test_ingress_egress_on_namespace(self):
        ...
        policy_types = ["Ingress", "Egress"]
        policy1 = self.setup_update_policy(name="igress-egress-policy-on-pod",
                                   namespace = self.ns3.name,
                                   policy_types = policy_types,
                                   ingress= ingress_list,
                                   egress= egress_list)
        ...
        # Verify that other pods which do not match egress criteria cannot communicate
        assert self.validate_wget(self.client1_pod_ns3, url2, expectation=False)
        assert self.validate_wget(self.client3_pod_ns3, url2, expectation=False)
        assert self.client1_pod_ns3.ping_with_certainty(self.web_pod_ns1.pod_ip,
                                                        expectation=False)
        assert self.client3_pod_ns3.ping_with_certainty(self.web_pod_ns1.pod_ip,
                                                        expectation=False)
        assert self.client1_pod_ns3.ping_with_certainty(self.web_pod_ns2.pod_ip,
                                                        expectation=False)
        assert self.client3_pod_ns3.ping_with_certainty(self.web_pod_ns2.pod_ip,
                                                        expectation=False)
        assert self.client1_pod_ns3.ping_with_certainty(self.client2_pod_ns3.pod_ip,
                                                        expectation=False)
        assert self.client3_pod_ns3.ping_with_certainty(self.client2_pod_ns3.pod_ip,
