                        ...
                        for ae in e.xpath ('./list/AclEntrySandeshData'):
                            ...
                            for c in ae:
                                if c.tag in ('src_port_l', 'dst_port_l', 'proto_l'):
                                    ace[c.tag] = {}
                                    # Validate data before looking for list elements as port_list is Not Available if protocol is ICMP
                                    cdata= c.xpath ('./list/SandeshRange')
                                    if cdata == []: ace[c.tag]= 'NA'
                                    else:     
                                        for pl in cdata[0]: ace[c.tag][pl.tag] = pl.text
                ...
                for e in vnl:
                    if e.tag == 'flow_entries':
                        ...
                        for ae in e.xpath ('./list/FlowSandeshData'):
                            # Dump following info for debugging in case of failure
                            chk_keys= ['ace_l', 'src', 'dst', 'source_vn', 'dest_vn', 'protocol', 'action_l', 'flow_uuid']
                            for i in ae.getchildren():
                                if i.tag == 'dest_vn' and i.text == '__UNKNOWN__':
                                    ...
                                    for k in chk_keys:
                                        if k == i.tag: self.log.info ("flow: %s, -->, %s" %(i.tag, i.text)); break
                                    return err_msg
                            ace = {}
                            p[e.tag].append (ace)
                            for c in ae:
...
l[0]={'protocol': '1', 'stats_bytes': '222180', 'stats_packets': '2645', 'setup_time_utc': '1371254131073195', 'sip': '1.1.1.253', 'src_port': '0', 'uuid': '3a95eaa5-87e5-4b37-a49a-15a406db8356', 'nat': 'disabled', 'mirror_port': '0', 'direction': 'ingress', 'implicit_deny': 'no', 'refcount': '4', 'setup_time': '2013-Jun-14 23:55:31.073195', 'vrf': '1', 'dest_vrf': '0', 'interface_idx': '3', 'flow_handle': '54518', 'dst_port': '0', 'action': '32', 'short_flow': 'no', 'dip': '2.1.1.253', 'mirror_ip': '0.0.0.0'}'''
    ...
    def get_vna_fetchflowrecord(self,vrf=None,sip=None,dip=None,sport=None,dport=None,protocol=None):
        ...
        path='Snh_FetchFlowRecord?vrf='+vrf+'&sip='+sip+'&dip='+dip+'&src_port='+sport+'&dst_port='+dport+'&protocol='+protocol
    ...
    def get_vna_route_in_mclist_by_key(self, vrf_id, key, ip, prefix):
         route_list=self.get_vna_active_route(vrf_id, ip, prefix)['path_list'][0]['nh']['mc_list']
...
#        if 1 == len (avn):
    ...
    def get_vna_xmpp_connection_status (self):
        ...
        for x in range (0,len(var)):   
            p = {}
            p[elem.xpath('./list/AgentXmppData')[x].xpath('./controller_ip')[0].tag] = elem.xpath('./list/AgentXmppData')[x].xpath('./controller_ip')[0].text
            p[elem.xpath('./list/AgentXmppData')[x].xpath('./cfg_controller')[0].tag] = elem.xpath('./list/AgentXmppData')[x].xpath('./cfg_controller')[0].text
            p[elem.xpath('./list/AgentXmppData')[x].xpath('./state')[0].tag] = elem.xpath('./list/AgentXmppData')[x].xpath('./state')[0].text
            l.append (p)
        return l
    ...
    def get_vna_diag_ping_res(self,src_ip='', src_port='', dst_ip='', dst_port='', proto='', vrf='',size='', count='', intv=''):
        ...
        '''
        ping_url="Snh_PingReq?source_ip=%s&source_port=%s&dest_ip=%s&dest_port=%s&protocol=%s&vrf_name=%s&packet_size=%s&count=%s&interval=%s" %(src_ip,src_port,dst_ip,dst_port,proto,vrf,size,count,intv)
        print ping_url
        ...
        if ping_resp != []:
            ...
            ping_sum_resp=self.ping_out.xpath('/__PingResp_list/PingSummaryResp')
            m=[]
    ...
    def get_vna_verify_diag_ping(self,src_ip='', src_port='', dst_ip='', dst_port='', proto='', vrf='',size='', count='', intv=''):
        ...
        req_rcv=0
        loss=0
        ping_count=self.get_vna_diag_ping_res(src_ip=src_ip, src_port=src_port, dst_ip=dst_ip, dst_port=dst_port, proto=proto, vrf=vrf,size=size, count=10, intv=intv)
        if ping_count == {}:
            result=False
        else:
            for i in range (0,len(ping_count['PingSummaryResp'])-1):
                if ping_count['PingSummaryResp'][i].keys()[0] == 'request_sent':
                    req_sent= int(ping_count['PingSummaryResp'][i].values()[0])
