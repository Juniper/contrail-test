...
from .base import RouteAggregateBase
import route_agg
...
class TestRouteAggregate(RouteAggregateBase, VerifySvcFirewall):
    ...
    def test_route_aggregation(self):
        ...
        '''
        ragg_name = get_random_name('ragg')
        ...
        agg_fixture = self.useFixture(RouteAggregateFixture(
                                      self.connections,agg_name=ragg_name, prefix=agg_prefix))
    ...
    def test_with_multiple_route_aggregation_prefixes(self):
        ...
        svc_info = self.verify_svc_chain(svc_img_name='cirros_in_net', service_mode='in-network',
                                     create_svms=True)
        ragg_1 = get_random_name('ragg')
        ragg_2 = get_random_name('ragg')
        svc_info = self.verify_svc_chain(svc_img_name='cirros_in_net', service_mode='in-network',
                                     create_svms=True)
        si = svc_info['si_fixture']
        left_vn = svc_info['left_vn_fixture']
        ...
        left_agg_fixture = self.useFixture(RouteAggregateFixture(
                                      self.connections,agg_name=ragg_1, prefix=left_agg_prefix))
        ...
        right_agg_fixture = self.useFixture(RouteAggregateFixture(
                                      self.connections,agg_name=ragg_2, prefix=right_agg_prefix))
