...
class TestPerms(testtools.TestCase, fixtures.TestWithFixtures):
    def setUp(self):
        ...
        self.key_stone_clients= KeystoneCommands(username= self.inputs.stack_user, password= self.inputs.stack_password, tenant= self.inputs.project_name, auth_url= auth_url )
    ...
    def keystone_create_users(self, user_list):
        ...
        for name in create_user_set:
            ...
            self.addCleanup( kc.roles.remove_user_role, user, role_dict[user_role[name]], admin_tenant)
    ...
    def set_perms(self, obj, mode = None, owner = None, group = None):
        ...
        self.logger.info( 'Current perms %s = %s' %(obj.get_fq_name(), self.print_perms(perms)))
    ...
    def test_all (self, ip='127.0.0.1', port=8082, domain_name='default-domain',
	  ...
	  10.Set IPAM perms such that owner/group has read/write permissions.try to update/read ipam with other user;should pass;else test fails
        ...
        self.keystone_create_users([('alice', 'alice123', 'staff'), ('bob', 'bob123', 'staff')])

        # user=admin, role=admin
        vnc_lib = VncApi(username= self.inputs.stack_user, password= self.inputs.stack_password, tenant_name= self.inputs.project_name, 
        api_server_host = ip, api_server_port = port)

        # user=bob, role=staff
        vnc_lib_bob = VncApi(username='bob', password='bob123', tenant_name= self.inputs.project_name, 
        api_server_host = ip, api_server_port = port)

        # user=alice, role=staff
        vnc_lib_alice = VncApi(username='alice', password='alice123', tenant_name= self.inputs.project_name, 
        api_server_host = ip, api_server_port = port)
...
#        self.logger.info( 'Created Project  %s ' %( str(project.get_fq_name())) )
        ...
        self.logger.info( 'Trying to create network-ipam in project as bob/staff ... should fail')
        try:
            ipam2 = NetworkIpam('default-network-ipam-2', project, IpamType("dhcp"))
            network_ipam_bob_id= vnc_lib_bob.network_ipam_create(ipam2)
            self.logger.error(  "Succeeded in creating network IPAM which should have failed ")
            testfail += 1
        except PermissionDenied as e:
            self.logger.info( 'Failed to create network IPAM ... Test passed!')
            testpass += 1

        self.logger.info(  'Resetting permissions to allow bob/staff to create network IPAM')
        self.set_perms(project, mode = '0777')
        vnc_lib.project_update(project)
        
        self.logger.info( 'Trying to create network IPAM in project as bob/staff ... should go through')
        try:
            ipam2 = NetworkIpam('default-network-ipam-2', project, IpamType("dhcp"))
            ipam2_id = vnc_lib_bob.network_ipam_create(ipam2)
            self.logger.info(  'Success in creating network IPAM ... Test passed!' )
            testpass += 1
        except PermissionDenied as e:
        ...
        self.set_perms(ipam2_obj, mode = '0774', owner = 'admin', group = 'foobar')
        vnc_lib.network_ipam_update(ipam2_obj)

        self.logger.info('') 
        self.logger.info( '############################# READ #######################################')
        self.logger.info( 'Reading IPAM as bob/staff ... should go through because of other permissions' )
        obj = vnc_lib_bob.network_ipam_read(id = ipam2_id)
        self.logger.info ( 'IPAM name=%s, uuid=%s' %(obj.get_fq_name(), obj.uuid) )

        self.logger.info( 'Resetting ownership and permission to disallow others to read IPAM' )
        self.set_perms(ipam2_obj, mode = '0770', owner = 'admin', group = 'foobar')
        vnc_lib.network_ipam_update(ipam2_obj)

        self.logger.info( 'Reading IPAM as bob/staff ... should fail' )
        try:
            obj = vnc_lib_bob.network_ipam_read(id = ipam2_id)
            self.logger.error( " Succeeded in reading IPAM which should have failed " )
            testfail += 1
        except PermissionDenied as e:
        ...
        self.logger.info( 'Resetting project ownership and permission to disallow delete' )
        project = vnc_lib.project_read(project.get_fq_name())
        self.set_perms(project, mode = '0775')
        vnc_lib.project_update(project)
        self.logger.info( 'Trying to delete VN in project as bob/staff ... should fail')
        try:
            vnc_lib_bob.network_ipam_delete (id = ipam2_id)
            self.logger.error(  " Succeeded in deleting IPAM which should not have been permitted" )
            testfail += 1
        except PermissionDenied as e:
            self.logger.info(  'Failed to delete IPAM ... Test passed!')
            testpass += 1

        self.logger.info( 'Resetting project ownership and permission to allow delete' )
        project = vnc_lib.project_read(project.get_fq_name())
        self.set_perms(project, mode = '0777')
        vnc_lib.project_update(project)
        self.logger.info( 'Trying to delete IPAM in project as bob/staff ... should succeed' )
        try:
            vnc_lib_bob.network_ipam_delete (id = ipam2_id)
            testpass += 1
        except PermissionDenied as e:
            self.logger.error( "Failed to delete the IPAM which should have succeeded ")
            testfail += 1

        self.logger.info( 'Trying to read IPAM as admin ... should fail')
        try:
            obj = vnc_lib.network_ipam_read(id = ipam2_id)
            self.logger.error( "Reading a IPAM succeded, which should have failed " )
            testfail += 1
        ...
        try:
            ...
            self.logger.error( "Failed to link IPAM which should have succeeded ")
            testfail += 1

        self.logger.info('')        
        self.logger.info( '########################### READ OWNER ##############################')
        self.logger.info( 'Set IPAM perms such that only owner has read/write permissions')
        ...
        try:
            ...
            self.logger.error( 'Read IPAM successfully which should have failed ')
            testfail += 1
        except PermissionDenied as e:
            self.logger.info( ' -> Failed to read IPAM ... Test passed!' )
            testpass += 1
        ...
        try:
            ...
            self.logger.info( ' -> name=%s, perms=%s' %(obj.get_fq_name(), self.print_perms(obj.get_id_perms())) )
            testpass += 1
        except PermissionDenied as e:
            self.logger.error( "Failed to read a IPAM as bob which should have passed")
            testfail += 1

        self.logger.info( '') 
        self.logger.info( '########################### UPDATE OWNER ##############################' )
        self.logger.info( 'Set IPAM perms such that only owner has write permissions' )
        ...
        try:
            ...
            self.logger.error( ' Was able to update the IPAM as Alice which should have failed ')
            testfail += 1
        except PermissionDenied as e:
            self.logger.info( ' unable to update IPAM as Alice. Test successful !')
            testpass += 1

        self.logger.info( '') 
        self.logger.info( '########################### UPDATE GROUP ##############################' )
        self.logger.info( 'Set IPAM perms such that owner/group has read/write permissions' )
        obj = vnc_lib.network_ipam_read(id = ipam.uuid)
        ...
        try:
            vnc_lib_alice.network_ipam_update(obj)
            self.logger.info( ' Was able to update the IPAM as Alice successfully')
            testpass += 1
        except PermissionDenied as e:
            testfail += 1
        ...
        self.logger.info( '') 
        self.logger.info( '########################### READ GROUP ##############################')
        self.logger.info( 'Trying to read IPAM as Alice .. should go through now' )
        try:
            obj = vnc_lib_alice.network_ipam_read(id = ipam.uuid)
            self.logger.info( ' -> name=%s, perms=%s' %(obj.get_fq_name(), self.print_perms(obj.get_id_perms())) )
            testpass += 1
        except PermissionDenied as e:
            self.logger.error( ' *** Failed to read IPAM as alice which should have gone through')
            testfail += 1
...
#        self.key_stone_clients.add_user_to_tenant(proj_name,'admin' , 'admin')
        ...
        try:
            self.key_stone_clients.create_user(user,password,email=email,tenant_name=self.inputs.project_name,enabled=True)
...
#                            api_server_host = ip, api_server_port = port)
            vnc_lib_test = VncApi(username='test', password='test123', tenant_name= proj_name, 
                            api_server_host = ip, api_server_port = port)
        ...
        try:
            ...
            self.key_stone_clients.create_user(user1,password1,email=email1,tenant_name=self.inputs.project_name,enabled=True)
            self.addCleanup( self.key_stone_clients.delete_user, user1)
            #Update projF with user test/test123 with member role
            self.key_stone_clients.add_user_to_tenant(proj_name,user1 , 'Member')
            # user=test, role=admin
            vnc_lib_test1 = VncApi(username='test1', password='test123', tenant_name= proj_name, 
                            api_server_host = ip, api_server_port = port)
        except Exception as e:
            self.logger.warn("User creation failed as exception %s"%(e))
#
        # create NetworkPolicy
        self.logger.info("Verifying that user %s/%s can create policy in project %s"%(user,password,proj_name))
        try:
            for i in range(1,4):
                policy_name = 'test_policy%s'%i
                policy = NetworkPolicy(policy_name, project)
                policy_id= vnc_lib_test.network_policy_create(policy)
                self.addCleanup( vnc_lib_test.network_policy_delete, id = policy_id )
                self.logger.info(  'Created network policy %s'%(str(policy.get_fq_name())) )
                testpass+= 1
        except PermissionDenied as e:
            self.logger.error( ' *** Failed to create policy which should have gone through')
            testfail += 1

        self.logger.info("Verifying that user %s/%s can create policy in project %s as role Member"%(user1,password1,proj_name))
        try:
            for i in range(1,4):
        ...
        try:
            ...
            vn1_fixture= self.useFixture(VNFixture(project_name= proj_name, connections= test_proj_connections1,
                     vn_name='vn_1', inputs= test_proj_inputs1, subnets= ['192.168.1.0/24']))
            testpass+= 1
        except PermissionDenied as e:
            self.logger.error( ' *** Failed to create vn which should have gone through')
            testfail += 1
        ...
        try:
            ...
            vn2_fixture= self.useFixture(VNFixture(project_name= proj_name, connections= test1_proj_connections1,
        ...
        try:
            ...
            self.logger.info("Verifying that Admin can still create policy,vn and vm....")
            # create NetworkPolicy
            self.logger.info("Verifying that user %s/%s can create policy in project %s"%(user,password,proj_name))
            try:
            ...
            self.logger.info("Verifying that %s/%s cannot create policy,vn and vm...."%(user1,password1))
    
            self.logger.info("Trying that user %s/%s to create policy in project %s as role Member"%(user1,password1,proj_name))
            try:
                ...
                self.logger.info(  'Created network policy %s'%(str(policy.get_fq_name())) )
                testfail+= 1
                self.logger.error( ' *** policy creation passed...should have failed')
            except PermissionDenied as e:
                self.logger.info( ' Failed to create policy ...')
                testpass += 1
    
            self.logger.info("Trying that user %s/%s to delete policy in project %s as role Member"%(user1,password1,proj_name))
            try:
#                policy_name = 'test1_policy_deny'
#                policy = NetworkPolicy(policy_name, project)
                vnc_lib_test1.network_policy_delete(id=policy_id)
#                self.addCleanup( vnc_lib_test1.network_policy_delete, id = policy_id )
                self.logger.info(  'Deleted network policy %s'%(str(policy_id)) )
                testfail+= 1
                self.logger.error( ' *** policy deletion passed...should have failed')
            except PermissionDenied as e:
                self.logger.info( ' Failed to delete policy ...')
                testpass += 1

            self.logger.info("Trying to Create vn in %s as user %s"%(proj_name,user1))
            try:
                vn_name = 'vn_denied'
                vn = VirtualNetwork(vn_name, project)
...
#                            api_server_host = ip, api_server_port = port)
        ...
        try:
            for i in range(1,4):
                ...
                policy_id[policy_name]= vnc_lib_test.network_policy_create(policy[policy_name])
                self.addCleanup( vnc_lib_test.network_policy_delete, id = policy_id[policy_name] )
                self.logger.info(  'Created network policy %s'%(str(policy[policy_name].get_fq_name())) )
                policy[policy_name]=vnc_lib_test.network_policy_read(id = policy_id[policy_name])
        except Exception as e:
            self.logger.error( ' *** Failed to create policy which should have gone through')
        ...
        try:
            ...
            vn_obj = self.useFixture( VNFixture(project_name= proj_name, connections= test_proj_connections1,
        ...
        net_obj = vnc_lib_test1.virtual_network_read(fq_name = vn_obj.api_vn_obj.get_fq_name())
        ...
        try:
            ...
            self.logger.error( "Failed to link Policy which should have succeeded ")
            testfail += 1
        finally:
        ...
        try:
            self.logger.info("Tryig to attach policy %s to vn %s as user %s"%(policy['test_policy1'].get_fq_name_str(), vn_obj.api_vn_obj.get_fq_name_str(),user1))
            vnc_lib_test1.virtual_network_update(net_obj)
            self.logger.error( 'Succeeded in linking Policy - should have failed... Test failed !')
            testfail += 1
        except PermissionDenied as e:
        ...
        net_obj.add_network_policy(policy['test_policy2'],VirtualNetworkPolicyType(sequence=SequenceType(major=0, minor=0)))
        try:
            vnc_lib_test1.virtual_network_update(net_obj)
            self.logger.info( 'Succeeded in linking Policy ... Test passed !')
            testpass += 1
        ...
        self.logger.info("Tryig to read policy %s  as user %s"%(policy['test_policy1'].get_fq_name_str(),user1))
        try:
            obj=vnc_lib_test1.network_policy_read(id = policy_id['test_policy1'])
        ...
        self.logger.info("Tryig to read policy %s  as user %s"%(policy['test_policy2'].get_fq_name_str(),user1))
        try:
            obj=vnc_lib_test1.network_policy_read(id = policy_id['test_policy2'])
            self.logger.info( 'Succeeded in reading Policy ... Test passed !')
            testpass += 1
        except PermissionDenied as e:
            self.logger.error( "Failed to read policy...should have succeeded ")
        ...
        self.logger.info("Tryig to read policy %s  as user %s"%(policy['test_policy3'].get_fq_name_str(),user1))
        try:
            obj=vnc_lib_test1.network_policy_read(id = policy_id['test_policy3'])
            self.logger.info( 'Succeeded in reading Policy ... should have failed !')
            testfail += 1
        except PermissionDenied as e:
            self.logger.info( "Failed to read policy...test passed ")
            testpass += 1

        self.logger.info("Tryig to read policy %s  as user %s"%(policy['test_policy3'].get_fq_name_str(),user))
        try:
            obj=vnc_lib_test.network_policy_read(id = policy_id['test_policy3'])
            self.logger.info( 'Succeeded in reading Policy ... Test passed !')
            testpass += 1
        except PermissionDenied as e:
            self.logger.error( "Failed to read policy...should have succeeded ")
...
#                            api_server_host = ip, api_server_port = port)
        ...
        net_obj = vnc_lib_test1.virtual_network_read(fq_name = vn_api_obj.get_fq_name())
        
        self.logger.info("Changing permissions on vn to disable read for others...")
        self.set_perms(net_obj, mode = '0773')
        vnc_lib_test1.virtual_network_update(net_obj)
        try:
            self.logger.info("trying to read vn %s as user %s"%(vn_api_obj.get_fq_name_str(),user1))
            obj=vnc_lib_test1.virtual_network_read(id = vn_obj.vn_id)
            self.logger.error( 'Succeeded in reading vn ... should have failed !')
            testfail += 1
        except PermissionDenied as e:
        ...
        try:
            self.logger.info("trying to read vn %s as user %s"%(vn_api_obj.get_fq_name_str(),user))
...
#@            testfail += 1
        ...
        self.logger.info("Changing permissions on vn to disable write for others...")
        ...
        try:
            ...
            self.addCleanup( vnc_lib_test.network_ipam_delete, id = network_ipam_id )
            self.logger.info(  'Created network ipam' )
        ...
        try:
            vnc_lib_test1.virtual_network_update(net_obj)
            self.logger.info( "Succeeded in writing into vn.. which should have failed! ")
            testfail += 1
        except PermissionDenied as e:
