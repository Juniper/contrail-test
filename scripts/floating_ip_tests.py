...
class TestFipCases(testtools.TestCase, ResourcedTestCase, fixtures.TestWithFixtures ):
    ...
    def test_communication_across_borrower_vm (self):
        ...
        fip_id1= fip_fixture.create_and_assoc_fip( fvn_fixture.vn_id, vn2_vm1_fixture.vm_id)
        ...
        if not result :
            self.logger.error('Test to ping between VMs %s and %s' %(vm1_name, vm2_name))
            assert result
        return True
    ...
    def test_mutual_floating_ip (self):
        ...
        fip_id1= fip_fixture1.create_and_assoc_fip(fvn1_fixture.vn_id, fvn2_vm1_fixture.vm_id)
        self.addCleanup( fip_fixture1.disassoc_and_delete_fip, fip_id1)
        assert fip_fixture1.verify_fip( fip_id1, fvn2_vm1_fixture, fvn1_fixture )
        ...
        fip_id2= fip_fixture2.create_and_assoc_fip(fvn2_fixture.vn_id, fvn1_vm1_fixture.vm_id)
    ...
    def test_exhust_Fip_pool_and_release_fip (self):
        ...
        # 2. Test with all the available floating IP pool used/ allocated to diffrent VMs
        result= True
        fip_pool_name1= 'some-pool1'
        fvn1_fixture= self.res.fvn1_fixture
        fvn2_fixture= self.res.fvn2_fixture
        ...
        fip_id1= fip_fixture1.create_and_assoc_fip(fvn3_fixture.vn_id, vn1_vm1_fixture.vm_id)
        assert fip_fixture1.verify_fip( fip_id1, vn1_vm1_fixture, fvn3_fixture )
        fip_id2= fip_fixture1.create_and_assoc_fip(fvn3_fixture.vn_id, vn1_vm2_fixture.vm_id)
        assert fip_fixture1.verify_fip( fip_id2, vn1_vm2_fixture, fvn3_fixture )
        fip_id3= fip_fixture1.create_and_assoc_fip(fvn3_fixture.vn_id, vn2_vm1_fixture.vm_id)
        assert fip_fixture1.verify_fip( fip_id3, vn2_vm1_fixture, fvn3_fixture )
        fip_id4= fip_fixture1.create_and_assoc_fip(fvn3_fixture.vn_id, vn2_vm2_fixture.vm_id)
        assert fip_fixture1.verify_fip( fip_id4, vn2_vm2_fixture, fvn3_fixture )

        self.logger.info('Here Floating IP pool is alreadu exhausted. Should not allow to add futher.Quantum Exception expected')
        fip_id5= fip_fixture1.create_and_assoc_fip(fvn3_fixture.vn_id, fvn1_vm1_fixture.vm_id)
        if fip_id5 != None:
            self.logger.error('FIP should not get created/asscocited as pool is already exhusted')
            result = result and False
        self.logger.info('Releasing one FIP.')
        fip_fixture1.disassoc_and_delete_fip(fip_id1) 

        if not vn1_vm1_fixture.ping_to_ip(fvn3_vm1_fixture.vm_ip):
            self.logger.info( "Here ping should fail from VM as FIP is removed ")
        else :
            self.logger.error("Ping should fail. But ping is successful even after removal of FIP from VM %s" %(vn1_vm1_name))
            result = result and False

        self.logger.info('Now FIP should get created and asscociated with %s' %(fvn1_vm1_name))
        fip_id5= fip_fixture1.create_and_assoc_fip(fvn3_fixture.vn_id, fvn1_vm1_fixture.vm_id)
        assert fip_fixture1.verify_fip( fip_id5, fvn1_vm1_fixture, fvn3_fixture )
        ...
        if not result :
            self.logger.error('Test Failed. Exhaustion of FIP pool test failed')
            assert result
        return result 
    ...
    def test_extend_fip_pool_runtime (self):    
        ...
        # 2. When the allocating VN has multiple subnet, borrower VM should able to communicate all the subnet
        result= True
        fip_pool_name1= 'some-pool1'
        ...
        self.logger.info('Here Floating IP pool is already exhausted. Should not allow to add futher.Quantum Exception expected')
        fip_id5= fip_fixture1.create_and_assoc_fip(fvn3_fixture.vn_id, fvn1_vm1_fixture.vm_id)
        if fip_id5 != None:
            self.logger.error('FIP should not get created/asscocited as pool is already exhusted')
            result = result and False
        ...
        additional_vm_fixture=self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections, vn_obj= fvn3_fixture.obj, vm_name= 'additional_vm'))
        ...
        fip_fixture1.disassoc_and_delete_fip(fip_id5)
        if not result :
            self.logger.error('Test Failed. Extension of FIP pool and communication multiple subnet test fail')
            assert result
        return result
    ...
    def test_service_restart_with_fip (self):
        ...
        #fip_fixture2.disassoc_and_delete_fip(fip_id2)
        if not result :
            self.logger.error('Test Failed for restart of agent and control node with floating IP')
            assert result
        return result
    ...
    def test_fip_with_traffic (self):
        ...
        fip_fixture1= self.useFixture(FloatingIPFixture( project_name= self.inputs.project_name, inputs = self.inputs,connections= self.connections, pool_name = fip_pool_name1, vn_id= fvn1_fixture.vn_id ))
        assert fip_fixture1.verify_on_setup()

        fip_id1= fip_fixture1.create_and_assoc_fip(fvn1_fixture.vn_id, vn1_vm1_traffic_fixture.vm_id)
        self.addCleanup( fip_fixture1.disassoc_and_delete_fip, fip_id1)
        assert fip_fixture1.verify_fip( fip_id1, vn1_vm1_traffic_fixture, fvn1_fixture)
    ...
    def test_removal_of_fip_with_traffic (self):
        ...
        assert(traffic_stats['status'] == True), err_msg
        self.logger.info ("-"*80)
        self.logger.info ("Removing/disassociating FIP here. Now Traffic should stopped")
        fip_fixture1.disassoc_and_delete_fip(fip_id1)
        sleep(2)

        # Poll live traffic
        traffic_stats= {}
        self.logger.info ("Traffic expected to stop flowing as FIP removed.Disruption expected.")
        for proto in traffic_proto_l:
            traffic_stats= traffic_obj[proto].getLiveTrafficStats()
        ...
        if flow_rec1 is not None:
            match = inspect_h1.match_item_in_flowrecord (flow_rec1,'short_flow','yes')
            if match is False :
                self.logger.error('Test Failed. After removal of FIP flow type should be short_flow. Flow details %s' %(flow_rec1))
                result = result and False
            match = inspect_h1.match_item_in_flowrecord (flow_rec1,'dst_vn','__UNKNOWN__')
            if match is False :
                self.logger.error('Test Failed. After removal of FIP destination VN should be unkwown. Flow details %s' %(flow_rec1))
                result = result and False
        # Verify Egress Traffic
        self.logger.info('Verifying Egress Flow Records')
        # Check VMs are in same agent or not. Need to compute source vrf accordingly 
        ...
        flow_rec2= inspect_h1.get_vna_fetchflowrecord(vrf=source_vrf,sip=fvn1_vm1_traffic_fixture.vm_ip,dip=fip_fixture1.fip[fip_id1],sport=dpi,dport=udp_src,protocol='17')
        if flow_rec2 is not None:
            self.logger.error('Test Failed. Egress Flow records entry should be removed after removal of FIP. It still exists.')
            self.logger.error('Flow record entry: %s' %(flow_rec2))
            result = result and False

        else:
            self.logger.info('Verification successful. Egress flow records removed')

        flow_rec3= inspect_h1.get_vna_fetchflowrecord(vrf=source_vrf,sip=fvn1_vm1_traffic_fixture.vm_ip,dip=vn1_vm1_traffic_fixture.vm_ip,sport=dpi,dport=udp_src,protocol='17')
        if flow_rec3 is not None:
            match = inspect_h1.match_item_in_flowrecord (flow_rec3,'short_flow','yes')
            if match is False :
                self.logger.error('Test Failed. After removal of FIP flow type should be short_flow. Flow details %s' %(flow_rec3))
                result = result and False
            match = inspect_h1.match_item_in_flowrecord (flow_rec3,'dst_vn','__UNKNOWN__')
            if match is False :                
                self.logger.error('Test Failed. After removal of FIP destination VN should be unkwown. Flow details %s' %(flow_rec3))
                result = result and False
        ...
        if not result :
            self.logger.error('Test Failed. Traffic not stopped/flow still exists after FIP removal')
            assert result
        return result
    ...
    def test_traffic_with_control_node_switchover(self):
        ...
        self.logger.info('Active control node from the Agent %s is %s' %(vn1_vm1_traffic_fixture.vm_node_ip, active_controller))
        ...
        total_streams= {}; total_streams['icmp']= 1; dpi= 9100; proto= 'icmp'
        for proto in traffic_proto_l:
            traffic_obj[proto]= {}; startStatus[proto]= {}
            traffic_obj[proto]= self.useFixture(traffic_tests.trafficTestFixture(self.connections))
            startStatus[proto]= traffic_obj[proto].startTraffic(num_streams=total_streams[proto], start_port=dpi, \
        ...
        self.logger.info('Active control node from the Agent %s is %s' %(vn1_vm1_traffic_fixture.vm_node_ip, new_active_controller))
        ...
        # Verify Ingress Traffic
        self.logger.info('Verifying Ingress Flow Record')
        flow_rec1= inspect_h1.get_vna_fetchflowrecord(vrf=vn1_vm1_traffic_fixture.agent_vrf_objs['vrf_list'][0]['ucindex'],sip=vn1_vm1_traffic_fixture.vm_ip,dip=fvn1_vm1_traffic_fixture.vm_ip,sport='0',dport='0',protocol='1')
        ...
        if vn1_vm1_traffic_fixture.vm_node_ip != fvn1_vm1_traffic_fixture.vm_node_ip:
            ...
            source_vrf = vrf_list['vrf_list'][0]['ucindex']
        self.logger.info('Verifying Egress Flow Records')
        flow_rec2= inspect_h1.get_vna_fetchflowrecord(vrf=source_vrf,sip=fvn1_vm1_traffic_fixture.vm_ip,dip=fip_fixture1.fip[fip_id1],sport='0',dport='0',protocol='1')
        if flow_rec2 is not None:
            self.logger.info('Verifying NAT in flow records')
            match = inspect_h1.match_item_in_flowrecord (flow_rec2,'nat','enabled')
            if match is False :
        ...
        self.logger.info('Checking the BGP peering from new active controler  %s' %(new_active_controller))
    ...
    def test_fip_in_uve (self):
        ...
        result= fip_fixture.verify_fip_in_uve(fip_fixture.fip[fip_id],vn1_vm1_fixture,fvn_fixture)
        #fip_fixture.disassoc_and_delete_fip(fip_id)
        if not result :
    ...
    def test_vm_restart_with_fip (self):
        ...
        fip_id1= fip_fixture1.create_and_assoc_fip(fvn1_fixture.vn_id, vn1_vm1_fixture.vm_id)
        self.addCleanup( fip_fixture1.disassoc_and_delete_fip, fip_id1)
    ...
    def test_vn_info_in_agent_with_fip (self):
        ...
        self.logger.info('Checking the VN informatiopn of FIP allocating VN is already present in agent of borrower VN or not')
        ...
        if vn_fq_name is None:
            self.logger.info('VN info for %s is not present in agent %s' %(fvn1_name,vn1_vm1_fixture.vm_node_ip))
        else:
            self.logger.error('VN info for %s is already present in agent %s. Setup problem. Existing the test here' %(fvn1_name,vn1_vm1_fixture.vm_node_ip))       
            result= result & False
            assert result
        ...
        vn_fq_name= inspect_h1.get_vna_vn(vn_name= fvn1_name)
        if vn_fq_name is None:
            self.logger.info('FIP allocating VN  %s is not present in agent  %s' %(fvn1_name,vn1_vm1_fixture.vm_node_ip))
            result= result & False
            fip_fixture1.disassoc_and_delete_fip(fip_id1)
            assert result
        else:
            self.logger.info('VN info for %s is present in agent %s.' %(fvn1_name,vn1_vm1_fixture.vm_node_ip))

        # Disasscotite the fixture here
        self.logger.info('Dis associating the FIP %s from VM  %s' %(fip_fixture1.fip[fip_id1],vn1_vm1_name)) 
        fip_fixture1.disassoc_and_delete_fip(fip_id1)
        
        vn_fq_name= inspect_h1.get_vna_vn(vn_name= fvn1_name)

        if vn_fq_name is None:
            self.logger.info('VN info for %s is no more  present in agent %s' %(fvn1_name,vn1_vm1_fixture.vm_node_ip))
        else:
            self.logger.error('VN info for %s is still present in agent %s after removal of FIP' %(fvn1_name,vn1_vm1_fixture.vm_node_ip))
            result= result & False
        if not result :
    ...
    def test_fip_with_policy (self):
        ...
        self.logger.info('Apply policy between VN %s and %s' %(fvn1_name,fvn2_name))
        fvn1_fixture.bind_policies([policy1_fixture.policy_fq_name], fvn1_fixture.vn_id)
        self.addCleanup(fvn1_fixture.unbind_policies , fvn1_fixture.vn_id ,[policy1_fixture.policy_fq_name])
        fvn2_fixture.bind_policies([policy2_fixture.policy_fq_name], fvn2_fixture.vn_id)
        self.addCleanup(fvn2_fixture.unbind_policies , fvn2_fixture.vn_id ,[policy2_fixture.policy_fq_name])
        ...
        fip_id2= fip_fixture1.create_and_assoc_fip(fvn1_fixture.vn_id, vn2_vm1_fixture.vm_id)
        ...
        self.logger.info('Ping from from VM %s to Other VM in different network with FIP %s ' %(vn1_vm1_name,fip_fixture1.fip[fip_id2]))
        if not vn1_vm1_fixture.ping_with_certainty(fip_fixture1.fip[fip_id2]):
            result = result and False

        self.logger.info('Ping from from VM %s to IP  %s in VN %s' %(vn1_vm1_name,fvn2_vm1_fixture.vm_ip, fvn2_vm1_name))
        if not vn1_vm1_fixture.ping_with_certainty(fvn2_vm1_fixture.vm_ip):
            result = result and False

        # Unbind
        fvn1_fixture.unbind_policies (fvn1_fixture.vn_id ,[policy1_fixture.policy_fq_name])
        fvn2_fixture.unbind_policies (fvn2_fixture.vn_id ,[policy2_fixture.policy_fq_name])

        sleep (5)

        if not vn1_vm1_fixture.ping_to_ip(fvn2_vm1_fixture.vm_ip):
            self.logger.info( "Here ping should fail from VM as Policy is removed from the VN")
        else :
            self.logger.error("Ping should fail. But ping is successful even after removal of policy")
            result = result and False
    ...
    def test_fip_pool_shared_across_project(self):
        ...
        self.demo_proj_inputs1=self.useFixture(ContrailTestInit( self.ini_file, stack_user='admin', stack_password='contrail123', project_fq_name=['default-domain','demo']))
        self.demo_proj_connections1= ContrailConnections(self.demo_proj_inputs1)
        ...
        assert fvn_fixture.verify_on_setup()
        fvn_fixture1= self.useFixture(VNFixture(project_name= self.inputs.project_name, connections= self.connections,
                     vn_name=fvn_name, inputs= self.inputs, subnets= fvn_subnets))
        ...
        assert vn1_fixture.verify_on_setup()
        vn2_fixture= self.useFixture(VNFixture(project_name= 'demo', connections= self.demo_proj_connections1,
        ...
        vm2_fixture= self.useFixture(VMFixture(project_name= 'demo', connections= self.demo_proj_connections1,
        ...
        self.logger.info('Adding project demo to FIP pool %s' %(fip_pool_name))
        project_obj = fip_fixture.assoc_project(fip_fixture, 'demo')

        # Asscociating FIP to VM under demo project 
        self.logger.info('Allocating FIP to VM %s in Demo project from VN %s under admin project' %(vm2_name,fvn_name))
        fip_id= fip_fixture.create_and_assoc_fip( fvn_fixture.vn_id, vm2_fixture.vm_id, project_obj)
        assert fip_fixture.verify_fip( fip_id, vm2_fixture, fvn_fixture )

        self.logger.info('FIP  pool is exhausted now. Trying to add FIP to VM under admin project. Should FAIL')
        fip_id1= fip_fixture.create_and_assoc_fip(fvn_fixture.vn_id, vm1_fixture.vm_id)
        if fip_id1 != None:
            self.logger.error('FIP should not get created/asscocited as pool is already exhusted')
            result = result and False
        self.logger.info('Releasing FIP to VM %s in Demo project from VN %s under admin project' %(vm2_name,fvn_name))
        fip_fixture.disassoc_and_delete_fip(fip_id)

        if result is True:
            self.logger.info('Allocating FIP to VM %s in admin  project from VN %s under admin project' %(vm1_name,fvn_name))
            fip_id2= fip_fixture.create_and_assoc_fip( fvn_fixture.vn_id, vm1_fixture.vm_id)
        ...
        self.logger.info('Removing project demo to FIP pool %s' %(fip_pool_name))
        project_obj = fip_fixture.deassoc_project(fip_fixture, 'demo')

        if not result :
            self.logger.error('Test Failed:Verify FIP Pool is shared accorss diffrent projects')
            assert result
        return True
    ...
    def test_communication_across__diff_proj(self):
        ...
        self.logger.info('Allocating FIP to VM %s in project %s from VN %s in project %s ' %(vm2_fixture.vm_name, projects[1], vn_names[0], projects[0]))
        fip_id= fip_fixture.create_and_assoc_fip( vn1_fixture.vn_id, vm2_fixture.vm_id, project_obj)
    ...
    def test_traffic_to_fip(self):
        ...
        for proto in traffic_proto_l:
            ...
            self.logger.info ("Status of start traffic : %s, %s, %s" %(proto, fvn1_vm1_traffic_fixture.vm_ip, startStatus[proto]))
            if startStatus[proto]['status'] != True:  result= False
        self.logger.info ("-"*80)
        ...
        sleep(5)
        for proto in traffic_proto_l:
            flow_record_data[proto] = self.analytics_obj.ops_inspect[self.inputs.collector_ips[0]].post_query('FlowRecordTable',dir=0,start_time=start_time,end_time='now',select_fields=['sourcevn', 'sourceip', 'destvn', 'destip','setup_time','teardown_time','agg-packets', 'agg-bytes','protocol'],where_clause=query[proto])
            flow_series_data[proto] = self.analytics_obj.ops_inspect[self.inputs.collector_ips[0]].post_query('FlowSeriesTable',dir=0,start_time=start_time,end_time='now',select_fields=['sourcevn', 'sourceip', 'destvn', 'destip','sum(packets)','flow_count','sum(bytes)','sum(bytes)'],where_clause= query[proto])
            msg = proto + " Flow count info is not matching with opserver flow series record"
            #self.assertEqual(flow_series_data[proto][0]['flow_count'],total_streams[proto],msg)
            assert(flow_series_data[proto][0]['flow_count'] == total_streams[proto]),msg
        ...
        for proto in traffic_proto_l:
            self.logger.info (" verify %s traffic status against to Analytics flow series data" %(proto))
            msg = proto + " Traffic Stats is not matching with opServer flow series data"
        ...
        for i in xrange(len(flow_series_data[proto])-1): 
            if flow_series_data[proto][i]['destip'] == fip_fixture1.fip[fip_id1]:    
                ...
                assert(flow_series_data[proto][i]['sum(packets)'] >= traffic_stats[proto]['total_pkt_sent']),msg
        ...
        for proto in traffic_proto_l:
            ...
            flow_series_data[proto] = self.analytics_obj.ops_inspect[self.inputs.collector_ips[0]].post_query('FlowSeriesTable',dir=0,start_time='now',end_time='now',select_fields=['sourcevn', 'sourceip', 'destvn', 'destip','sum(packets)','flow_count','sum(bytes)','sum(bytes)'],where_clause= query[proto])
    ...
    def test_ping_to_fip_using_diag (self):
        ...
        self.logger.info("Pinging using diag introspect from IP %s to IP %s" %(fvn1_vm1_fixture.vm_ip,fip_fixture1.fip[fip_id1]))
        result=inspect_h1.get_vna_verify_diag_ping(src_ip=fvn1_vm1_fixture.vm_ip,dst_ip=fip_fixture1.fip[fip_id1],vrf=fvn1_vm1_fixture.agent_vrf_objs['vrf_list'][0]['name'],proto='17')
        if not result :
            self.logger.error('Test to ping uding diag between VMs %s and %s' %(fvn1_vm1_fixture.vm_ip, fip_fixture1.fip[fip_id1]))
            assert result
        return result
    ...
    def test_tcp_transfer_from_fip_vm(self):
        ...
        policy_fixture = self.useFixture(PolicyFixture(policy_name= policy_name,
...
#                vn_obj= fvn_fixture.obj, vm_name= fvm_name))
        ...
        assert fvm_fixture.verify_on_setup()
        fvm_fixture1= self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections,
        ...
        assert fip_fixture.verify_fip( fip_id, vm1_fixture, fvn_fixture )
        fip_id1= fip_fixture.create_and_assoc_fip( fvn_fixture1.vn_id, vm1_fixture.vm_id)
        ...
        # Check the floating IP provider VNs should commmunicate with each other
        self.logger.info('Ping should fail here. %s and %s should not able to communicate with each oether' %(fvm_name1, fvm_name))
        if fvm_fixture1.ping_to_ip( fvm_fixture.vm_ip ):
            result = result and False
        # Check after disscocition of floating ip communication should and only should stop from that network
        fip_fixture.disassoc_and_delete_fip(fip_id)
        self.logger.info('Ping should fail here as floating IP pool is already released')
        ...
        fip_fixture1.disassoc_and_delete_fip(fip_id1)
        if not result :
            self.logger.error('Test to check multiple floating ip for single VM has failed')
            assert result
        return True
    #end test_floating_ip
