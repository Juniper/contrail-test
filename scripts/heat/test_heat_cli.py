...
class TestHeatCli(BaseHeatTest):
	...
	def setUpClass(cls):
		...
		cls.heat_client = heat_client.HeatClient(cls.heat_url, token = cls.auth_token) 
		cls.heat_client = cls.heat_client.get_client() 
		cls.heat_cli = heat_client.HeatCli(cls.heat_client,cls.inputs, cls.auth_token, cls.auth_url)
	...
	def test_heat_stacks_create(self):
		...
		self.verify_all_vms_in_stack(self.heat_client,self.heat_cli.stack_name)
