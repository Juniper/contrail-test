...
class StaticRouteTableBase(BaseNeutronTest, test_v1.BaseTestCase_v1):
    ...
    def delete_int_route_table(self):
        self.static_table_handle.delete_interface_route_table(self.intf_table_to_right_obj.uuid)
        self.static_table_handle.delete_interface_route_table(self.intf_table_to_left_obj.uuid)

    def add_network_table_to_vn(self, vn1_fix, vn2_fix): 

        if str(self.vm1_fixture.vm_ips[0])[:-1] == str(self.right_vm_fixture.vm_ip)[:-1]:
            self.nw_handle_to_left =self.static_table_handle.create_route_table(prefixes = [str(self.vn1_fixture.get_cidrs()[0])],\
            ...
            self.static_table_handle.bind_network_route_table_to_vn(vn_uuid = vn2_fix.uuid, nw_route_table_obj=self.nw_handle_to_left)
        else:
            self.nw_handle_to_right =self.static_table_handle.create_route_table(prefixes = [str(self.vn2_fixture.get_cidrs()[0])],\
                                            name="network_table_left_to_right", \
                                            next_hop = self.vm1_fixture.vm_ips[0], \
                                            parent_obj = self.project.project_obj, \
                                            next_hop_type='ip-address', \
            ...
            self.static_table_handle.bind_network_route_table_to_vn(vn_uuid = vn1_fix.uuid, nw_route_table_obj=self.nw_handle_to_right)
    ...
    def del_nw_route_table(self):
        ...
        self.static_table_handle.delete_network_route_table(self.nw_handle_to_left.uuid)
    ...
    def bind_network_table(self, src_vn_fix, dst_vn_fix):
        self.static_table_handle.bind_network_route_table_to_vn(vn_uuid = src_vn_fix.uuid, nw_route_table_obj=self.nw_handle_to_right)
        self.static_table_handle.bind_network_route_table_to_vn(vn_uuid = dst_vn_fix.uuid, nw_route_table_obj=self.nw_handle_to_left)
    ...
    def test_interop_with_neutron_router(self):
        ...
        assert self.left_vm_fixture.ping_with_certainty(self.right_vm_fixture.vm_ip)
