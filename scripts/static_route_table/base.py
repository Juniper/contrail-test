...
class StaticRouteTableBase(test_v1.BaseTestCase_v1):
    ...
    def add_interface_route_table(self, src_vn_fix, dst_vn_fix, port1_fix):
        ...
        vm_uuid = str(port1_fix.get_vmi_ids()[id_entry])
        port1_fix.provision_static_route(prefix = str(dst_vn_fix.get_cidrs()[0]), tenant_name = self.inputs.project_name, virtual_machine_interface_id = vm_uuid)

        id_entry = self.inputs.project_fq_name[0] + ':' + self.inputs.project_fq_name[1] + ':' + self.vn2_name
        vm_uuid = str(port1_fix.get_vmi_ids()[id_entry])
        port1_fix.provision_static_route(prefix = str(src_vn_fix.get_cidrs()[0]), tenant_name = self.inputs.project_name, virtual_machine_interface_id = vm_uuid)

    def add_network_table_to_vn(self, vn1_fix, vn2_fix): 

        if str(self.vm1_fixture.vm_ips[0])[:-1] == str(self.right_vm_fixture.vm_ip)[:-1]:
            vn1_fix.provision_network_table(prefix = self.left_vm_fixture.vm_ip + '/32',\
                                            name="network_table_left_to_right", \
                                            next_hop = self.vm1_fixture.vm_ips[0], \
                                            parent_obj = self.project.project_obj, \
                                            vnc_handle=self.vnc_lib, \
            ...
            vn1_fix.provision_network_table(prefix = self.left_vm_fixture.vm_ip + '/32',\
                                            ...
                                            vn_uuid = vn1_fix.uuid)
        if str(self.vm1_fixture.vm_ips[0])[:-1] == str(self.right_vm_fixture.vm_ip)[:-1]:
            vn2_fix.provision_network_table(prefix = self.left_vm_fixture.vm_ip + '/32',\
    ...
    def check_route_in_agent(self, expected_next_hops):
        ...
        if not next_hops:
            result = False
            assert result, 'Covering route not found in the Agent'
        else:
            self.logger.info('Covering route found in the Agent')
        import pdb;pdb.set_trace()
        if (len(next_hops) != expected_next_hops):
            result = False
