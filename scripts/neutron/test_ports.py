...
class TestPorts(BaseNeutronTest):
    ...
    def test_zombie_tap_interface(self):
        '''Test Zombie Tap-interface
            ...
            delete the port
            check wheather still tap interface present or not''' 
        result = True
        vn1_name = get_random_name('vn1')
        vm1_name = get_random_name('vm1')
        ...
        '''if not vm_tap_intf:
            self.logger.error('Tap interface not present for %s'  %vm1_fixture.vm_name)
            result = result and False
            assert result,
        self.delete_port(port1_obj['id'])
        vm_tap_intf=vm1_fixture.get_tap_intf_of_vm()
        if vm_tap_intf:
            self.logger.info('Tap interface still present for vm %s'%vm1_fixture.vm_name)
            return False'''
        return True
