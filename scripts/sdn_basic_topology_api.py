...
class sdn_basic_config_api ():
    def __init__(self, domain= 'default-domain', project= 'admin', username= None, password= None):
        ...
        self.vn_policy=  {'vnet0': ['policy0'], 'vnet1': ['policy1'], 'vnet2': [], 'vnet3': ['policy2'], 'vnet4': ['policy3']}
        ##
        # Define VM's
        ...
        self.rules['policy0']= [
            PolicyRuleType(direction='>', protocol='icmp', dst_addresses= [AddressType(virtual_network = 'any')],src_addresses=[AddressType(virtual_network = 'local')], dst_ports=[PortType(80, 80)], action_list = ActionListType(simple_action='deny'), src_ports=[PortType(-1, -1)]) ,
            PolicyRuleType(direction='>', protocol='tcp', dst_addresses= [AddressType(virtual_network = 'any')],src_addresses=[AddressType(virtual_network = 'local')], dst_ports=[PortType(80, 80)], action_list = ActionListType(simple_action='deny'), src_ports=[PortType(-1, -1)])
                               ]

        self.rules['policy1']= [
            PolicyRuleType(direction='>', protocol='icmp', dst_addresses= [AddressType(virtual_network = 'any')],src_addresses=[AddressType(virtual_network = 'local')], dst_ports=[PortType(80, 80)], action_list = ActionListType(simple_action='pass'), src_ports=[PortType(-1, -1)]) , 
            PolicyRuleType(direction='>', protocol='tcp', dst_addresses= [AddressType(virtual_network = 'any')],src_addresses=[AddressType(virtual_network = 'local')], dst_ports=[PortType(80, 80)], action_list = ActionListType(simple_action='pass'), src_ports=[PortType(-1, -1)])
                               ]

        self.rules['policy2']= []

        self.rules['policy3']= [
            PolicyRuleType(direction='<>', protocol='udp', dst_addresses= [AddressType(virtual_network = 'any')],src_addresses=[AddressType(virtual_network = 'local')], dst_ports=[PortType(80, 80)], action_list = ActionListType(simple_action='pass'), src_ports=[PortType(-1, -1)]) ,
            PolicyRuleType(direction='>', protocol='any', dst_addresses= [AddressType(virtual_network = 'any')],src_addresses=[AddressType(virtual_network = 'local')], dst_ports=[PortType(80, 80)], action_list = ActionListType(simple_action='pass'), src_ports=[PortType(80, 100)])
                               ]
        # end __init__
...
class sdn_multiple_vn_single_policy_config_api ():
    def __init__(self, domain= 'default-domain', project= 'admin', username= None, password= None):
        ...
        self.vn_policy=  {'vnet0': ['policy0'], 'vnet1': ['policy0'], 'vnet2': ['policy0'], 'vnet3': ['policy0'], 'vnet4': ['policy0']}
        ##
        # Define VM's
        # VM distribution on available compute nodes is handled by nova scheduler or contrail vm naming scheme
        self.vn_of_vm=  {'vmc0': 'vnet0', 'vmc1': 'vnet1', 'vmc2': 'vnet2', 'vmc3': 'vnet3', 'vmc4': 'vnet4'}
        ##
        # Define network policy rules
        self.rules= {}
        self.rules['policy0']= [
            PolicyRuleType(direction='<>', protocol='icmp', dst_addresses= [AddressType(virtual_network = 'any')],src_addresses=[AddressType(virtual_network = 'local')], dst_ports=[PortType(-1, -1)], action_list = ActionListType(simple_action='pass'), src_ports=[PortType(-1, -1)]) ,
                               ]
