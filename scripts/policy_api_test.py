...
#    @classmethod
    ...
    def test_create_api_policy(self):
        ...
        if not vb:
            self.logger.error( "VN %s object is not present is API server" %vn1_name)
        ...
        self.logger.info( "policy %s is created with rules using API Server" %policy_name)
        ...
        if not pol:
            self.logger.error( "policy %s object is not present in API server" %policy_name)
        ...
        if not vn_in_quantum:
            self.logger.info("VN %s is not present in the quantum server" %vn1_name)
            self.assertIsNotNone(vn_in_quantum, "VN is not present on quantum server")

        # verify vn_policy data on quantum after association
        #vn_assoc_policy_quantum = vn_in_quantum['network']['contrail:policys'][0][2]
        vn_assoc_policy_quantum = self.get_current_policies_bound(self.vnc_lib, vn_id)
        vn_assoc_policy_quantum = str(vn_assoc_policy_quantum)
        self.logger.info("verifying vn_policy data on the quantum server for policy %s and vn %s" %(policy_name, vn1_name))
        self.assertEqual(vn_assoc_policy_quantum, policy_obj1.name, 'associaton policy data on vn is missing from quantum')

        policy_in_quantum = self.quantum_fixture.get_policy_if_present(policy_name=pol.name, project_name=self.inputs.project_name)
        if not policy_in_quantum:
            self.logger.info("policy %s is not present in the quantum server" %pol.name)
            self.assertIsNotNone(policy_in_quantum, "policy is not present on quantum server")
	...
	assert self.verify_policy_in_api_quantum_server(pol, policy_in_quantum)
        ...
        if vn_delete:
            self.logger.info("VN %s is still present on the API server" %vn1_name)
            self.assertIsNone(vn_delete, "VN delete failed")
        self.logger.info("VN %s is successfully deleted using API server" %vn1_name)

        # delete network policy
        np_delete = self.vnc_lib.network_policy_delete(id=str(policy_rsp))
        if np_delete:
            self.logger.info("policy %s is still present on the API server" %policy_name)
            self.assertIsNone(np_delete, "policy delete failed")
        self.logger.info("policy %s is successfully deleted using API server" %policy_name)
        return True
    ...
    def test_associate_disassociate_api_policy(self):
        ...
        vn_obj.add_network_policy(policy_obj,
            vnc_api.VirtualNetworkPolicyType(sequence=vnc_api.SequenceType(major=0, minor=0)))
        self.logger.info( "trying to associate policy %s to vn %s" %(policy_name, vn1_name))
        vn_update_rsp = self.vnc_lib.virtual_network_update(vn_obj)

        if vn_update_rsp == None :
            self.logger.error( "policy %s assocation with vn %s failed" %(policy_name, vn1_name))
            self.assertIsNone(vn_update_rsp, "policy association failed")
        self.logger.info( "policy %s assocation with vn %s is successful" %(policy_name, vn1_name))
       
        # delete policy with reference
        try:
            self.vnc_lib.network_policy_delete(id=str(policy_rsp))
            deleted_policy = []
            self.logger.error("policy %s got deleted even though we had association with VN %s" %(policy_name, vn1_name))
            self.assertIsNotNone(delete_policy, "delete policy with reference test failed")
        except RefsExistError as e:
             self.logger.info( 'RefsExistError:Check passed that the policy cannot be deleted when the VN is associated to it.')
		
	# dis-associate a policy from vn 
	vn_update_rsp = None
	vn_obj.del_network_policy(policy_obj)
	self.logger.info( "trying to dis-associate policy %s from vn %s" %(policy_name, vn1_name))
	vn_update_rsp = self.vnc_lib.virtual_network_update(vn_obj)

        if vn_update_rsp == None :
            self.logger.error( "policy %s dis-assocation with vn %s failed" %(policy_name, vn1_name))
            self.assertIsNone(vn_update_rsp, "policy dis-association failed")
        self.logger.info( "policy %s dis-assocation with vn %s is successful" %(policy_name, vn1_name))
    ...
    def test_policy_with_local_keyword_across_multiple_vn(self):
        ...
        for i in range(num_rules):
            ...
            matching_rule_action[proto]= topo.rules[policy][i].action_list.simple_action
        ...
        for vm in topo.vmc_list:
            if vm != dst_vm:
                ...
                self.logger.info ("Verify ping to vm %s from vm %s" %(dst_vm, vm))
                ...
                if ret != True : result= False; msg.extend([result_msg, policy])
    ...
    def test_policy_rules_scaling_with_ping_api(self):
        ...
        policy_objs_list = policy_test_utils._create_n_policy_n_rules(self, number_of_policy, valid_rules, number_of_dummy_rules,option='api')
        self.logger.info('Create VN and associate %d policy' %(number_of_policy))

        vn_fixture= {} ;vm_fixture = {}
        ref_tuple = []
        ...
        for vn_name in vn_names:
            vn_fixture[vn_name]= self.useFixture( VirtualNetworkTestFixtureGen(self.vnc_lib, virtual_network_name = vn_name,
            ...
            vn_read = self.vnc_lib.virtual_network_read(id=str(vn_fixture[vn_name]._obj.uuid))
            if not vn_read:
        ...
        for vm_name in vm_names:
            vn_read= self.vnc_lib.virtual_network_read(id=str(vn_fixture[vn_of_vm[vm_name]]._obj.uuid))
            vn_quantum_obj= self.quantum_fixture.get_vn_obj_if_present(vn_read.name)
            ...
            vm_fixture[vm_name]= self.useFixture(VMFixture(project_name= self.inputs.project_name,
            ...
            vm_node_ip= self.inputs.host_data[self.nova_fixture.get_nova_host_of_vm(vm_fixture[vm_name].vm_obj)]['host_ip']
            ...
            if vm_verify_out == False:
                m= "%s - vm verify in agent after launch failed" %vm_node_ip; err_msg.append(m)
                return {'result':vm_verify_out, 'msg': err_msg}
        for vm_name in vm_names:
            out= self.nova_fixture.wait_till_vm_is_up( vm_fixture[vm_name].vm_obj )
            if out == False: return {'result':out, 'msg':"VM failed to come up"}
            else: vm_fixture[vm_name].install_pkg("Traffic")
        ...
        ret= vm_fixture[vm_names[0]].ping_with_certainty( dst_vm_ip, expectation=True )
        result_msg= "vm ping test result to vm %s is: %s" %(dst_vm, ret)
        self.logger.info (result_msg)
        if ret != True : result= False; msg.extend(["ping test failure with scaled policy and rules:", result_msg])
        self.assertEqual(result, True, msg)
        return True
    ...
    def _create_policy(self, policy_name, rules_list):
        ...
        for rule_dict in rules_list:
            ...
            if 'src_ports' in rule_dict : 
                if  type(rule_dict['src_ports']) is tuple or type(rule_dict['src_ports']) is list :
                    new_rule['src_ports']= [vnc_api.PortType(rule_dict['src_ports'][0], rule_dict['src_ports'][1])]
                elif rule_dict['src_ports'] == 'any':
                    new_rule['src_ports']= [vnc_api.PortType(-1, -1)]
                else :
                    self.logger.error( "Error in Source ports arguments, should be (Start port, end port) or any " )
                    return None             
            # Format Dest ports
            if 'dst_ports' in rule_dict:
                if 'dst_ports' in rule_dict and type(rule_dict['dst_ports']) is tuple or type(rule_dict['dst_ports']) is list :
                    new_rule['dst_ports']= [vnc_api.PortType( rule_dict['dst_ports'][0], rule_dict['dst_ports'][1])]
                elif rule_dict['dst_ports'] == 'any':
                    new_rule['dst_ports']= [vnc_api.PortType(-1, -1)]
                else :
                    self.logger.error( "Error in Destination ports arguments, should be (Start port, end port) or any " )
                    return None    
            
            source_vn= ':'.join(self.project_fq_name) + ':' +  new_rule[ 'source_network' ]
            dest_vn= ':'.join(self.project_fq_name) + ':' +  new_rule[ 'dest_network' ]
            # handle 'any' network case
            if rule_dict['source_network'] == 'any': source_vn= 'any'
            if rule_dict['dest_network'] == 'any': dest_vn= 'any'
            # end code to handle 'any' network
            new_rule[ 'source_network' ]= [vnc_api.AddressType(virtual_network = source_vn)] 
            new_rule[ 'dest_network' ]= [vnc_api.AddressType(virtual_network = dest_vn)] 
            np_rules.append( vnc_api.PolicyRuleType(direction = new_rule['direction'],
                             simple_action = new_rule['simple_action'],
                             protocol = new_rule['protocol'], 
                             src_addresses = new_rule['source_network'],
                             src_ports = new_rule['src_ports'],
                             application = new_rule['application'],
                             dst_addresses =  new_rule['dest_network'], 
        ...
        policy_obj = vnc_api.NetworkPolicy(policy_name , network_policy_entries = pol_entries, parent_obj = proj)
        return policy_obj
    #end  _create_policy
    ...
    def verify_policy_in_api_quantum_server(self, api_policy_obj, quantum_policy_obj):
        ...
        self.logger.info("====Verifying data for %s in API_Server ======" %(api_policy_obj.fq_name[2]))
        self.api_s_policy_obj= self.api_s_inspect.get_cs_policy(policy= api_policy_obj.fq_name[2], refresh= True)
        self.api_s_policy_obj_x= self.api_s_policy_obj['network-policy']

        # compare policy_fq_name
        out= policy_test_utils.compare_args('policy_fq_name', api_policy_obj.fq_name , quantum_policy_obj['policy']['fq_name'])
        if out: err_msg.append(out)
        # compare policy_uuid
        out= policy_test_utils.compare_args('policy_uuid', api_policy_obj.uuid , quantum_policy_obj['policy']['id'])
        if out: err_msg.append(out)
        # compare policy_rules
        out= policy_test_utils.compare_args('policy_rules', self.api_s_policy_obj_x['network_policy_entries']['policy_rule'],
            quantum_policy_obj['policy']['entries']['policy_rule'])
        if out: err_msg.append(out)

        if err_msg != []: result= False; err_msg.insert(0, me + ":" + api_policy_obj.fq_name[2])
        self.logger.info ("verification: %s, status: %s message: %s" %(me, result, err_msg))
