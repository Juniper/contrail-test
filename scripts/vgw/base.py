...
class BaseVgwTest(test.BaseTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.connections = ContrailConnections(cls.inputs, project_name = cls.inputs.project_name, 
						username = cls.inputs.stack_user, password = cls.inputs.stack_password,
                                                 logger = cls.logger)
    ...
    def setup_common_objects(cls):
        ...
        cls.project_fixture = ProjectFixture(
            vnc_lib_h = cls.vnc_lib, project_name = cls.inputs.project_name, connections = cls.connections)
        ...
        for key in cls.inputs.vgw_data[0]:
            for vgw in cls.inputs.vgw_data[0][key]:
                ...
                cls.vgw_vn_list[cls.inputs.vgw_data[0]
                                 [key][vgw]['vn']]['host'] = key
                if cls.inputs.vgw_data[0][key][vgw].has_key('gateway-routes'):
                    cls.vgw_vn_list[cls.inputs.vgw_data[0][key][vgw]['vn']][
        ...
        for key in cls.vgw_vn_list:
            vn = VNFixture(
                        project_name=cls.inputs.project_name, connections=cls.connections,
                        inputs=cls.inputs, vn_name=key.split(":")[3], subnets=cls.vgw_vn_list[key]['subnet'])
