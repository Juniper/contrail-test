...
class TestDSNAT(BaseDSNAT):
    ...
    def test_dsnat_basic(self):
        ...
        assert self.verify_fabric_ip_as_floating_ip(test_vm1, vn_fixture.vn_fq_name)
        assert self.verify_fabric_ip_as_floating_ip(test_vm2, vn_fixture.vn_fq_name)
        ...
        assert not self.verify_fabric_ip_as_floating_ip(test_vm1, vn_fixture.vn_fq_name), (
            'FIP list of VMI expected to be empty')

        assert test_vm1.ping_with_certainty(test_vm2.vm_ip)
        assert test_vm1.ping_with_certainty(self.inputs.cfgm_ip,expectation=False)
    ...
    def test_dsnat_with_different_forwarding_mode(self):
        ...
        assert self.verify_fabric_ip_as_floating_ip(vm1_fixture, vn1_fixture.vn_fq_name)
        assert self.verify_fabric_ip_as_floating_ip(vm2_fixture, vn1_fixture.vn_fq_name)
        ...
        #set VN forwarding mode as l2_l3 and verify
        self.set_vn_forwarding_mode(vn1_fixture, forwarding_mode='l2_l3')
    ...
    def test_dsnat_with_sg(self):
        ...
        default_sg = SecurityGroupFixture(self.connections,
        ...
        secgrp_fixture = self.useFixture(SecurityGroupFixture(
            ...
            secgrp_name=self.sg_allow_tcp, secgrp_entries=rule,option='neutron'))
    ...
    def test_dsnat_with_policy(self):
        ...
        assert self.verify_fabric_ip_as_floating_ip(vm1_fixture, vn1_fixture.vn_fq_name)
        assert self.verify_fabric_ip_as_floating_ip(vm2_fixture, vn2_fixture.vn_fq_name)

        assert vm1_fixture.ping_with_certainty(vm2_fixture.vm_ip), (
            'Ping failed between VNs with allow-policy')

        assert vm1_fixture.ping_with_certainty(self.inputs.cfgm_ip), (
    ...
    def test_dsnat_with_floating_ip(self):
        ...
        fip_fixture = self.create_floatingip(floating_vn)
        ...
        prefixes = []
        prefixes.append(self.inputs.cfgm_ip+'/32')
    ...
    def test_dsnat_through_floatingvn(self):
        ...
        fip_fixture = self.create_floatingip(fvn_fixture)
        ...
        assert fvn_vm1.ping_with_certainty(self.inputs.cfgm_ip)
    ...
    def test_dsnat_with_secondary_ip(self):
        ...
        vIP = self.configure_app_for_port_list(vn1_fixture, [vm1_fixture, vm2_fixture])

        assert vm1_fixture.ping_with_certainty(vm2_fixture.vm_ip)
        self.logger.info('Ping fabric IP with source IP as %s' %vIP)
        assert vm1_fixture.ping_with_certainty(other_opt='-I '+vIP, ip=self.inputs.cfgm_ip),\
