        ...
        ContrailTestInit(
            self.ini_file, stack_user=self.project_fixture[
                ...
                self.topo.project].password, project_fq_name=[
                    'default-domain', self.topo.project]))
    ...
    for vm in self.topo.vmc_list:
        if self.skip_verify == 'no':
            ...
            while True:
                ...
                retry += 1
                if vm_verify_out or retry > retry_count:
                    break
...
def createStaticRouteBehindVM(self):
    ...
    for vm_name in self.topo.vm_static_route:
        ...
        obj.provision_static_route(
            prefix=prefix,
            virtual_machine_id=vm_uuid,
            tenant_name=self.topo.project,
            virtual_machine_interface_ip=vm_ip,
    ...
    if 'fvn_vm_map' in dir(self.topo):
        ...
        for vn_name in self.topo.fvn_vm_map:
            ...
            self.logger.info(
                ...
                (fip_pool_name,
                 self.fip_fixture_dict[vn_name].pub_vn_name,
                 self.topo.project))
