...
def _create_project_return_connection(self,projectname):
    ...
    kc = ksclient.Client(username= self.inputs.stack_user, password= self.inputs.stack_password, 
    ...
    for name in create_user_set:
        ...
        self.logger.info('Created User:%s with Role:%s for Project:%s ' %(name, user_role[name], new_tenant))
        for new_tenant in [projectname]:
    ...
    # Projects
    self.new_proj_inputs=self.useFixture(ContrailTestInit( self.ini_file, stack_user=user_list[0][0], 
