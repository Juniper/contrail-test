...
class TestBasicRTFilter(BaseRtFilterTest):
    ...
    def test_vn_rt_entry(self):
        '''
        Description:  Validate the entry of the VN's Route Target in the rt_group and  bgp.rtarget.0 table on the control nodes
        Test steps:
        1. Create a VM in a VN.
        2. Check the rt_group and  bgp.rtarget.0 table on the control nodes.
        Pass criteria: The route target of the VN and the VM IP should be seen in the respective tables.
    ...
    def test_user_def_rt_entry(self):
        '''
        Description: Validate the entry and deletion of the VN's user-added Route Target in the rt_group and  bgp.rtarget.0 table on the control nodes
        Test steps:
        1. Create a VM in a VN.
        2. Add a route-target entry to the VN.
        2. Check the rt_group and  bgp.rtarget.0 table on the control nodes.
        Pass criteria: The system-defined, user-defined route target of the VN and the VM IP should be seen in the respective tables.
        Maintainer : ganeshahv@juniper.net
        '''
        vn1_name = get_random_name('vn30')
        vn1_subnets = [get_random_cidr()]
    ...
    def test_rt_entry_persistence_across_restarts(self):
        '''
        Description: Validate the persistence of Route Target entry in the rt_group and bgp.rtarget.0 table on the control nodes
        ...
        Pass criteria: The system-defined, user-defined route target of the VN and the VM IP should be seen in the respective tables, even after the restarts.
        Maintainer : ganeshahv@juniper.net
        '''
        vn1_name = get_random_name('vn30')
        vn1_subnets = [get_random_cidr()]
    ...
    def test_vpnv4_route_entry_only_on_RT_interest_receipt(self):
        '''
        Description: Validate the presence of route in the bgp.l3vpn.0 table only when a RT interest is generated
        Test steps:
        0. Check env variable MX_GW_TEST is set to 1. This confirms MX presence in the setup.
        1. Create a VM in a VN.
        2, Enable control-node peering with MX.
        3. Check that with RT-Filtering enabled, we should not see route 0/0 from Mx in the bgp.l3vpn.0 table.
    ...
    def test_dep_routes_two_vns_with_same_rt(self):
        '''
        Description: Validate that dep_routes are seen in the RTGroup Table under the route-target which is common to two different networks
        Test steps:
        1. Create 2 VNs and a VM in each.
        2. Add the same RT-entry to both the VNs.
        Pass criteria: dep_routes are seen in the RTGroup Table under the route-target which is common to two different networks
        Maintainer : ganeshahv@juniper.net
        '''
        vn1_name = get_random_name('vn30')
    ...
    def test_rt_entry_with_multiple_ctrl_nodes(self):
        '''
        Description: Validate that the dep_routes in the RTGroup Table and paths in the bgp.l3vpn.0 corresponding to the VM is seen only in the control_node, the VM's compute node has a session with.
        Test steps:
        1. Have a setup with more than 2 control nodes.
        2. Create a VN and a VM in it.
        Pass criteria: Check the dep_routes in the RTGroup Table and paths in the bgp.l3vpn.0 table of the ctrl node that the compute node has a XMPP peering with.
        Maintainer : ganeshahv@juniper.net
        '''
