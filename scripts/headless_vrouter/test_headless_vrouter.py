...
class TestHeadlessVrouter(testtools.TestCase, fixtures.TestWithFixtures):

    def setUp(self):
        super(TestHeadlessVrouter, self).setUp()
    ...
    def cleanUp(self):
        super(TestHeadlessVrouter, self).cleanUp()
    # end cleanUp
    ...
    def test_traffic_connections_while_control_nodes_go_down(self):
        ...
        for each_compute in self.inputs.compute_ips:
            self.compute_fixture_dict[each_compute] = self.useFixture(ComputeNodeFixture(
                connections=self.connections, node_ip=each_compute, username=self.inputs.username, password=self.inputs.password))
            mode = self.compute_fixture_dict[each_compute].get_agent_headless_mode()
            if mode is False:
                self.compute_fixture_dict[each_compute].set_agent_headless_mode()
        #
    ...
    def test_config_add_change_while_control_nodes_go_down(self):
        ...
        VM22_fixture = self.useFixture(VMFixture(connections=project1_instance.project_connections, \
                                                 vn_obj=vnet2_instance.obj, \
                                                 vm_name='VM22', \
                                                 project_name=project1_instance.project_name))

        # create new IPAM
        ipam3_obj = self.useFixture(IPAMFixture(project_obj=project1_instance, name='ipam3'))
        ipam4_obj = self.useFixture(IPAMFixture(project_obj=project1_instance, name='ipam4'))

        # create new VN
        VN3_fixture = self.useFixture(VNFixture(project_name=project1_instance.project_name, \
        ...
        VN4_fixture = self.useFixture(VNFixture(project_name=project1_instance.project_name, \
                                                connections=project1_instance.project_connections, \
                                                vn_name='VN4', \
                                                inputs=project1_instance.inputs, \
                                                subnets=['10.4.1.0/24'], \
        ...
        policy34_fixture = self.useFixture(PolicyFixture(policy_name=policy_name, \
                                                        rules_list=rules, \
                                                        inputs=project1_instance.inputs, \
                                                        connections=project1_instance.project_connections))

        # create VN to policy mapping in a dict of policy list.
        vn_policys = {VN3_fixture.vn_name : [policy_name], VN4_fixture.vn_name : [policy_name]}
        ...
        # attach policy to VN
        VN3_policy_fixture = self.useFixture(VN_Policy_Fixture(connections=project1_instance.project_connections, \
        ...
        VN4_policy_fixture = self.useFixture(VN_Policy_Fixture(connections=project1_instance.project_connections, \
        ...
        # add VM to new VN
        VM31_fixture = self.useFixture(VMFixture(connections=project1_instance.project_connections, \
                                                 vn_obj=VN3_fixture.obj, \
                                                 vm_name='VM31', \
                                                 project_name=project1_instance.project_name))

        VM41_fixture = self.useFixture(VMFixture(connections=project1_instance.project_connections, \
        ...
        test_flag = 0
        if ((VN3_fixture.verify_vn_in_api_server() == True) and \
            (VN3_fixture.verify_vn_not_in_agent() == True) and \
            (VN3_fixture.verify_vn_policy_in_api_server()['result'] == True)):
            self.logger.info("Verification of VN3 PASSED while control nodes down.")
        else:
            self.logger.error("Verification of VN3 FAILED while control nodes down.")
            test_flag = 1

        if ((VN4_fixture.verify_vn_in_api_server() == True) and \
            (VN4_fixture.verify_vn_not_in_agent() == True) and \
            (VN4_fixture.verify_vn_policy_in_api_server()['result'] == True)):
            self.logger.info("Verification of VN4 PASSED while control nodes down.")
        else:
            self.logger.error("Verification of VN4 FAILED while control nodes down.")
            test_flag = 1

        if ((VM22_fixture.verify_vm_launched() == True) and \
            (VM22_fixture.verify_vm_in_api_server() == True)):
            self.logger.info("Verification of VM22 PASSED while control nodes down.")
        else:
            self.logger.error("Verification of VM22 FAILED while control nodes down.")
            test_flag = 1

        if ((VM31_fixture.verify_vm_launched() == True) and \
            (VM31_fixture.verify_vm_in_api_server() == True)):
            self.logger.info("Verification of VM31 PASSED while control nodes down.")
        else:
            self.logger.error("Verification of VM31 FAILED while control nodes down.")
            test_flag = 1

        if ((VM41_fixture.verify_vm_launched() == True) and \
            (VM41_fixture.verify_vm_in_api_server() == True)):
            self.logger.info("Verification of VM41 PASSED while control nodes down.")
        else:
            self.logger.error("Verification of VM41 FAILED while control nodes down.")
            test_flag = 1

        # check ping failure between the two VM's
        try:
            if (config_topo['project1']['vm']['VM11'].ping_with_certainty(
                   VM22_fixture.vm_ip, expectation=False)) == True :
                self.logger.info("Ping failed as expected between VM11 and VM22.")
            else:
                self.logger.error("Ping passed unexpectedly between VM11 and VM22.")
                test_flag = 1
        except:
        ...
        try:
            VM31_fixture.ping_with_certainty(VM41_fixture.vm_ip, expectation=False)
        except:
        ...
        # wait for 3 to 5 sec for configuration sync from control nodes to the agents.
        time.sleep(5)
        ...
        if ((VM22_fixture.verify_vm_in_agent() == True) and \
            (VM31_fixture.verify_vm_in_agent() == True) and \
            (VM41_fixture.verify_vm_in_agent() == True)):
        ...
        assert VM31_fixture.ping_with_certainty(VM41_fixture.vm_ip, expectation=True)
        assert VM41_fixture.ping_with_certainty(VM31_fixture.vm_ip, expectation=True)

        # verification routines.
        if ((VN3_fixture.verify_on_setup() == True) and \
            (VN4_fixture.verify_on_setup() == True) and \
            (VM22_fixture.verify_on_setup() == True) and \
            (VM31_fixture.verify_on_setup() == True) and \
            (VM41_fixture.verify_on_setup() == True)):
            self.logger.info("All verifications passed after controllers came up in headless agent mode")
        else:
            self.logger.error("Verifications FAILED after controllers came up in headless agent mode")
            return False
