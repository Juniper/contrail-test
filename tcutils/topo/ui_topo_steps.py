...
def createPort(self, option='contrail'):
    ...
    for port in self.topo.port_list:
        ...
        if not self.webui.create_port(net, subnet, mac, state, port_name, fixed_ip, fip, sg, device_owner):
...
def createRouter(self, option='contrail'):
    ...
    for router in self.topo.router_list:
        ...
        if not self.webui.create_router(router_name, networks, state, gateway, snat):
            result = result and False
    return result
    # end createPort
...
def createDnsServer(self, option='contrail'):
    ...
    for dserver in self.topo.dns_server_list:
        ...
        if not self.webui.create_dns_server(server_name, domain_name, rr_order, fip_record, ipam_list, ttl, dns_forwarder):
            result = result and False
    return result
# end createDnsServer
...
def createDnsRecord(self, option='contrail'):
    ...
    for dns_record in self.topo.dns_record_list:
        ...
        if not self.webui.create_dns_record(server_name, host_name, ip_address, record_type, dns_class, ttl):
            result = result and False
    return result
# end createDnsRecord
