...
# retry = 10
# includeservice = {'10.204.217.7': 'supervisor-control,contrail-control', '10.204.216.72': 'supervisor-vrouter,contrai# l-vrouter-agent'}
# (boolval, ret) = self.stat.check_status(nodes, includeservice, retry)
# The return value in boolval will be False if error present, True otherwise and in ret will be a list of dict specifyi# ng node, service and error for that node and service if present:
# boolval = False
# ret = [{'Node': '10.204.216.72', 'Service': 'contrail-svc-monitor', 'Error': 'contrail-svc-monitor          inactive #     \r'}, {'Node': '10.204.217.11', 'Service': 'contrail-schema', 'Error': 'contrail-schema               inactive   #        \r'}]
# Usage 2:
# nodes = ['10.204.217.7', '10.204.216.72']
# retry = 5
# ret = self.stat.get_status(nodes, retry)
# The return value in ret will be a list of dict specifying node, service and error for that node and service if presen# t:
...
class Constatuscheck:
    ...
    def __init__(self, inputs=None):
        if not inputs:
            sanity_params = os.environ.get('TEST_CONFIG_FILE')
