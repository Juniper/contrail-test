...
from tcutils.contrail_status_check import Constatuscheck
...
# match two rules (type DiscoveryServiceAssignmentType)
    ...
    def create_rule(self, fq_name, publisher_prefix, publisher_type, *subscriber_prefix_type):    
        ...
        self.logger.info("Total number of subscribers mentioned in this rule are %d" % int(subscriber_number))
    ...
    def read_rule(self, fq_name):
        ...
        for rule in dsa_rules:
            ...
            if entry:
                ...
                self.logger.info("Total subscribers mentioned in the rule are %d" % len(subs))
                pub_str = '%s/%d,%s,%s,%s' % \
                ...
                self.logger.debug("Rule: %s %s %s" % ('', pub_str, sub_str))
            idx += 1
    ...
    def verify_client_subscription_to_expected_publisher(self, ds_ip, client_type, client_ip, service_type):
        ...
        for elem in publisher_node_ips:
            ...
            if publisher_net != expected_pub_network:
                ...
                result = False
        self.logger.debug("Expected publisher was from network : %s " % expected_pub_network)
        self.logger.debug("Actual publishers were from network : %s" % publisher_networks)
        if result == True:
    ...
    def verify_subscribe_request_with_diff_instances_rules(self, ds_ip=None):
        ...
        try:
            ...
            self.logger.debug("# The instances of publishers allocated to TestClient by the discovery server are %d #" % instances_allocated)
            self.logger.debug("# The total number of publishers running of such types are %d #" % len(self.inputs.cfgm_control_ips))
        ...
        try:
            ...
            instances_allocated = len(client_subscribed_service_id)
            self.logger.debug("# The instances of publishers allocated to TestClient by the discovery server are %d #" % instances_allocated)
            self.logger.debug("# The total number of publishers running of such types are %d #" % instances_allocated)
            if ifmap_server_count == instances_allocated or (ifmap_server_count > 3 and instances_allocated == 3):
                self.logger.info("# Instance field working as expected #")
            else:
                self.logger.error(" # Instance field not working as expected.#")
    ...
    def verify_rule_when_service_oper_down(self, ds_ip=None):
        ...
        sleep (90)
        try:
            self.logger.debug("# Verifying that as publisher is operationally down, the mentioned subscriber in rule do not get any instance of Publisher #")      
            ...
            if instances_allocated==0:
                ...
                result = False
                self.logger.error("# Even if rule is present and publisher in rule is oper *down*, some publisher got assigned to the subscriber contrail-vrouter-agent running on %s .#", self.inputs.compute_control_ips[0])
                service_IPs = []
                for i in range (0,instances_allocated):
                    service_endpoint = self.get_service_endpoint_by_service_id(ds_ip,client_subscribed_service_id[i])
                    service_IPs.append(service_endpoint[0][0])
            ...
            self.logger.debug("#### Waiting for 5 seconds so that the client subscribe to the new subscriber as soon as it comes operationally up ##")
            sleep(5)
            self.logger.debug("# Verifying that as publisher is oper up, the mentioned subscriber in rule gets the same instance of Publisher as mentione din rule #")      
            client_subscribed_service_id = self.get_subscribed_service_id(ds_ip, client=(self.inputs.compute_control_ips[0],"contrail-vrouter-agent:0"), service="dns-server")
            instances_allocated = len(client_subscribed_service_id)
            service_IPs = []
            for i in range (0,instances_allocated):
            ...
            if instances_allocated==1 and service_IPs[0]==self.inputs.bgp_control_ips[0]:
                ...
                result = False
                self.logger.error("# Even if rule is present and publisher in rule is oper *up*, some different publishers or no publisher got assigned to the subscriber contrail-vrouter-agent running on %s .#", self.inputs.compute_control_ips[0])
