...
#                    uid = (id,'IfmapServer')
    ...
    def add_keystone_auth_conf_file(self, auth="keystone", add_values = "True"):
        ...
        if add_values == "True":
            ...
            self.modify_conf_file('set', 'contrail-discovery', 'KEYSTONE', 'admin_user', "admin" , username, password)
            self.modify_conf_file('set', 'contrail-discovery', 'KEYSTONE', 'admin_password', "contrail123", username, password)
            self.modify_conf_file('set', 'contrail-discovery', 'KEYSTONE', 'admin_tenant_name', "admin" , username, password)
        conf_file = '/etc/contrail/contrail-discovery.conf '
        cmd = 'cat ' + conf_file
        for ip in self.inputs.cfgm_ips:
            out_put = self.inputs.run_cmd_on_server(ip, cmd, username, password)
    ...
    def white_list_subscribers_conf_file(self, *subscriber_ips):
        ...
        self.modify_conf_file('set', 'contrail-discovery', 'DEFAULTS', 'white_list_subscribe',subscriber_list, username, password)
        conf_file = '/etc/contrail/contrail-discovery.conf '
        cmd = 'cat ' + conf_file
        for ip in self.inputs.cfgm_ips:
            out_put = self.inputs.run_cmd_on_server(ip, cmd, username, password)
...
# match two rules (type DiscoveryServiceAssignmentType)
    ...
    def publish_requests_with_keystone(self, ds_ip, operation, service_type, service_id, operation_status = "up" ):
        try:
            vnc = VncApi(self.inputs.stack_user, self.inputs.stack_password, self.inputs.project_fq_name[1], "127.0.0.1", '8082')
        except Exception as e:
            print '*** %s' % str(e)
    ...
    def verify_subscribe_request_with_diff_instances_rules(self, ds_ip=None):
        ...
        try:
            ...
            self.logger.info("############### The total number of publishers running of such types are %d ###############" % len(self.inputs.cfgm_control_ips))
    ...
    def verify_rule_when_service_oper_down(self, ds_ip=None):
        ...
        sleep (90)
        try:
            self.logger.info("############### Verifying that as publisher is operationally down, the mentioned subscriber in rule do not get any instance of Publisher ###############")      
    ...
    def verify_multiple_rule_same_subscriber(self, ds_ip=None):
        ...
        try:
            ...
            for i in range (0,instances_allocated):
                    service_endpoint = self.get_service_endpoint_by_service_id(ds_ip,client_subscribed_service_id[i])
                    service_IPs.append(service_endpoint[0][0])
            if instances_allocated==2 and service_IPs[0] in self.inputs.bgp_control_ips and service_IPs[1] in self.inputs.bgp_control_ips:
                self.logger.info("############### Client contrail-vrouter-agent running on %s is subscribed to expected xmpp-server as the rule is restricting it to do that ###############" % self.inputs.compute_control_ips[0])
                pass
            else:
                result = False
    ...
    def verify_rules_preferred_over_auto_load_balance(self, ds_ip=None):
        ...
        for i in range(0,len(self.inputs.compute_control_ips)):
            self.create_rule('default-discovery-service-assignment', self.inputs.bgp_control_ips[0], 'xmpp-server', self.inputs.compute_control_ips[i], 'contrail-vrouter-agent:0')
        self.read_rule('default-discovery-service-assignment')
        for i in range(0,len(self.inputs.compute_control_ips)): 
            Result1 = self.find_rule('default-discovery-service-assignment', self.inputs.bgp_control_ips[0], 'xmpp-server', self.inputs.compute_control_ips[i], 'contrail-vrouter-agent:0')
            if Result1 == False:
                self.logger.warn("############### While searching for the configured rule, it was not found. Configuration failed ###############")
                result = False
        self.logger.info("################## Waiting for 60 seconds so that TTL expiry for all subscriber happens ###########")
        ...
        try:
            in_use_list = []
            for i in range(0,len(self.inputs.bgp_control_ips)): 
                in_use_list_elem = self.get_service_in_use(ds_ip, (self.inputs.bgp_control_ips[i],'xmpp-server'))
            ...
            sys.exit(1)  
        for i in range(0,len(self.inputs.compute_control_ips)): 
            self.delete_rule('default-discovery-service-assignment', self.inputs.bgp_control_ips[0], 'xmpp-server', self.inputs.compute_control_ips[i], 'contrail-vrouter-agent:0')
        self.read_rule("default-discovery-service-assignment")
        for i in range(0,len(self.inputs.compute_control_ips)): 
            Result1 = self.find_rule('default-discovery-service-assignment', self.inputs.bgp_control_ips[0], 'xmpp-server', self.inputs.compute_control_ips[i], 'contrail-vrouter-agent:0')
            if Result1 == True:
    ...
    def verify_service_in_use_list(self, ds_ip=None):
        ...
        try:
            ...
            if instances_allocated == len(self.inputs.cfgm_control_ips) and sum_in_use_aftr_subscribe_request > sum_in_use_bfr_subscribe_request:
    ...
    def verify_white_list_security(self, ds_ip=None):
        ...
        try:
            ...
            DiscoveryServerUtils.POST_HEADERS = {'Content-type': 'application/json' , 'X-Forwarded-For': "1.1.1.1"}
            ...
            if self.get_all_services_by_service_name(ds_ip, service="Test_Pub_1") == []:
                result = False
                self.logger.warn("############ Failure!! The requested publish request not accepted by discovery server even if the IP was present in Publisher white list  ###########")
            else:
            ...
            if self.get_all_services_by_service_name(ds_ip, service="Test_Pub_2") == []:
                ...
                result = False
                self.logger.warn("############ Failure!! The requested publish request accepted by discovery server even if the IP was not present in Publisher white list  ###########")
            ...
            self.subscribe_service_from_discovery(ds_ip, service="IfmapServer", instances="2", client_id=self.inputs.compute_names[0]+":TestClient_1", remote_addr= self.inputs.compute_control_ips[0], client_type= "TestClient_1")
            if self.get_subscribed_service_id(ds_ip, client=(self.inputs.compute_control_ips[0],"TestClient_1"), service="IfmapServer") == []:
                result = False
                self.logger.warn("############ Failure!! The requested subscribe request not accepted by discovery server even if the IP was present in Subscriber white list  ###########")
            else:
            ...
            DiscoveryServerUtils.POST_HEADERS = {'Content-type': 'application/json' , 'X-Forwarded-For': "3.3.3.3"}
            self.subscribe_service_from_discovery(ds_ip, service="IfmapServer", instances="2", client_id=self.inputs.compute_names[0]+":TestClient_2", remote_addr= self.inputs.compute_control_ips[0], client_type= "TestClient_2")
            if self.get_subscribed_service_id(ds_ip, client=(self.inputs.compute_control_ips[0],"TestClient_2"), service="IfmapServer") == []:
                ...
                result = False
                self.logger.warn("############ Failure!! The requested subscribe request accepted by discovery server even if the IP was not present in Subscriber white list  ###########")
            ...
            if self.get_all_services_by_service_name(ds_ip, service="Test_Pub_2") == []:
                result = False
                self.logger.warn("############ Failure!! The requested publish request not accepted by discovery server even after deleting publish white list  ###########")   
            else:
            ...
            if self.get_subscribed_service_id(ds_ip, client=(self.inputs.compute_control_ips[0],"TestClient_2"), service="IfmapServer") == []: 
                result = False
                self.logger.warn("############ Failure!! The requested subscribe request not accepted by discovery server even if Subscriber white list has been deleted  ###########")
            else:
    ...
    def verify_keystone_auth_security(self, ds_ip=None):
        ...
        try:
            ...
            self.logger.info("############### Verify that all requests fails if Auth is True and credentials are not mentioned ############### ")
            response = self.publish_requests_with_keystone(ds_ip,operation="oper-state", operation_status="up", service_id=self.inputs.cfgm_names[0], service_type="IfmapServer")
            if response != 200:
                ...
                self.logger.info("Failure!! Even if authetication is True and credentials are not configured, the oper-state change request is successful")
                result = False
            response = self.publish_requests_with_keystone(ds_ip,operation="admin-state", operation_status="up", service_id=self.inputs.cfgm_names[0], service_type="IfmapServer")
            if response != 200:
                ...
                self.logger.info("Failure!! Even if authetication is True and credentials are not configured, the admin-state change request is successful")
                result = False
            response = self.publish_requests_with_keystone(ds_ip,operation="load-balance", service_id=self.inputs.cfgm_names[0], service_type="IfmapServer")
            if response != 200:
                self.logger.info("Success!! As authetication is True and credentials are not configured, the load-balance request has failed")
            else:
                self.logger.info("Failure!! Even if authetication is True and credentials are not configured, the load-balance request is successful")
                result = False
            ...
            if response == 200:
                ...
                self.logger.info("Failure!! Even if authetication is True and credentials are configured, the oper-state change request has failed")
                result = False
            response = self.publish_requests_with_keystone(ds_ip,operation="admin-state", operation_status="up", service_id=self.inputs.cfgm_names[0], service_type="IfmapServer")
            if response == 200:
                ...
                self.logger.info("Failure!! Even if authetication is True and credentials are  configured, the admin-state change request has failed")
                result = False
            response = self.publish_requests_with_keystone(ds_ip,operation="load-balance", service_id=self.inputs.cfgm_names[0], service_type="IfmapServer")
            if response == 200:
                ...
                self.logger.info("Failure!! Even if authetication is True and credentials are configured, the load-balance request has failed")
                result = False
        except Exception as e:
            print '*** %s' % str(e)
            sys.exit(1)
        self.logger.info("############### Deleting the auth configurations from contrail-discovery.conf file ############### ")
