...
def get_build_sku(openstack_node_ip, openstack_node_password='c0ntrail123'):
    build_sku=None
    host_str='%s@%s' % ('root', openstack_node_ip)
    ...
    while not build_sku and tries:
        try:
            output = run_fab_cmd_on_node(host_str, pswd, cmd, timeout=10)
