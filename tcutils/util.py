...
def add_knob_to_container(obj, node_ip, container_name, level='DEFAULT',
    ...
    username = obj.inputs.host_data[node_ip]['username']
    password = obj.inputs.host_data[node_ip]['password']

    obj.logger.info('Running %s on %s' % (issue_cmd, node_ip))
    obj.inputs.run_cmd_on_server(node_ip, issue_cmd, username, password, pty=True,
                                as_sudo=True)
    ...
    obj.inputs.run_cmd_on_server(node_ip, issue_cmd, username, password, pty=True,
                                as_sudo=True)
    #obj.inputs.restart_container([node_ip], container_name)
