...
class PassParcentageCalculator(object):
    ...
    def stop_on_failure(self):
        ...
        if self.percentage >= int(self._args.count):
            print 'Failed tests %s percent corssed the expected limit %s percent'%(str(self.percentage),str(self._args.count))
	    sys.exit(1) 
	else:
	    print 'Failures within limit %s percent'%(str(self._args.count))
            sys.exit(0)
    # end stop_on_failure 
    ...
    def _parse_args(self, args_str):
        ...
        parser.add_argument(
            "--files",nargs='+' 
		,help="Files to be checked for test counts")
        parser.add_argument("--count", help="Max number of tests expected to be failed")
