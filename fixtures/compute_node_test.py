...
class ComputeNodeFixture(fixtures.Fixture):
    ...
    def cleanUp(self):
        ...
        if do_cleanup:
            self.logger.debug("Check & reset agent params to defaults")
            if self.get_config_flow_aging_time != 180:
                restart_sup_vrouter=True
                self.set_flow_aging_time(180)
            if self.get_config_per_vm_flow_limit != 100:
                restart_sup_vrouter=True
                self.set_per_vm_flow_limit(100)
            if restart_sup_vrouter:
