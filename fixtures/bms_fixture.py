...
import logging
import fixtures
from tcutils.util import retry, search_arp_entry, get_random_name, get_intf_name_from_mac
...
class BMSFixture(fixtures.Fixture):
    def __init__(self,
        ...
        self.name = name
        bms_dict = self.inputs.bms[name]
        self.tor = kwargs.get('tor') or bms_dict['tor']
        tor_port = kwargs.get('tor') or bms_dict['tor_port']
        ...
        self.lif = tor_port+'.'+unit
        self.vnc_h = connections.orch.vnc_h
        self.vlan_id = port_fixture.vlan_id if port_fixture else \
                       vlan_id
    ...
    def setup_bms(self):
        ...
        self.mvlanintf = 'mvlan-%s'%pvlanintf
        macaddr = 'address %s'%self.bms_mac if self.bms_mac else ''
        self.run('ip link add %s link %s type %s macvlan mode bridge'%(
                 mvlanintf, pvlanintf, macaddr))
    ...
    def run_dhclient(self, timeout=60):
        ...
        output = self.run_namespace('timeout %s dhclient -v %s'%(
                              timeout, self.mvlanintf), timeout=30)
