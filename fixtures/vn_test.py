...
#    def __init__(self, connections, vn_name, inputs, policy_objs= [], subnets=[], project_name= 'admin', router_asn='64512', rt_number=None, ipam_fq_name=None, option = 'api'):
    def __init__(self, connections, vn_name, inputs, policy_objs= [], subnets=[], project_name= 'admin', router_asn='64512', rt_number=None, ipam_fq_name=None, option = 'quantum'):
...
#        self.api_vn_obj = VirtualNetwork(name = self.vn_name, parent_obj= self.project_obj.project_obj)
        try:
            self.api_vn_obj = VirtualNetwork(name = vn_name, parent_obj=project.project_obj)
            if not self.verify_if_vn_already_present(self.api_vn_obj ,project.project_obj):
                self.vn_id = self.vnc_lib_h.virtual_network_create(self.api_vn_obj)
                with self.lock:
                    self.logger.info("Virtual network %s created using api-server"%(self.vn_name))
            else:
                with self.lock:
                    self.logger.info("VN %s already present"%(self.vn_name))
                self.vn_id = self.get_vn_uid(self.api_vn_obj,project.project_obj.uuid)    
    ...
    def setUp(self):
        ...
        if self.rt_number is not None:
            self.add_route_target(self.ri_name, self.router_asn, self.rt_number)
...
#        t_pol_api.daemon = True
        ...
        time.sleep(1)
        if self.policy_objs:
            t_pol_op = threading.Thread(target=self.verify_vn_policy_in_vn_uve, args=())
...
#                           
    ...
    def verify_vn_in_api_server(self):
        ...
        """
        self.api_verification_flag = True
        self.api_s_vn_obj= self.api_s_inspect.get_cs_vn(project = self.project_name, vn= self.vn_name, refresh= True)
        if not self.api_s_vn_obj :
            self.logger.warn( "VN %s is not found in API-Server" %(self.vn_name))
            self.api_verification_flag = self.api_verification_flag and False
            return False
        if self.api_s_vn_obj['virtual-network']['uuid'] != self.vn_id:
            self.logger.warn( "VN Object ID %s in API-Server is not what was created" %(self.vn_id))
            self.api_verification_flag = self.api_verification_flag and False
            return False
        
        subnets= self.api_s_vn_obj['virtual-network']['network_ipam_refs'][0]['attr']['ipam_subnets']
        for vn_subnet in self.vn_subnets:
            ...
            for subnet in subnets:
                if subnet['subnet']['ip_prefix'] == str(IPNetwork(vn_subnet).ip):
                    subnet_found=True
            if not subnet_found:
                self.logger.warn( "VN Subnet IP %s not found in API-Server for VN %s" %(IPNetwork(vn_subnet).ip, self.vn_name))
                self.api_verification_flag = self.api_verification_flag and False
                return False
        #end for  
        self.api_s_route_targets= self.api_s_inspect.get_cs_route_targets( vn_id= self.vn_id)
        if not self.api_s_route_targets:
            errmsg = "Route targets not found in API-Server for VN %s" % self.vn_name
            self.logger.error(errmsg)
            self.api_verification_flag = self.api_verification_flag and False
            return False
        self.rt_names= self.api_s_inspect.get_cs_rt_names( self.api_s_route_targets )
        self.api_verification_flag = self.api_verification_flag and True
        self.logger.info( "Verifications in API Server for VN %s passed" %(self.vn_name))
        self.api_s_routing_instance= self.api_s_inspect.get_cs_routing_instances( vn_id= self.vn_id) 
        return True
    ...
    def verify_vn_policy_in_vn_uve(self):
        ...
        try:
            for ip in self.inputs.collector_ips:
                self.policy_in_vn_uve=self.analytics_obj.get_vn_uve_attched_policy(ip,vn_fq_name=self.vn_fq_name)
                self.logger.info("Attached policy in vn %s uve %s"%(self.vn_name,self.policy_in_vn_uve))
                policy_list=[]
                for elem in self.policy_objs:
                    policy=elem['policy']['fq_name']
                    policy_name=str(policy[0])+':'+(str(policy[1]))+':'+(str(policy[2]))
...
#                    result=result and True
    ...
    def verify_vn_policy_not_in_vn_uve(self):
        ...
        for ip in self.inputs.collector_ips:
            ...
            if self.policy_in_vn_uve:
                self.logger.warn("Attached policy not deleted in vn %s uve"%(self.vn_name)) 
                result=result and False
            else:
                result=result and True
        return result
    ...
    def get_allowed_peer_vns_by_policy(self):
        ...
        pol_name_list= []; allowed_peer_vns= []
        ...
        if pol_name_list:
            for pol in pol_name_list:
                pol_object = self.api_s_inspect.get_cs_policy(policy=pol,refresh= True)
                pol_rules = pol_object['network-policy']['network_policy_entries']['policy_rule']
                self.logger.debug ("vn: %s, inspecting following rules for route verification: %s" %(self.vn_fq_name, pol_rules))
                for rule in pol_rules:
                    ...
                    rule_vns= []
                    src_vn= rule['src_addresses'][0]['virtual_network']; rule_vns.append(src_vn)
                    dst_vn= rule['dst_addresses'][0]['virtual_network']; rule_vns.append(dst_vn)
                    if self.vn_fq_name in rule_vns:
                        rule_vns.remove(self.vn_fq_name)
                        # Consider peer VN route only if the action is set to pass
                        if rule['action_list']['simple_action'] == 'pass':
                            self.logger.debug ("Local VN: %s, Peer VN %s is a valid peer" %(self.vn_fq_name, rule_vns[0]))
                            allowed_peer_vns.append( rule_vns[0])
                        else:
                            self.logger.debug ("Local VN: %s, skip route to VN %s as the action is not set to allow" %(self.vn_fq_name, rule_vns[0]))
                    elif 'any' in rule_vns:
                        if rule['action_list']['simple_action'] == 'pass':
                            self.logger.debug ("any VN is a valid pair for this vn %s" %(self.vn_fq_name))
                            allowed_peer_vns.append('any')
                    else:
                        self.logger.info ("Local VN: %s, skip the VNs in this rule as the local VN is not listed & the rule is a no-op: %s" %(self.vn_fq_name, rule_vns))
    ...
    def verify_vn_policy_in_api_server(self):
        ...
        result= True; err_msg= []; out= None
        self.logger.info("====Verifying policy data for %s in API_Server ======" %(self.vn_name))
        self.api_s_vn_obj= self.api_s_inspect.get_cs_vn(project = self.project_name, vn= self.vn_name, refresh= True)
        try:
            vn_pol= self.api_s_vn_obj['virtual-network']['network_policy_refs']
        except:
            self.logger.debug ("=>VN %s has no policy to be verified" %(self.vn_name))
        ...
        if len(vn_pol) != len(self.policy_objs):
            ...
            for policy in self.policy_objs:
                self.logger.debug('%s, %s' %(policy['policy']['id'], policy['policy']['fq_name']))

        # Compare attached policy_fq_names & uuid's
        for policy in vn_pol:
            fqn= policy['to']; id= policy['uuid']
            self.logger.info("==>Verifying data for policy with id: %s, fqn: %s" %(id, fqn))
            # check if policy with this id exists in quantum
            d= policy_test_utils.get_dict_with_matching_key_val ('id', id, self.policy_objs, 'policy')
            if d['state'] == None: err_msg.append(d['ret'])
            else:
                out= policy_test_utils.compare_args('policy_fqn', fqn, d['ret']['policy']['fq_name'])    
                if out: err_msg.append(out)

        if err_msg : result= False; err_msg.insert(0, me + ":" + self.vn_name)
   ...
   # end verify_vn_policy_in_api_server
    ...
    def verify_vn_not_in_api_server(self):
        ...
        if self.api_s_inspect.get_cs_vn(project = self.project_name, vn= self.vn_name, refresh= True):
            self.logger.warn( "VN %s is still found in API-Server" %(self.vn_name))
    ...
    def verify_vn_in_control_nodes(self):
        ...
        for cn in self.inputs.bgp_ips:
            cn_config_vn_obj= self.cn_inspect[cn].get_cn_config_vn(vn_name=self.vn_name, project=self.project_name)
            if not cn_config_vn_obj:
                self.logger.warn('Control-node %s does not have VN %s info ' %(cn, self.vn_name) )
                self.cn_verification_flag = self.cn_verification_flag and False
                return False
            self.logger.debug( "Control-node %s : VN object is : %s" %(cn, cn_config_vn_obj))
            if self.vn_fq_name not in cn_config_vn_obj['node_name'] :
                self.logger.warn( 'IFMAP View of Control-node is not having the VN detail of %s' %(self.vn_fq_name))
                self.cn_verification_flag = self.cn_verification_flag and False
                return False
            ...
            if not cn_object:
                self.logger.warn('No Routing Instance found in CN %s with name %s' %(cn, self.ri_name) )
                self.cn_verification_flag = self.cn_verification_flag and False
                return False
            try:
                rt_names= self.api_s_inspect.get_cs_rt_names( self.api_s_route_targets )
                if cn_object['export_target'][0] not in rt_names:
                    self.logger.warn( "Route target %s for VN %s is not found in Control-node %s" %(rt_names, self.vn_name, cn))
                    self.cn_verification_flag = self.cn_verification_flag and False
                    return False
            except Exception as e:
                self.logger.exception("Got exception from control node verification as %s"%(e) )
                self.cn_verification_flag = self.cn_verification_flag and False
                return False
        #end for
        self.logger.info( 'Control-node Config, RI and RT verification for VN %s passed' %(self.vn_name) ) 
    ...
    def verify_vn_policy_not_in_api_server(self,policy_name):
        ...
        self.logger.info("====Verifying policy %s data removed from %s in API_Server ======" %(policy_name,self.vn_name))
        found = False
        ...
        if not pol_ref:
            self.logger.info("=> VN %s has no reference policys" %(self.vn_name))
            return found
    ...
    def verify_vn_not_in_control_nodes(self):
        ...
        for cn in self.inputs.bgp_ips:
            cn_object=self.cn_inspect[cn].get_cn_routing_instance( ri_name= self.ri_name)        
            if cn_object:
                self.logger.warn( "Routing instance for VN %s is still found in Control-node %s" %(self.vn_name, cn) )
                result= result and False
        ...
        if result :
            self.logger.info( "Routing instances and Config db in Control-nodes does not have VN %s info" %(self.vn_name) )
        return result 
    ...
    def verify_vn_not_in_agent(self):
        ...
        for compute_ip in self.inputs.compute_ips:
            ...
            vn= inspect_h.get_vna_vn(project=self.project_name, vn_name= self.vn_name)
            print vn
            if vn : 
                self.logger.warn('VN %s is still found in %s ' %( self.vn_name, compute_ip ))
                return False
                self.not_in_agent_verification_flag = False
            vrf_objs= inspect_h.get_vna_vrf_objs( project= self.project_name, vn_name= self.vn_name)
            if len( vrf_objs['vrf_list']) != 0 :
                self.logger.warn('VRF %s for VN %s is still found in agent %s' %( str(self.vrf_name), self.vn_name, compute_ip ) )
                self.not_in_agent_verification_flag = False
                return False 
            self.logger.info('VN %s is not present in Agent %s ' %( self.vn_name, compute_ip ) )
        #end for 
        self.not_in_agent_verification_flag = True
        return True
    ...
    def del_route_target(self, routing_instance_name, router_asn, route_target_number):
        ...
        if rtgt_val not in net_obj.get_route_target_list().get_route_target():
            self.logger.error( "%s not configured for VN %s" %(rtgt_val, rt_inst_fq_name[:-1]))
            result = False
    ...
    def verify_vn_route_target(self, policy_peer_vns):
        ...
        compare_rt_export = policy_test_utils.compare_list(exp_rt['rt_export'],act_rt_export)
        compare_rt_import = policy_test_utils.compare_list(exp_rt['rt_import'],act_rt_import)
    ...
    def get_rt_info (self):
        ...
        if dst_vn_name_list:
            for vn_name in dst_vn_name_list:
                vn_obj = self.vnc_lib_h.virtual_network_read(fq_name_str=vn_name)
                rt_list = self.api_s_inspect.get_cs_route_targets(vn_id = vn_obj.uuid)
            ...
            if (self.option == 'api'):
                ...
                for i in range(12):
                    if not self.quantum_fixture.delete_vn(self.vn_id):
                        ...
                        self.logger.warn("%s. Deleting the VN %s failed" % (i, self.vn_name))
                        self.logger.info("%s. Retry deleting the VN %s " % (i, self.vn_name))
                        sleep(10)
                    else:
                        break
...
#                assert self.verify_vn_not_in_control_nodes()
    ...
    def bind_policies(self, policy_fq_names, vn_id):
        ...
        if len(policy_fq_names) != 0:
            ...
            net_rsp = self.quantum_fixture.update_network(vn_id, {'network': net_req})
            self.logger.debug( 'Response for mapping policy(s) with vn '+ str(net_rsp) )
        # Update VN obj 
        self.update_vn_object()
        return net_rsp
    #end bind_policy     
    ...
    def unbind_policies(self, vn_id, policy_fq_names=[] ):
        ...
        for policy_name in policy_fq_names:
            if not policy_name in policys:
                self.logger.error('Policy %s is not bound to VN ID %s ' %(policy_name, vn_id) )
                return None
            else:
    ...
    def _find_subnets(self):
        ...
        for vn_name, net in self.vn_name_net.items():
            if type(net) is list:
                ...
                vn_subnets.update({vn_name : self._subnet(net, self.subnet_count)})
        return vn_subnets
