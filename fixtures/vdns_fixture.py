...
class VdnsFixture(ContrailFixture):
    ...
    def setUp(self):
        ...
        self.vdns_fix = self.useFixture(VirtualDnsTestFixtureGen(self.vnc_lib,virtual_DNS_name=self.vdns_name,virtual_DNS_data = self.dns_data,auto_prop_val=True))
    ...
    def verify_on_setup(self):
        ...
        try:
            vdns = self.vnc_lib.virtual_DNS_read(fq_name=self.obj.get_fq_name())
            self.logger.debug("VDNS: %s created succesfully", self.obj.get_fq_name())
        except NoIdError:
        ...
        if not retval1:
            ...
            errmsg = errmsg + "\n VDNS server " + self.obj.get_fq_name() + " info not found not found in control node"
        ...
        if not retval2:
           retval = True and False
           errmsg = errmsg + "\n VDNS server "+ self.obj.get_fq_name()+ " info not found not found in API server"
    ...
    def verify_vdns_in_control_node(self):
        ...
        for cn in self.inputs.bgp_ips:
            try:
                cn_s_dns = self.cn_inspect[cn].get_cn_vdns(vdns = str(self.obj.name))
                ...
                return False
        if msg != '':
            self.logger.info("VDNS info is not matching with control node data\n %s:",msg)
    ...
    def verify_vdns_in_api_server(self):
        ...
        result = True
        api_s_dns = self.api_s_inspect.get_cs_dns(vdns_name = str(self.obj.name),refresh=True)
        print api_s_dns
        msg = ''
        try:
            ...
            for data in api_vdns_data:
                if str(exp_vdns_data.__dict__[data]) != str(api_vdns_data.get(data)) :
                    result = result and False
                    msg = msg + '\nvdns '+ data + ' is not matching with api server data'
        except Exception as e:
            # Return false if we get an key error and for retry
            return False

        if msg != '':
            self.logger.info("VDNS info is not matching with API server\n %s:",msg)
    ...
    def verify_vdns_not_in_api_server(self):
        ...
        if self.api_s_inspect.get_cs_dns(vdns_name = str(self.obj.name),refresh=True) is not None:
            ...
            self.logger.info("VDNS information %s removed from the API Server", self.obj.name)
            return True
        ...
        for cn in self.inputs.bgp_ips:
            ...
            if cn_s_dns:
               ...
               return False
                ...
                self.logger.info("VDNS information %s removed in the Control node", self.obj.name)
                return True
    ...
    def verify_on_cleanup(self):
        ...
        try:
            ...
            errmsg = errmsg + "VDNS info " + self.vdns_fq_name + 'still not removed'
            self.logger.warn(errmsg)
            return False, errmsg
        except NoIdError:
            self.logger.info("VDNS info: %s deleted successfully." % self.vdns_fq_name)
...
class VdnsRecordFixture(ContrailFixture):
    ...
    def setUp(self):
        self.logger.debug("Creating VDNS record data : %s", self.vdns_record_name)
        super(VdnsRecordFixture, self).setUp() 
        self.vdns_rec_fix = self.useFixture(VirtualDnsRecordTestFixtureGen(self.vnc_lib,virtual_DNS_record_name=self.vdns_record_name,parent_fixt=self.parent,virtual_DNS_record_data = self.vdns_record_data,auto_prop_val=True))
        self.obj = self.vdns_rec_fix.getObj()
        self.vdns_rec_fq_name = self.obj.get_fq_name_str()
    def cleanUp(self):
        self.logger.debug("Deleting VDNS record data: %s",self.vdns_record_name)
    ...
    def verify_on_setup(self):
        ...
        self.logger.info("In verify_on_setup")
        try:
            vdns_rec = self.vnc_lib.virtual_DNS_record_read(fq_name=self.obj.get_fq_name())
            self.logger.debug("VDNS record: %s created succesfully", self.obj.get_fq_name())
        except NoIdError:
        ...
        if not ret_val1:
            retval = True and False
            errmsg = errmsg + "\n VDNS record " + self.obj.get_fq_name() + " is info not found in the control node\n"
            self.logger.error( "VDNS record info not found not found in control node")
        self.logger.info("Verify VDNS record  is shown in the control node")
        ret_val2 = self.verify_vdns_rec_in_cn_node()
        if not ret_val2:
           retval = True and False
           errmsg = errmsg + "\n VDNS record " + self.obj.get_fq_name() + " is info not found in the control node\n"
           self.logger.error( "VDNS record info not found not found in the control node")
    ...
    def verify_on_cleanup(self):
        ...
        errmsg = ''
        try:
            vdns = self.vnc_lib.virtual_DNS_record_read(fq_name=self.vdns_rec_fq_name)
            errmsg = errmsg + 'VDNS record '+ self.vdns_rec_fq_name + ' info still not removed'
            self.logger.warn(errmsg)
            return False, errmsg
        except NoIdError:
            self.logger.info("VDNS record info: %s deleted successfully." ,self.vdns_rec_fq_name)
        return retval,errmsg
    ...
    def verify_vdns_rec_in_cn_node(self):
        ...
        for cn in self.inputs.bgp_ips:
            try:
                cn_s_dns = self.cn_inspect[cn].get_cn_vdns_rec(vdns = str(self.parent.getObj().name),rec_name = str(self.obj.name))
                if self.obj.get_fq_name_str() not in cn_s_dns['node_name']:
                    result = result and False
                    msg = msg + '\nvdns name info not matching with control name data'
                act_cn_vdns_rec_data = cn_s_dns['obj_info']['data']['virtual-DNS-record-data']
        ...
        if msg != '':
            self.logger.info("VDNS record info is not matching with control node data\n %s:",msg)
        return result
        self.logger.info('Outside verify_vdns_rec_in_cn_node')
    ...
    def verify_vdns_rec_in_api_server(self):
        ...
        result = True
        api_s_dns_rec = self.api_s_inspect.get_cs_dns_rec(rec_name = self.vdns_record_name,vdns_name = self.parent.getObj().name,refresh =True)
        msg = ''
        try:
            ...
            api_vdns_rec_data = api_s_dns_rec['virtual-DNS-record']['virtual_DNS_record_data']
            exp_vdns_rec_data = self.obj.get_virtual_DNS_record_data()
            for data in api_vdns_rec_data:
                if str(exp_vdns_rec_data.__dict__[data]) != str(api_vdns_rec_data.get(data)) :
                    result = result and False
                    msg = msg + '\nvdns '+ data + ' is not matching with api server DNS record data'
        except Exception as e:
            # Return false if we get an key error and for retry
            return False
        if msg != '':
            self.logger.info("VDNS record info is not matching with API Server data\n %s:",msg)
    ...
    def verify_vdns_rec_not_in_api_server(self):
        ...
        if self.api_s_inspect.get_cs_dns_rec(rec_name = self.vdns_record_name,vdns_name = self.parent.getObj().name,refresh=True) is not None:
            ...
            self.logger.info("VDNS record information %s removed from the API Server", self.obj.name)
            return True
        ...
        for cn in self.inputs.bgp_ips:
            ...
            if cn_s_dns:
               ...
               return False
                ...
                self.logger.info("VDNS record information %s removed in the Control node", self.obj.name)
                return True
