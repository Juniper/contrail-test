...
class VdnsFixture(fixtures.Fixture):
        ...
        for cn in self.inputs.bgp_ips:
            try:
                cn_s_dns = self.cn_inspect[cn].get_cn_vdns(
                    vdns=str(self.obj.name), domain = self.inputs.admin_domain )
    ...
    def verify_vdns_in_api_server(self):
        ...
        api_s_dns = self.api_s_inspect.get_cs_dns(
            vdns_name=str(self.obj.name), refresh=True, domain = self.inputs.admin_domain)
        print(api_s_dns)
        msg = ''
        try:
    ...
    def verify_vdns_not_in_api_server(self):
        ...
        if self.api_s_inspect.get_cs_dns(vdns_name=str(self.obj.name), refresh=True, domain = self.inputs.admin_domain) is not None:
