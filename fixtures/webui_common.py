...
class webui_common:
    ...
    def get_collectors_list_ops(self):
        url = 'http://' + self.inputs.collector_ip + \
            ':8081/analytics/uves/collectors'
        obj = self.jsondrv.load(url)
        return obj
    ...
    def get_bgp_routers_list_ops(self):
        url = 'http://' + self.inputs.collector_ip + \
            ':8081/analytics/uves/bgp-routers'
        obj = self.jsondrv.load(url)
        return obj
    ...
    def get_rows(self):
        return self.browser.find_element_by_class_name('k-grid-content').find_element_by_tag_name(
    ...
    def click_monitor_instances_basic_in_webui(self, row_index):
        ...
        self.wait_till_ajax_done()
        rows[row_index +
             1].find_element_by_class_name('k-detail-cell').find_elements_by_tag_name('li')[0].find_element_by_tag_name('a').click()
        self.wait_till_ajax_done()
    ...
    def click_monitor_vrouters_in_webui(self):
        ...
        mon_net_networks = WebDriverWait(self.browser, self.delay).until(
            lambda a: a.find_element_by_id('mon_infra_compute'))
    ...
    def click_monitor_common_advance_in_webui(self, row_index):
        ...
        self.wait_till_ajax_done()
        self.browser.find_element_by_class_name(
            'contrail').find_element_by_class_name('icon-code').click()
        self.wait_till_ajax_done()
    # end click_monitor_common_advance_in_webui
    ...
    def click_monitor_networks_advance_in_webui(self, row_index):
        ...
        self.wait_till_ajax_done()
        rows[row_index +
             1].find_element_by_class_name('k-detail-cell').find_elements_by_tag_name('li')[1].find_element_by_tag_name('a').click()
        self.wait_till_ajax_done()
    ...
    def get_expanded_api_data_in_webui(self, row_index):
        ...
        rows = self.browser.find_element_by_tag_name(
            'tbody').find_elements_by_tag_name('tr')
    ...
    def get_vm_basic_view(self):
        domArry = json.loads(self.browser.execute_script(
            "var eleList = $('td.k-detail-cell').find('div'),dataSet = []; for(var i = 0; i < eleList.length-1; i++){if(eleList[i].className == 'span2' && eleList[i + 1].className == 'span10'){dataSet.push({key : eleList[i].getElementsByTagName('label')[0].innerHTML,value:eleList[i+1].innerHTML});}} return JSON.stringify(dataSet);"))
        return domArry
    ...
    def match_ops_values_with_webui(self, complete_ops_data, webui_list):
        ...
        for ops_items in complete_ops_data:
            ...
            if not match_flag:
                self.logger.error(
                    "ops_key %s ops_value %s not found/matched in webui" %
    ...
    def match_ops_with_webui(self, complete_ops_data, merged_arry):
        ...
        self.logger.info("opserver data to be matched : %s" %
                         complete_ops_data)
        self.logger.info(self.dash)
        ...
        for i in range(len(complete_ops_data)):
            ...
            for j in range(len(merged_arry)):
                ...
                # self.logger.info("ops key [%s] : value [%s] webui key [%s] : value [%s]" %(
                # item_ops_key, item_ops_value, item_webui_key,
                # item_webui_value))
                ...
                # print only for testing##
                # elif item_ops_key == item_webui_key :
                #     self.logger.info("ops key %s : value %s not matched with webui key %s : value %s" %(
                # item_ops_key, item_ops_value, item_webui_key,
                # item_webui_value))
            if not matched_flag:
                ...
                if key_found_flag:
                    self.logger.error(
                        "ops key %s : value %s not matched in webui key-value pairs list %s" %
                        (item_ops_key, item_ops_value, webui_match_try_list))
                else:
                    self.logger.error(
                        "ops key %s : value %s not found in webui" %
                        (item_ops_key, item_ops_value))
                #self.browser.get_screenshot_as_file('ops_key_'+item_ops_key+'_value_'+ '-'.join(item_ops_value) + self.date_time_string()+'.png' )
                not_matched_count += 1
                ...
                no_error_flag = False
        self.logger.info("total ops key-value count is %s" %
                         (str(len(complete_ops_data))))
        self.logger.info("total ops key-value match is %s" %
                         (str(match_count)))
        self.logger.info("total ops key-value not matched count is %s" %
                         str(not_matched_count))
        self.logger.info("total ops key-value match skipped count is %s" %
                         str(skipped_count))
