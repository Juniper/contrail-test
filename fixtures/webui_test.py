...
class webui_test:
    ...
    def create_vn_in_webui(self, fixture):
        try:
            fixture.obj = fixture.quantum_fixture.get_vn_obj_if_present(
                fixture.vn_name, fixture.project_name)
            if not fixture.obj:
                ...
                self.browser.find_element_by_id(
                    'txtIPBlock').send_keys(fixture.vn_subnets)
                ...
                try:
                    if self.browser.find_element_by_id('infoWindow'):
                        error_header = self.browser.find_element_by_id(
                            'modal-header-title').text
                        error_text = self.browser.find_element_by_id(
                            'short-msg').text
                        self.logger.error('error occured : %s ' %
                                          (error_header))
                        self.logger.error(
                            'error occured while creating vn %s msg is %s ' %
                            (fixture.vn_name, error_text))
                        self.logger.info('Capturing screenshot of error msg .')
                        self.browser.get_screenshot_as_file(
                            'create_vn_error' + fixture.vn_name + self.webui_common.date_time_string() + '.png')
                        self.logger.info('Captured screenshot create_vn_error' +
                                         fixture.vn_name + self.webui_common.date_time_string() + '.png')
                        self.browser.find_element_by_id(
                            'infoWindowbtn0').click()
                        self.logger.info(
                            ' VN %s creation failed using webui .' %
                            (fixture.vn_name))
                except NoSuchElementException:
                    self.logger.info('created VN %s using webui .' %
                                     (fixture.vn_name))
                    pass
            else:
            ...
            fixture.vn_fq_name = ':'.join(
                fixture.obj['network']['contrail:fq_name'])
    ...
    def verify_analytics_nodes_ops_basic_data_in_webui(self):
        ...
        for n in range(len(analytics_nodes_list_ops)):
            ...
            if not match_flag:
                ...
                self.webui_common.click_monitor_analytics_nodes_basic_in_webui(
                    match_index)
                ...
                modified_ops_data.extend(
                    [{'key': 'Hostname', 'value': host_name}, {'key': 'Generators', 'value': generators_count},
                     {'key': 'IP Address', 'value': ip_address}, {'key': 'CPU', 'value': cpu}, {'key': 'Memory', 'value': memory}, {'key': 'Version', 'value': version}])
    ...
    def verify_config_nodes_ops_basic_data_in_webui(self):
        ...
        for n in range(len(config_nodes_list_ops)):
            ...
            if not match_flag:
                ...
                self.webui_common.click_monitor_config_nodes_basic_in_webui(
                    match_index)
                dom_basic_view = self.webui_common.get_basic_view_infra()
                ops_basic_data = []
                ...
                if not reduced_process_keys_dict:
                    ...
                    overall_node_status_string = [
                        'Down since ' + status for status in overall_node_status_down_time]
        ...
        for n in range(len(vrouters_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[0].text == ops_vrouter_name:
                    self.logger.info(
                        "vrouter name %s found in webui..going to match basic details now" % (ops_vrouter_name))
                    self.logger.info(self.dash)
                    match_index = i
                    match_flag = 1
                    break
            if not match_flag:
                ...
                self.webui_common.click_monitor_vrouters_basic_in_webui(
                    match_index)
                dom_basic_view = self.webui_common.get_basic_view_infra()
                # special handling for overall node status value
                node_status = self.browser.find_element_by_id('allItems').find_element_by_tag_name('p').get_attribute(
                ...
                if not reduced_process_keys_dict:
                    ...
                    overall_node_status_string = [
                        'Down since ' + status for status in overall_node_status_down_time]
                modified_ops_data.extend(
                    [{'key': 'Flow Count', 'value': flow_count_string}, {'key': 'Hostname', 'value': host_name}, {'key': 'IP Address', 'value': ip_address}, {'key': 'Networks', 'value': networks}, {'key': 'Instances', 'value': instances},
                     {'key': 'CPU', 'value': cpu}, {'key': 'Memory', 'value': memory}, {'key': 'Version', 'value': version}, {'key': 'vRouter Agent', 'value': contrail_vrouter_string}, {'key': 'Overall Node Status', 'value': overall_node_status_string}])
                self.webui_common.match_ops_with_webui(
                    modified_ops_data, dom_basic_view)
                if self.webui_common.match_ops_with_webui(modified_ops_data, dom_basic_view):
                    self.logger.info(
    ...
    def verify_vrouter_ops_advance_data_in_webui(self):
        ...
        for n in range(len(vrouters_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[0].text == ops_vrouter_name:
                    self.logger.info(
                        "vrouter name %s found in webui..going to match advance details now" % (ops_vrouter_name))
                    self.logger.info(self.dash)
                    match_index = i
                    match_flag = 1
                    break
    ...
    def verify_bgp_routers_ops_basic_data_in_webui(self):
        ...
        for n in range(len(bgp_routers_list_ops)):
            ...
            if not match_flag:
                ...
                self.webui_common.click_monitor_control_nodes_basic_in_webui(
                    match_index)
                dom_basic_view = self.webui_common.get_basic_view_infra()
                # special handling for overall node status value
                ...
                modified_ops_data = []
                modified_ops_data.extend(
                    [{'key': 'Peers', 'value': bgp_peers_string}, {'key': 'Hostname', 'value': host_name}, {'key': 'IP Address',
                                                                                                            'value': ip_address}, {'key': 'CPU', 'value': cpu}, {'key': 'Memory', 'value': memory}, {'key': 'Version', 'value': version}])
                self.webui_common.match_ops_with_webui(
                    modified_ops_data, dom_basic_view)
                if self.webui_common.match_ops_with_webui(modified_ops_data, dom_basic_view):
                    self.logger.info(
        ...
        for n in range(len(bgp_routers_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[0].text == ops_bgp_router_name:
                    self.logger.info(
                        "bgp router name %s found in webui..going to match advance details now" % (ops_bgp_router_name))
                    match_flag = 1
                    match_index = i
                    break
            if not match_flag:
                ...
                self.logger.info(
                    ...
                    (ops_bgp_router_name))
                self.webui_common.click_monitor_control_nodes_advance_in_webui(
                    match_index)
                dom_arry = self.webui_common.parse_advanced_view()
                dom_arry_str = self.webui_common.get_advanced_view_str()
                dom_arry_num = self.webui_common.get_advanced_view_num()
    ...
    def verify_analytics_nodes_ops_advance_data_in_webui(self):
        ...
        for n in range(len(analytics_nodes_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[0].text == ops_analytics_node_name:
                    self.logger.info(
                        "analytics node name %s found in webui..going to match advance details now" %
                        (ops_analytics_node_name))
                    match_flag = 1
                    match_index = i
                    break
            if not match_flag:
                ...
                self.logger.info(
                    ...
                    (ops_analytics_node_name))
                self.webui_common.click_monitor_analytics_nodes_advance_in_webui(
                    match_index)
                analytics_nodes_ops_data = self.webui_common.get_details(
                    analytics_nodes_list_ops[n]['href'])
                dom_arry = self.webui_common.parse_advanced_view()
        ...
        for k in range(len(vm_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[2].text == ops_uuid:
                    self.logger.info(
                        "vm uuid %s matched in webui..going to match basic view details now" % (ops_uuid))
                    self.logger.info(self.dash)
                    match_index = i
                    match_flag = 1
            ...
            if not match_flag:
                ...
                self.webui_common.click_monitor_instances_basic_in_webui(
                    match_index)
                self.logger.info(
                ...
                if self.webui_common.match_ops_values_with_webui(complete_ops_data, dom_arry_intf):
                    self.logger.info(
                        "ops uves virtual instaces basic view data matched in webui")
                    return True
                else:
                    self.logger.error(
                        "ops uves virtual instances basic view data match failed in webui")
                    return False
        ...
        for k in range(len(vn_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[1].text == ops_fq_name:
                    self.logger.info(
                        "vn fq_name %s matched in webui..going to match basic view details now" % (ops_fq_name))
                    self.logger.info(self.dash)
                    match_index = i
                    match_flag = 1
                    vn_fq_name = rows[
                        i].find_elements_by_tag_name('td')[1].text
                    break
            if not match_flag:
                self.logger.error(
                ...
                self.webui_common.click_monitor_networks_basic_in_webui(
                    match_index)
                self.logger.info(
                ...
                complete_ops_data.extend(
                    [ops_data_ingress, ops_data_egress, ops_data_acl_rules, ops_data_interfaces_count, ops_data_vrf])
    ...
    def verify_config_nodes_ops_advance_data_in_webui(self):
        ...
        for n in range(len(config_nodes_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[0].text == ops_config_node_name:
                    self.logger.info(
                        "config node name %s found in webui..going to match advance view details now" % (ops_config_node_name))
                    match_flag = 1
                    match_index = i
                    break
            if not match_flag:
                ...
                self.logger.info(
                    ...
                    (ops_config_node_name))
                self.webui_common.click_monitor_config_nodes_advance_in_webui(
                    match_index)
                config_nodes_ops_data = self.webui_common.get_details(
                    config_nodes_list_ops[n]['href'])
                dom_arry = self.webui_common.parse_advanced_view()
        ...
        for n in range(len(vn_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[1].text == ops_fqname:
                    self.logger.info(
                        "vn fq name %s found in webui..going to match advance view details now" % (ops_fqname))
                    self.logger.info(self.dash)
                    match_index = i
                    match_flag = 1
                    break
            if not match_flag:
                ...
                self.webui_common.click_monitor_networks_advance_in_webui(
                    match_index)
                vn_ops_data = self.webui_common.get_details(
                    vn_list_ops[n]['href'])
                dom_arry = self.webui_common.parse_advanced_view()
    ...
    def verify_vm_ops_advance_data_in_webui(self):
        ...
        for k in range(len(vm_list_ops)):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('td')[2].text == ops_uuid:
                    self.logger.info(
                        "vm uuid %s matched in webui..going to match advance view details now" % (ops_uuid))
                    self.logger.info(self.dash)
                    match_index = i
                    match_flag = 1
                    break
            if not match_flag:
                ...
                self.webui_common.click_monitor_instances_advance_in_webui(
                    match_index)
                self.logger.info(
    ...
    def verify_vn_api_data_in_webui(self):
        ...
        for i in range(ln):
            ...
            self.logger.info(
                "VN details for %s got from API server and going to match in webui : " % (vn_list[i]))
            j = 0
            for j in range(len(rows)):
                self.webui_common.click_configure_networks_in_webui()
                self.browser.get_screenshot_as_file(
                    'config_net_verify_api' + self.webui_common.date_time_string() + '.png')
                ...
                if (rows[j].find_elements_by_tag_name('td')[2].get_attribute('innerHTML') == details['virtual-network']['fq_name'][2]):
                    ...
                    if rows[j].find_elements_by_tag_name('td')[4].text == ip_block:
                        self.logger.info("VN %s : ip block matched" %
                                         (vn_name))
                    ...
                    if (ui_ip_block.split(' ')[0] == ':'.join(details['virtual-network']['network_ipam_refs'][0]['to']) and ui_ip_block.split(
                        ...
                        self.logger.info(
                            "VN %s basic details in webui network config page matched with api data " % (vn_name))
                    else:
                        self.logger.error(
                            "VN %s basic details in webui network config page not matched with api data" % (vn_name))
                        self.browser.get_screenshot_as_file(
                            'verify_vn_api_data_webui_basic_details_failed' + self.webui_common.date_time_string() + '.png')
                    ...
                    if network_dict[details['virtual-network']['virtual_network_properties']['forwarding_mode']] == forwarding_mode:
                        self.logger.info(" VN %s : forwarding mode matched " %
                                         (vn_name))
                    else:
                        self.logger.error(
                            "VN %s : forwarding mode not matched" % (vn_name))
                        self.browser.get_screenshot_as_file(
                            'verify_vn_api_data_forwarding_mode_match_failed' + self.webui_common.date_time_string() + '.png')
                    ...
                    if vxlan_api == vxlan:
                        self.logger.info(" VN %s : vxlan matched " %
                                         (vn_name))
                    else:
                        self.logger.error("VN %s : vxlan not matched" %
                                          (vn_name))
                        self.browser.get_screenshot_as_file(
                            'verify_vn_api_basic_data_vxlan_failed.png')
                    xpath = "//label[contains(text(), 'Floating IP Pools')]"
                    driver = rows[j + 1]

                    if details['virtual-network'].has_key('floating_ip_pools'):
                        floating_ip_length_api = len(
                            details['virtual-network']['floating_ip_pools'])
                        self.logger.info(
                            " %s FIP/s exist in api for network %s " %
                            (floating_ip_length_api, vn_name))

                        if self.webui_common.check_element_exists_by_xpath(driver, xpath):
                            fip_ui = rows[
                                j + 1].find_element_by_xpath("//label[contains(text(), 'Floating IP Pools')]/..").text.split('\n')[1:]
                            for n in range(floating_ip_length_api):
                                fip_api = details[
                                    'virtual-network']['floating_ip_pools'][n]['to']
                                if fip_ui[n] == fip_api[3] + ' (' + fip_api[0] + ':' + fip_api[1] + ')':
                                    self.logger.info(
                                        " %s FIP/s matched in webui with api data " % (fip_api))
                        else:
                            self.logger.error(
                                "fip element mismatch happened in webui and api ")
                            self.browser.get_screenshot_as_file(
                                'verify_vn_monitor_page.png')
                    else:
                        self.logger.info(
                            "Not verifying FIP as it is not found in API ")
                    rows[j].find_elements_by_tag_name(
                        'td')[0].find_element_by_tag_name('a').click()
                    break

                elif (j == range(len(rows))):
                    self.logger.info("%s is not matched in webui" %
                                     (details['virtual-network']['fq_name'][2]))
            ...
            for j in range(len(rows)):
                ...
                if(fq_name == vn_list[i]['name']):
                    ...
                    self.logger.error(" %s VN not found in monitor page " %
                                      (fq_name))
            details = self.webui_common.get_vn_details_api(
                vn_list[i]['href'])
    ...
    def verify_vn_in_webui(self, fixture):
        ...
        if self.webui_common.verify_uuid_table(fixture.vn_id):
            self.logger.info("VN %s UUID verified in webui table " %
                             (fixture.vn_name))
        else:
            self.logger.error(
                "VN %s UUID Verification failed in webui table " %
                (fixture.vn_name))
            self.browser.get_screenshot_as_file(
                'verify_vn_configure_page_ip_block.png')
        fixture.obj = fixture.quantum_fixture.get_vn_obj_if_present(
        ...
        if self.webui_common.verify_fq_name_table(full_fq_name, fq_type):
            self.logger.info("fq_name %s found in fq Table for %s VN" %
                             (fixture.vn_fq_name, fixture.vn_name))
        else:
            self.logger.error("fq_name %s failed in fq Table for %s VN" %
                              (fixture.vn_fq_name, fixture.vn_name))
            self.browser.get_screenshot_as_file(
                'setting_page_configure_fq_name_error.png')
        return True
        ...
        try:
            if not self.proj_check_flag:
                ...
                if not current_project == fixture.project_name:
                    ...
                    WebDriverWait(self.browser_openstack,
                                  self.delay).until(ajax_complete)
                ...
                if(rows_os[i].find_elements_by_tag_name('td')[1].text == fixture.vm_name):
                    ...
                    while not vm_active:
                        ...
                        if(vm_active_status1 == 'Active' or vm_active_status2 == 'Active'):
                            self.logger.info(
                                "%s status is Active now in openstack" % (fixture.vm_name))
                            vm_active = True
                            time.sleep(5)
        ...
        try:
            ...
            for i in range(len(rows)):
                ...
                if(vm_name == fixture.vm_name and fixture.vm_obj.id == vm_uuid and fixture.vn_name == vm_vn):
                    self.logger.info(
                        "VM %s vm found now will verify basic details" %
                        (fixture.vm_name))
                    retry_count = 0
                    while True:
            ...
            for i in range(len(rows)):
                if(rows[i].find_elements_by_tag_name('a')[1].text == fixture.vn_fq_name.split(':')[0] + ":" + fixture.project_name + ":" + fixture.vn_name):
                    ...
                    rows = self.webui_common.get_rows()
                    vm_ids = rows[
                        i + 1].find_element_by_xpath("//div[contains(@id, 'basicDetails')]").find_elements_by_tag_name('div')[15].text
            ...
            if self.webui_common.verify_uuid_table(fixture.vm_id):
                self.logger.info("UUID %s found in UUID Table for %s VM" %
                                 (fixture.vm_name, fixture.vm_id))
            else:
                self.logger.error("UUID %s failed in UUID Table for %s VM" %
                                  (fixture.vm_name, fixture.vm_id))
            fq_type = 'virtual_machine'
            full_fq_name = fixture.vm_id + ":" + fixture.vm_id
            if self.webui_common.verify_fq_name_table(full_fq_name, fq_type):
                self.logger.info("fq_name %s found in fq Table for %s VM" %
                                 (fixture.vm_id, fixture.vm_name))
            else:
                self.logger.error("fq_name %s failed in fq Table for %s VM" %
                                  (fixture.vm_id, fixture.vm_name))
    ...
    def create_floatingip_pool_webui(self, fixture, pool_name, vn_name):
        try:
            self.webui_common.click_configure_networks_in_webui()
            rows = WebDriverWait(self.browser, self.delay).until(
                lambda a: a.find_element_by_id('gridVN'))
            rows = WebDriverWait(rows, self.delay).until(
                lambda a: a.find_element_by_tag_name('tbody'))
            rows = WebDriverWait(rows, self.delay).until(
                lambda a: a.find_elements_by_tag_name('tr'))
            for net in rows:
                if (net.find_elements_by_tag_name('td')[2].get_attribute('innerHTML') == fixture.vn_name):
                    ...
                    pool_con.find_element_by_class_name(
                        'k-multiselect-wrap').click()
                    self.webui_common.wait_till_ajax_done()
                    ip_ul = self.browser.find_element_by_xpath(
    ...
    def create_and_assoc_fip_webui(self, fixture, fip_pool_vn_id, vm_id, vm_name, project=None):
        try:
            ...
            for net in rows:
                if (net.find_elements_by_tag_name('td')[2].get_attribute('innerHTML') == fixture.vn_name):
                    ...
                    fip = self.browser.find_element_by_id(
                        "ddFipPool_listbox").find_elements_by_tag_name('li')
                    ...
                    self.webui_common.wait_till_ajax_done()
                    fip = self.browser.find_element_by_id(
                        "ddAssociate_listbox").find_elements_by_tag_name('li')
                    for i in range(len(fip)):
            ...
            self.logger.info(
                "Error while creating floating ip and associating it to a VM Test.")
