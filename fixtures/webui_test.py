            ...
            if not fixture.policy_obj:
                ...
                for index, rule in enumerate(fixture.rules_list):
                    ...
                    textbox_rule_items = rules.find_elements_by_class_name(
                        "span1") 
                    textbox_rule_items[0].find_element_by_tag_name('input').clear()
                    textbox_rule_items[2].find_element_by_tag_name('input').clear()
                    textbox_rule_items[0].find_element_by_tag_name('input').send_keys(src_port) 
                    textbox_rule_items[2].find_element_by_tag_name('input').send_keys(dst_port)
        ...
        for vns in range(len(vn_list_api['virtual-networks'])):
            ...
            for i in range(len(rows)):
                ...
                if rows[i].find_elements_by_tag_name('div')[2].text == api_fq_name:
                    ...
                    dom_arry_basic.append(
                        {'key': 'admin_state_grid_row', 'value': rows[i].find_elements_by_tag_name('div')[5].text})
                    break
            if not match_flag:
                self.logger.error(
                    ...
                    for ip in range(len(api_data_basic['network_ipam_refs'])):
                        ...
                        for ip_sub in range(len_ipams):
                            ...
                            if dhcp_api:
                                ...
                                dhcp_api = 'Disabled'
                            cidr_string = net_ipam_refs['attr']['ipam_subnets'][ip_sub]['subnet']['ip_prefix'] + '/' + str(net_ipam_refs['attr']['ipam_subnets'][ip_sub]['subnet']['ip_prefix_len']) + ':' + net_ipam_refs['attr']['ipam_subnets'][ip_sub]['default_gateway']
                ...
                if api_data_basic.has_key('floating_ip_pools'):
                    for fip in range(len(api_data_basic['floating_ip_pools'])):
                        ...
                        fip_string = fip_api[3] + ' (' + fip_api[0] + ':' + fip_api[1] + ' )'
