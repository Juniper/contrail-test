...
class HeatFixture(fixtures.Fixture):
    ...
    def __init__(
        ...
        self.heat_url = 'http://%s:%s/v1/%s' % (
            self.cfgm_ip, self.heat_port, self.project_id)
    ...
    def create_stack(self, stack_name, template, env):
        ...
        self.logger.info('Creating Stack %s' % stack_name)
        return stack_obj
    # end create_stack

    def delete_stack(self, stack_name):
        self.logger.info('Deleting Stack %s' % stack_name)
    ...
    def wait_till_stack_created(self):
        for stack in self.obj.stacks.list():
            if stack.stack_status == 'CREATE_COMPLETE':
                return True
            else:
                self.logger.info('Stack is in %s status.Retrying...'%stack.stack_status)
