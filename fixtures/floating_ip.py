...
class FloatingIPFixture(fixtures.Fixture):
    ...
    def is_fip_pool_present(self, pool_name):
        ...
        self.pub_vn_name= self.pub_vn_obj.name
        try: 
            fip_pool_dict = self.vnc_lib_h.floating_ip_pools_list( parent_id = self.vn_id )
            #Bug 532
    ...
    def verify_fip_pool_in_control_node(self):
        ...
        for cn in self.inputs.bgp_ips:
            cn_object= self.cn_inspect[cn].get_cn_config_fip_pool( vn_name= self.pub_vn_name, fip_pool_name = self.pool_name , project= self.project_name )
            if not cn_object:
    ...
    def create_and_assoc_fip(self, fip_pool_vn_id, vm_id , project = None): 
        ...
        try:
            ...
            self.logger.error('Failed to create or asscociate FIP. Error: %s' %(sys.exc_info()[0]))
            return None   
    #end create_and_assoc_fip
    
    def verify_fip(self, fip_id, vm_fixture, fip_vn_fixture):
        result= True
        fip= self.quantum_fixture.get_floatingip(fip_id)['floatingip']['floating_ip_address']
        self.fip[fip_id]= fip
    ...
    def verify_no_fip( self, fip_id, fip_vn_fixture):
        result= True
        fip= self.fip[fip_id]
    ...
    def verify_fip_in_control_node(self, fip, vm_fixture, fip_vn_fixture):
        for cn in self.inputs.bgp_ips:
            ...
            cn_routes=self.cn_inspect[cn].get_cn_route_table_entry(ri_name= fip_vn_fixture.ri_name, prefix= fip+'/32') 
            if not cn_routes :
                self.logger.warn('No route found for %s in Control-node %s ' %(fip, cn ) )
                return False
            if cn_routes[0]['next_hop'] != vm_fixture.vm_node_data_ip : 
                self.logger.warn( 'Expected next-hop for %s in Control-node %s : %s, Found : %s' %( fip, cn,vm_node_data_ip, cn_routes[0]['next_hop'] ) )
                return False
    ...
    def verify_fip_in_agent(self, fip, vm_fixture, fip_vn_fixture):
        for compute_ip in self.inputs.compute_ips:
            ...
            if vn is None:
                continue
            agent_vrf_objs= inspect_h.get_vna_vrf_objs(vn_name= fip_vn_fixture.vn_name)
    ...
    def verify_fip_in_uve(self, fip, vm_fixture, fip_vn_fixture):
        ...
        found_vn = 0
        result = False
        vm_intf=self.analytics_obj.get_ops_vm_uve_interface(collector=self.inputs.collector_ip , uuid=vm_fixture.vm_id)
        for item in vm_intf:
    ...
    def create_floatingips(self, fip_pool_vn_id, count= 1):
        ...
        fip_resp = self.quantum_fixture.list_floatingips(tenant_id = self.project_obj.uuid)
        return fip_resp
    ...
    def create_floatingip(self, fip_pool_vn_id, project_obj = None):
        ...
        fip_resp = self.quantum_fixture.create_floatingip(fip_pool_vn_id, project_obj.uuid)
        return fip_resp
    #end create_floatingip
    ...
    def assoc_floatingip(self, fip_id, vm_id ):
        ...
        if update_dict['port_id'] :
            fip_resp= self.quantum_fixture.update_floatingip(fip_id, {'floatingip': update_dict})
            return fip_resp
        else:
            return None
    ...
    def assoc_project(self, fip_fixture , project , domain = 'default-domain') :
        ...
        project_fq_name= ['default-domain', project]
        self.new_project_obj = fip_fixture.vnc_lib_h.project_read(fq_name = project_fq_name)
        

        # Associate project with floating IP pool 
        result = self.new_project_obj.add_floating_ip_pool( fip_fixture.fip_pool_obj )
        fip_fixture.vnc_lib_h.project_update( self.new_project_obj )
        self.new_project_obj = fip_fixture.vnc_lib_h.project_read(fq_name = project_fq_name)
        return self.new_project_obj
    #end assoc_project

    def deassoc_project(self, fip_fixture , project , domain = 'default-domain') :
        result = True
        self.logger.info( 'De-associting Floting IP with project %s' %(project))
        ...
        result = self.new_project_obj.del_floating_ip_pool( fip_fixture.fip_pool_obj )
        fip_fixture.vnc_lib_h.project_update( self.new_project_obj )
        self.new_project_obj = fip_fixture.vnc_lib_h.project_read(fq_name = project_fq_name)
        return self.new_project_obj
    #end assoc_project 
