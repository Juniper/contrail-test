...
class OpenstackOrchestrator(ContrailApi):
   ...
   def get_vm_by_id(self, vm_id, **kwargs):
       return self.nova_h.get_vm_by_id(vm_id, **kwargs)
   ...
   def assoc_floating_ip(self, fip_id, vm_id, option='orch', **kwargs):
       ...
       self._log.debug('Associating FIP ID %s with Port ID %s' %(fip_id,
                          update_dict['port_id']))
   ...
   def disassoc_floating_ip(self, fip_id, option='orch', **kwargs):
       ...
       self._log.debug('Disassociating port from FIP ID : %s' %(fip_id))
       fip_resp = self.quantum_h.update_floatingip(fip_id,
                       {'floatingip': update_dict})
       return fip_resp
   ...
   def create_security_group(self, sg_name, project_obj, sg_entries, option='orch', **kwargs):
       if option == 'contrail':
           return super(OpenstackOrchestrator, self).create_security_group(sg_name=sg_name,
                                project_obj=project_obj, sg_entries=sg_entries, **kwargs)
       sg = self.quantum_h.create_security_group(sg_name)
       if not sg:
           self._log.error("security group creation failed through quantum")
           return False
       ...
       for rule in secgrp_rules:
           ...
           if rule['src_addresses'][0].has_key('security_group'):
               if rule['src_addresses'][0]['security_group'] == 'local':
                   ...
                   if rule['dst_addresses'][0]['security_group'] != None:
                       remote_group_id = self.get_security_group(sg_id=rule['src_addresses'][0]['security_group']).uuid
           if rule['dst_addresses'][0].has_key('security_group'):
               if rule['dst_addresses'][0]['security_group'] == 'local':
                   ...
                   if rule['dst_addresses'][0]['security_group'] != None:
                      remote_group_id = self.get_security_group(sg_id=rule['dst_addresses'][0]['security_group']).uuid
           ...
           if direction == 'ingress':
               try:
                   ...
                   self._log.error("error while creating sg rule through quantum")
                   return False
           if direction == 'egress':
               try:
