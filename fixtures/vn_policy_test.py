           ...
           if self.policy_obj[self.vn] :
	         ...
	         self.logger.info ('Associated Policy:%s to %s'%(policy_fq_names,self.vn))
                   ...
                   for conf_policy in self.policy_obj[self.vn]:
                       self.vn_obj[self.vn].add_network_policy(conf_policy,vnc_api.VirtualNetworkPolicyType(sequence=vnc_api.SequenceType(major=0, minor=0)))
                   vn_update_rsp = self.vnc_lib.virtual_network_update(self.vn_obj[self.vn]._obj) 
           ...
           if policy_of_vn :
              if self.option == 'openstack' :
                 ...
                 self.logger.info ('Detached Policy:%s from %s'%(policy_fq_names,self.vn))
                   ...
                   vn_update_rsp = None
                   for conf_policy in self.policy_obj[self.vn]:
                       self.vn_obj[self.vn]._obj.del_network_policy(conf_policy)
                   vn_update_rsp = self.vnc_lib.virtual_network_update(self.vn_obj[self.vn]._obj)
