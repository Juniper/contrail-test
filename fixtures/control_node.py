...
class CNFixture(fixtures.Fixture ):
    ...
    def setUp(self):
        ...
        if not self.is_cn_present(self.router_ip):
           ...
           self.logger.info('Creating Peering in control node with ip %s' %(self.router_ip))
        else :
           self.logger.info('Control nodel %s already present, not creating it' %(self.router_ip))
    ...
    def create_cn_node (self,router_ip, router_type):
        ...
        router_params = BgpRouterParams(vendor = router_type, autonomous_system = int(self.router_asn),
    ...
    def verify_on_setup(self):
        ...
        if not self.verify_peer_in_control_nodes():
            ...
            self.logger.error( "Either Control node %s does not have any BGP peer or is not in Established state" %(self.router_ip))
    ...
    def is_cn_present(self,router_ip):
        ...
        result = False
        present_router_list=[]
        rt_inst_obj = self.vnc_lib_h.routing_instance_read(fq_name = ['default-domain', 'default-project',
                                                               'ip-fabric', '__default__'])
        fq_name = rt_inst_obj.get_fq_name()
        bgp_router_list = self.vnc_lib_h.bgp_routers_list(parent_fq_name = fq_name)
        bgp_router_ids = [bgp_dict['uuid'] for bgp_dict in bgp_router_list['bgp-routers']]
        for id in bgp_router_ids:
            present_router_list.append(self.vnc_lib_h.bgp_router_read(id = id).bgp_router_parameters.address)
    ...
    def verify_peer_in_control_nodes(self):
        ...
        for entry1 in self.inputs.bgp_ips:
            cn_bgp_entry=self.cn_inspect[entry1].get_cn_bgp_neigh_entry(encoding='BGP')
            if not cn_bgp_entry:
                result = False
                self.logger.error('Control Node %s does not have any BGP Peer' %(self.router_ip))
            else:
                for entry in cn_bgp_entry:
                    if entry ['state'] != 'Established':
                        ...
                        self.logger.info('With Peer %s peering is Current State is %s ' %(entry ['peer'] , entry['state']) )
        return result 
    ...
    def restart_control_node(self, host_ips= []):
        ...
        for host in host_ips:
            ...
            self.logger.info('Restarting %s.service in %s' %(service_name,self.inputs.host_data[host]['name']))
    ...
    def stop_control_node(self, host_ips= []):
        ...
        for host in host_ips:
            username= self.inputs.host_data[host]['username']
            password= self.inputs.host_data[host]['password']
            self.logger.info('Stoping %s.service in %s' %(service_name,self.inputs.host_data[host]['name']))
    ...
    def start_control_node(self, host_ips= []):
        ...
        for host in host_ips:
            username= self.inputs.host_data[host]['username']
            password= self.inputs.host_data[host]['password']
            self.logger.info('Starting %s.service in %s' %(service_name,self.inputs.host_data[host]['name']))
    ...
    def cleanUp(self):
        ...
        if do_cleanup :
            ...
            self.logger.info( "Deleting the Control Node %s " %(self.router_ip))
            #TODO Add verification after cleanup here
        else:
            self.logger.info( 'Skipping the deletion of the Control Node %s ' %(self.router_ip) )
    #end cleanUp 
