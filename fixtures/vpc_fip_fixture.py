...
#        self.project_name = project_name
    ...
    def setUp(self):
        ...
        self.c_fip_fixture = self.useFixture(FloatingIPFixture(project_name= self.fip_vn_fixture.project_name,
    ...
    def associate_floating_ip(self, fip_allocation_id, instance_id):
        out = self.ec2_base._shell_with_ec2_env('euca-associate-address -a %s %s' % (fip_allocation_id,
                                                              instance_id), True).split('\t')
        if out:
            fip_allocation_id = out[1]
            self.logger.info('Associated Floating IP (Assoc ID : %s) to Instance %s' % (fip_allocation_id,
                                                                          instance_id))
            return True
        else: 
            return False
    ...
    def create_and_assoc_fip(self, instance_id):
        ...
        if not self.associate_floating_ip(fip_alloc_id, instance_id) :
            self.logger.error('Error while applying FIP to instance %s' % (instance_id))
            return (None,None)
    ...
    def release_floating_ip(self,fip_allocation_id, fip):
        out = self.ec2_base._shell_with_ec2_env('euca-release-address %s' % fip_allocation_id, True)
        if out:
            self.logger.info('Floating IP (Alloc ID %s) %s released' % (fip, fip_allocation_id))
            return True
        else: 
            return False
    ...
    def verify_fip(self, floating_ip):
        out =  self.ec2_base._shell_with_ec2_env('euca-describe-addresses --filter domain=vpc| grep %s' %(floating_ip), True).split('\n')
        ...
        for ip in out:
            ...
            if ip[0] == floating_ip:
                ...
                if ip[2].split('-')[0] == 'eipalloc' :
                    ...
                    self.logger.warn('Floating IP allocation or association id problem')
                break

        if not foundIp:
            self.logger.warn('Floating IP allocation or association id verification failed')
        return foundIp
    # end verify_fip        
    ...
    def cleanUp(self):
        if self.already_present:
            self.logger.debug('VM was not created by this fixture..Skipping deletion')
