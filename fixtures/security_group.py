...
class SecurityGroupFixture(ContrailFixture):
    def __init__(self, inputs, connections, domain_name, project_name, secgrp_name,
    ...
    def replace_rules(self, rules):
        ...
        self.logger.info("Replace all the rules of this security group %s with the new rules" % self.secgrp_name)
    ...
    def verify_secgrp_in_api_server(self):
        ...
        if not self.api_s_secgrp_obj:
            ...
            self.logger.info("Security group %s found in the API Server", self.secgrp_name)
            return True, None

    def verify_on_setup(self):
        try:
            secgrp = self.vnc_lib_h.security_group_read(fq_name=self.secgrp_fq_name)
            self.logger.debug("Security group: %s created succesfully", self.secgrp_fq_name)
        except NoIdError:
    ...
    def verify_secgrp_not_in_api_server(self):
        ...
        if self.api_s_secgrp_obj:
            ...
            self.logger.info("Security group %s removed from the API Server", self.secgrp_name)
            return True, None
    ...
    def verify_on_cleanup(self):
        try:
            ...
            self.logger.info("Security group: %s deleted successfully." % self.secgrp_fq_name)
