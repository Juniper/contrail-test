...
class ContrailVncApi(object):
    ...
    def create_service_appliance_set(self, sas_name=None, virtualization_type='physical-device'):
        gsc_obj = self._vnc.global_system_config_read(
            fq_name=['default-global-system-config'])
        sas_name = get_random_name('pnf_sas')
    ...
    def create_service_appliance(self, sas_id, pnf_left_attachment_point, pnf_right_attachment_point, pnf_left_intf, pnf_right_intf, virtualization_type='physical-device'):
        default_gsc_name = 'default-global-system-config'
        gsc_obj = self._vnc.global_system_config_read(
            fq_name=[default_gsc_name])
        sa_name = get_random_name('pnf_sa')
        sa_fq_name = [default_gsc_name, sa_name]
        sas_obj = self._vnc.service_appliance_set_read(
            id=sas_id)
        sa_obj = ServiceAppliance(sa_name, sas_obj)
        kvp_array = []
        kvp = KeyValuePair("left-attachment-point",
                           default_gsc_name + ':' + pnf_left_attachment_point)
        kvp_array.append(kvp)
        kvp = KeyValuePair("right-attachment-point",
                           default_gsc_name + ':' + pnf_right_attachment_point)
        kvp_array.append(kvp)
        kvps = KeyValuePairs()
    ...
    def create_port_tuple(self, si_id, left_lr, right_lr):
