...
class SwiftHelper(object):
    ...
    def create_image(self, image_name):
        ...
        try:
            ...
            sha1 = self.get_sha1(exact_path_to_file)
            md5 = self.get_md5(exact_path_to_file)
            self.download_image(image_name)
            with open(exact_path_to_file, 'rb') as file_to_upload:
                self.obj.put_object(container=self.container_name, obj=actual_name_of_file, contents=file_to_upload, headers={
                                    'X-Object-Meta-md5': md5, 'X-Object-Meta-sha1': sha1})
        except:
            self.logger.info(
                'UPLOAD FOR FILE %s TO SWIFT FAILED' % (file_name))
    # end create_image

    def delete_image(self, file_name):
        try:
            self.obj.delete_object(
                container=self.container_name, obj=file_name)
        except:
            self.logger.info(
                'DELETION OF FILE %s FROM SWIFT CONTAINER FAILED' % (file_name))
    # end delete_image
    ...
    def download_image(self, image_name):
        ...
        actual_name_of_file = image_info['name']
        full_path = 'http://%s/%s/%s' % (webserver,
                                         location, actual_name_of_file)
        ...
        try:
            ...
            self.logger.exception(
                'Errors while executing the following command: %s\n' % (command))
            self.logger.exception('%s' % (stderr))
            raise
    # end download_image
