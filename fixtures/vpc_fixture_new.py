...
class VPCFixture(fixtures.Fixture):
    ...
    def _get_admin_role_id(self):
        roles = self.run_cmd_on_os_node('(source /etc/contrail/keystonerc; keystone role-get admin)', 
                       ).split('\n')
    ...
    def _get_tenant_id(self, tenantName):
        tenants = self.run_cmd_on_os_node('(source /etc/contrail/openstackrc; keystone tenant-get %s)' % tenantName, ).split('\n')
    ...
    def _add_admin_role_to_tenant(self):
        ...
        if not tenantId:
            ...
            return False
        self.run_cmd_on_os_node('(source /etc/contrail/keystonerc ; keystone user-role-add --user %s --role %s --tenant %s)' % (userId, roleId, tenantId))
    ...
    def verify_vpc(self):
        if not self.vpc_id:
            ...
            return False
        verify_vpc_output = self.ec2_base._shell_with_ec2_env('euca-describe-vpcs |grep %s' % (self.vpc_id), True).split('\n')[0].split(' ')
    ...
    def terminate_instance(self):
        out = self.ec2_base._shell_with_ec2_env('euca-terminate-instances %s' % (self.instance_id), True).split('\t')
    ...
    def allocate_floating_ip(self):
        ...
        out = self.ec2_base._shell_with_ec2_env('euca-allocate-address -d vpc', True).split('\t')
        if out:
    ...
    def release_floating_ip(self):
        out = self.ec2_base._shell_with_ec2_env('euca-release-address %s' % self.fip_allcation_id, True)
        if out:
    ...
    def verify_floating_ip(self):
        out =  self.ec2_base._shell_with_ec2_env('euca-describe-addresses', True).split('\n')
        foundIp = False
    ...
    def associate_floating_ip(self):
        ...
        if out:
            ...
            self.logger.info('Associate Floating IP %s to Instance %s' % (self.floating_ip, self.instance_id))
            return True
    ...
    def disassociate_floating_ip(self):
        out = self.ec2_base._shell_with_ec2_env('euca-disassociate-address %s' % ( self.fip_allcation_id),True)
        if out == 'True':
            self.floating_ip_association = False
            self.fip_allcation_id.replace('eipassoc', 'eipalloc')
            self.logger.info('Floating IP %s disassociated from instance %s' % (self.floating_ip, self.instance_id))
            return True
        else: return False
    ...
    def create_acl_rule(self, acl_id, rule):
        ...
        acl_keys = ['protocol','action','cidr','fromPort','toPort','direction']
    ...
    def verify_route_table(self, rtb_id ):
        out = self.ec2_base._shell_with_ec2_env('euca-describe-route-tables %s' %(rtb_id), True).split('\n')
        found_rtb = False
...
True
    ...
    def create_security_group(self, sg_name):
        out = self.ec2_base._shell_with_ec2_env('euca-create-security-group -d sanity_test_group -v %s %s' % (self.vpc_id, sg_name), True).split('\t')
    ...
    def verify_security_group(self, sg_name ):
        out = self.ec2_base._shell_with_ec2_env('euca-describe-group', True).split('\n')
        foundGroup = False
