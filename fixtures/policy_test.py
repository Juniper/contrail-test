...
class PolicyFixture(fixtures.Fixture ):
    ...
    def setUp(self):
        ...
        self.policy_fq_name= self.quantum_fixture.get_policy_fq_name( self.policy_obj)
    #end setUp
    ...
    def _create_policy(self, policy_name, rules_list):
        ...
        for rule_dict in rules_list:
            ...
            #end for
            new_rule['action_list']['simple_action']= rule_dict['simple_action']                        
            #Format Source ports
            if 'src_ports' in rule_dict : 
                if  type(rule_dict['src_ports']) is tuple or type(rule_dict['src_ports']) is list :
                    new_rule['src_ports']= [PortType( rule_dict['src_ports'][0], rule_dict['src_ports'][1])]
            ...
            if 'dst_ports' in rule_dict:
                if 'dst_ports' in rule_dict and type(rule_dict['dst_ports']) is tuple or type(rule_dict['dst_ports']) is list :
                    new_rule['dst_ports']= [PortType( rule_dict['dst_ports'][0], rule_dict['dst_ports'][1])]
            ...
            new_rule[ 'source_network' ]= [AddressType(virtual_network = source_vn)] 
            new_rule[ 'dest_network' ]= [AddressType(virtual_network = dest_vn)] 
    ...
    def tx_user_def_rule_to_aces(self, test_vn, rules):
        ...
        translator= {'direction': 'direction', 'simple_action': 'simple_action', 
        ...
        for rule in user_rules_tx:
            ...
            for port in ['src_port_l', 'dst_port_l']:
                if rule[port] == 'any': rule[port]= {'max': '65535', 'min': '0'}
                else: # only handling single or continuous range for port
                    if len(rule[port]) == 2 :
                        rule[port] = {'max': str(rule[port][1]),
                        'min': str(rule[port][0])}
                    else:
                        self.logger.error ("user input port_list not handled by verification")
    ...
    def verify_policy_in_vna (self, scn):
        ...
        for policy in scn.policy_list:
            for test_vn in scn.policy_vn[policy]:
                user_rules_tx[policy]= self.tx_user_def_rule_to_aces(test_vn, scn.rules[policy])
        
        # Step 2: Aggregate rules by network
        rules_by_vn= {}
        for vn in scn.vnet_list:
            tmp_vn_rules=[]; rules_by_vn[vn]= []
            ...
            rules_by_vn[vn]= policy_test_utils.trim_realign_rules (rules_by_vn[vn])

        # Step 3: Translate user-rules-> ACEs to system format and update ACE IDs
        for vn in scn.vnet_list:
            if rules_by_vn[vn] != []:
                rules_by_vn[vn]= self.tx_user_def_aces_to_system (vn, rules_by_vn[vn])
        ...
        for compNode in self.inputs.compute_ips:
            ...
            for vn in vnCn:
                ...
                ret= policy_test_utils.compare_rules_list(rules_by_vn[vn], cn_vna_rules_by_vn[vn])
                if ret:
                    ...
                    for r in rules_by_vn[vn]: self.logger.debug (r)
            ...
            self.logger.info ("Verify rules not expected to be in CN if no VN-VM in CN")
            self.logger.info("CN: %s, Check for unexpected data" %(compNode))
            vn_not_of_cn= []; skip_vn_not_of_cn= 0
            ...
            for vn in vn_not_of_cn:
                ...
                if vn_exists:    
                    ...
                    msg= "CN: " + str(compNode) + ", VN: " + str(vn) + \
                        " seeing unexpected rules in VNA" + str(cn_vna_rules_by_vn[vn])
                    err_msg[compNode]= {vn: msg}
                else:
    ...
    def verify_policy_in_api_server(self):
        ...
        self.logger.info("====Verifying data for %s in API_Server ======" %(self.policy_name))
        self.api_s_policy_obj= self.api_s_inspect.get_cs_policy(domain= self.project_fq_name[0], project= self.project_fq_name[1], policy= self.policy_name, refresh= True)
        self.api_s_policy_obj_x= self.api_s_policy_obj['network-policy']

        # compare policy_fq_name
        out= policy_test_utils.compare_args('policy_fq_name', self.api_s_policy_obj_x['fq_name'] , self.policy_fq_name)
        if out: err_msg.append(out)
        # compare policy_uuid
        out= policy_test_utils.compare_args('policy_uuid', self.api_s_policy_obj_x['uuid'], self.policy_obj['policy']['id'])
        if out: err_msg.append(out)
        ...
        if err_msg != []: result= False; err_msg.insert(0, me + ":" + self.policy_name)
    ...
    def verify_policy_not_in_api_server(self):
        ...
        pol_dict = self.vnc_lib.network_policys_list(parent_id=proj, parent_fq_name=proj.fq_name)

        pol_list = pol_dict.get('network-policys') 
        for policy in pol_list:
            if (policy['fq_name'][2]== self.policy_name):
                pol_found = True
                self.logger.info( "policy %s is still found in API-Server" %(self.policy_name))
                break
        if not pol_found:
            self.logger.info( "policy %s is not found in API Server" %(self.policy_name))
        return pol_found
    ...
    def verify_policy_in_control_nodes(self):
        ...
        for cn in self.inputs.bgp_ips:
            ...
            if not cn_config_policy_obj:
                msg= "IFMAP View of Control-node %s is missing policy %s" %(cn, self.policy_fq_name)
            ...
            out= policy_test_utils.compare_args('policy_rules', cn_rules, self.policy_obj['policy']['entries']['policy_rule'], exp_name= 'cn_rules', act_name= 'quantum_rules')
            if out: 
                msg= "Rules view in control-node %s is not matching, detailed msg follows %s" %(cn, out)
                err_msg.append(msg)
