...
import argparse
import sys
import fabfile.testbeds.testbed  as testbed
...
class SmgrFixture(fixtures.Fixture):
    ...
    def __init__(self, connections, inputs, smgr_config_ini="./smgr_input.ini", test_local=False):
        self.connections = connections
        self.inputs = inputs
        self.smgr_config_ini = smgr_config_ini
        self.test_local = test_local
        self.params = read_ini_file(smgr_config_ini)
...
# end add_vns()
    ...
    def get_pref_vns_id(self):
        vns_id = None
        params=read_ini_file(config_ini)
        if  params.has_key('vns_id'):
            vns_id = params['vns_id']
        else:
            vns_id = get_vns_id()
