...
class QuantumFixture(fixtures.Fixture):
    ...
    def setUp(self):
        ...
        for idx in range(self._vn_count):
            ...
            self.logger.info ("Created VN %s, tid %s, id %s" % (net_req['name'], self.tid, vn_obj['id']))
            for jdx in range(self._vms_per_vn):
                #import pdb; pdb.set_trace()
                #port_obj = self.q_create_port( 'vmybz-%d-%d' % (idx,jdx), vn_obj['id'])
                port_obj = self.n_create_port( 'vmybz-%d-%d' % (idx,jdx),vn_obj['id'],self.tid)
    ...
    def n_create_port(self,name,netid,tid):
        try:
            ...
            self.obj.show_network(network = netid, fields = ['tenant_id'])
            self.tm[ii] += cmt() - ct ; ii +=1 ; ct = cmt() 
            self.obj.list_ports(device_owner = ['network:dhcp'], network_id = netid)
            self.tm[ii] += cmt() - ct ; ii +=1 ; ct = cmt() 
            return rport
