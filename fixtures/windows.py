...
class WindowsOrchestrator(Orchestrator):
    ...
    def run_cmd_on_windows(self, target_host, command, powershell=False,
    ...
    def _create_docker_on_windows(self, network, docker_name, target_host, image, username='Administrator',
    	...
    	docker_create_command = "docker run -id --network {} --name {} {} powershell".format(
    		...
    		docker_name,
    		image)
    	std_out, std_err, status_code = self.run_cmd_on_windows(
    ...
    def _create_docker_network(self, network, target_host, username='Administrator',
    	password='Contrail123!', transport='ntlm', ignore_error=False, subnet=''):

	if subnet:
    	    network_create_cmd = "docker network create --ipam-driver windows -d Contrail \
    	        --opt tenant={} --opt network={} {} --subnet={}".format(
	        self.inputs.project_name,network, network, subnet)
	else:
    	    network_create_cmd = "docker network create --ipam-driver windows -d Contrail \
    	        --opt tenant={} --opt network={} {} ".format(
	        self.inputs.project_name,network, network)
    	std_out, std_err, status_code = self.run_cmd_on_windows(
    		target_host=target_host,
	...
	if not node_name:	
		...
		assert False, "No valid compute nodes present !"
	...
	for subnet in vn_objs[0].network_ipam_refs[0]['attr'].ipam_subnets:
	    subnet_network = str(
		vn_objs[0].network_ipam_refs[0]['attr'].ipam_subnets[0].subnet.ip_prefix)+'/'+str(
		vn_objs[0].network_ipam_refs[0]['attr'].ipam_subnets[0].subnet.ip_prefix_len)
...
class VM(WindowsOrchestrator):
	...
	cmd = 'docker restart {}'.format(self.name)
	self.run_cmd_on_windows(
	    target_host=self.host_ip,
	    command=cmd,
	    powershell=True)
