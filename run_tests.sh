...
if ! options=$(getopt -o VNnfuUsthdC:lLm:F:T:c: -l virtual-env,no-virtual-env,no-site-packages,force,update,upload,sanity,parallel,help,debug,config:logging,logging-config,send-mail:features:tags:concurrency: -- "$@")
then
    # parse error
    usage
    exit 1
