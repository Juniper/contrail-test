...
docker_build () {
  ...
  build_arg_opts+=" --build-arg FROM_TAG=${FROM_TAG}"
...
Usage: $0 test [OPTIONS]
  ...
  --from-tag      BASE_TAG      Specify contrail-base-test container tag to use. Defaults to 'latest'.
...
EOF
    ...
    if [[ -z $FROM_TAG ]]; then
        echo "FROM_TAG(--from-tag) is unspecified, using 'latest'."; echo
        FROM_TAG='latest'
    fi
