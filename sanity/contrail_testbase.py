...
#        self.webServer, self.webServerPath=config.get('Basic','webServer').split(':')
        ...
        self.runStartTimeStr = datetime.datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S")
...
#        self.logFile= self.logPath +'/sanity.log'+ ts
        ...
        self.sumLogFile = self.logPath + \
            '/sanity.log.' + self.build_id + '.summary' + ts
        ...
        # HTML Report file
        self.htmlFile = self.logPath + '/sanity.log.' + \
            self.build_id + ts + '.html'
    ...
    def createHTMLFile(self):
        header = '<html> <head> <title> Sanity Report [Build %s] - %s- </title> </head> <body>' % (
            self.build_id, self.runStartTimeStr)
        header += '<h1>  Sanity Report [Build %s] </h2> <br>Start Time : %s <br> <p>' % (
            self.build_id, self.runStartTimeStr)
        self._result_colors = {
            'PASSED':      'lime',
            'FAILED':      'red',
    ...
    def sendMail(self, file):
        ...
        msg['Subject'] = 'Sanity Report [Build %s] %s' % (
            self.build_id, self.runStartTimeStr)
...
#        s = smtplib.SMTP('172.24.192.33','587')
    ...
    def uploadResults(self):
        ...
        try:
            with settings(
                host_string=self.webServer, user=self.webServerUser, password=self.webServerPassword,
    ...
    def writeHTMLResults(self):
        ...
        for index in table:
            ...
            colored_result = HTML.TableCell(
                result_str[index[2]], bgcolor=color)
            t.rows.append([index[0], index[1], colored_result])
        # end for
        htmlcode = str(t)
        self.htmlHandle.write(htmlcode)
        self.htmlHandle.write(
            '<p>Total Tests  : ' + str(self.totalTests) + '<br>')
        self.htmlHandle.write(
            'Passed Tests  : ' + str(self.passedTests) + '<br>')
        self.htmlHandle.write(
            'Failed Tests  : ' + str(self.failedTests) + '<br>')
        self.htmlHandle.write(
            '<p>Run Duration : ' + str(self.runDuration) + '<br>')
        logFileName = self.logFile.split('/')[-1]
        link = 'http://' + self.webServer + '/' + \
            self.webRoot + '/logs/' + logFileName
    ...
    def indicateStartTest(self, testId, description):
        self.logger.info('-' * 80)
        self.logger.info('Test ID ' + str(testId)
                         + ' : ' + description.upper())
        self.logger.info('')
    ...
    def indicateEndTest(self, testId, description, result):
        if result == 1:
            ...
            resultStr = 'FAILED'
        self.logger.info('Test ID ' + str(testId) +
                         ' : [' + resultStr + '] ' + description.upper())
        self.logger.info('-' * 80)
        self.logger.info('')
        self.sumLogger.info(
            ' %03s. %s : [ %6s ]', testId, "{:<60}".format(description.upper()), resultStr)
    # end indicateEndTest
