...
class SetupMachines:
    ...
    def __init__(self, args=''):
        ...
        local("cp %s %s/fabfile/testbeds/testbed.py" %
              (testbed_file, self.fab_dir))
    ...
    def parse_args(self, args):
        ...
        parser.add_argument(
            '-p', "--skip_install", dest="skip_install", action='store_true',
            help="Skip installing latest packages from repo")
#        parser.add_argument('-s', "--single_node",dest= "single_node",
# help = "IP of the All-in-one Single node setup. Ignores the (-i)ini_file
# option if supplied")
...
#            ls_out = run("find . -maxdepth 1 -type f -name '*'", capture = True)
            ...
            print "output : " + ls_out
            existing_repos = [
                repo for repo in ls_out.split() if not re.match('./contrail*', repo)]
            ...
            self._template_substitute_write(CONTRAIL_FEDORA_TEMPL,
                                            {'__contrail_fedora_path__':
                                                self.contrail_fedora_repo},
                                            '%s/contrail_fedora.repo' % (self._temp_dir_name))
            self._template_substitute_write(CONTRAIL_PKGS_TEMPL,
                                            {'__contrail_pkgs_path__':
                                                self.contrail_pkgs_repo},
...
#            local("sudo createrepo .")
    ...
    def setup_cgroup_entry(self):
        try:
            ret = run(
                "sudo grep -q '^cgroup_device_acl' /etc/libvirt/qemu.conf")
        except SystemExit as e:
    ...
    def install_packages(self, host):
        ...
        if 'bgp' in role_types:
            ...
            self.install_or_upgrade_pkg('contrail-dns')
        if 'cfgm' in role_types:
            packages = [
                'openstack-nova', 'openstack-quantum', 'openstack-cinder',
...
#        if 'compute' in role_types and 'cfgm' not in role_types :
        if 'compute' in role_types:
            packages = [
                'contrail-agent', 'openstack-utils', 'openstack-nova-compute',
                'contrail-libs']
            for package in packages:
                self.install_or_upgrade_pkg(package)
    ...
    def set_agent_config(self, bgp_ip0, bgp_ip1, collector_ip):
        ...
        agent_file = '/tmp/agent.conf'
        agent_tree = ET.parse(agent_file)
        agent_root = agent_tree.getroot()
        agent_elem = agent_root.find('agent')
        for srv in agent_elem.findall('xmpp-server'):
            agent_elem.remove(srv)

        pri_xmpp_elem = ET.Element('xmpp-server')
        pri_xmpp_ip = ET.SubElement(pri_xmpp_elem, 'ip-address')
        pri_xmpp_ip.text = '%s' % (bgp_ip0)
        agent_elem.append(pri_xmpp_elem)

        if bgp_ip1:
            sec_xmpp_elem = ET.Element('xmpp-server')
            sec_xmpp_ip = ET.SubElement(sec_xmpp_elem, 'ip-address')
            sec_xmpp_ip.text = '%s' % (bgp_ip1)
            agent_elem.append(sec_xmpp_elem)

        agent_tree = agent_tree.write('%s/agent.conf' % (temp_dir_name))
        local("sudo cp %s/agent.conf %s" % (temp_dir_name, agent_file))
        local("sudo rm %s/agent.conf" % (temp_dir_name))
        put(agent_file, '/etc/contrail/agent.conf')
        with cd('/tmp'):
            run("sudo sed 's/COLLECTOR=.*/COLLECTOR=%s/g' /etc/contrail/agent_param > agent_param.new" %
                (collector_ip))
            run("sudo mv agent_param.new /etc/contrail/agent_param")
    # end

    def run_setup_vnc_scripts(self, host):
        host_ip = str(IPNetwork(host['ip']).ip)
        pwd = host['password']
        role_types = [role['type'] for role in host['roles']]
        print "Roles for host %s is %s" % (host_ip, role_types)
        for role in host['roles']:
            if role['type'] == 'cfgm':
                collector_ip = socket.gethostbyname(
                    role['params']['collector'])
                with cd(INSTALLER_DIR):
                    run("./setup-vnc-cfgm.py --self_ip %s --collector_ip %s" %
                        (host_ip, collector_ip))
        for role in host['roles']:
            if role['type'] == 'bgp':
                collector_ip = socket.gethostbyname(
                    role['params']['collector'])
                cfgm_ip = socket.gethostbyname(role['params']['cfgm'])
                collector_ip = socket.gethostbyname(
                    role['params']['collector'])
                with cd(INSTALLER_DIR):
                    run("export PASSWORD=%s && ./setup-vnc-control.py --self_ip %s --cfgm_ip %s --collector_ip %s"
                        % (pwd, host_ip, cfgm_ip, collector_ip))
#            if role['type'] == 'compute' and 'cfgm' not in role_types :
            if role['type'] == 'compute':
                cfgm_ip = socket.gethostbyname(role['params']['cfgm'])
                bgp_ips = [socket.gethostbyname(hostname)
                           for hostname in role['params']['bgp']]
                if len(bgp_ips) == 1:
                    bgp_ips.append(bgp_ips[0])
                collector_ip = socket.gethostbyname(
                    role['params']['collector'])
                with cd(INSTALLER_DIR):
                    run("export PASSWORD=%s && ./setup-vnc-vrouter.py --self_ip %s --cfgm_ip %s --collector_ip %s \
                        --control_1_ip %s --control_2_ip %s" % (pwd, host_ip, cfgm_ip, collector_ip, bgp_ips[0],
                                                                bgp_ips[1]))
            if role['type'] == 'collector':
                with cd(INSTALLER_DIR):
                    run("./setup-vnc-collector.py")
            if role['type'] == 'webui':
                cfgm_ip = socket.gethostbyname(role['params']['cfgm'])
                with cd(INSTALLER_DIR):
                    run("./setup-vnc-webui.py --cfgm_ip %s" % (cfgm_ip))
        # end for
    # end run_setup_vnc_scripts
    ...
    def clean_remnants(self, host):
        ...
        all_services = [
            'redis', 'mysqld', 'openstack-nova-novncproxy', 'qpidd', 'ifmap', 'openstack-cinder-volume', 'openstack-cinder-scheduler', 'openstack-cinder-api', 'openstack-glance-registry', 'openstack-glance-api', 'openstack-nova-xvpvncproxy', 'openstack-nova-scheduler', 'openstack-nova-objectstore', 'openstack-nova-metadata-api', 'openstack-nova-consoleauth', 'openstack-nova-console',
            'openstack-nova-compute', 'openstack-nova-cert', 'openstack-nova-api', 'contrail-vncserver', 'contrail-analyzer', 'puppetmaster', 'puppetagent', 'contrail-opserver', 'openstack-keystone', 'contrail-qe', 'contrail-collector', 'quantum-server', 'contrail-svc-monitor', 'contrail-schema', 'contrail-control', 'contrail-api', 'contrail-webui', 'contrail-webui-middleware', 'contrail-vrouter', 'contrail-dns']
    ...
    def clean_DB(self, database):
     # Getting the token
        ...
        run('mysql -u root --password=%s -e \'drop database %s;\'' %
            (token, database))
    # end def
    ...
    def setup(self):
        ...
        for host in self.json_data['hosts']:
            host_ip = str(IPNetwork(host['ip']).ip)
            if host_ip == self.cfgmIP:
                cfgm = host
            with settings(host_string='%s@%s' % (host['username'], host_ip),
                          password=host['password'], warn_only=True):
                self.clean_remnants(host)
                if not self.args.skip_install:
                    self.setup_repo()
                    self.install_packages(host)
# if host_ip == self.cfgmIP:  # Run cfgm setup first
#                    self.run_setup_vnc_scripts(host)

        self.run_fab_scripts()

#        for host in self.json_data['hosts'] :
#            host_ip=str(IPNetwork(host['ip']).ip)
#            if host_ip == self.cfgmIP : cfgm= host
#            with settings( host_string='%s@%s' %(host['username'], host_ip),
#                    password= host['password'], warn_only=True):
#                if host_ip != self.cfgmIP:
#                    self.run_setup_vnc_scripts(host)

        # Reboot the cfgm node last
        for host in self.json_data['hosts']:
            host_ip = str(IPNetwork(host['ip']).ip)
            with settings(host_string='%s@%s' % (host['username'], host_ip),
                          password=host['password'], warn_only=True):
                if not self.args.dont_reboot and host_ip != self.cfgmIP:
                    self.check_and_reboot(host_ip)
        with settings(host_string='%s@%s' % (cfgm['username'], self.cfgmIP),
                      password=cfgm['password'], warn_only=True):
            self.enable_api_server_reset(host)
            if not self.args.dont_reboot:
                self.check_and_reboot(self.cfgmIP)
    # end setup
