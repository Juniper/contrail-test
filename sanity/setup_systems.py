...
#        if 'compute' in role_types and 'cfgm' not in role_types :
    ...
    def set_agent_config(self, bgp_ip0, bgp_ip1, collector_ip):
        ...
        with cd('/tmp'):
            run("sudo sed 's/COLLECTOR=.*/COLLECTOR=%s/g' /etc/contrail/agent_param > agent_param.new" %(collector_ip))
    ...
    def run_setup_vnc_scripts(self, host):
        ...
        for role in host['roles']:
            if role['type'] == 'cfgm':
                collector_ip= socket.gethostbyname( role['params']['collector'])
                with cd(INSTALLER_DIR):
                    run("./setup-vnc-cfgm.py --self_ip %s --collector_ip %s" %(host_ip, collector_ip))
        for role in host['roles']:
            if role['type'] == 'bgp':
                collector_ip= socket.gethostbyname( role['params']['collector'])
...
#            if role['type'] == 'compute' and 'cfgm' not in role_types :
            if role['type'] == 'compute' :
                cfgm_ip= socket.gethostbyname( role['params']['cfgm'])
                bgp_ips= [socket.gethostbyname(hostname) for hostname in role['params']['bgp']]
                if len(bgp_ips) == 1:
