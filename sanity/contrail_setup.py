...
class ContrailSetup:
    ...
    def setup(self):
        ...
        try:
            ...
            logging.exception('Exception while authenticating with %s' %(self.ip))
        ...
        self._nc=mynovaclient.Client('2', username='admin', project_id= 'admin', api_key='contrail123',
                                    auth_url='http://'+self.openstack_ip+':5000/v2.0')
    ...
    def get_host_ip_of_vm(self,vmUUID):
        for host in self.hostIPs: 
            link='http://' +host+ ':'+self.agentPort+'/Snh_ItfReq?name= '
...
#                return 0
            ...
            vmPassword='cubswin:)'
            logger.info('Pinging from '+str(vmObj)+' to '+ destIP + ' with size(bytes)'+ str(size) +'..... ')
            env.disable_known_hosts= True
            self.put_key_file_to_host(host) # Copy id file from cfgm-ip to compute node
...
#                    output=run('ping -s '+ str(size)+ ' -c '+ str(count)+' '+destIP)
                output=run('ssh -o StrictHostKeyChecking=no -i '+self.tmp_key_file + ' root@'+local_ip+' \"ping -s '+ str(size)+ ' -c '+ str(count)+' '+destIP+'\"')
    ...
    def get_local_ip_of_vm(self, hostIP, vmUUID):
        logger=logging.getLogger(self.logScenario)
        try:
            output=os.system('wget http://'+hostIP+':8085/Snh_ItfReq?name= -O '+self.tmpFile) 
    ...
    def is_server_present(self,vmName):
        ...
        try:
            ...
            logging.exception('Exception occured while checking if server is present')
            is_present=0
        return is_present
    ...
    def create_vn(self, vnName, vnSubnet):
        logger=logging.getLogger(self.logScenario)
        try:
            logger.debug('VN to be created : '+vnName+', Subnet '+vnSubnet)
            net_req = {'name': vnName}
    ...
    #        self.vn1.subnet=vnSubnet
            ...
            logger.exception('Some Exception occured while creating Virtual Network')
            return None
    # end create_vn
    ...
    def delete_vn(self, vnName):
        ...
        try:
            ...
            logger.debug('Response for deleting vn '+vnName+' : ' + net_rsp)
        except exceptions.QuantumClientException,e:
            logger.exception('Some Quantum Exception Occured while deleting Virtual Network')
...
#        import pdb; pdb.set_trace()
    ...
    def create_floatingip(self, count =1 ):
        ...
        fip_resp = self._quantum.list_floatingips(tenant_id = self.proj_obj.uuid)
    ...
    def delete_floatingip(self, fip_obj_list ):
        ...
        for i in fip_obj_list['floatingips']:
            ...
            self._quantum.delete_floatingip(fip_obj_list['floatingips'][index]['id'])
    ...
    def assoc_floatingip(self, fip_id, vm_id):
        ...
        if update_dict['port_id'] :
            fip_resp= self._quantum.update_floatingip(fip_id, {'floatingip': update_dict})
            return fip_resp
        else:   
            return None
    #end 
    ...
    def is_vn_in_bgp(self, vn_name):
        ...
        for host in self.bgpIPs:
            link='http://' +host+ ':'+self.bgpPort+'/Snh_ShowRoutingInstanceReq?name='
            output= web_invoke(link)
            if output and vn_fqdn in output: 
                logger.debug('VN '+ vn_fqdn+' found in control-node of '+host)
                return 1
    ...
    def is_vn_in_agent(self, vn_name):
        ...
        vrf=vn_fqdn+':'+vn_name
        for host in self.hostIPs:
            link='http://' +host+ ':'+self.agentPort+'/Snh_VrfListReq?name='+vn_fqdn
            output= web_invoke(link)
            if output and vn_fqdn in output:
    ...
    def get_image(self, image_name):
        with settings(host_string= self.ip, username= self.username, password= self.password,
                      warn_only=True,abort_on_prompts=False):
