...
class Test_Sanity(ContrailTestBase):
    def __init__(self, args=''):
        ...
        ContrailTestBase.__init__(self, self.iniFile, self.build_id, self.single_node)
        self.node=ContrailSetup(self.username, self.password, self.provFile, self.single_node, self.key, self.logScenario)
    ...
    def test_process_state(self):
        ...
        control_services=['contrail-control', 'contrail-dns', 'contrail-named']
        ...
        collector_services= ['redis', 'contrail-collector', 'contrail-opserver', 'contrail-qe']
    ...
    def test_create_delete_vn(self, vn_name, vn_subnet):
        result= 1
        self.logger.info('Starting test to create a Virtual Network '+vn_name+' with subnet '+vn_subnet)
        ...
        if  self.node.is_vn_in_quantum(vn) == 1 :
            self.logger.info('VN '+vn_name+' with UUID '+ vnUUID+ ' is found in the Networks list..OK')
        else :
            self.logger.error('VN'+vn_name+' with UUID '+ vnUUID + ' is not found in the Networks List..Not OK')
            result = result & 0
        ...
        if  (vn_in_quantum == 0 or  self.node.is_vn_in_bgp(vn_name) or self.node.is_vn_in_agent(vn_name)) :
            self.logger.info('VN '+vn_name+' with UUID '+ vnUUID+ ' is deleted from the Networks list..OK')
        else :
            self.logger.error('VN'+vn_name+' with UUID '+ vnUUID + ' is still found in the Networks List..Not OK')
            result = result & 0
        if result :
            self.logPass('VN '+ vn_name + ' with Subnet '+ vn_subnet + '  is created and deleted fine.OK')
        else:
            self.logFail('VN '+ vn_name + ' with Subnet '+ vn_subnet + ' creation/deletion had problems!')
    ...
    def test_create_delete_vm(self, vn_name, vn_subnet, count):
        ...
        for i in range(0,count):
            ...
            self.logger.info('')
            self.logger.info('Starting Test to create VM '+vmName+' in VN '+vn_name+', Subnet '+vn_subnet)
            vm = self.node.create_vm(vmName, vn_name)
#            self.sleep(60)
            self.node.wait_till_vm_up(vm)
            self.node.get_vm_detail(vm)
            if vm.status == 'ACTIVE':
                self.logger.info('VM '+vmName + ' is correctly in ACTIVE state')
            else:
                self.logger.error('VM '+ vmName + ' is not in ACTIVE state. Current state : '+vm.status)
                result = result & 0
#            import pdb; pdb.set_trace()
            ...
            if result: 
                ...
                if ('Lease of '+vmIP+' obtained' in console_output):
                    ...
                    self.logger.error('Unable to figure out whether VM got an IP or not')
                    self.logger.debug('Console output for VM '+vmName+': '+ console_output)

            self.logger.info('Continuing to delete the VMs')
            vm.delete()
            self.sleep(30)
            if self.node.is_server_present(vmName=vmName):
                self.logger.error('VM '+vmName+' still seems to be present even after deleting!')
                self.logger.error('Current VM list' + str(self.node._nc.servers.list()))
                result = result & 0
            else:
                self.logger.info('Vm '+ vmName+' is removed from nova list after deleting..OK')
        #end for 
        ...
        vn_in_quantum= self.node.is_vn_in_quantum(vn)
        if  (vn_in_quantum == 0 or self.node.is_vn_in_bgp(vn_name) or self.node.is_vn_in_agent(vn_name)) :
            self.logger.info('VN '+ vn_name +' is deleted from the Networks list..OK')
        else :
            self.logger.error('VN'+ vn_name +'  is still found in the Networks List..Not OK')
            result = result & 0
    ...
    def test_ping_between_vms(self, vn_name, vn_subnet, min_size= 56, max_size= 57, size_step= 1):
        ...
        for i in range(0,2): 
            vmName=self.node.get_compute_host().next() + '-vm'+repr(i)
            self.logger.info('')
            self.logger.info('Starting to create VM '+vmName+' in VN '+vnName+', Subnet '+vnSubnet)
...
#            self.node.get_vm_detail(vm)
            ...
            if result :
                ...
                if len(vm.addresses[vnName]) > 0:
                    ...
                    self.logger.error('VM did not seem to have got an IP at all')
                    result= result & 0
        #end for
        vm0=vmList[0]
        vm1=vmList[1]
...
#            vmName=self.hostname+ '-'+ vmList[i].name
    ...
    def test_floating_ip(self):
        ...
        self.logger.info('Creating a VM in VN '+vnName+' and another VM in floating-IP-VN '+ fVNName)
...
#        self.sleep(60)
        ...
        for tvm in [ vm, fvm]:
            if tvm.status == 'ACTIVE':
                self.logger.info('VM '+tvm.name + ' is correctly in ACTIVE state')
            else:
                self.logger.error('VM '+ tvm.name + ' is not in ACTIVE state. Current state : '+tvm.status)
                result = result & 0

        fip_pool_obj = self.node.create_floatingip_pool(fip_pool_name=fip_pool_name, net_id=fVNid)
        ...
        if not fip_resp:
            self.logger.error('Unable to associate floating IP with VM. Response : '+str(fip_resp))
            result= result & 0
        else:
        ...
        if result : 
            if self.node.ping_to_ip(vm, fvm.addresses[fVNName][0]['addr'], username=self.username, password=self.password):
                self.logger.info('VM is able to ping a IP in floating IP Network')
            else :
                self.logger.error('VM is not able to ping a IP in floating IP Network')
                result= result & 0
        ...
        if not self.node.delete_vn(fVNName):
            self.logger.error('Deleting vn %s failed.. Please check quantum logs ' %(fVNName))
            result = result & 0
        if not self.node.delete_vn(vnName):
            self.logger.error('Deleting vn %s failed.. Please check quantum logs ' %(vnName))
            result = result & 0
        return result
    ...
    def test_ping_between_vns_with_policy(self, vn1, vn2, policy_dict, rev_policy_dict):
        result =1
        self.logger.info('Creating VNs : '+vn1['name']+' and '+ vn2['name'])
        ...
        self.logger.info('Creating and binding policies for : '+vn1['name']+' and '+ vn2['name'])
#        policy_dict['source_vn']=source_vn['network']['contrail:fq_name']
#        policy_dict['dest_vn']=dest_vn['network']['contrail:fq_name']
        resp=self.node.create_and_bind_policy( source_vn['network']['id'], policy_dict)
        if resp is None:
            return 0 
        
#        rev_policy_dict['source_vn']=dest_vn['network']['contrail:fq_name']
#        rev_policy_dict['dest_vn']=source_vn['network']['contrail:fq_name']
        resp=self.node.create_and_bind_policy( dest_vn['network']['id'], rev_policy_dict)
        if resp is None:
            return 0
        
        self.logger.info('Creating VMs '+vn1['vm_name']+ ' and ' + vn2['vm_name'] + ' in each of the VNs')
...
#        self.sleep(150)
        ...
        if result: 
            if self.node.ping_to_ip(vm1, vm2.addresses[vn2['name']][0]['addr'], username=self.username, password=self.password):
                self.logger.info('VM is able to ping a IP in a different VN using policy')
            else :
                self.logger.error('VM is not able to ping a IP in a different VN using policy')
                result= result & 0
...
if __name__ == "__main__":
    ...
    x.runTest(x.test_ping_between_vns_with_policy, 
                    ...
                    vn1={'name':'vn5', 'subnet':'19.1.1.0/24', 'vm_name':x.node.get_compute_host().next() +'-vn1vm1'},
                    vn2={'name':'vn6', 'subnet':'20.1.1.0/24', 'vm_name':x.node.get_compute_host().next() +'-vn2vm1'},
                    policy_dict={'name':'policy4','source_vn':'vn5', 'source_port':'any','source_subnet':None,
                        'protocol':'any', 'dest_vn':'vn6', 'dest_port':'any', 'dest_subnet':None, 'action':'pass'},
                    rev_policy_dict={'name':'rev_policy6','source_vn':'vn6', 'source_port':'any','source_subnet':None,
