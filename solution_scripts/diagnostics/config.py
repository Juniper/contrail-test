...
def get_vn_type(vn_name):
    vn_group_list = ['Private_SNAT_VN','Private_VN','Private_LB_VIP_VN','Private_LB_Pool_VN',
                     'SNAT_GW_VN','Public_FIP_VN','Private_SC_MGMT_VN','Private_SC_Left_VN','Private_SC_Right_VN','Private_VSRX_BGP_VN','Private_VSRX_MX_VN','Private_SC_Auto_Left_VN','Private_SC_Auto_Right_VN']
...
class PortTuples(Base):
    ...
    def list_port_tuples(self,*arg,**kwarg):
        ...
        ports_list = self.connections.vnc_lib.port_tuples_list()['port-tuples']
    ...
    def delete_port_tuple(self,*arg,**kwarg):
        ...
        self.connections.vnc_lib.port_tuple_delete(id=port_tuple_id)

    def delete_port_tuples(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        ports_list = self.list_port_tuples(conn_obj_list=conn_obj_list,tenant_list=tenant_list)
        if not ports_list:
           return
        kwargs_list = []
        ...
        self.delete_port_tuple(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
...
class Policy(Base):
    ...
    @timeit
    def create_policies(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
    ...
    def delete_policy(self,*arg,**kwarg):
        ...
        self.connections = kwarg['connection_obj'].connections
        self.logger      = kwarg['connection_obj'].logger
        policys  = self.connections.vnc_lib.network_policys_list(parent_id=tenant_uuid)['network-policys']
    ...
    def delete_policies(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        kwargs_list = []
        for tenant in tenant_list:
            if tenant['fq_name'] == ['default-domain','admin']:
             continue
    ...
    def detach(self,tenant_fq_name,vn_fq_name):
    ...
    @timeit
    def not_used_attach_policies(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
        policy_count = tenant_conf.get('policy,count',None)
        if policy_count is None:
    ...
    def detach_policy(self,*arg,**kwarg):
        ...
        self.logger      = kwarg['connection_obj'].logger
        self.connections.api_server_inspect = self.connections.get_api_server_inspect_handles()
        self.detach(tenant_fq_name,vn_fq_name)

    def detach_policies(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        kwargs_list = []
        vn_obj = VN(None)
        vn_list = vn_obj.list_vn(tenant_list=tenant_list,conn_obj_list=conn_obj_list)
        if not vn_list:
           return
        for vn in vn_list:
           tenant_fq_name,vn_id,vn_fq_name = vn
...
class AnalyticsConfig(Base):
    ...
    def analytics(self,*arg,**kwarg):
        ...
        self.connections = kwarg['connection_obj'].connections
        self.logger      = kwarg['connection_obj'].logger
        analytics_obj = self.connections.get_analytics_object()
        oi_handle=self.connections.get_opserver_inspect_handles()
        oi_handle['172.16.70.3'].get_hrefs_to_all_UVEs_of_a_given_UVE_type(uveType=None)
    ...
    def analyticss(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties,dpdk):
        ...
        if len(kwargs_list):
           kwargs = {'kwargs_list': kwargs_list} if len(kwargs_list) > 1 else kwargs_list[0]
           self.analytics(debug_enabled=True,count=thread_count,conn_obj_list=conn_obj_list,**kwargs)

class ProjectConfig(Base):

    def create(self,domain_name,tenant_name):
        self.fixture = ProjectFixture(domain_name=domain_name,project_name=tenant_name, connections=self.connections)
        self.fixture.setUp()
    ...
    def create_tenant(self,*arg,**kwarg):
        ...
        project_id  = self.create(domain_name,tenant_name)

    def create_tenants(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
        existing_projects_list = self.list_projects(conn_obj_list=conn_obj_list)
        kwargs_list = []
        for tenant in tenant_list:
            if tenant['fq_name'] == ['default-domain','admin']:
             continue
    ...
    @timeit
    def update_security_groups(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties=False):
        if update_properties:
           return
        kwargs_list = []
        for tenant in tenant_list:
    ...
    def delete_tenant(self,*arg,**kwarg):
        ...
        self.logger      = kwarg['connection_obj'].logger
        try:
          proj_obj = self.connections.vnc_lib.project_read(id=tenant_uuid)
        except NoIdError:
          self.logger.warn("project %s not found..skipping delete"%str(tenant_fq_name))
          return True
        try:
          self.delete(tenant_uuid)
          return True
        except NoIdError:
          return True
          ...
          self.logger.warn("ERROR seen during delete_tenant:%s...retrying.."%str(tenant_fq_name))
          traceback.print_exc(file=sys.stdout)
          return False
    
    def delete_tenants(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        kwargs_list = []
        for tenant in tenant_list:
            if tenant['fq_name'] == ['default-domain','admin']:
             continue
...
class vDNS(Base):
    ...
    def create_record(self,*arg,**kwarg):
      try:
        ...
        try:
          self.connections.vnc_lib.virtual_DNS_record_read(fq_name=[tenant_domain_name,forwarder.split(':')[-1],rec_name])
          self.logger.warn("Record: %s:%s found..skipping create"%(tenant_domain_name,rec_name))
          return
        except NoIdError:
    ...
    def create_vdns(self,*arg,**kwarg):
        ...
        try:
            domain_obj = self.connections.vnc_lib.domain_read(fq_name=[tenant_domain_name])
        except NoIdError:
            domain_obj = None
        ...
        try:
          self.connections.vnc_lib.virtual_DNS_read(fq_name=[u'%s'%tenant_domain_name,u'%s'%name])
        ...
        #domain_obj =  Domain(name=tenant_domain_name)
    ...
    @timeit
    def create_data_vdns_tree(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
    ...
    def delete_record_per_tenant(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        kwargs_list = []
        for tenant in tenant_list:
            if tenant['fq_name'] == ['default-domain','admin']:
             continue
       ...
       for i in xrange(10):
         ...
         for vdns_id in vdns_ids :
           ...
           if not vdns_info.has_key('%s,down'%vdns_name) or len(vdns_info['%s,down'%vdns_name]) == 0:
              ...
              self.connections.vnc_lib.virtual_DNS_delete(id=vdns_id)
     
    @Process.wrapper
    def list_domains(self,*arg,**kwarg):
        self.connections  = kwarg['connection_obj'].connections
        self.logger       = kwarg['connection_obj'].logger
        domains = self.connections.vnc_lib.domains_list() ['domains']
        return domains
    ...
    def delete_vdns(self,conn_obj_list):
       ...
       for domain in domains:
           ...
           self.delete_vdns_tree(count=1,**kwargs)
       return
...
class IPAM(Base):
    ...
    def create_ipam(self,*arg,**kwarg):
        ...
        try:
           ...
           pass
        domain_obj = self.connections.vnc_lib.virtual_DNS_read(fq_name=[domain_name,u"%s"%domain_server_name])
    ...
    @timeit
    def create_ipams(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
    ...
    def delete_ipam(self,*arg,**kwarg):
        ...
        self.connections.project_name        = tenant_name
        self.connections.inputs.project_name = tenant_name
        ipams = self.connections.vnc_lib.network_ipams_list(parent_id=tenant_uuid)['network-ipams']
        for ipam in ipams:
            ...
            try:
               ...
               self.connections.vnc_lib.network_ipam_delete(id=ipam['uuid'])
            except NoIdError,TypeError:
               pass
            except RefsExistError:
    ...
    def delete_ipams(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        kwargs_list = []
        for tenant in tenant_list:
            if tenant['fq_name'] == ['default-domain','admin']:
             continue
...
class VN(Base):
    ...
    def create(self, name, subnets=[], ipam_name_list=[], external=False,shared=False,disable_gateway=False,rt_number=None,project_obj=None,forwarding_mode=None,project_name=None):
        kwargs = dict()
    ...
    def get_vn_ids(self,*arg,**kwarg):
        ...
        for tenant in tenant_list:
            ...
            tenant_uuid    = tenant['uuid']
            try:
              project_obj = self.connections.vnc_lib.project_read(id=tenant_uuid)
            except NoIdError:
              self.logger.warn("tenant :%s missing..skipping..."%str(tenant_fq_name))
              continue
    ...
    def delete_extend_to_physical_routers(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
    ...
    def create_vn(self,*arg,**kwarg):
        ...
        domain_name,tenant_name  = tenant_fq_name
        project_obj = self.connections.vnc_lib.project_read(fq_name=tenant_fq_name)
        self.connections.project_name        = tenant_name
        self.connections.inputs.project_name = tenant_name
        ...
        if not vn_obj:
           ...
           if use_fixture:
              subnets = []
              for i,ipv4_cidr in enumerate(ipv4_cidr_list):
              ...
              self.create(vn_name,subnets,ipam_fq_name_list,external,shared,disable_gateway,rt_number,project_obj,fwd_mode,self.connections.inputs.project_name)
              ...
              for i,ipv6_cidr in enumerate(ipv6_cidr_list):
                  ...
                  ipam_sn_lst.append(ipam_sn)
              if len(ipam_fq_name_list) == 1:
                 ipam_obj = self.connections.vnc_lib.network_ipam_read(fq_name=ipam_fq_name_list[0])
        ...
        if attach_qos:
          ...
          try:
            qos_config_obj = self.connections.vnc_lib.qos_config_read(fq_name=qos_fq_name)
            vn_obj.add_qos_config(qos_config_obj)
          except: 
            pass

        if use_fixture or update_properties: # VN is supposed to have been created already.just do update
           self.connections.vnc_lib.virtual_network_update(vn_obj)
        else:
           try:
    ...
    def update_import_rt(self,*arg,**kwarg):
        ...
        data_vn_obj  = self.connections.vnc_lib.virtual_network_read(fq_name=data_vn_fq_name)
        rt_vn_obj    = self.connections.vnc_lib.virtual_network_read(fq_name=rt_vn_fq_name)
        ...
        self.connections.vnc_lib.virtual_network_update(data_vn_obj)
  
    @timeit
    def create_vns(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        kwargs_list = []
        ...
        for tenant in tenant_list:
            ...
            for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
                ...
                for vn_indx in xrange(vn_count):
                    ...
                    vn_fq_name = tenant_fq_name + [vn_name]
                    if not update_properties and vn_fq_name in existing_vns:
                       continue

                    ipv4_cidr_list = []
                    if ipv4_cidr is True:
                       for i in xrange(subnet_count):
                         v4_cidr    = generate_v4_cidr(tenant_fq_name,vn_type,already_allocated_cidr)
                         already_allocated_cidr.append(v4_cidr)
                         ipv4_cidr_list.append(v4_cidr)
                    elif ipv4_cidr :
                       v4_cidr = ipv4_cidr
                       already_allocated_cidr.append(v4_cidr)
                       ipv4_cidr_list.append(v4_cidr)
        ...
        if len(kwargs_list) != 0:
           kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
           self.update_import_rt(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
    ...
    def delete_vn_by_name_process(self,*arg,**kwarg):
        ...
        self.connections = kwarg['connection_obj'].connections
        self.logger      = kwarg['connection_obj'].logger
        net_list = self.connections.vnc_lib.virtual_networks_list(parent_id=tenant_uuid)['virtual-networks']
        vn_id = None
        vn_fq_name = tenant_fq_name + [vn_name]
        for net in net_list:
    ...
    def not_used_delete_vn_by_name(self,args,kwargs):
        tenant_fq_name,vn_name = args
        p = mp.Process(target=self.delete_vn_by_name_process,args=(tenant_fq_name,vn_name))
        p.start()
        p.join()
    ...
    def delete_vn(self,*arg,**kwarg):
        ...
        for inst_ip in inst_ip_ref:
            self.connections.vnc_lib.instance_ip_delete(id=inst_ip['uuid']) 
        try:
    ...
    def delete_vns(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        ...
        for i in xrange(5):
          if i != 0 :
             time.sleep(60)
          vn_list = self.list_vn(tenant_list=tenant_list,conn_obj_list=conn_obj_list)
          if not vn_list:
             return True
...
class VM(Base):
    ...
    def retrieve_vm_info(self,*arg,**kwarg):
      try:  
        ...
        si_instances = self.connections.vnc_lib.service_instances_list()['service-instances']
        for tenant in tenant_list:
           if tenant['fq_name'] == ['default-domain','admin']:
             continue
           tenant_id = tenant['uuid']
    ...
    def create(self, name, vn_id_dict,subnet_id_dict,mgmt_network_first,image='ubuntu',userdata=None,flavor="m1.small",zone=None):
        self.fixture = VMFixture(connections=self.connections, vm_name=name, image_name=image,userdata=userdata,zone=zone)
        self.fixture.flavor = flavor
        if subnet_id_dict['mgmt'] :
           ...
           mgmt_port = self.fixture.quantum_h.create_port(mgmt_vn_id,mgmt_subnet_id)
           data_port = self.fixture.quantum_h.create_port(data_vn_id,data_subnet_id)
           if mgmt_network_first:
              self.fixture.port_ids = [mgmt_port['id'],data_port['id']]
           else:
              self.fixture.port_ids = [data_port['id'],mgmt_port['id']]
        else:
    ...
    def check_if_vm_already_exists(self,existing_vms,conf):
        ...
        for vm in existing_vms:
            if vm['tenant'] != tenant_fq_name or vm['vm_name'] != vm_name:
               continue
      ...
      try:
        ...
        if not vm_obj:
           ...
           for additional_vn_name in additional_vn_name_list:
              ...
              #additional_vn_d = {}
              # additional_vn_d['name'] = additional_vn_obj['network']['id']
           ...
           try:
             self.create(vm_name,vn_id_dict,subnet_id_dict,mgmt_network_first,image_name,userdata,flavor,zone)
        ...
        if attach_port_mirror in [True,False] or fat_flow is not None or disable_policy is not None:
           ...
           #self.fixture.wait_till_vm_is_up()
           if vm_obj is None:
              vm_obj = self.connections.vnc_lib.virtual_machine_read(id=self.fixture.vm_obj.id)
        else:
           return True
        ...
        if attach_port_mirror in [True,False] or fat_flow is not None or disable_policy is not None:
           ...
           for vmi_ref in vmi_refs:       
               vmi_obj = self.connections.vnc_lib.virtual_machine_interface_read(id=vmi_ref['uuid'])
    ...
    def list_subnets(self,*arg,**kwarg):
        ...
        for tenant in tenant_list:
            tenant_id  = tenant['uuid']
            ipams_list = self.connections.vnc_lib.network_ipams_list(parent_id=tenant_id)['network-ipams'] or []
            for ipam in ipams_list:
                ipam_obj = self.connections.vnc_lib.network_ipam_read(id=ipam['uuid'])
    ...
    @timeit
    def create_vms(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties,dpdk):
        subnet_dict = self.list_subnets(tenant_list=tenant_list,conn_obj_list=conn_obj_list)
        kwargs_list         = []   
        bgp_vm_kwargs_list  = []   
        mgmt_vn_name     = global_conf['mgmt,vn_name']
        port_mirror_conf = tenant_conf['port_mirror_conf']
        vm_obj = VM(None)
        existing_vms = vm_obj.list_vms(conn_obj_list=conn_obj_list,tenant_list=tenant_list)
        #nova_h = NovaHelper(inputs=conn_obj_list[0].inputs, project_name='admin',
        #                      username='admin', password='c0ntrail123',update_ssh_key=True)
        ...
        if len(bgp_vm_kwargs_list) :
           kwargs = {'kwargs_list': bgp_vm_kwargs_list} if len(bgp_vm_kwargs_list) > 1 else bgp_vm_kwargs_list[0]
           self.create_vm(count=1,conn_obj_list=conn_obj_list,**kwargs)
        if len(kwargs_list):
           kwargs = {'kwargs_list': kwargs_list} if len(kwargs_list) > 1 else kwargs_list[0]
    ...
    def delete_vmi(self,*arg,**kwarg):
        ...
        for inst_ip in inst_ips:
            ...
            try:
              inst_ip_obj = self.connections.vnc_lib.instance_ip_read(id=inst_ip_id)
              inst_ip_obj.del_virtual_machine_interface(vmi_obj)
              self.connections.vnc_lib.instance_ip_update(inst_ip_obj)
        ...
        for lr in lr_refs:
            try:
              lr_obj = self.connections.vnc_lib.logical_router_read(id=lr['uuid'])
            except NoIdError,TypeError:
              lr_obj = None
              print "INFO: LR update failed.. NoIdError",lr['uuid']
            if lr_obj:
              lr_obj.del_virtual_machine_interface(vmi_obj)
              self.connections.vnc_lib.logical_router_update(lr_obj)
        try:
           vmi_obj = self.connections.vnc_lib.virtual_machine_interface_read(id=vmi_id)
        except NoIdError,TypeError:
           ## TypeError: ('__init__() takes exactly 2 arguments (1 given)', <class 'cfgm_common.exceptions.NoIdError'>, ()) TO_FIX
        ...
        if inst_ips or li_refs or lr_refs:
           ...
           #handleRefsExistError()
        try:
          self.connections.vnc_lib.virtual_machine_interface_delete(id=vmi_id)
        except NoIdError,TypeError:
          pass
        except RefsExistError:
    ...
    def delete_vm(self,*arg,**kwarg):
        ...
        for si_ref in si_refs:
            try:
              si_obj = self.connections.vnc_lib.service_instance_read(id=si_ref['uuid'])
              vm_obj.del_service_instance(si_obj)
              self.connections.vnc_lib.virtual_machine_update(vm_obj)
            except NoIdError:
              pass
        if si_vm:
           return
        try:
          ...
          #except ConnectionError,ClientException,Exception:
          pass # retry loop in delete_vms will take care of this.

    def delete_vmis(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):

        for i in xrange(10):
            vmis_list = self.list_vmis(conn_obj_list=conn_obj_list,tenant_list=tenant_list)
            if not vmis_list:
               return
    ...
    def delete_vms(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        for i in xrange(10):
           if i != 0 :
              time.sleep(60) 
           print "iteration",i
           ret = self.list_vms(conn_obj_list=conn_obj_list,tenant_list=tenant_list)
           vms_list = []
           if ret:
              vms_list.extend(ret)
           if len(vms_list) == 0:
...
class FloatingIPPool(Base):
    ...
    @timeit
    def create_fip_pools(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
    ...
    @timeit
    def delete_fip_pools(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties=False):
        if update_properties:
           return
        kwargs_list = []
        for tenant in tenant_list:
    ...
    def associate_fip(self,*arg,**kwarg):
        ...
        domain_name,tenant_name = tenant_fq_name 
        self.connections.project_name = self.connections.inputs.project_name = tenant_name

        # delete default-fip-pool
        try:
           fip_pool_fq_name = tenant_fq_name + [fip_gw_vn_name,'floating-ip-pool']
           pool_obj = self.connections.vnc_lib.floating_ip_pool_read(fq_name=fip_pool_fq_name)
           project_obj = self.connections.vnc_lib.project_read(fq_name=tenant_fq_name)
           project_obj.del_floating_ip_pool(pool_obj)
           self.connections.vnc_lib.project_update(project_obj)
    ...
    @timeit
    def associate_fips(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
        vn_index_replace_str      = tenant_conf['vn,index,replace_str']
        ...
        vn_obj = VN(None)
        ret    = vn_obj.get_vn_ids(conn_obj_list=conn_obj_list,tenant_list=tenant_list_l)
        if ret:
           vn_info_dict.update(ret)
        if not fip_shared :
        ...
        for tenant in tenant_list:
            ...
            for vn_info in tenant_conf['tenant,vn_group_list']:
                ...
                vm_list = vm_obj.list_vms(conn_obj_list=conn_obj_list,tenant_list=[tenant],vn_list=vn_names_list)
                if not vm_list:
                   continue
                for vm in vm_list:
                    tenant         = vm['tenant']
...
class LogicalRouterConfig(Base):
    ...
    def create(self, name, tenant_id,vn_ids=[], gw=None):
        ...
        #self.connections.vnc_lib.logical_router_create(lr)
    ...
    def create_logical_router(self,*arg,**kwarg):
        ...
        try:
          self.create(router_name,tenant_uuid,gw=gw)
        except:
          traceback.print_exc(file=sys.stdout)

    @timeit
    def create_logical_routers(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
        ...
        if len(kwargs_list) == 0:
           return
        kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
        self.create_logical_router(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
    ...
    def delete_logical_router(self,*arg,**kwarg):
        ...
        for lr in lrs:
            ...
            if vn_ref:
               for vn in vn_ref:
                   try:
                     vn_obj = self.connections.vnc_lib.virtual_machine_read(id=vn['uuid'])
            ...
            if vn_ref or vmi_ref:
               self.connections.vnc_lib.logical_router_update(router_obj)
            self.delete(uuid)

    def delete_logical_routers(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        kwargs_list = []
        for tenant in tenant_list:
            if tenant['fq_name'] == ['default-domain','admin']:
               continue
    ...
    @timeit
    def attach_vns_to_logical_routers(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
           return
        gw_vn_group_index           = None
        fip_vn_group_index          = None
        ...
        vn_info_dict      = {}
        vn_obj = VN(None)
        ret = vn_obj.get_vn_ids(conn_obj_list=conn_obj_list,tenant_list=tenant_list)
        if ret:
           vn_info_dict.update(ret)
        kwargs            = {}
        router_info_dict  = {}
...
class Lbaas(Base):
     ...
     def create_lb_pool(self,*arg,**kwarg):
         ...
         lb_pools = self.connections.vnc_lib.loadbalancer_pools_list(parent_id=tenant_uuid)['loadbalancer-pools']
     ...
     def delete_lb_pools(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
     ...
     def delete_lb_members(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
        kwargs_list = []
        for tenant in tenant_list:
            if tenant['fq_name'] == ['default-domain','admin']:
               continue
     ...
     def create_lb_members(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        ...
        for indx,tenant in enumerate(tenant_list):
          ...
          if not pool_vn_name:
             return
          vms_list     = vm_obj.list_vms(conn_obj_list=conn_obj_list,tenant_list=[tenant],vn_list=[pool_vn_name])
          if not vms_list:
     ...
     def create_lb_vip_associate_fip(self,*arg,**kwarg):
         ...
         vip_obj = self.connections.vnc_lib.virtual_ip_read(fq_name=vip_fq_name)
         ...
         proj_obj      = self.connections.vnc_lib.project_read(fq_name=tenant_fq_name)
         self.connections.orch = self.connections.get_orch_h()
         ...
         self.quantum_h.update_floatingip(fip_id,{'floatingip': update_dict})
     ...
     def delete_lb_vip(self,*arg,**kwarg):
         ...
         tenant_fq_name   = tenant['fq_name']
         tenant_uuid      = tenant['uuid']
         virt_ips_list = self.connections.vnc_lib.virtual_ips_list(parent_id=tenant_uuid)['virtual-ips']
     ...
     def delete_lb_vips(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
     ...
     def create_lb_vips_associate_fips(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
         ...
         if len(kwargs_list) == 0:
            return
         kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
         self.create_lb_vip_associate_fip(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
     ...
     def create_health_monitor(self,*arg,**kwarg):
         ...
         self.connections = kwarg['connection_obj'].connections
         self.logger      = kwarg['connection_obj'].logger
         hms = self.connections.vnc_lib.loadbalancer_healthmonitors_list(parent_id=tenant_uuid)['loadbalancer-healthmonitors']
         if hms:
     ...
     def delete_health_monitors(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
...
class RouterConfig(Base):
      ...
      def create_logical_interfaces(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
          if update_properties:
             return
      ...
      def delete_logical_interface(self,*arg,**kwarg):
          ...
          for li in lis:
              ...
              if delete_li or len(vmis) == 0:
                 try:
                   self.connections.vnc_lib.logical_interface_delete(id=li['uuid'])
                 except NoIdError:
                   pass
                 except RefsExistError:
      ...
      def delete_logical_interfaces(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list=[]):
      ...
      def associate_fip_to_vmi(self,*arg,**kwarg):
         ...
         try:
            ...
            return
         vmis = self.connections.vnc_lib.virtual_machine_interfaces_list(parent_id=tenant_uuid)['virtual-machine-interfaces']
         attach_fip_vns = []
         for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
            tenant_index = get_tenant_index(tenant_conf,tenant_fq_name)
            for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
      ...
      def associate_fip_to_vmis(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
         if update_properties:
            return
         kwargs_list = []
         vn_index    = 0
         ...
         if fip_shared:
            ...
            tenant_list_l = tenant_list[:]
         vn_obj = VN(None)
         ret = vn_obj.get_vn_ids(tenant_list=tenant_list_l,conn_obj_list=conn_obj_list)
         if ret:
            vn_info_dict.update(ret)
         for tenant in tenant_list:
             if tenant['fq_name'] == ['default-domain','admin']:
...
class VrouterGlobalConfig(Base):
    ...
    def create_qos(self,*arg,**kwarg):
        ...
        global_qos_obj = self.connections.vnc_lib.global_qos_config_read(fq_name=global_qos_fq_name)
        ...
        gs_conf_obj = self.connections.vnc_lib.global_system_config_read(fq_name=[u'default-global-system-config'])
        qos_obj.add_global_system_config(gs_conf_obj)
        self.connections.vnc_lib.qos_config_create(qos_obj)
    ...
    def create_qoss(self,count,conn_obj_list,global_conf):
        ...
        if len(kwargs_list):
           kwargs = {'kwargs_list': kwargs_list} if len(kwargs_list) > 1 else kwargs_list[0]
           self.create_qos(count=count,conn_obj_list=conn_obj_list,**kwargs)
    ...
    def list_qos(self,*arg,**kwarg):
        ...
        self.logger      = kwarg['connection_obj'].logger
        try:
          return self.connections.vnc_lib.qos_configs_list().get('qos-configs',[])
        except:
          return []
    ...
    def delete_qos(self,*arg,**kwarg):
        ...
        self.connections = kwarg['connection_obj'].connections
        self.logger      = kwarg['connection_obj'].logger
        self.connections.vnc_lib.qos_config_delete(id=qos_uuid)

    def delete_qoss(self,count,conn_obj_list,global_conf):

        qos_list = self.list_qos(conn_obj_list=conn_obj_list)
        kwargs_list = []
        for qos in qos_list:
            kwargs = {}
        ...
        if len(kwargs_list):
           kwargs = {'kwargs_list': kwargs_list} if len(kwargs_list) > 1 else kwargs_list[0]
           self.delete_qos(count=count,conn_obj_list=conn_obj_list,**kwargs)
    ...
    def update_conf(self,*arg,**kwarg):
          ...
          self.connections = connection_obj.connections
          self.logger      = connection_obj.logger
          global_vr_conf_obj  = self.connections.vnc_lib.global_vrouter_config_read(fq_name=[u'default-global-system-config', u'default-global-vrouter-config'])
          ...
          self.connections.vnc_lib.global_vrouter_config_update(global_vr_conf_obj)
          ...
          self.connections.vnc_lib.global_system_config_update(gs_conf_obj)
...
class Bgpaas(Base):
     ...
     def create(self,*arg,**kwarg):
         ...
         bgpaas_obj = None
         try:
            bgpaas_obj = self.connections.vnc_lib.bgp_as_a_service_read(fq_name=service_fq_name)
         except NoIdError:
            pass
     ...
     def create_bgpaas(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        if update_properties:
            return
     ...
     def delete(self,*arg,**kwarg):
         ...
         tenant           = kwarg['tenant']
         tenant_fq_name   = tenant['fq_name']
         services_list    = self.connections.vnc_lib.bgp_as_a_services_list()
         ...
         for service in services_list:
             ...
             if [dom,t_name] != tenant_fq_name:
                continue
             try:
               bgpaas_obj = self.connections.vnc_lib.bgp_as_a_service_read(id=service['uuid'])
             except NoIdError:
               continue
     ...
     def delete_bgpaas(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
...
class ServicesConfig(Base):
      ...
      def retrieve_sc_info(self,*arg,**kwarg):
          ...
          for tenant in tenant_list:
             ...
             vn_pairs_list = []
             for policy in policys:
                 pol_obj = self.connections.vnc_lib.network_policy_read(id=policy['uuid']) 
      ...
      def create_service_template(self,*arg,**kwarg):
          ...
          try:
            ...
            self.logger.warn("Service template: %s already exists"%str(st_fq_name))
            return
          except NoIdError:
      ...
      def create_service_health_check(self,*arg,**kwarg):
          ...
          self.connections.vnc_lib.service_health_check_create(hc_obj)

      def create_service_health_checks(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties=False):
          ...
          if len(kwargs_list) == 0:
             return
          kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
          self.create_service_health_check(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
      ...
      def list_service_health_checks(self,*arg,**kwarg):
          ...
          tenant_list      = kwarg['tenant_list']
          tenant_list_t    = map(lambda x:x['fq_name'],tenant_list)
          hc_list = self.connections.vnc_lib.service_health_checks_list()['service-health-checks'] or []
          hc_info = {}
          for hc in hc_list:
      ...
      def delete_service_health_check(self,*arg,**kwarg):
          ...
          self.logger      = kwarg['connection_obj'].logger
          hc_uuid          = kwarg['uuid']
          hc_obj = self.connections.vnc_lib.service_health_check_read(id=hc_uuid)
          si_ref = hc_obj.get_service_instance_refs() or []
          for si in si_ref:
              si_obj = self.connections.vnc_lib.service_instance_read(id=si['uuid'])
              hc_obj.del_service_instance(si_obj)
          self.connections.vnc_lib.service_health_check_update(hc_obj)
          self.connections.vnc_lib.service_health_check_delete(id=hc_uuid)

      def delete_service_health_checks(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties=False):
          hc_info_dict = self.list_service_health_checks(conn_obj_list=conn_obj_list,tenant_list=tenant_list)
          tenant_list_t  = map(lambda x:x['fq_name'],tenant_list)
          kwargs_list = []
          ...
          if len(kwargs_list) == 0:
             return
          kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
          self.delete_service_health_check(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
      ...
      def create_service_instance(self,*arg,**kwarg):
          ...
          self.connections.api_server_inspect = self.connections.get_api_server_inspect_handles()
          try:
            si_obj = self.connections.vnc_lib.service_instance_read(fq_name=si_fq_name)
          except NoIdError:
            si_obj = None
          ...
          if update_properties and si_obj:
             ...
             if change_config:
                si_obj.set_service_instance_properties(si_prop)
                self.connections.vnc_lib.service_instance_update(si_obj)
          ...
          fixture = SvcInstanceFixture(
                ...
                svc_template=st_obj, if_list=if_list,mgmt_vn_name=mgmt_vn_name,
                left_vn_name=left_vn_name, right_vn_name=right_vn_name, do_verify=False, max_inst=max_inst, static_route=if_st_route_list,route_community=if_route_comm_list,az_name=az_name,host_name=host_name,ha_mode=ha_mode)
          try:
            fixture.setUp()
          except RefsExistError:
            pass
          ...
          if int(si_version) == 2:
             ...
             for vm_indx in range(max_inst):
                 vm_fixture = VM(None)
                 vm_fixture = VMFixture(connections=self.connections, vm_name="si-vm-%s-%d"%(si_name,vm_indx), image_name=si_image,zone=az_name)
             ...
             st_obj   = self.connections.vnc_lib.service_template_read(id=st_uuid)
             st_props = st_obj.get_service_template_properties()
             if_list  = st_props.get_interface_type()
             for vm_indx in range(max_inst):
                 pt_obj  = PortTuple(name=u'pt_%s_%d'%(si_name,vm_indx), parent_obj=si_obj)
                 pt_uuid = self.connections.vnc_lib.port_tuple_create(pt_obj)
             ...
             for si_vm in si_vms:
                 si_vm_obj = self.connections.vnc_lib.virtual_machine_read(id=si_vm['uuid'])
          ...
          if health_check:
             ...
             hc_obj = self.connections.vnc_lib.service_health_check_read(id=health_check_uuid)
      ...
      @timeit
      def create_service_instances(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties,dpdk):
          service_instances = tenant_conf.get('service_instances',None)
          service_templates = global_conf.get('service_templates',None)
          serial_service_chain  = tenant_conf.get('serial_service_chain',[])
          if len(serial_service_chain):
          ...
          health_check_ids = self.list_service_health_checks(conn_obj_list=conn_obj_list,tenant_list=tenant_list)
          ...
          if len(create_policy_kwargs_list) != 0:
             ...
             policy_obj.create_policy(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
          ...
          if len(attach_policy_kwargs_list) != 0:
             ...
             policy_obj = Policy(None)
             policy_obj.attach_policy(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
      ...
      def delete_service_instance(self,*arg,**kwarg):
          ...
          self.logger      = kwarg['connection_obj'].logger
          domain_name,tenant_name = tenant_fq_name
          si = self.connections.vnc_lib.service_instances_list() 
          ...
          for service_instance in service_instances:
             ...
             if [dom,t_name] == tenant_fq_name:
                ...
                self.connections.vnc_lib.service_instance_delete(id=uuid)

      def delete_service_instances(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list):
          kwargs_list = []
          for tenant in tenant_list:
              if tenant['fq_name'] == ['default-domain','admin']:
                 continue
          ...
          if len(kwargs_list) == 0:
             return
          kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
          self.delete_service_instance(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
      ...
      def create_static_routes(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
          ...
          if len(kwargs_list) == 0:
             return
          kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
          self.create_static_route(count=thread_count,conn_obj_list=conn_obj_list,**kwargs)
