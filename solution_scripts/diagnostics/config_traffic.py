...
class CIDR:
  ...
  """
  def get_next_cidr(self,offset):
    lock.acquire()
    ip = IPNetwork(self.cidr)[0]
    new_ip = ipaddr.IPAddress(ip) + 256 * self.offset
...
class Policy(Base):
    ...
    def attach_policies(self,thread_count,global_conf,tenant_conf,tenant_name):
        ...
        kwargs_list = []
        vns_list    = []
        for tenant_indx,tenant_name in enumerate(tenant_name_list):
        ...
        for vn in vns_list:
           ...
           tenant_indx = tenant_name_list.index(tenant_name)
           kwargs = {}
           kwargs['tenant_name'] = tenant_name
    ...
    def detach_policies(self,thread_count,global_conf,tenant_conf,tenant_name):
        ...
        kwargs_list = []
        vn_list     = []
        for tenant_name in enumerate(tenant_name_list):
...
class vDNS(Base):
    ...
    def create_vdns(self,args,kwargs):
        ...
        vdns_data = VirtualDnsType(domain_name=dns_domain, dynamic_records_from_client=dyn_updates, record_order=rec_order, default_ttl_seconds=int(ttl),next_virtual_DNS=next_vdns,reverse_resolution=reverse_resolution)
    ...
    for tenant_name in tenant_name_list:
       ...
       for vn in net_list:
           ...
           for vm,vn_name in vms_filtered:
               ...
               vm_info['mgmt,ip_addr'] = addr[mgmt_vn_name] [0]['addr']
               vm_info['data,ip_addr'] = addr[vn_name][0]['addr']
    ...
    def create_vns(self,thread_count,global_conf,tenant_conf,tenant_name):
        ...
        for tenant_name in tenant_name_list:
            ...
            for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
                ...
                vn_name_pattern      = vn_info['vn,name,pattern']
                if not re.search('Private',vn_name_pattern):
                   for i in xrange(tenant_index*int(vn_count)):
                     data_cidr_obj.get_next_cidr()
                   cidr_list         = [data_cidr_obj.get_next_cidr()for x in xrange(vn_count)]
                else:
                   cidr_list         = [data_cidr_obj.get_next_cidr()for x in xrange(vn_count)]
                rt_number            = vn_info.get('route_target,rt_number') 
                rt_index             = rt_index_list[vn_group_index]
                ...
                for vn_indx in xrange(vn_count):
                    ...
                    if rt_number:
                       kwarg['rt_number'] = int(rt_number) + rt_index
                       rt_index += 1
                    else:
                       kwarg['rt_number']       = None
                    kwarg['external_flag']   = external_flag
                    kwarg['disable_gateway'] = False
                    kwargs_list.append(kwarg)
                rt_index_list[vn_group_index] = rt_index
