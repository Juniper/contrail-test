...
def handle_lbaas_traffic(test, tenant, vip_list, client_traffic_commands, remote_file_commands, pvn_vm_list = [], psvn_vm_list = []):
   ...
   if ex_srv_ip:
      ...
      computed_lbaas_command = [run, computed_run, 'httpload', '-rate', str(computed_lbaas_rate), '-fetches',\
      ...
      for vm in vm_list:
         if not vm['is_bms']:
            computed_vm_lbaas_command = [run, computed_run, 'httpload', '-rate', str(computed_lbaas_rate), '-fetches',\
                                str(computed_lbaas_rate * int(test.traffic_conf['sampling_interval'])), url_lb_file]
            ...
            computed_bms_lbaas_command = [run, computed_run, 'httpload', '-rate', str(computed_lbaas_rate), '-fetches',\
                                str(computed_lbaas_rate * int(test.traffic_conf['sampling_interval'])), url_lb_file]
...
def build_kill_traffic_commands(test, tenants):
   ...
   kill_vn_list = ['Private_VN', 'Private_SNAT', 'Public_FIP']
   vms = {}
...
def process_client_results(client_results):
   ...
   title = ['Throughput_Success', 'Retrans', 'Throughput_Fail',\
   ...
   for item in [httpload_short, httpload_lls, httpload_lb]:
      if not item.get('total',None):
         item['total'] = [(0.0,)*7]*len(iperf3['total'])
...
def iperf3_stats_handler(iperf3, output, address, command):
   ...
   if output.get('error', None):
      ...
      for index,sample in enumerate(output['intervals']):
         mbps_success = float(sample['sum']['bits_per_second'] / 1000000.0)
