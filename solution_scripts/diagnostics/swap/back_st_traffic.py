...
def build_traffic_commands(traffic_command, client_traffic_commands, src_vm={}, dst_vm={}, external_server_ip=""):
...
def final_client_traffic_command(traffic_command, client_traffic_commands, src, dst, port):
   ...
   if "ping" in traffic_command:
      ...
      traffic_command.extend(["-p", port])
      temp = traffic_command[:]
      client_traffic_commands.append((src,temp))
      del traffic_command[-1]
   elif ("iperf3" in traffic_command):
      traffic_command.extend(["-c", dst, "-p", port])
      temp = traffic_command[:]
      client_traffic_commands.append((src,temp))
      del traffic_command[-1]
      ...
      if pvn_vm_list:
         ...
         for c,s,p in pvn_c_s_list:
            if c and s:
	          ...
	          build_traffic_commands(lls_urls, remote_file_commands, src_vm=s)
                  httpload_url_short = 'http://'+s['name']+"/4kb\n"
               else:
                  ...
                  httpload_url_short = 'http://'+ ex_srv_ip+'/256kb\n'
            ...
            build_traffic_commands(httpload_url_short, remote_file_commands, src_vm=c)
            build_traffic_commands(lls_urls, remote_file_commands, src_vm=c)
