...
from systemtest_traffic import *
...
class Test(object):
    ...
    def get_vm_info(self,tenant_name):
        
        retrieve_vm_info(tenant_name=tenant_name,global_conf=self.global_conf,tenant_conf=self.tenant_conf)
    ...
    def cleanup_global_config(self):
        ...
        args_list   = [1]
        kwargs_list = [{'tenant_name':'admin','ipam_name':ipam_name}]  
        ipam_obj    = IPAM(None)
    ...
    def parse_cli_args(self,args):
        ...
        tenant_name_prefix = args.tenant_name_prefix if args.tenant_name_prefix else self.tenant_conf['tenant,name_prefix']
        ...
        tenant_index_end   = 650 if tenant_index[0] == tenant_index[1] else int(tenant_index[1])

        if tenant_name and string.upper(tenant_name) == "ALL" :
           # for delete/traffic only
           existing_tenants = retrieve_configured_tenant_list() 
           ...
           if create:
              while len(tenant_name_list) != tenant_count:
                    ...
                    if tenant_name in tenant_name_list:
                       continue
                    tenant_name_list.append(tenant_name)
           elif create == False: # --delete/traffic only
              existing_tenants = retrieve_configured_tenant_list() 
              filtered_tenants = []
              for tenant in existing_tenants:
              ...
              if not self.global_conf['tenant_index_random']:
                 tenant_name_list = existing_tenants[0:tenant_count] 
              else:
    ...
    def parse_config(self):
        ...
        if tenant_traffic_block:
           ...
           self.traffic_conf['pvn_ext_port_start'] = tenant_traffic_block['external_server']['pvn_ext_port_start']
           self.traffic_conf['snat_ext_port_start'] = tenant_traffic_block['external_server']['snat_ext_port_start']
           self.traffic_conf['ping'] = tenant_traffic_block['client_comm']['c_ping']
           self.traffic_conf['c_iperf3'] = tenant_traffic_block['client_comm']['c_iperf3'] + ["-t", tenant_traffic_block['duration']]
           self.traffic_conf['s_iperf3'] = tenant_traffic_block['server_comm']['s_iperf3']
           self.traffic_conf['httpload_lls'] = tenant_traffic_block['client_comm']['c_httpload_lls'] + \
                                               ['-seconds', tenant_traffic_block['duration']] + [url_lls_file]

           self.traffic_conf['httpload_short'] = tenant_traffic_block['client_comm']['c_httpload_short'] + \
                                               ['-seconds', tenant_traffic_block['duration']] + [url_short_file]
    ...
    def configure_global_config(self):
        vdns_obj = vDNS(None)
        vdns_obj.create_mgmt_vdns_tree(self.thread_count,self.global_conf,self.tenant_conf)
