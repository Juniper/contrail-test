...
class VM(Base):
    ...
    def create(self, name, vn_id_dict,subnet_id_dict,mgmt_network_first,image='ubuntu',userdata=None,flavor="m1.small",zone=None):
        ...
        if subnet_id_dict['mgmt'] :
           ...
           if mgmt_network_first:
              self.fixture.vn_ids = [vn_id_dict['mgmt'],vn_id_dict['data']]
           else:
              self.fixture.vn_ids = [vn_id_dict['data'],vn_id_dict['mgmt']]
        self.fixture.setUp()
    ...
    def create_vm(self,*arg,**kwarg):
        ...
        data_subnet_id = kwarg['data,subnet_id']
        mgmt_subnet_id = kwarg['mgmt,subnet_id']
        zone               = kwarg.get('zone_name',None)
        mgmt_network_first = kwarg.get('mgmt_network_first',False)
        additional_vn_name = kwarg.get('additional_vn_name',None)
        ...
        if not vm_obj:
           ...
           if additional_vn_name is not None:
           ...
           if additional_vn_name is not None:
               vn_ids.append(additional_vn_obj['network']['id'])
    ...
    @timeit
    def create_vms(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
            ...
            for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
              ...
              for vn_indx  in xrange(vn_count):
                 ...
                 if bgp_vm_count is not None:
                    ...
                    additional_vn_name  = vn_info['bgp_vm,additional_vn']
    ...
    def delete_vm(self,*arg,**kwarg):
        ...
        try:
          ...
          traceback.print_exc(file=sys.stdout)
          handleRefsExistError()
        except ConnectionError,ClientException:
      ...
      def associate_fip_to_vmis(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
         ...
         for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
             ...
             if re.search('Public_FIP_VN',vn_name_pattern):
                fip_vn_name_pattern = vn_name_pattern
         fip_shared = tenant_conf.get('fip,shared',False)
         if fip_vn_name_pattern is None and fip_shared is None:
            return
         vn_info_dict = {}
         if fip_shared:
            tenant_list_l = tenant_list + fip_pool_tenant_fq_name
...
class Bgpaas(Base):
     ...
     def create(self,*arg,**kwarg):
         ...
         if bgpaas_obj is None:
            ...
            vmis   = vm_obj.get_virtual_machine_interface_back_refs()
            for vm in vmis:
                vmi_obj  = self.connections.vnc_lib.virtual_machine_interface_read(id=vm['uuid'])
     ...
     def create_bgpaas(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
        ...
        for i,tenant in enumerate(tenant_list):
            ...
            tenant_fq_name = tenant['fq_name']
            for vm in vms:
                if vm['tenant_fq_name'] != tenant_fq_name or not re.search('bgp.vm',vm['vm_name']):
                   continue
                vm_id = vm['id']
                break
            if vm_id is None:
...
class ServicesConfig(Base):
      ...
      def retrieve_sc_info(self,*arg,**kwarg):
          ...
          for tenant in tenant_list:
             ...
             try:
                ...
                continue
             policys = proj_obj.get_network_policys()
             vn_pairs_list = []
             for policy in policys:
      ...
      def create_service_instance(self,*arg,**kwarg):
          ...
          az_name       = kwarg.get('az_name','ANY')
          host_name     = kwarg.get('host_name','ANY')
          tenant        = kwarg['tenant']
          tenant_fq_name = tenant['fq_name']
          domain_name,tenant_name = tenant_fq_name
          ...
          if int(si_version) == 2:
             ...
             for vm_indx in range(max_inst):
                 vm_fixture = VM(None)
                 vm_fixture = VMFixture(connections=self.connections, vm_name="si-vm-%s-%d"%(si_name,vm_indx), image_name=si_image)
      ...
      @timeit
      def create_service_instances(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_list,update_properties):
          service_instances = tenant_conf.get('service_instances',None)
          service_templates = global_conf.get('service_templates',None)
          serial_service_chain  = tenant_conf.get('serial_service_chain',[])
          if len(serial_service_chain):
