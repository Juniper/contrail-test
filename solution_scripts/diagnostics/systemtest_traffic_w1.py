...
def build_ping_pvn_fip(traffic_command, client_traffic_commands, vm):
    build_traffic_commands(traffic_command, client_traffic_commands,\
           		   src_vm=vm, dst_vm=vm)
    pvn_vm = get_one_random_vm(tenants[tenant], u'Private_VN')
    build_traffic_commands(traffic_command, client_traffic_commands,\
			   src_vm=vm, dst_vm=pvn_vm)
    fip_vn_vm = get_one_random_vm(tenants[tenant], u'Public_FIP')
    build_traffic_commands(traffic_command, client_traffic_commands,\
			   src_vm=vm, dst_vm=fip_vn_vm)
    gw_vn_vm = get_one_random_vm(tenants[tenant], u'SNAT_GW')
    if gw_vn_vm:
       build_traffic_commands(traffic_command, client_traffic_commands,\
			      src_vm=vm, dst_vm=gw_vn_vm)

def build_ping_pvn_fip_ext(traffic_command, client_traffic_commands, vm, ex_srv_ip):
    ping_check_pvn_fip(traffic_command, client_traffic_commands, vm)
    build_traffic_commands(traffic_command, client_traffic_commands,\
			   src_vm=vm, external_server_ip=ex_srv_ip)
    build_traffic_commands(traffic_command, client_traffic_commands,\
			   external_server_ip=ex_srv_ip, dst_vm=vm)

def build_ping_psvn(traffic_command, client_traffic_commands, vm):
    build_traffic_commands(traffic_command, client_traffic_commands,\
			   src_vm=vm, dst_vm=vm)
    gw_vn_vm = get_one_random_vm(tenants[tenant], u'SNAT_GW')
    build_traffic_commands(traffic_command, client_traffic_commands,\
...
def build_ping_psvn_ext(traffic_command, client_traffic_commands,\
			vm, ex_srv_ip):
    build_ping_psvn(traffic_command, client_traffic_commands, vm)
    build_traffic_commands(traffic_command, client_traffic_commands,\
			   src_vm=vm, external_server_ip=ex_srv_ip)

def build_ping_lb_ext(traffic_command, client_traffic_commands, vip, ex_srv_ip):    
    build_traffic_commands(traffic_command, client_traffic_commands,\
			   external_server_ip=ex_srv_ip, dst_vm=vip)
    build_ping_lb(traffic_command, client_traffic_commands, vip)

def build_ping_lb(traffic_command, client_traffic_commands, vip):
    fip_vn_vm = get_one_random_vm(tenants[tenant], u'Public_FIP')
    if fip_vn_vm:
       build_traffic_commands(traffic_command, client_traffic_commands,\
			      src_vm=fip_vn_vm, dst_vm=vip)
...
def build_traffic_commands(traffic_command,\
   ...
   if src_vm and dst_vm:
      if dst_vm['name']:
         final_client_traffic_command(traffic_command, client_traffic_commands,\
                                      src_vm['ip_addr,mgmt'], dst_vm['name'], port)

         if dst_vm['ip_addr,fip'] and 'ping' in traffic_command:
            fip_name = fip_to_name(dst_vm['ip_addr,fip'])
            final_client_traffic_command(traffic_command, client_traffic_commands,\
                                         src_vm['ip_addr,mgmt'], fip_name, port)
      else:
         fip_name = fip_to_name(dst_vm['ip_addr,fip'])
         final_client_traffic_command(traffic_command, client_traffic_commands,\
                                       src_vm['ip_addr,mgmt'], fip_name, port)
      ...
      final_client_traffic_command(traffic_command, client_traffic_commands,\
                                   src_vm['ip_addr,mgmt'], external_server_ip, port)
   elif external_server_ip and dst_vm:
      final_client_traffic_command(traffic_command, client_traffic_commands,\
                                   external_server_ip, dst_vm['ip_addr,fip'], port)
   elif src_vm:
      final_client_traffic_command(traffic_command, client_traffic_commands,\
                                   src_vm['ip_addr,mgmt'], dst_vm, port)
   elif external_server_ip:
      final_client_traffic_command(traffic_command, client_traffic_commands,\
                                   external_server_ip, dst_vm, port)
   else:
      return

def final_client_traffic_command(traffic_command,\
                                 client_traffic_commands,\
                                 src, dst, port):
      ...
      for vn in tenants[tenant]:
         if ((u'Private_VN' in vn) and pvn_fip and ex_srv_ip):
            for vm in tenants[tenant][vn]:
               build_ping_pvn_fip_ext(traffic_command, client_traffic_commands, vm, ex_srv_ip)
         elif u'Private_VN' in vn and pvn_fip:
            for vm in tenants[tenant][vn]:
               build_ping_pvn_fip(traffic_command, client_traffic_commands, vm)
            ...
            for vm in tenants[tenant][vn]:
               build_ping_psvn_ext(traffic_command, client_traffic_commands, vm, ex_srv_ip)
         elif u'Private_SNAT_VN' in vn:
            for vm in tenants[tenant][vn]:
               build_ping_psvn_ext(traffic_command, client_traffic_commands, vm)
         elif u'Private_LB_VIP' in vn and ex_srv_ip:
            for vip in vn:
               build_ping_lb_ext(traffic_command, client_traffic_commands, vip, ex_srv_ip)
         elif u'Private_LB_VIP' in vn:
            for vip in vn:
               build_ping_lb(traffic_command, client_traffic_commands, vm)
         else:
            continue
