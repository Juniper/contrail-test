...
def build_ping_service_chain(tenant, vn_pair, left_vms, right_vms,\
                             traffic_command, client_traffic_commands):
...
def check_vn_prop(test, tenant, vn):
   ...
   vn_name = ''.join(tenant_num.group(0).lower().split('.'))+'.test_id1.'+vn
   ...
   if vn_info:
      ...
      if vn_info['ipv6_cidr']:
         traffic_command = test.traffic_conf['ping6']
      else:
         traffic_command = test.traffic_conf['ping']

      return fip_attached, traffic_command
   else:
      return False, []
...
def ping_check_setup(test, tenants, sc_info, filename = "ping_result.txt"):
   ...
   for tenant in tenants:
      for vn in tenants[tenant]:
         fip_attached, traffic_command = check_vn_prop(test, tenant, vn)
   ...
   for tenant in sc_info:
      for vn_pair in sc_info[tenant]:
         ...
         fip_attached, traffic_command = check_vn_prop(test, tenant, vn)
         build_ping_service_chain(tenant, vn_pair, left_vms, right_vms, traffic_command, client_traffic_commands)
...
def build_bgpaas_ping(vm, traffic_command, client_traffic_commands):
   ...
   build_traffic_commands(traffic_command, client_traffic_commands, src=vm['ip_addr,mgmt'], dst=ipv4)
