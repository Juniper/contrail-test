...
def delete_mgmt_vn(conn_obj_list,thread_count,global_conf,tenant_conf):
    ...
    kwargs_list = []
    if len(pr_obj_list) != 0 and vn_ids.has_key("default-domain:admin:%s"%mgmt_vn_name):
       kwargs = {}
       kwargs['pr_obj_list'] = pr_obj_list
       kwargs['vn_id']     = vn_ids["default-domain:admin:%s"%mgmt_vn_name]
    ...
    if vn_ids.has_key("default-domain:admin:%s"%mgmt_vn_name):
       kwarg = {}
...
def create_mgmt_vn_ipam(conn_obj_list,thread_count,global_conf,tenant_conf,update_properties=False):
    ...
    conf = {}
    conf['ipam_fq_name_list']  = [['default-domain','admin',ipam_name]]
    ...
    #conf['ipam_name'] = "test.ipam"
    #conf['vn_name']   = "test-mgmt-vn1"
    vn_obj  = VN(None)
...
class Test(object):
    ...
    def get_vm_info(self):
        ...
        tenant_name_list = self.global_conf['tenant_name_list']
        vm_obj = VM(None)
        vm_info = vm_obj.retrieve_vm_info(conn_obj_list=conn_obj_list,tenant_name_list=tenant_name_list,global_conf=self.global_conf,tenant_conf=self.tenant_conf)
        svc_obj = ServiceTemplateConfig(None)
        sc_info = svc_obj.retrieve_sc_info(conn_obj_list=conn_obj_list,tenant_name_list=tenant_name_list)
    ...
    def parse_config(self):
        ...
        for i in xrange(len(tenant_network_conf)):
            ...
            if tenant_network_conf[i].has_key('bgp_vm'):
              ...
              vn_info['bgp_vm,additional_vn'] = tenant_network_conf[i]['bgp_vm'].get('additional_vn',None)
    ...
    def configure_global_config(self):
        ...
        func_arg1 = conn_obj_list,self.thread_count,self.global_conf,self.tenant_conf,['admin'],update_properties
        project_obj = ProjectConfig(None)
    ...
    def configure_tenant(self):
        ...
        func_arg = conn_obj_list,self.thread_count,self.global_conf,self.tenant_conf,tenant_name_list_conf,update_properties
        ...
        func_arg = conn_obj_list,self.thread_count,self.global_conf,self.tenant_conf,tenant_name_list,update_properties
