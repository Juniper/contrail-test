...
def get_vn_type(vn_name):
    vn_group_list = ['Private_SNAT_VN','Private_VN','Private_LB_VIP_VN','Private_LB_Pool_VN',
                     'SNAT_GW_VN','Public_FIP_VN','Private_SC_Left_VN','Private_SC_Right_VN','Private_VSRX_BGP_VN','Private_VSRX_MX_VN']
...
def generate_cidr(tenant_name,vn_type,already_allocated_cidr):
    ...
    ip_group['Private_VSRX_BGP_VN'] = [i for i in xrange(19,20)]
    ip_group['Private_VSRX_MX_VN']  = [i for i in xrange(20,21)]
...
class VN(Base):
    ...
    def create(self, name, subnets=[], ipam_id=None, external=False,shared=False,disable_gateway=False,rt_number=None,project_obj=None,forwarding_mode=None,project_name=None):
        kwargs = dict()
    ...
    def create_vn(self,*arg,**kwarg):
      try:
        ...
        ipv6_cidr       = kwarg.get('ipv6_cidr',None)
        self.connections = kwarg['connection_obj'].connections
        self.logger      = kwarg['connection_obj'].logger
        ...
        self.connections.project_name = project_fq_name[-1]
        self.connections.inputs.project_name = project_fq_name[-1]
        ...
        if use_fixture:
           ...
           subnets = [{'cidr':cidr,'name':vn_name+"_subnet0"}]
           if ipv6_cidr:
              subnets.append({'cidr':ipv6_cidr,'name':vn_name+"_subnet1"})
           self.create(vn_name,subnets,ipam_id,external,shared,disable_gateway,rt_number,project_obj,fwd_mode,self.connections.inputs.project_name)
           vn_obj = self.connections.vnc_lib.virtual_network_read(fq_name=vn_fq_name)
           return (extend_to_pr,vn_fq_name,vn_obj.uuid)
        else:
           ...
           if ipv6_cidr:
              ...
              ipam_sn1 = IpamSubnetType(subnet=SubnetType(ipv6_network, int(ipv6_prefix)),addr_from_start=True)
              ipam_sn_lst.append(ipam_sn1)
           vn_obj.add_network_ipam(ipam_obj,VnSubnetsType(ipam_sn_lst))
    ...
    def create_vns(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_name_list):
        ...
        for tenant_name in tenant_name_list:
            ...
            for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
                ...
                ipv6_cidr            = vn_info.get('ipv6_cidr',None)
                ipv4_cidr            = vn_info.get('ipv4_cidr',None)
        ...
        kwargs = {'kwargs_list': kwargs_list } if len(kwargs_list) > 1 else kwargs_list[0]
        extend_flag_vn_ids = self.create_vn(count=1,conn_obj_list=conn_obj_list,**kwargs)
...
class VM(Base):
    ...
    def create(self, name, vn_ids, image='ubuntu',userdata=None,flavor="m1.small"):
    ...
    def create_vm(self,*arg,**kwarg):
        ...
        if additional_vn_name is not None:
           additional_vn_obj    = quantum_h.get_vn_obj_if_present(additional_vn_name,uuid)
        ...
        if mgmt_network_first == True:
           vn_ids = [mgmt_vn_obj['network']['id'],data_vn_obj['network']['id']]
        else:
           vn_ids = [data_vn_obj['network']['id'],mgmt_vn_obj['network']['id']]
        if additional_vn_name is not None:
            vn_ids.append(additional_vn_obj['network']['id'])
        uid = self.create(vm_name,vn_ids,image_name,userdata,flavor)
        return 
              ...
              for vn_indx  in xrange(vn_info['count']):
                 ...
                 if vm_count is not None:
                    ...
                    for vm_indx in xrange(vm_count):
                        ...
                        conf['mgmt_network_first'] = vn_info['vm,mgmt_network_first']
                 ...
                 if bgp_vm_count is not None:
                    ...
                    for vm_indx in xrange(bgp_vm_count):
                        ...
                        conf['mgmt_network_first'] = vn_info['bgp_vm,mgmt_network_first']
...
class Bgpaas(Base):
     ...
     def delete(self,*arg,**kwarg):
         ...
         self.logger      = kwarg['connection_obj'].logger
         tenant_name      = kwarg['tenant_name']
         services_list = self.connections.vnc_lib.bgp_as_a_services_list()['bgp-as-a-services']
         for service in services_list:
             ...
             try:
               ...
               continue
             vmi_refs = bgpaas_obj.virtual_machine_interface_refs
             for vmi in vmi_refs:
                 vmi_obj = self.connections.vnc_lib.virtual_machine_interface_read(id=vmi['uuid'])
